...
void IndexBuffer::CreateIndexBuffer(const unsigned int numIndices,
                                    const DWORD usage,
                                    const IndexBuffer::Format format,
                                    ...
                                    const deviceNumber dN)
{
  ...
  const HRESULT hr =
      (dN == PRIMARY_DEVICE ? m_pd3dPrimaryDevice : m_pd3dSecondaryDevice)
...
IndexBuffer* IndexBuffer::CreateAndFillIndexBuffer(const unsigned int numIndices,
                                                   const WORD* indices,
                                                   const deviceNumber dN)
{
#ifdef ENABLE_SDL
...
void IndexBuffer::lock(const unsigned int offsetToLock,
                       ...
                       const DWORD flags)
{
  ...
  {
    *dataBuffer = malloc(
        this->sizeToLock); //!! does not init the buffer from the IBuffer data if flags is set accordingly (i.e. WRITEONLY, or better: create a new flag like 'PARTIALUPDATE'?)
...
void IndexBuffer::bind()
{
  ...
  {
    CHECKD3D(
        (m_dN == PRIMARY_DEVICE ? m_pd3dPrimaryDevice : m_pd3dSecondaryDevice)->SetIndices(m_ib));
    m_curIndexBuffer = this;
  }
#endif
}
