...
bool StackTrace::InitSymbols()
{
  ...
  {
    DWORD options =
        SYMOPT_FAIL_CRITICAL_ERRORS | SYMOPT_DEFERRED_LOADS | SYMOPT_LOAD_LINES | SYMOPT_UNDNAME;
...
int StackTrace::GetCallStack(void* vcontext, Address* callStack, int maxDepth, int entriesToSkip)
{
#if defined(_M_ARM64)
#pragma message("Warning: No CPU stack debug implemented yet")
  uintptr_t ebpReg[2];
  ...
  int numEntries(0);
  while (::StackWalk64(IMAGE_FILE_MACHINE_I386, process, thread, &stackFrame, context, 0,
...
int StackTrace::GetSymbolInfo(Address address, char* symbol, int maxSymbolLen)
{
  ...
  if (SymGetLineFromAddr64(hCurrentProcess, address64, &displacementLine, &lineInfo))
  {
    ...
    {
      fileLineChars = _snprintf_s(symbol, maxSymbolLen, _TRUNCATE, " %s(%u+%04u byte(s))", fileName,
                                  lineInfo.LineNumber, displacementLine);
    }
    else
    {
      fileLineChars =
          _snprintf_s(symbol, maxSymbolLen, _TRUNCATE, " %s(%u)", fileName, lineInfo.LineNumber);
    }
...
void StackTrace::GetCallStack(void* vcontext, bool includeArguments, char* symbol, int maxSymbolLen)
{
  ...
  while (maxSymbolLen > 0 &&
         ::StackWalk64(IMAGE_FILE_MACHINE_I386, ::GetCurrentProcess(), ::GetCurrentThread(),
                       &stackFrame, context, nullptr, /*Internal_ReadProcessMemory,*/
