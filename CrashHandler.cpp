...
#pragma message("Warning: No CPU exception debug output implemented yet")
  ...
  {
    fprintf(
        f, "Attempt to %s 0x%08X\n",
        (exceptionPtrs->ExceptionRecord->ExceptionInformation[0] == 1 ? "write to" : "read from"),
        exceptionPtrs->ExceptionRecord->ExceptionInformation[1]);
  }
...
{
  const HANDLE hDump = ::CreateFile(fileName, GENERIC_WRITE, FILE_SHARE_READ, 0, CREATE_ALWAYS,
  ...
  if (hDump != INVALID_HANDLE_VALUE)
  {
    ...
    const MINIDUMP_TYPE dumpType =
        (MINIDUMP_TYPE)(MiniDumpWithPrivateReadWriteMemory | MiniDumpWithThreadInfo |
                        MiniDumpWithUnloadedModules);

    const BOOL success = ::MiniDumpWriteDump(::GetCurrentProcess(), ::GetCurrentProcessId(), hDump,
...
void WriteMemoryStatus(FILE* f, const rde::MemoryStatus& status)
{
  ...
  fprintf(f, "Total Free: %uK (%uM) bytes\n", status.totalFree >> 10, status.totalFree >> 20);
...
#ifdef _WIN64
  fprintf(f,
          "RAX=%08X RBX=%08X RCX=%08X RDX=%08X\n"
          ...
          ctx->EFlags, ctx->SegCs, ctx->SegDs, ctx->SegSs, ctx->SegEs, ctx->SegFs, ctx->SegGs);
#else
  fprintf(f,
