...
PaintSur::PaintSur(const HDC hdc,
                   const float zoom,
                   const float offx,
                   const float offy,
                   const int width,
...
void PaintSur::PolygonImage(const std::vector<RenderVertex>& rgv,
                            HBITMAP hbm,
                            const float left,
                            const float top,
                            const float right,
                            ...
                            const int bitmapheight)
{
  ...
  if (GetWinVersion() >=
      2600) // For everything newer than Windows XP: use the alpha in the bitmap (RGB needs to be premultiplied with alpha, too, then! see CopyTo_ConvertAlpha())
  {
  ...
  else // do XOR trick for masking (draw image, draw black polygon, draw image again, and the XOR will do an implicit mask op)
  {
    ...
    StretchBlt(m_hdc, ix, iy, ix2 - ix, iy2 - iy, hdcNew, 0, 0, bitmapwidth, bitmapheight,
               SRCINVERT);
...
void PaintSur::Arc(const float x,
                   const float y,
                   const float radius,
                   const float pt1x,
                   const float pt1y,
...
void PaintSur::Image(const float x,
                     const float y,
                     const float x2,
                     const float y2,
                     HDC hdcSrc,
...
void PaintSur::SetObject(ISelect* const psel)
{
  if ((m_psel != nullptr) &&
      (psel !=
       nullptr)) // m_psel can be null when rendering a blueprint or other item which has no selection feedback
  {
