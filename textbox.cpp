...
HRESULT Textbox::Init(PinTable* ptable, float x, float y, bool fromMouseClick)
{
  ...
  return InitVBA(
      fTrue, 0,
      nullptr); //ApcProjectItem.Define(ptable->ApcProject, GetDispatch(), axTypeHostProjectItem/*axTypeHostClass*/, L"Textbox", nullptr);
}
...
void Textbox::SetDefaults(bool fromMouseClick)
{
  ...
  else
  {
    ...
    m_d.m_fontcolor =
        LoadValueIntWithDefault("DefaultProps\\TextBox", "FontColor", RGB(255, 255, 255));
    m_d.m_intensity_scale =
        LoadValueFloatWithDefault("DefaultProps\\TextBox", "IntensityScale", 1.0f);
    m_d.m_tdr.m_TimerEnabled =
        LoadValueBoolWithDefault("DefaultProps\\TextBox", "TimerEnabled", false) ? true : false;
    m_d.m_tdr.m_TimerInterval =
        LoadValueIntWithDefault("DefaultProps\\TextBox", "TimerInterval", 100);
    m_d.m_talign = (TextAlignment)LoadValueIntWithDefault("DefaultProps\\TextBox", "TextAlignment",
                                                          TextAlignRight);
...
void Textbox::RenderDynamic()
{
  ...
  const bool dmd = (m_d.m_isDMD || StrStrI(m_d.m_sztext.c_str(), "DMD") !=
  ...
  else if (m_texture)
  {
    ...
    g_pplayer->Spritedraw(x, y, width, height, 0xFFFFFFFF,
                          pd3dDevice->m_texMan.LoadTexture(m_texture, false),
                          m_d.m_intensity_scale);
...
void Textbox::PreRenderText()
{
  ...
  DrawText(hdc, m_d.m_sztext.c_str(), (int)m_d.m_sztext.length(), &rcOut,
...
HRESULT Textbox::InitLoad(IStream* pstm,
                          PinTable* ptable,
                          int* pid,
                          int version,
