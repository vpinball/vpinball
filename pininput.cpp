...
#ifdef __STANDALONE__
   m_gamepadOverride = settings.LoadValueWithDefault(Settings::Standalone, "GamepadOverride"s, true);
   if (m_gamepadOverride) {
      PLOGI.printf("Gamepad override enabled, using SDL GameController API");
   }
   else {
      PLOGI.printf("Gamepad override disabled, using SDL Joystick API");
   }
#endif
}

#ifdef ENABLE_SDL_INPUT
void PinInput::RefreshSDLInput()
{
   if (!m_gamepadOverride) {
      if (m_pSDLRumbleDevice) {
         SDL_HapticClose(m_pSDLRumbleDevice);
...
void PinInput::HandleInputSDL(DIDEVICEOBJECTDATA *didod)
{
   ...
   while (SDL_PollEvent(&e) != 0 && j<32)
   {
      ...
      case SDL_CONTROLLERAXISMOTION:
         if (m_gamepadOverride && e.caxis.axis < 6) {
              PLOGI.printf("SDL_CONTROLLERAXISMOTION: axis=%d, value=%d", e.caxis.axis, e.caxis.value);
      ...
      case SDL_CONTROLLERBUTTONUP:
         if (m_gamepadOverride && e.cbutton.button < 32) {
            PLOGI.printf("SDL_CONTROLLERBUTTONDOWN/UP: button=%d, type=%d", e.cbutton.button, e.type);
      ...
      case SDL_JOYAXISMOTION:
         if (!m_gamepadOverride && e.jaxis.axis < 6) {
            PLOGI.printf("SDL_JOYAXISMOTION: axis=%d, value=%d", e.jaxis.axis, e.jaxis.value);
      ...
      case SDL_JOYBUTTONUP:
         if (!m_gamepadOverride && e.jbutton.button < 32) {
            PLOGI.printf("SDL_JOYBUTTONDOWN/UP: button=%d, type=%d", e.jbutton.button, e.type);
