...
void PinInput::LoadSettings()
{
  ...
  m_override_default_buttons =
      LoadValueBoolWithDefault("Player", "PBWDefaultLayout", m_override_default_buttons);
  ...
  m_enableMouseInPlayer =
      LoadValueBoolWithDefault("Player", "EnableMouseInPlayer", m_enableMouseInPlayer);
  m_enableCameraModeFlyAround =
      LoadValueBoolWithDefault("Player", "EnableCameraModeFlyAround", m_enableCameraModeFlyAround);
  m_enable_nudge_filter =
      LoadValueBoolWithDefault("Player", "EnableNudgeFilter", m_enable_nudge_filter);
...
BOOL CALLBACK DIEnumJoystickCallback(LPCDIDEVICEINSTANCE lpddi, LPVOID pvRef)
{
  ...
  hr = ppinput->m_pDI->CreateDevice(lpddi->guidInstance, &ppinput->m_pJoystick[ppinput->e_JoyCnt],
                                    nullptr);
  if (FAILED(hr))
  {
  ...
  if (hr == S_OK)
  {
    if (!WzSzStrCmp(dstr.wsz, "PinballWizard"))
      ...
      ppinput->uShockType =
          USHOCKTYPE_SIDEWINDER; // set type 3 = Microsoft SideWinder Freestyle Pro
  ...
  hr = ppinput->m_pJoystick[ppinput->e_JoyCnt]->SetCooperativeLevel(
      ppinput->m_hwnd, DISCL_NONEXCLUSIVE | DISCL_BACKGROUND);
  ...
  hr = ppinput->m_pJoystick[ppinput->e_JoyCnt]->EnumObjects(EnumObjectsCallback, (VOID*)pvRef,
                                                            DIDFT_ALL);
...
void PinInput::PushQueue(DIDEVICEOBJECTDATA* const data,
                         const unsigned int app_data /*, const U32 curr_time_msec*/)
{
    ...
    if (hr == S_OK || hr == S_FALSE)
    {
      ...
      if ((hr == S_OK || hr == DI_BUFFEROVERFLOW) && (m_hwnd == GetForegroundWindow()))
      {
        if (g_pplayer->m_throwBalls || g_pplayer->m_ballControl) // debug ball throw functionality
        {
          ...
          if ((mouseState.rgbButtons[2] & 0x80) && !rightMouseButtonDown && !leftMouseButtonDown &&
              !middleMouseButtonDown)
          {
            POINT curPos;
            GetCursorPos(&curPos);
          ...
          if (g_pplayer->m_ballControl && !g_pplayer->m_throwBalls && leftMouseButtonDown &&
              !rightMouseButtonDown && !middleMouseButtonDown)
          {
            POINT curPos;
            GetCursorPos(&curPos);
            didod[0].dwData = 4;
...
void PinInput::handleInputXI(DIDEVICEOBJECTDATA* didod)
{
  ...
  static constexpr mappingElement mappingTable[] = {{XINPUT_GAMEPAD_A, DIJOFS_BUTTON0},
  ...
  if (m_inputDeviceXI != -2 &&
      (m_inputDeviceXI == -1 || (xie = XInputGetState(m_inputDeviceXI, &state)) != ERROR_SUCCESS))
  {
    m_inputDeviceXI = -1;
  ...
  if (xie ==
      ERROR_DEVICE_NOT_CONNECTED) // XInputGetState can cause quite some overhead, especially if no devices connected! Thus disable the polling if nothing connected
    m_inputDeviceXI = -2;
  ...
  {
    if ((m_inputDeviceXIstate.Gamepad.wButtons & mappingTable[i].xi) !=
        (state.Gamepad.wButtons & mappingTable[i].xi))
    {
...
void PinInput::PlayRumble(const float lowFrequencySpeed,
                          const float highFrequencySpeed,
                          const int ms_duration)
{
  if (m_rumbleMode == 0)
    return;
...
#ifdef ENABLE_SDL_INPUT
      if (m_rumbleDeviceSDL)
        SDL_HapticRumblePlay(m_rumbleDeviceSDL,
...
void PinInput::Init(const HWND hwnd)
{
  ...
  HRESULT hr;
#ifdef USE_DINPUT8
  hr = DirectInput8Create(g_pvp->theInstance, DIRECTINPUT_VERSION, IID_IDirectInput8,
                          (void**)&m_pDI, nullptr);
#else
  ...
  hr = m_pKeyboard->SetCooperativeLevel(hwnd, DISCL_NONEXCLUSIVE |
                                                  DISCL_FOREGROUND); //!! exclusive necessary??
  ...
  {
#ifdef USE_DINPUT8
    m_pDI->EnumDevices(DI8DEVCLASS_GAMECTRL, DIEnumJoystickCallback, this,
                       DIEDFL_ATTACHEDONLY); //enum Joysticks
#else
    m_pDI->EnumDevices(DIDEVTYPE_JOYSTICK, DIEnumJoystickCallback, this,
                       DIEDFL_ATTACHEDONLY); //enum Joysticks
#endif
  }
...
void PinInput::UnInit()
{
  ...
  SystemParametersInfo(SPI_SETSTICKYKEYS, sizeof(STICKYKEYS), &m_StartupStickyKeys,
                       SPIF_SENDCHANGE);
...
void PinInput::FireKeyEvent(const int dispid, int keycode)
{
  ...
  if (g_pplayer->m_ptable->m_tblMirrorEnabled)
  {
    ...
    if (keycode == g_pplayer->m_rgKeys[eLeftFlipperKey])
      keycode = g_pplayer->m_rgKeys[eRightFlipperKey];
    else if (keycode == g_pplayer->m_rgKeys[eRightFlipperKey])
      keycode = g_pplayer->m_rgKeys[eLeftFlipperKey];
    else if (keycode == g_pplayer->m_rgKeys[eLeftMagnaSave])
      keycode = g_pplayer->m_rgKeys[eRightMagnaSave];
    else if (keycode == g_pplayer->m_rgKeys[eRightMagnaSave])
      keycode = g_pplayer->m_rgKeys[eLeftMagnaSave];
    else if (keycode == DIK_LSHIFT)
      keycode = DIK_RSHIFT;
    else if (keycode == DIK_RSHIFT)
      keycode = DIK_LSHIFT;
  ...
  if (g_pplayer->m_cameraMode)
  {
    ...
    else if (keycode == g_pplayer->m_rgKeys[eStartGameKey] && dispid == DISPID_GameEvents_KeyDown)
    {
      if (!g_pvp->m_povEdit)
      {
        g_pplayer->m_ptable->m_BG_layback[g_pplayer->m_ptable->m_BG_current_set] =
            g_pplayer->m_ptable->m_backupLayback;
        g_pplayer->m_ptable->m_BG_rotation[g_pplayer->m_ptable->m_BG_current_set] =
            g_pplayer->m_ptable->m_backupRotation;
        g_pplayer->m_ptable->m_BG_inclination[g_pplayer->m_ptable->m_BG_current_set] =
            g_pplayer->m_ptable->m_backupInclination;
        g_pplayer->m_ptable->m_BG_xlatex[g_pplayer->m_ptable->m_BG_current_set] =
            g_pplayer->m_ptable->m_backupOffset.x;
        g_pplayer->m_ptable->m_BG_xlatey[g_pplayer->m_ptable->m_BG_current_set] =
            g_pplayer->m_ptable->m_backupOffset.y;
        g_pplayer->m_ptable->m_BG_xlatez[g_pplayer->m_ptable->m_BG_current_set] =
            g_pplayer->m_ptable->m_backupOffset.z;
        g_pplayer->m_ptable->m_BG_scalex[g_pplayer->m_ptable->m_BG_current_set] =
            g_pplayer->m_ptable->m_backupScale.x;
        g_pplayer->m_ptable->m_BG_scaley[g_pplayer->m_ptable->m_BG_current_set] =
            g_pplayer->m_ptable->m_backupScale.y;
        g_pplayer->m_ptable->m_BG_scalez[g_pplayer->m_ptable->m_BG_current_set] =
            g_pplayer->m_ptable->m_backupScale.z;
        g_pplayer->m_ptable->m_BG_FOV[g_pplayer->m_ptable->m_BG_current_set] =
            g_pplayer->m_ptable->m_backupFOV;
    ...
    else if (keycode == g_pplayer->m_rgKeys[eAddCreditKey] && dispid == DISPID_GameEvents_KeyDown &&
             g_pvp->m_povEdit)
    {
      g_pvp->Quit();
    }
  ...
  else
  {
    ...
    // Mixer volume only
    gMixerKeyDown =
        (keycode == g_pplayer->m_rgKeys[eVolumeDown] && dispid == DISPID_GameEvents_KeyDown);
    gMixerKeyUp =
        (keycode == g_pplayer->m_rgKeys[eVolumeUp] && dispid == DISPID_GameEvents_KeyDown);
...
void PinInput::autostart(const U32 msecs, const U32 retry_msecs, const U32 curr_time_msec)
{
  ...
  if ((m_firedautostart > 0) && // Initialized.
      ...
      ((curr_time_msec - m_firedautostart) >
       100)) // Start button has been down for at least 0.10 seconds.
  {
    // Release start.
  ...
  if ((m_as_down == 0) && // Start button is up.
          (((m_as_didonce == 1) && !started() &&
            ((curr_time_msec - m_firedautostart) >
             retry_msecs))) || // Not started and last attempt was at least AutoStartRetry seconds ago.
      ((m_as_didonce == 0) && ((curr_time_msec - m_firedautostart) >
                               msecs))) // Never attempted and autostart time has elapsed.
  {
    // Press start.
    m_firedautostart = curr_time_msec;
    m_as_down = 1;
...
void PinInput::Joy(const unsigned int n, const int updown, const bool start)
{
  ...
  if (m_joyvolumedown == n)
    FireKeyEvent(updown, g_pplayer->m_rgKeys[eVolumeDown]);
  if (m_joylefttilt == n)
    FireKeyEvent(updown, g_pplayer->m_rgKeys[eLeftTiltKey]);
  if (m_joycentertilt == n)
    FireKeyEvent(updown, g_pplayer->m_rgKeys[eCenterTiltKey]);
  if (m_joyrighttilt == n)
    FireKeyEvent(updown, g_pplayer->m_rgKeys[eRightTiltKey]);
  if (m_joymechtilt == n)
    FireKeyEvent(updown, g_pplayer->m_rgKeys[eMechanicalTilt]);
  if (m_joydebugballs == n)
    FireKeyEvent(updown, g_pplayer->m_rgKeys[eDBGBalls]);
  if (m_joydebugger == n)
    FireKeyEvent(updown, g_pplayer->m_rgKeys[eDebugger]);
  if (m_joylockbar == n)
    FireKeyEvent(updown, g_pplayer->m_rgKeys[eLockbarKey]);
  if (m_joycustom1 == n)
    FireKeyEvent(updown, m_joycustom1key);
  if (m_joycustom2 == n)
    FireKeyEvent(updown, m_joycustom2key);
  if (m_joycustom3 == n)
    FireKeyEvent(updown, m_joycustom3key);
  if (m_joycustom4 == n)
    FireKeyEvent(updown, m_joycustom4key);
  if (m_joypmbuyin == n)
    FireKeyEvent(updown, DIK_2);
  if (m_joypmcoin3 == n)
    FireKeyEvent(updown, DIK_5);
  if (m_joypmcoin4 == n)
    FireKeyEvent(updown, DIK_6);
  if (m_joypmcoindoor == n)
    FireKeyEvent(updown, DIK_END);
  if (m_joypmcancel == n)
    FireKeyEvent(updown, DIK_7);
  if (m_joypmdown == n)
    FireKeyEvent(updown, DIK_8);
  if (m_joypmup == n)
    FireKeyEvent(updown, DIK_9);
  if (m_joypmenter == n)
    FireKeyEvent(updown, DIK_0);
}
...
void PinInput::ProcessThrowBalls(const DIDEVICEOBJECTDATA* __restrict input)
{
  if (input->dwData == 1 || input->dwData == 3)
  {
    ...
    if (g_pplayer->m_ptable->m_BG_rotation[g_pplayer->m_ptable->m_BG_current_set] != 0.f &&
        g_pplayer->m_ptable->m_BG_rotation[g_pplayer->m_ptable->m_BG_current_set] != 360.f)
    {
    ...
    else
    {
      ...
      if (!ballGrabbed)
      {
        const float z = (input->dwData == 3) ? g_pplayer->m_ptable->m_glassheight
                                             : g_pplayer->m_ptable->m_tableheight;
        Ball* const pball = g_pplayer->CreateBall(vertex.x, vertex.y, z, vx, vy, 0,
                                                  (float)g_pplayer->m_debugBallSize * 0.5f,
                                                  g_pplayer->m_debugBallMass);
...
void PinInput::ProcessJoystick(const DIDEVICEOBJECTDATA* __restrict input, int curr_time_msec)
{
  ...
  if (input->dwOfs >= DIJOFS_BUTTON0 && input->dwOfs <= DIJOFS_BUTTON31)
  {
    ...
    const bool start =
        ((curr_time_msec - m_firedautostart) > g_pplayer->m_ptable->m_tblAutoStart) ||
        m_pressed_start || started();
    if (input->dwOfs == DIJOFS_BUTTON0)
    {
      if (((uShockType == USHOCKTYPE_PBWIZARD) || (uShockType == USHOCKTYPE_VIRTUAPIN)) &&
          !m_override_default_buttons) // plunge
        FireKeyEvent(updown, g_pplayer->m_rgKeys[ePlungerKey]);
    ...
    {
      if (((uShockType == USHOCKTYPE_PBWIZARD) || (uShockType == USHOCKTYPE_VIRTUAPIN)) &&
          !m_override_default_buttons) // right
        FireKeyEvent(updown, g_pplayer->m_rgKeys[eRightFlipperKey]);
    ...
    {
      if (((uShockType == USHOCKTYPE_PBWIZARD) || (uShockType == USHOCKTYPE_ULTRACADE) ||
           (uShockType == USHOCKTYPE_VIRTUAPIN)) &&
          !m_override_default_buttons)
    ...
    {
      if (((uShockType == USHOCKTYPE_PBWIZARD) || (uShockType == USHOCKTYPE_VIRTUAPIN)) &&
          !m_override_default_buttons) // volume down
        FireKeyEvent(updown, g_pplayer->m_rgKeys[eVolumeDown]);
      else
        Joy(4, updown, start);
    }
    ...
    {
      if (((uShockType == USHOCKTYPE_PBWIZARD) || (uShockType == USHOCKTYPE_VIRTUAPIN)) &&
          !m_override_default_buttons) // volume up
        FireKeyEvent(updown, g_pplayer->m_rgKeys[eVolumeUp]);
      else
        Joy(5, updown, start);
    }
    ...
    {
      if (((uShockType == USHOCKTYPE_PBWIZARD) || (uShockType == USHOCKTYPE_VIRTUAPIN)) &&
          !m_override_default_buttons) // pause menu
      {
        if (DISPID_GameEvents_KeyDown == updown)
          g_pplayer->m_closeDown = true;
      }
    ...
    {
      if (((uShockType == USHOCKTYPE_PBWIZARD) || (uShockType == USHOCKTYPE_VIRTUAPIN)) &&
          !m_override_default_buttons && !m_disable_esc) // exit
    ...
    {
      if (((uShockType == USHOCKTYPE_PBWIZARD) || (uShockType == USHOCKTYPE_VIRTUAPIN)) &&
          !m_override_default_buttons) // left
        FireKeyEvent(updown, g_pplayer->m_rgKeys[eLeftFlipperKey]);
      else
        Joy(10, updown, start);
    }
    ...
    {
      if (((uShockType == USHOCKTYPE_PBWIZARD) || (uShockType == USHOCKTYPE_VIRTUAPIN)) &&
          !m_override_default_buttons) // left 2
        FireKeyEvent(updown, g_pplayer->m_rgKeys[eLeftMagnaSave]);
    ...
    {
      if (((uShockType == USHOCKTYPE_PBWIZARD) || (uShockType == USHOCKTYPE_VIRTUAPIN)) &&
          !m_override_default_buttons) // coin 1
        FireKeyEvent(updown, g_pplayer->m_rgKeys[eAddCreditKey]);
      else
        Joy(12, updown, start);
    }
    ...
    {
      if (((uShockType == USHOCKTYPE_PBWIZARD) || (uShockType == USHOCKTYPE_VIRTUAPIN)) &&
          !m_override_default_buttons) // coin 2
        FireKeyEvent(updown, g_pplayer->m_rgKeys[eAddCreditKey2]);
    ...
    else if (input->dwOfs == DIJOFS_BUTTON14)
    {
      ...
      {
        if (started() ||
            !g_pplayer->m_ptable->m_tblAutoStartEnabled) // Check if we have started a game yet.
        {
          if (DISPID_GameEvents_KeyDown == updown)
          {
            m_first_stamp = curr_time_msec;
  ...
  else //end joy buttons
  {
    ...
    { // with selectable axes added to menu, giving priority in this order... X Axis (the Left/Right Axis), Y Axis
      ...
      case DIJOFS_Z:
      {
        if (g_pplayer)
        {
          ...
          { // with the ability to use rZ for plunger, checks to see if
            if (uShockType ==
                USHOCKTYPE_PBWIZARD) // the override is used and if so, if Plunger is set to Rz or
          ...
          { // if X or Y ARE NOT chosen for this axis and Plunger IS chosen for this axis...
            if (uShockType == USHOCKTYPE_GENERIC)
              g_pplayer->MechPlungerIn(!m_plunger_reverse ? -(int)input->dwData
                                                          : (int)input->dwData);
          }
        }
        break;
      ...
      case DIJOFS_RZ:
      {
        if (g_pplayer)
        {
          if ((uShockType == USHOCKTYPE_PBWIZARD) && m_override_default_buttons &&
              (m_plunger_axis == 6))
            g_pplayer->MechPlungerIn((int)input->dwData);
...
void PinInput::ProcessKeys(
    /*const U32 curr_sim_msec,*/ int
        curr_time_msec) // last one is negative if only key events should be fired
{
  ...
  while ((input = GetTail(/*curr_sim_msec*/)))
  {
    if (input->dwSequence == APP_MOUSE && g_pplayer)
    {
      ...
      else
      {
        if (input->dwOfs == 1 && m_joylflipkey == 25)
          FireKeyEvent((input->dwData & 0x80) ? DISPID_GameEvents_KeyDown : DISPID_GameEvents_KeyUp,
                       (DWORD)g_pplayer->m_rgKeys[eLeftFlipperKey]);
        if (input->dwOfs == 2 && m_joyrflipkey == 26)
          FireKeyEvent((input->dwData & 0x80) ? DISPID_GameEvents_KeyDown : DISPID_GameEvents_KeyUp,
                       (DWORD)g_pplayer->m_rgKeys[eRightFlipperKey]);
        if (input->dwOfs == 3 && m_joyplungerkey == 27)
          FireKeyEvent((input->dwData & 0x80) ? DISPID_GameEvents_KeyDown : DISPID_GameEvents_KeyUp,
                       (DWORD)g_pplayer->m_rgKeys[ePlungerKey]);
      }
    }
    ...
    if (input->dwSequence == APP_KEYBOARD)
    {
      ...
      else if (((input->dwOfs == (DWORD)g_pplayer->m_rgKeys[eEscape]) && !m_disable_esc) ||
               (input->dwOfs == (DWORD)g_pplayer->m_rgKeys[eExitGame]))
      {
        // Check if we have started a game yet.
        if (started() || !g_pplayer->m_ptable->m_tblAutoStartEnabled)
        {
