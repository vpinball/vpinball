...
void PinInput::Init(const HWND hwnd)
{
   ...
   if (!std::filesystem::exists(path)) {
      string srcPath = g_pvp->m_szMyPath + "assets" + PATH_SEPARATOR_CHAR + "Default_gamecontrollerdb.txt";
      if (std::filesystem::exists(srcPath))
         std::filesystem::copy(srcPath, path);
      else {
         PLOGE.printf("Default gamecontrollerdb.txt not found in %s", srcPath.c_str());
      }
   }
