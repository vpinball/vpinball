...
bool Mesh::LoadAnimation(const char* fname, const bool flipTV, const bool convertToLeftHanded)
{
  ...
  if (idx == string::npos)
  {
    ShowError("Can't find sequence of obj files! The file name of the sequence must be "
              "<meshname>_x.obj where x is the frame number!");
    return false;
  }
  idx++;
...
void Primitive::CreateRenderGroup(const Collection* const collection)
{
  ...
  SAFE_BUFFER_RELEASE(m_vertexBuffer);
  VertexBuffer::CreateVertexBuffer(m_numGroupVertices, 0, MY_D3DFVF_NOTEX2_VERTEX, &m_vertexBuffer,
                                   PRIMARY_DEVICE);
...
void Primitive::SetDefaults(bool fromMouseClick)
{
  ...
  m_d.m_SideColor = fromMouseClick
                        ? LoadValueIntWithDefault(strKeyName, "SideColor", RGB(150, 150, 150))
                        : RGB(150, 150, 150);

  m_d.m_visible = fromMouseClick ? LoadValueBoolWithDefault(strKeyName, "Visible", true) : true;
  m_inPlayState = m_d.m_visible;
  m_d.m_staticRendering =
      fromMouseClick ? LoadValueBoolWithDefault(strKeyName, "StaticRendering", true) : true;
  m_d.m_drawTexturesInside =
      fromMouseClick ? LoadValueBoolWithDefault(strKeyName, "DrawTexturesInside", false) : false;

  // Position (X and Y is already set by the click of the user)
  m_d.m_vPosition.z =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "Position_Z", 0.0f) : 0.0f;
  ...
  m_d.m_aRotAndTra[0] =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "RotAndTra0", 0.0f) : 0.0f;
  m_d.m_aRotAndTra[1] =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "RotAndTra1", 0.0f) : 0.0f;
  m_d.m_aRotAndTra[2] =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "RotAndTra2", 0.0f) : 0.0f;
  m_d.m_aRotAndTra[3] =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "RotAndTra3", 0.0f) : 0.0f;
  m_d.m_aRotAndTra[4] =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "RotAndTra4", 0.0f) : 0.0f;
  m_d.m_aRotAndTra[5] =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "RotAndTra5", 0.0f) : 0.0f;
  m_d.m_aRotAndTra[6] =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "RotAndTra6", 0.0f) : 0.0f;
  m_d.m_aRotAndTra[7] =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "RotAndTra7", 0.0f) : 0.0f;
  m_d.m_aRotAndTra[8] =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "RotAndTra8", 0.0f) : 0.0f;
  ...
  m_d.m_color = fromMouseClick ? LoadValueIntWithDefault(strKeyName, "Color", RGB(255, 255, 255))
                               : RGB(255, 255, 255);

  m_d.m_edgeFactorUI =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "EdgeFactorUI", 0.25f) : 0.25f;
  m_d.m_collision_reductionFactor =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "CollisionReductionFactor", 0.f) : 0.f;

  m_d.m_collidable =
      fromMouseClick ? LoadValueBoolWithDefault(strKeyName, "Collidable", true) : true;
  m_d.m_toy = fromMouseClick ? LoadValueBoolWithDefault(strKeyName, "IsToy", false) : false;
  m_d.m_disableLightingTop = dequantizeUnsigned<8>(
      fromMouseClick ? LoadValueIntWithDefault(strKeyName, "DisableLighting", 0)
                     : 0); // stored as uchar for backward compatibility
  m_d.m_disableLightingBelow =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "DisableLightingBelow", 0.f) : 0.f;
  m_d.m_reflectionEnabled =
      fromMouseClick ? LoadValueBoolWithDefault(strKeyName, "ReflectionEnabled", true) : true;
  m_d.m_backfacesEnabled =
      fromMouseClick ? LoadValueBoolWithDefault(strKeyName, "BackfacesEnabled", false) : false;
  m_d.m_displayTexture =
      fromMouseClick ? LoadValueBoolWithDefault(strKeyName, "DisplayTexture", false) : false;
  m_d.m_objectSpaceNormalMap =
      fromMouseClick ? LoadValueBoolWithDefault(strKeyName, "ObjectSpaceNormalMap", false) : false;
}
...
void Primitive::GetHitShapes(vector<HitObject*>& pvho)
{
  ...
  const unsigned int reduced_vertices =
      max((unsigned int)pow((double)m_vertices.size(),
                            clamp(1.f - m_d.m_collision_reductionFactor, 0.f, 1.f) * 0.25f + 0.75f),
          420u); //!! 420 = magic
...
void Primitive::AddHitEdge(vector<HitObject*>& pvho,
                           std::set<std::pair<unsigned, unsigned>>& addedEdges,
                           const unsigned i,
                           const unsigned j,
                           const Vertex3Ds& vi,
...
void Primitive::RecalculateMatrices()
{
  ...
  // translation matrix
  Matrix3D Tmatrix;
  Tmatrix.SetTranslation(m_d.m_vPosition.x, m_d.m_vPosition.y,
                         m_d.m_vPosition.z + m_ptable->m_tableheight);
...
void Primitive::UIRenderPass2(Sur* const psur)
{
  ...
  if (!m_d.m_displayTexture)
  {
    ...
    {
      if (!m_d.m_use3DMesh || (m_d.m_edgeFactorUI >= 1.0f) ||
          (m_mesh.NumVertices() <= 100)) // small mesh: draw all triangles
      {
        for (size_t i = 0; i < m_mesh.NumIndices(); i += 3)
        {
  ...
  psur->Line(m_d.m_vPosition.x - 10.0f, m_d.m_vPosition.y, m_d.m_vPosition.x + 10.0f,
             m_d.m_vPosition.y);
  psur->Line(m_d.m_vPosition.x, m_d.m_vPosition.y - 10.0f, m_d.m_vPosition.x,
             m_d.m_vPosition.y + 10.0f);
...
void Primitive::CalculateBuiltinOriginal()
{
  ...
  for (int i = 0; i < m_d.m_Sides; ++i)
  {
    ...
    // calculate bottom
    Vertex3D_NoTex2* const bottomVert =
        &m_mesh.m_vertices[i + 1 + m_d.m_Sides + 1]; // bottompoint at side
  ...
  for (int i = 0; i < m_d.m_Sides; i++)
  {
    ...
    Vertex3D_NoTex2* const bottomVert =
        &m_mesh.m_vertices[i + 1 + m_d.m_Sides + 1]; // bottom point at side
...
void Primitive::UpdateStatusBarInfo()
{
  ...
  if (m_d.m_use3DMesh)
  {
    const string tbuf = "Vertices: " + std::to_string(m_mesh.NumVertices()) +
                        " | Polygons: " + std::to_string(m_mesh.NumIndices());
...
void Primitive::RenderObject()
{
  ...
  if (!m_d.m_useAsPlayfield)
  {
    ...
    pd3dDevice->SetRenderStateCulling(m_d.m_backfacesEnabled && mat->m_bOpacityActive
                                          ? RenderDevice::CULL_CW
    ...
    if (pin && nMap)
    {
      pd3dDevice->basicShader->SetTechnique(mat->m_bIsMetal
                                                ? "basic_with_texture_normal_isMetal"
                                                : "basic_with_texture_normal_isNotMetal");
    ...
    if (m_d.m_addBlend)
    {
      ...
      pd3dDevice->basicShader->SetFlasherColorAlpha(
          vec4(color.x * color.w, color.y * color.w, color.z * color.w, color.w));
    }
    else
    {
      const vec4 color = convertColor(m_d.m_color, m_d.m_alpha * (float)(1.0 / 100.0));
...
void Primitive::RenderSetup()
{
  ...
  SAFE_BUFFER_RELEASE(m_vertexBuffer);
  VertexBuffer::CreateVertexBuffer((unsigned int)m_mesh.NumVertices(), 0, MY_D3DFVF_NOTEX2_VERTEX,
                                   &m_vertexBuffer, PRIMARY_DEVICE);
...
HRESULT Primitive::SaveData(IStream* pstm, HCRYPTHASH hcrypthash, const bool backupForPlay)
{
  ...
  if (m_d.m_use3DMesh && !backupForPlay)
  {
    ...
    bw.WriteStruct(FID(M3DX), m_mesh.m_vertices.data(),
                   (int)(sizeof(Vertex3D_NoTex2) * m_mesh.NumVertices()));
#else
    ...
    {
#ifndef COMPRESS_MESHES
      bw.WriteStruct(FID(M3DI), m_mesh.m_indices.data(),
                     (int)(sizeof(unsigned int) * m_mesh.NumIndices()));
#else
...
HRESULT Primitive::InitLoad(IStream* pstm,
                            PinTable* ptable,
                            int* pid,
                            int version,
                            ...
                            HCRYPTKEY hcryptkey)
{
  ...
  if (version <
      1011) // so that old tables do the reorderForsyth on each load, new tables only on mesh import, so a simple resave of a old table will also skip this step
  {
    WaitForMeshDecompression(); //!! needed nowadays due to multithreaded mesh decompression
...
bool Primitive::LoadToken(const int id, BiffReader* const pbr)
{
  switch (id)
  {
    ...
    case FID(DILI):
    {
      int tmp;
      pbr->GetInt(tmp);
      m_d.m_disableLightingTop =
          (tmp == 1) ? 1.f : dequantizeUnsigned<8>(tmp); // backwards compatible hacky loading!
      break;
    }
    case FID(DILB):
    ...
    case FID(DIPT):
      ...
      break;
    case FID(M3DN):
      pbr->GetString(m_d.m_meshFileName);
      break;
    case FID(M3VN):
    {
    ...
    case FID(M3AX):
    {
      ...
      const int error = uncompress((unsigned char*)frameData.m_frameVerts.data(), &uclen, c,
                                   m_compressedAnimationVertices);
      if (error != Z_OK)
      {
        char err[128];
    ...
    case FID(M3CX):
    {
      ...
      g_pPrimitiveDecompressThreadPool->enqueue(
          [uclen, c, this]
          {
            ...
            const int error = uncompress((unsigned char*)m_mesh.m_vertices.data(), &uclen2, c,
                                         m_compressedVertices);
            if (error != Z_OK)
            {
              char err[128];
    ...
    case FID(M3CI):
    {
      ...
      if (m_numVertices > 65535)
      {
        ...
        g_pPrimitiveDecompressThreadPool->enqueue(
            ...
            {
              mz_ulong uclen2 = uclen;
              const int error = uncompress((unsigned char*)m_mesh.m_indices.data(), &uclen2, c,
                                           m_compressedIndices);
              if (error != Z_OK)
              {
                char err[128];
                sprintf_s(err, "Could not uncompress (large) primitive index data, error %d",
                          error);
                ShowError(err);
              }
              free(c);
      ...
      else
      {
        ...
        g_pPrimitiveDecompressThreadPool->enqueue(
            [uclen, c, this]
            {
              ...
              mz_ulong uclen2 = uclen;
              const int error =
                  uncompress((unsigned char*)tmp.data(), &uclen2, c, m_compressedIndices);
              if (error != Z_OK)
              {
                char err[128];
                sprintf_s(err, "Could not uncompress (small) primitive index data, error %d",
                          error);
                ShowError(err);
              }
              free(c);
        ...
        case BN_CLICKED:
          switch (LOWORD(wParam))
          {
            case IDOK:
            {
              ...
              const bool convertToLeftHanded =
                  IsDlgButtonChecked(hwndDlg, IDC_CONVERT_COORD_CHECK) == BST_CHECKED;
              const bool importAbsolutePosition =
                  IsDlgButtonChecked(hwndDlg, IDC_ABS_POSITION_RADIO) == BST_CHECKED;
              const bool centerMesh = IsDlgButtonChecked(hwndDlg, IDC_CENTER_MESH) == BST_CHECKED;
              const bool importMaterial =
                  IsDlgButtonChecked(hwndDlg, IDC_IMPORT_MATERIAL) == BST_CHECKED;
              const bool importAnimation =
                  IsDlgButtonChecked(hwndDlg, IDC_IMPORT_ANIM_SEQUENCE) == BST_CHECKED;
              const bool doForsyth =
                  IsDlgButtonChecked(hwndDlg, IDC_IMPORT_NO_FORSYTH) == BST_UNCHECKED;
              if (importMaterial)
              {
              ...
              if (prim->m_mesh.LoadWavefrontObj(szFileName, flipTV, convertToLeftHanded))
              {
                  ...
                  if (prim->m_mesh.LoadAnimation(szFileName, flipTV, convertToLeftHanded))
                  {
                    if (centerMesh)
                    {
                      for (size_t t = 0; t < prim->m_mesh.m_animationFrames.size(); t++)
                      {
                        for (size_t i = 0; i < prim->m_mesh.m_vertices.size(); i++)
                        {
                          prim->m_mesh.m_animationFrames[t].m_frameVerts[i].x -=
                              prim->m_mesh.middlePoint.x;
                          prim->m_mesh.m_animationFrames[t].m_frameVerts[i].y -=
                              prim->m_mesh.middlePoint.y;
                          prim->m_mesh.m_animationFrames[t].m_frameVerts[i].z -=
                              prim->m_mesh.middlePoint.z;
                        }
                      }
                    }
                  }
                ...
                if (doForsyth)
                {
                  unsigned int* const tmp =
                      reorderForsyth(prim->m_mesh.m_indices, (int)prim->m_mesh.NumVertices());
                  if (tmp != nullptr)
                  {
                    memcpy(prim->m_mesh.m_indices.data(), tmp,
                           prim->m_mesh.NumIndices() * sizeof(unsigned int));
                    delete[] tmp;
                  }
                }
            ...
            case IDC_BROWSE_BUTTON:
            {
                                        ...
                                        "Wavefront obj file (*.obj)\0*.obj\0", "obj", 0))
              {
                ...
                if (index != std::string::npos)
                {
                  ...
                  index++;
                  prim->m_d.m_meshFileName =
                      szFileName[0].substr(index, szFileName[0].length() - index);
                }
...
{
  DialogBoxParam(m_vpinball->theInstance, MAKEINTRESOURCE(IDD_MESH_IMPORT_DIALOG),
                 m_vpinball->GetHwnd(), ObjImportProc, (size_t)this);
#if 1
  return false;
#else
  ...
  int ans = m_vpinball->MessageBox("Do you want to mirror the object?",
                                   "Convert coordinate system?", MB_YESNO | MB_DEFBUTTON2);
  if (ans == IDYES)
  {
  ...
  {
    ans = m_vpinball->MessageBox("Do you want to convert texture coordinates?", "Confirm",
...
void Primitive::ExportMeshDialog()
{
  ...
  if (m_vpinball->SaveFileDialog(szInitialDir, szFileName, "Wavefront obj file (*.obj)\0*.obj\0",
                                 "obj", OFN_OVERWRITEPROMPT | OFN_HIDEREADONLY))
  {
...
{
  *pVal = FTOVB((!g_pplayer || m_vhoCollidable.empty()) ? m_d.m_collidable
                                                        : m_vhoCollidable[0]->m_enabled);
...
void Primitive::SetDefaultPhysics(bool fromMouseClick)
{
  static constexpr char strKeyName[] = "DefaultProps\\Primitive";
  m_d.m_elasticity =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "Elasticity", 0.3f) : 0.3f;
  m_d.m_elasticityFalloff =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "ElasticityFalloff", 0.5f) : 0.5f;
