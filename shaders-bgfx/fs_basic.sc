...
uniform float objectSpaceNormalMap; // FIXME float uniforms are not supported: group or declare as vec4
...
uniform float refractionThickness; // FIXME float uniforms are not supported: group with tint
...
uniform float mirrorFactor; // FIXME float uniforms are not supported: group with normal
...
vec3 compute_reflection(const vec2 screenSpace, const vec3 N)
{
   ...
   return smoothstep(0.5, 0.9, dot(mirrorNormal, N)) * mirrorFactor * texture2D(tex_reflection, screenSpace + 0.5 * w_h_height.xy).rgb;
}
...
#endif
{
   ...
   const vec4 proj = mul(u_proj, vec4(refracted_pos, 1.0));
   ...
   const vec4 proj_base = mul(vec4(pos, 1.0), u_proj); 
...
void main() {
   ...
   BRANCH if (cBase_Alpha.a < 1.0)
   {
      ...
      if (fDisableLighting_top_below.y < 1.0)
         ...
         color.xyz += mix(sqrt(diffuse)*texture2DLod(tex_base_transmission, gl_FragCoord.xy * u_viewTexel.xy, 0.0).rgb*color.a, vec3_splat(0.), fDisableLighting_top_below.y); //!! depend on normal of light (unknown though) vs geom normal, too?
   }
...
#ifdef DX9
      color.rgb = mix(compute_refraction(v_worldPos.xyz, gl_FragCoord.xy * u_viewTexel.xy, N, V), color.rgb, cBase_Alpha.a);
#else
