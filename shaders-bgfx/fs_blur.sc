...
SAMPLER2D(s_tex_fb_filtered,  0);
   ...
   #define NO_CENTER
   #define ARRAY_SIZE 5
   const ARRAY_BEGIN(float, offset, ARRAY_SIZE) 0.65323, 2.42572, 4.36847, 6.31470, 8.26547 ARRAY_END();
   const ARRAY_BEGIN(float, weight, ARRAY_SIZE) 0.19923, 0.18937, 0.08396, 0.02337, 0.00408 ARRAY_END();
...
#ifdef HORIZONTAL
   #define OFFSET vec2(u_viewTexel.x*offset[i], 0.0)
#else
   #define OFFSET vec2(u_viewTexel.y*offset[i], 0.0)
#endif
...
void main()
{
   ...
   UNROLL for(int i = 0; i < ARRAY_SIZE; ++i)
      result += (texture2DLod(s_tex_fb_filtered, v_texcoord0.xy + OFFSET, 0.0).xyz
                +texture2DLod(s_tex_fb_filtered, v_texcoord0.xy - OFFSET, 0.0).xyz) * weight[i];
#else
   vec3 result = texture2DLod(s_tex_fb_filtered, v_texcoord0.xy, 0.0).xyz * weight[0];
