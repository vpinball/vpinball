...
// uniform float fSceneScale; // FIXME float uniforms are not supported. group with something else
...
{ 
   ...
   const vec3 lightDir = (( mul(u_view, vec4(lightPos[i].xyz, 1.0)) ).xyz - pos) / fSceneScale; 
#else
...
vec3 lightLoop(const vec3 pos, vec3 N, const vec3 V, vec3 diffuse, vec3 glossy, const vec3 specular, const float edge, const float fix_normal_orientation, const bool is_metal) // input vectors (N,V) are normalized for BRDF evals
{
   ...
   BRANCH if (!is_metal && (diffuseMax > 0.0))
      color += DoEnvmapDiffuse(normalize(( mul(vec4(N,0.0), u_view) ).xyz), diffuse); // trafo back to world for lookup into world space envmap // actually: mul(vec4(N,0.0), u_viewInverseInverseTranspose), but optimized to save one matrix

   BRANCH if ((glossyMax > 0.0) || (specularMax > 0.0))
   {
	   vec3 R = (2.0*NdotV)*N - V; // reflect(-V,n);
	   R = normalize(( mul(vec4(R,0.0), u_view) ).xyz); // trafo back to world for lookup into world space envmap // actually: mul(vec4(R,0.0), u_viewInverseInverseTranspose), but optimized to save one matrix
