...
void BASS_ErrorMapCode(const int code, string& text)
{
  switch (code)
  {
    ...
    case BASS_ERROR_FILEOPEN:
      text = "can't open the file";
      break;
    case BASS_ERROR_DRIVER:
      text = "can't find a free/valid driver";
      break;
    case BASS_ERROR_BUFLOST:
      text = "the sample buffer was lost";
      break;
    case BASS_ERROR_HANDLE:
      text = "invalid handle";
      break;
    case BASS_ERROR_FORMAT:
      text = "unsupported sample format";
      break;
    case BASS_ERROR_POSITION:
      text = "invalid position";
      break;
    case BASS_ERROR_INIT:
      text = "BASS_Init has not been successfully called";
      break;
    case BASS_ERROR_START:
      text = "BASS_Start has not been successfully called";
      break;
    case BASS_ERROR_SSL:
      text = "SSL/HTTPS support isn't available";
      break;
    case BASS_ERROR_ALREADY:
      text = "already initialized/paused/whatever";
      break;
    case BASS_ERROR_NOTAUDIO:
      text = "file does not contain audio";
      break;
    case BASS_ERROR_NOCHAN:
      text = "can't get a free channel";
      break;
    case BASS_ERROR_ILLTYPE:
      text = "an illegal type was specified";
      break;
    case BASS_ERROR_ILLPARAM:
      text = "an illegal parameter was specified";
      break;
    ...
    case BASS_ERROR_DEVICE:
      text = "illegal device number";
      break;
    case BASS_ERROR_NOPLAY:
      text = "not playing";
      break;
    case BASS_ERROR_FREQ:
      text = "illegal sample rate";
      break;
    case BASS_ERROR_NOTFILE:
      text = "the stream is not a file stream";
      break;
    case BASS_ERROR_NOHW:
      text = "no hardware voices available";
      break;
    case BASS_ERROR_EMPTY:
      text = "the MOD music has no sequence data";
      break;
    case BASS_ERROR_NONET:
      text = "no internet connection could be opened";
      break;
    case BASS_ERROR_CREATE:
      text = "couldn't create the file";
      break;
    case BASS_ERROR_NOFX:
      text = "effects are not available";
      break;
    case BASS_ERROR_NOTAVAIL:
      text = "requested data/action is not available";
      break;
    case BASS_ERROR_DECODE:
      text = "the channel is/isn't a 'decoding channel'";
      break;
    case BASS_ERROR_DX:
      text = "a sufficient DirectX version is not installed";
      break;
    ...
    case BASS_ERROR_VERSION:
      text = "invalid BASS version (used by add-ons)";
      break;
    case BASS_ERROR_CODEC:
      text = "codec is not available/supported";
      break;
    case BASS_ERROR_ENDED:
      text = "the channel/file has ended";
      break;
    case BASS_ERROR_BUSY:
      text = "the device is busy";
      break;
    case BASS_ERROR_UNSTREAMABLE:
      text = "unstreamable file";
      break;
...
HRESULT PinSound::ReInitialize()
{
  ...
  if (!IsWav())
  {
    const SoundConfigTypes SoundMode3D =
        (m_outputTarget == SNDOUT_BACKGLASS)
            ? SNDCFG_SND3D2CH
            : (SoundConfigTypes)LoadValueIntWithDefault("Player", "Sound3D", (int)SNDCFG_SND3D2CH);

    SetDevice();
    m_BASSstream =
        BASS_StreamCreateFile(TRUE, m_pdata, 0, m_cdata,
                              (SoundMode3D != SNDCFG_SND3D2CH)
                                  ? (BASS_SAMPLE_3D | BASS_SAMPLE_MONO)
                                  : 0 /*| BASS_SAMPLE_LOOP*/ //!! mono really needed? doc claims so
        );
    ...
    if (m_BASSstream == 0)
    {
      ...
      const string bla = "BASS music/sound library cannot create stream \"" + m_szPath +
                         "\" (error " + std::to_string(code) + ": " + bla2 + ")";
  ...
  dsbd.dwFlags = DSBCAPS_STATIC | DSBCAPS_CTRLVOLUME | DSBCAPS_CTRLFREQUENCY |
                 ((SoundMode3D != SNDCFG_SND3D2CH) ? DSBCAPS_CTRL3D : DSBCAPS_CTRLPAN);
  ...
  if ((SoundMode3D == SNDCFG_SND3D2CH) && (m_wfx.nChannels == 1) &&
      ((m_wfx.wFormatTag == WAVE_FORMAT_PCM) || (m_wfx.wFormatTag == WAVE_FORMAT_IEEE_FLOAT)))
  {
  ...
  if (FAILED(hr = m_pDSBuffer->Lock(0, dsbd.dwBufferBytes, &pbData, &dwLength, &pbData2, &dwLength2,
                                    0L)))
  {
...
void PinSound::Play(const float volume,
                    const float randompitch,
                    const int pitch,
                    const float pan,
                    const float front_rear_fade,
                    const int flags,
                    const bool restart)
{
  if (IsWav())
  ...
  else if (m_BASSstream)
  {
    ...
    BASS_ChannelSetAttribute(m_BASSstream, BASS_ATTRIB_VOL,
                             sqrtf(saturate(volume * (float)(1.0 / 100.)))); // to match VP legacy
    ...
    if (randompitch > 0.f)
    {
      ...
      BASS_ChannelSetAttribute(m_BASSstream, BASS_ATTRIB_FREQ,
                               freq + freq * randompitch * rndh * rndh -
                                   freq * randompitch * rndl * rndl * 0.5f);
    }
    else if (pitch != 0)
    {
      float freq;
    ...
    switch (SoundMode3D)
    {
      ...
      {
        const BASS_3DVECTOR v(PinDirectSound::PanTo3D(pan), 0.0f,
                              PinDirectSound::PanTo3D(front_rear_fade));
        BASS_ChannelSet3DPosition(m_BASSstream, &v, nullptr, nullptr);
        BASS_Apply3D();
        break;
      }
      ...
      {
        const BASS_3DVECTOR v(PinDirectSound::PanTo3D(pan), 0.0f,
                              -((PinDirectSound::PanTo3D(front_rear_fade) + 3.0f) / 2.0f));
        BASS_ChannelSet3DPosition(m_BASSstream, &v, nullptr, nullptr);
        BASS_Apply3D();
        break;
      }
      ...
      {
        const BASS_3DVECTOR v(PinDirectSound::PanSSF(pan), 0.0f,
                              PinDirectSound::FadeSSF(front_rear_fade));
        BASS_ChannelSet3DPosition(m_BASSstream, &v, nullptr, nullptr);
        BASS_Apply3D();
        break;
      }
...
PinSound* AudioMusicPlayer::LoadFile(const string& strFileName)
{
  ...
  if (pps->IsWav()) // only use old direct sound code and wav reader if playing wav's
  {
    ...
    if (FAILED(hr = pWaveSoundRead->Read(pps->m_cdata, (BYTE*)pps->m_pdata, &cbWavSize)))
    {
    ...
    VOID* pbData2 = nullptr;
    DWORD dwLength, dwLength2;
    if (FAILED(hr = pps->m_pDSBuffer->Lock(0, pps->m_cdata, &pbData, &dwLength, &pbData2,
                                           &dwLength2, 0L)))
    {
  ...
  else
  {
    ...
    pps->SetDevice();
    pps->m_BASSstream =
        BASS_StreamCreateFile(TRUE, pps->m_pdata, 0, pps->m_cdata,
                              (SoundMode3D != SNDCFG_SND3D2CH)
                                  ? (BASS_SAMPLE_3D | BASS_SAMPLE_MONO)
                                  : 0 /*| BASS_SAMPLE_LOOP*/ //!! mono really needed? doc claims so
        );
    ...
    if (pps->m_BASSstream == 0)
    {
      ...
      string bla2;
      BASS_ErrorMapCode(code, bla2);
      const string bla = "BASS music/sound library cannot load \"" + strFileName + "\" (error " +
                         std::to_string(code) + ": " + bla2 + ")";
      g_pvp->MessageBox(bla.c_str(), "Error", MB_ICONERROR);
      return nullptr;
    }
  }
...
void PinDirectSoundWavCopy::PlayInternal(const float volume,
                                         const float randompitch,
                                         const int pitch,
                                         const float pan,
                                         const float front_rear_fade,
                                         ...
                                         const bool restart)
{
  ...
  if (m_ppsOriginal != this)
  {
    if (randompitch > 0.f)
    {
      ...
      const float rndh = rand_mt_01();
      const float rndl = rand_mt_01();
      m_pDSBuffer->SetFrequency(freq + (DWORD)((float)freq * randompitch * rndh * rndh) -
                                (DWORD)((float)freq * randompitch * rndl * rndl * 0.5f));
    }
    else if (pitch != 0)
    {
      DWORD freq;
  ...
  const SoundConfigTypes SoundMode3D =
      (m_ppsOriginal->m_outputTarget == SNDOUT_BACKGLASS)
          ? SNDCFG_SND3D2CH
          : (SoundConfigTypes)LoadValueIntWithDefault("Player", "Sound3D", (int)SNDCFG_SND3D2CH);
  ...
  {
    case SNDCFG_SND3DALLREAR:
      m_pDS3DBuffer->SetPosition(PinDirectSound::PanTo3D(pan), 0.0f, -PinDirectSound::PanTo3D(1.0f),
                                 DS3D_IMMEDIATE);
      break;
    case SNDCFG_SND3DFRONTISFRONT:
      m_pDS3DBuffer->SetPosition(PinDirectSound::PanTo3D(pan), 0.0f,
                                 PinDirectSound::PanTo3D(front_rear_fade), DS3D_IMMEDIATE);
      break;
    case SNDCFG_SND3DFRONTISREAR:
      m_pDS3DBuffer->SetPosition(PinDirectSound::PanTo3D(pan), 0.0f,
                                 -PinDirectSound::PanTo3D(front_rear_fade), DS3D_IMMEDIATE);
    ...
    case SNDCFG_SND3D6CH:
      ...
      break;
    case SNDCFG_SND3DSSF:
      m_pDS3DBuffer->SetPosition(PinDirectSound::PanSSF(pan), 0.0f,
                                 PinDirectSound::FadeSSF(front_rear_fade), DS3D_IMMEDIATE);
      break;
    case SNDCFG_SND3D2CH:
    default:
      if (pan != 0.f)
