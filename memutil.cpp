...
void __cdecl MemLeakAlert(void* pUserData, size_t nBytes)
{
  ...
  sprintf_s(szT,
            "Memory leak at 0x%.8x.\nStack trace:\n0x%.8x\n0x%.8x\n0x%.8x\n0x%.8x\n0x%.8x\n\nWould "
            "you like to debug?  (Cancel skips reporting any other leaks)",
            (int)pUserData + PREEXTRA, st[0], st[1], st[2], st[3], st[4]);
  const int ans =
      g_pvp->MessageBox(szT, "Memory Leak", MB_YESNOCANCEL | MB_ICONWARNING | MB_DEFBUTTON2);
  switch (ans)
  {
    case IDYES:
    {
...
void* operator new(unsigned int cb)
{
  ...
  for (int i = 0; i < 5; i++)
  {
    StackWalk(IMAGE_FILE_MACHINE_I386, hProcess, hThread, &stFrame, &stCtx, DoMemoryRead,
              SwFunctionTableAccess, SwGetModuleBase, nullptr);
