...
{
  char tbuf[128];
  sprintf_s(tbuf, "Length: %.3f | Height: %.3f", m_vpinball->ConvertToUnit(m_d.m_length),
            m_vpinball->ConvertToUnit(m_d.m_height));
...
{
  m_d.m_length =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Gate", "Length", 100.f) : 100.f;
  m_d.m_height =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Gate", "Height", 50.f) : 50.f;
  m_d.m_rotation =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Gate", "Rotation", -90.f) : -90.f;
  m_d.m_showBracket =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Gate", "ShowBracket", true) : true;
  m_d.m_type = fromMouseClick
                   ? (GateType)LoadValueIntWithDefault("DefaultProps\\Gate", "GateType", GateWireW)
                   : GateWireW;
  m_d.m_collidable =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Gate", "Collidable", true) : true;
  m_d.m_angleMin =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Gate", "AngleMin", 0.f) : 0.f;
  m_d.m_angleMax = fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Gate", "AngleMax",
                                                              (float)(M_PI / 2.0))
                                  : (float)(M_PI / 2.0);
  m_d.m_visible =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Gate", "Visible", true) : true;
  m_d.m_tdr.m_TimerEnabled =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Gate", "TimerEnabled", false)
                     : false;
  m_d.m_tdr.m_TimerInterval =
      fromMouseClick ? LoadValueIntWithDefault("DefaultProps\\Gate", "TimerInterval", 100) : 100;
  ...
  m_d.m_twoWay =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Gate", "TwoWay", true) : true;
  m_d.m_reflectionEnabled =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Gate", "ReflectionEnabled", true)
                     : true;
}
...
{
  return RADTOANG(g_pplayer ? m_phitgate->m_gateMover.m_angleMax
                            : m_d.m_angleMax); // player active value
}
...
{
  m_d.m_elasticity =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Gate", "Elasticity", 0.3f) : 0.3f;
  m_d.m_friction =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Gate", "Friction", 0.02f) : 0.02f;
  m_d.m_damping = fromMouseClick
                      ? LoadValueFloatWithDefault("DefaultProps\\Gate", "AntiFriction", 0.985f)
                      : 0.985f;
  m_d.m_scatter =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Gate", "Scatter", 0.f) : 0.f;
  m_d.m_gravityfactor =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Gate", "GravityFactor", 0.25f)
                     : 0.25f;
}
...
void Gate::GetHitShapes(vector<HitObject*>& pvho)
{
  ...
  {
    m_plineseg =
        new LineSeg(rgv[0], rgv[1], height, height + (float)(2.0 * PHYS_SKIN)); //!! = ball diameter
...
void Gate::UpdateWire()
{
  ...
  fullMatrix.RotateXMatrix(m_d.m_twoWay ? m_phitgate->m_gateMover.m_angle
                                        : -m_phitgate->m_gateMover.m_angle);
  tempMat.RotateZMatrix(ANGTORAD(m_d.m_rotation));
  tempMat.Multiply(fullMatrix, fullMatrix);

  Matrix3D vertMatrix;
  tempMat.SetScaling(m_d.m_length, m_d.m_length,
                     m_d.m_length * m_ptable->m_BG_scalez[m_ptable->m_BG_current_set]);
  tempMat.Multiply(fullMatrix, vertMatrix);
  tempMat.SetTranslation(m_d.m_vCenter.x, m_d.m_vCenter.y,
                         m_d.m_height * m_ptable->m_BG_scalez[m_ptable->m_BG_current_set] +
                             m_baseHeight);
...
void Gate::RenderObject()
{
  ...
  if (m_d.m_showBracket)
    pd3dDevice->DrawIndexedPrimitiveVB(RenderDevice::TRIANGLELIST, MY_D3DFVF_NOTEX2_VERTEX,
                                       m_bracketVertexBuffer, 0, gateBracketNumVertices,
                                       m_bracketIndexBuffer, 0, gateBracketNumIndices);
  // render wire
  pd3dDevice->DrawIndexedPrimitiveVB(RenderDevice::TRIANGLELIST, MY_D3DFVF_NOTEX2_VERTEX,
                                     m_wireVertexBuffer, 0, m_numVertices, m_wireIndexBuffer, 0,
                                     m_numIndices);
...
void Gate::GenerateBracketMesh(Vertex3D_NoTex2* buf)
{
  ...
  for (int i = 0; i < gateBracketNumVertices; i++)
  {
    ...
    buf[i].z = vert.z * m_d.m_length * m_ptable->m_BG_scalez[m_ptable->m_BG_current_set] +
               (m_d.m_height * m_ptable->m_BG_scalez[m_ptable->m_BG_current_set] + m_baseHeight);
...
{
  SAFE_BUFFER_RELEASE(m_bracketIndexBuffer);
  m_bracketIndexBuffer = IndexBuffer::CreateAndFillIndexBuffer(gateBracketNumIndices,
                                                               gateBracketIndices, PRIMARY_DEVICE);

  SAFE_BUFFER_RELEASE(m_bracketVertexBuffer);
  VertexBuffer::CreateVertexBuffer(gateBracketNumVertices, 0, MY_D3DFVF_NOTEX2_VERTEX,
                                   &m_bracketVertexBuffer, PRIMARY_DEVICE);
  ...
  SAFE_BUFFER_RELEASE(m_wireIndexBuffer);
  m_wireIndexBuffer =
      IndexBuffer::CreateAndFillIndexBuffer(m_numIndices, m_indices, PRIMARY_DEVICE);

  SAFE_BUFFER_RELEASE(m_wireVertexBuffer);
  VertexBuffer::CreateVertexBuffer(m_numVertices, USAGE_DYNAMIC, MY_D3DFVF_NOTEX2_VERTEX,
                                   &m_wireVertexBuffer, PRIMARY_DEVICE);
...
HRESULT Gate::InitLoad(IStream* pstm,
                       PinTable* ptable,
                       int* pid,
                       int version,
  ...
  switch (id)
  {
    ...
    case FID(GATY):
    {
      ...
      if (m_d.m_type < GateWireW ||
          m_d.m_type >
              GateLongPlate) // for tables that were saved in the phase where m_type could've been undefined
        m_d.m_type = GateWireW;
      break;
    }
    case FID(VCEN):
...
STDMETHODIMP Gate::put_Collidable(VARIANT_BOOL newVal)
{
  if (g_pplayer)
  {
    ...
    if (m_phitgate->m_enabled)
      m_phitgate->m_gateMover.m_angleMin = 0;
  }
  else
  {
...
STDMETHODIMP Gate::Move(int dir,
                        float speed,
                        float angle) //move non-collidable gate, for graphic effects only
{
  if (g_pplayer)
  {
    m_phitgate->m_gateMover.m_hitDirection = false;
    ...
    if (!dir || angle != 0) // if no direction or non-zero angle
    {
      ...
      if (angle < m_d.m_angleMin)
        angle = m_d.m_angleMin;
      else if (angle > m_d.m_angleMax)
        angle = m_d.m_angleMax;
...
{
  *pVal = !g_pplayer ? m_d.m_damping
                     : powf(m_phitgate->m_gateMover.m_damping, (float)(1.0 / PHYS_FACTOR));
