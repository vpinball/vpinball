...
BaseTexture* BaseTexture::CreateFromFreeImage(FIBITMAP* dib)
{
  ...
  while (!success)
  {
    ...
    if ((pictureHeight > maxTexDim) || (pictureWidth > maxTexDim))
    {
      ...
      dibResized = FreeImage_Rescale(
          dib, newWidth, newHeight,
          FILTER_BILINEAR); //!! use a better filter in case scale ratio is pretty high?
    }
    ...
    const bool rgbf = (img_type == FIT_FLOAT) || (img_type == FIT_DOUBLE) ||
                      (img_type == FIT_RGBF) ||
                      (img_type == FIT_RGBAF); //(FreeImage_GetBPP(dibResized) > 32);
    const bool has_alpha = !!FreeImage_IsTransparent(dibResized);
    // already in correct format?
    if (((img_type == FIT_BITMAP) && (FreeImage_GetBPP(dibResized) == 32)) ||
        (img_type == FIT_RGBF))
    {
      dibConv = dibResized;
    }
    else
    {
      ...
      if (dibResized != dib)
      { // did we allocate a rescaled copy?
    ...
    try
    {
      tex = new BaseTexture(FreeImage_GetWidth(dibConv), FreeImage_GetHeight(dibConv),
                            rgbf ? RGB_FP : RGBA, rgbf ? false : has_alpha);
    ...
    catch (...)
    {
      ...
      { // did we allocate a copy from conversion?
...
static FIBITMAP* HBitmapToFreeImage(HBITMAP hbmp)
{
  ...
  /*const int Success =*/GetDIBits(dc, hbmp, 0, FreeImage_GetHeight(dib), FreeImage_GetBits(dib),
...
HRESULT Texture::SaveToStream(IStream* pstream, const PinTable* pt)
{
  ...
  if (!m_ppb)
  {
    ...
    // 32-bit picture
    LZWWriter lzwwriter(pstream, (int*)m_pdsBuffer->data(), m_width * 4, m_height,
                        m_pdsBuffer->pitch());
...
bool Texture::LoadFromMemory(BYTE* const data, const DWORD size)
{
  ...
  if (maxTexDim <= 0) // only use fast JPG path via stbi if no texture resize must be triggered
  {
    ...
    unsigned char* const __restrict stbi_data =
        stbi_load_from_memory(data, size, &x, &y, &channels_in_file, 4);
    if (stbi_data) // will only enter this path for JPG files
    {
      ...
      {
        tex = new BaseTexture(
            x, y, BaseTexture::RGBA,
            channels_in_file ==
                4); //!! stbi at the moment does not support alpha channel JPGs, so channels_in_file will be 3 or 1
      }
      // failed to get mem?
      catch (...)
      {
...
bool Texture::LoadToken(const int id, BiffReader* const pbr)
{
  switch (id)
  {
    ...
    case FID(BITS):
    {
      ...
      // 32-bit picture
      LZWReader lzwreader(pbr->m_pistream, (int*)m_pdsBuffer->data(), m_width * 4, m_height,
                          m_pdsBuffer->pitch());
      lzwreader.Decoder();
...
void Texture::CreateGDIVersion()
{
  ...
  if (g_pvp->m_table_played_via_command_line ||
      g_pvp
          ->m_table_played_via_SelectTableOnStart) // only do anything in here (and waste memory/time on it) if UI needed (i.e. if not just -Play via command line is triggered or selected on VPX start with the file popup!)
  {
  ...
  StretchDIBits(hdcNew, 0, 0, m_width, m_height, 0, 0, m_width, m_height,
...
{
  const HBITMAP hbm = (HBITMAP)LoadImage(g_pvp->theInstance, MAKEINTRESOURCE(id), IMAGE_BITMAP, 0,
