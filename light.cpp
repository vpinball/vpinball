...
void Light::SetDefaults(bool fromMouseClick)
{
  ...
  m_d.m_falloff =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Light", "Falloff", 50.f) : 50.f;
  m_d.m_falloff_power = fromMouseClick
                            ? LoadValueFloatWithDefault("DefaultProps\\Light", "FalloffPower", 2.0f)
                            : 2.0f;
  m_d.m_state = fromMouseClick ? (LightState)LoadValueIntWithDefault("DefaultProps\\Light",
  ...
  m_d.m_tdr.m_TimerEnabled =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Light", "TimerEnabled", false)
                     : false;
  m_d.m_tdr.m_TimerInterval =
      fromMouseClick ? LoadValueIntWithDefault("DefaultProps\\Light", "TimerInterval", 100) : 100;
  m_d.m_color = fromMouseClick
                    ? LoadValueIntWithDefault("DefaultProps\\Light", "Color", RGB(255, 255, 0))
                    : RGB(255, 255, 0);
  m_d.m_color2 = fromMouseClick ? LoadValueIntWithDefault("DefaultProps\\Light", "ColorFull",
                                                          RGB(255, 255, 255))
                                : RGB(255, 255, 255);
  ...
  m_d.m_intensity =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Light", "Intensity", 1.0f) : 1.0f;
  m_d.m_transmissionScale =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Light", "TransmissionScale", 0.5f)
                     : 0.f; // difference in defaults is intended
  ...
  m_d.m_fadeSpeedDown =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Light", "FadeSpeedDown", 0.2f)
                     : 0.2f;
  m_d.m_BulbLight =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Light", "Bulb", false) : false;
  m_d.m_imageMode =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Light", "ImageMode", false) : false;
  m_d.m_showBulbMesh = fromMouseClick
                           ? LoadValueBoolWithDefault("DefaultProps\\Light", "ShowBulbMesh", false)
                           : false;
  m_d.m_staticBulbMesh =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Light", "StaticBulbMesh", true)
                     : true;
  m_d.m_showReflectionOnBall =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Light", "ShowReflectionOnBall", true)
                     : true;
  m_d.m_meshRadius = fromMouseClick
                         ? LoadValueFloatWithDefault("DefaultProps\\Light", "ScaleBulbMesh", 20.0f)
                         : 20.0f;
  m_d.m_modulate_vs_add =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Light", "BulbModulateVsAdd", 0.9f)
                     : 0.9f;
  m_d.m_bulbHaloHeight =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Light", "BulbHaloHeight", 28.0f)
                     : 28.0f;
}
...
void Light::UIRenderPass1(Sur* const psur)
{
  ...
  psur->SetFillColor(
      m_ptable->RenderSolid() ? (((m_d.m_color & 0xFEFEFE) + (m_d.m_color2 & 0xFEFEFE)) / 2) : -1);
  psur->SetObject(this);
...
void Light::GetHitShapesDebug(vector<HitObject*>& pvho)
{
  ...
  switch (m_d.m_shape)
  {
    ...
    default:
    {
      Hit3DPoly* const pcircle =
          new Hit3DPoly(m_d.m_vCenter.x, m_d.m_vCenter.y, height, m_d.m_falloff, 32);
...
{
  return !m_backglass ? (m_d.m_depthBias + viewDir.x * m_d.m_vCenter.x +
                         viewDir.y * m_d.m_vCenter.y + viewDir.z * m_surfaceHeight)
                      : 0.f;
}
...
void Light::RenderBulbMesh()
{
  ...
  pd3dDevice->DrawIndexedPrimitiveVB(RenderDevice::TRIANGLELIST, MY_D3DFVF_NOTEX2_VERTEX,
                                     m_bulbSocketVBuffer, 0, bulbSocketNumVertices,
                                     m_bulbSocketIndexBuffer, 0, bulbSocketNumFaces);
  ...
  pd3dDevice->basicShader->Begin(0);
  pd3dDevice->DrawIndexedPrimitiveVB(RenderDevice::TRIANGLELIST, MY_D3DFVF_NOTEX2_VERTEX,
                                     m_bulbLightVBuffer, 0, bulbLightNumVertices,
                                     m_bulbLightIndexBuffer, 0, bulbLightNumFaces);
  pd3dDevice->basicShader->End();
}
...
void Light::RenderDynamic()
{
  ...
  if (m_d.m_BulbLight ||
      (!m_d.m_BulbLight && (m_surfaceTexture == (offTexel = m_ptable->GetImage(m_d.m_szImage))) &&
       (offTexel != nullptr) && !m_backglass &&
  ...
  const vec4 center_range(centerHUD.x, centerHUD.y, !m_backglass ? m_surfaceHeight + 0.05f : 0.0f,
                          1.0f / max(m_d.m_falloff, 0.1f));
  ...
  if (!m_d.m_BulbLight)
  {
    ...
    if (offTexel != nullptr)
    {
      ...
      pd3dDevice->classicLightShader->SetTechniqueMetal(SHADER_TECHNIQUE_light_with_texture,
  ...
  else
  {
    ...
    pd3dDevice->SetRenderState(
        RenderDevice::DESTBLEND,
        RenderDevice::
            INVSRC_COLOR); // but also modulate the light first with the underlying elements by (1+lightcontribution, e.g. a very crude approximation of real lighting)
    ...
    if (m_d.m_showBulbMesh) // blend bulb mesh hull additive over "normal" bulb to approximate the emission directly reaching the camera
    {
      ...
      pd3dDevice->lightShader->SetFloat(
          ...
          0.00001f); // additive, but avoid full 0, as it disables the blend
    ...
    pd3dDevice->lightShader->SetFloat(
        ...
        (g_pplayer->m_current_renderstage == 0)
            ? min(max(m_d.m_modulate_vs_add, 0.00001f), 0.9999f)
            : 0.00001f); // avoid 0, as it disables the blend and avoid 1 as it looks not good with day->night changes // in the separate bulb light render stage only enable additive
  }
  ...
  pd3dDevice->DrawIndexedPrimitiveVB(RenderDevice::TRIANGLELIST,
                                     (!m_backglass) ? MY_D3DFVF_NOTEX2_VERTEX
                                                    : MY_D3DTRANSFORMED_NOTEX2_VERTEX,
                                     m_customMoverVBuffer, 0, m_customMoverVertexNum,
  ...
  if (!m_d.m_BulbLight &&
      ...
      !m_backglass) // See above: // TOTAN and Flintstones inserts break if alpha blending is disabled here.
  {
...
void Light::PrepareMoversCustom()
{
  ...
  IndexBuffer::CreateIndexBuffer(m_customMoverIndexNum, 0, IndexBuffer::FMT_INDEX16,
                                 &m_customMoverIBuffer,
                                 m_backglass ? SECONDARY_DEVICE : PRIMARY_DEVICE);
  ...
  const DWORD vertexType =
      (!m_backglass) ? MY_D3DFVF_NOTEX2_VERTEX : MY_D3DTRANSFORMED_NOTEX2_VERTEX;
  VertexBuffer::CreateVertexBuffer(m_customMoverVertexNum, 0, vertexType, &m_customMoverVBuffer,
                                   m_backglass ? SECONDARY_DEVICE : PRIMARY_DEVICE);
...
void Light::UpdateCustomMoverVBuffer()
{
  ...
  for (unsigned int t = 0; t < m_customMoverVertexNum; t++)
  {
    ...
    else
    {
      ...
      buf[t].nx =
          1.f; //!! for backglass we use no vertex shader (D3DDECLUSAGE_POSITIONT), thus w component is actually mapped to nx, and that must be 1
      buf[t].ny =
          (pv0->x *
           (float)(1.0 /
                   EDITOR_BG_WIDTH)); //!! abuses normal to pass tex coord via TEXCOORD1.xy to shader for non-bulbs :/
...
void Light::RenderSetup()
{
  ...
  {
    SAFE_BUFFER_RELEASE(m_bulbLightIndexBuffer);
    m_bulbLightIndexBuffer = IndexBuffer::CreateAndFillIndexBuffer(
        bulbLightNumFaces, bulbLightIndices, m_backglass ? SECONDARY_DEVICE : PRIMARY_DEVICE);

    SAFE_BUFFER_RELEASE(m_bulbLightVBuffer);
    VertexBuffer::CreateVertexBuffer(bulbLightNumVertices, 0, MY_D3DFVF_NOTEX2_VERTEX,
                                     &m_bulbLightVBuffer,
                                     m_backglass ? SECONDARY_DEVICE : PRIMARY_DEVICE);
    ...
    for (unsigned int i = 0; i < bulbLightNumVertices; i++)
    {
      ...
      buf[i].z =
          bulbLight[i].z * m_d.m_meshRadius * m_ptable->m_BG_scalez[m_ptable->m_BG_current_set] +
          m_surfaceHeight;
    ...
    SAFE_BUFFER_RELEASE(m_bulbSocketIndexBuffer);
    m_bulbSocketIndexBuffer = IndexBuffer::CreateAndFillIndexBuffer(
        bulbSocketNumFaces, bulbSocketIndices, m_backglass ? SECONDARY_DEVICE : PRIMARY_DEVICE);

    SAFE_BUFFER_RELEASE(m_bulbSocketVBuffer);
    VertexBuffer::CreateVertexBuffer(bulbSocketNumVertices, 0, MY_D3DFVF_NOTEX2_VERTEX,
                                     &m_bulbSocketVBuffer,
                                     m_backglass ? SECONDARY_DEVICE : PRIMARY_DEVICE);
    ...
    for (unsigned int i = 0; i < bulbSocketNumVertices; i++)
    {
      ...
      buf[i].z =
          bulbSocket[i].z * m_d.m_meshRadius * m_ptable->m_BG_scalez[m_ptable->m_BG_current_set] +
          m_surfaceHeight;
...
HRESULT Light::InitLoad(IStream* pstm,
                        PinTable* ptable,
                        int* pid,
                        int version,
...
{
  menu.EnableMenuItem(ID_WALLMENU_FLIP,
                      MF_BYCOMMAND | ((m_d.m_shape != ShapeCustom) ? MF_GRAYED : MF_ENABLED));
  menu.EnableMenuItem(ID_WALLMENU_MIRROR,
                      MF_BYCOMMAND | ((m_d.m_shape != ShapeCustom) ? MF_GRAYED : MF_ENABLED));
  menu.EnableMenuItem(ID_WALLMENU_ROTATE,
                      MF_BYCOMMAND | ((m_d.m_shape != ShapeCustom) ? MF_GRAYED : MF_ENABLED));
  menu.EnableMenuItem(ID_WALLMENU_SCALE,
                      MF_BYCOMMAND | ((m_d.m_shape != ShapeCustom) ? MF_GRAYED : MF_ENABLED));
  menu.EnableMenuItem(ID_WALLMENU_ADDPOINT,
                      MF_BYCOMMAND | ((m_d.m_shape != ShapeCustom) ? MF_GRAYED : MF_ENABLED));
}
...
void Light::Scale(const float scalex,
                  const float scaley,
...
void Light::setInPlayState(const LightState newVal)
{
  if (newVal != m_inPlayState) // state changed???
  {
    ...
    if (g_pplayer)
    {
      ...
      {
        m_timenextblink =
            g_pplayer->m_time_msec; // Start pattern right away // + m_d.m_blinkinterval;
