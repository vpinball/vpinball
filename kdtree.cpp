...
void HitKD::Init(vector<HitObject*>& vho)
{
  ...
  if (m_num_items > m_max_items)
  {
    ...
    /* NB:
      ...
      */
    m_nodes.resize((m_num_items * 2 + 1) &
...
void HitKDNode::CreateNextLevel(const unsigned int level, unsigned int level_empty)
{
  ...
  const Vertex3Ds vdiag(m_rectbounds.right - m_rectbounds.left,
                        ...
                        m_rectbounds.zhigh - m_rectbounds.zlow);
  ...
  const Vertex3Ds vcenter((m_rectbounds.left + m_rectbounds.right) * 0.5f,
                          (m_rectbounds.top + m_rectbounds.bottom) * 0.5f,
                          (m_rectbounds.zlow + m_rectbounds.zhigh) * 0.5f);
  if (axis == 0)
  {
  ...
  memcpy(&m_hitoct->m_org_idx[m_children[0].m_start], &m_hitoct->tmp[m_children[0].m_start],
         m_children[0].m_items * sizeof(unsigned int));
  memcpy(&m_hitoct->m_org_idx[m_children[1].m_start], &m_hitoct->tmp[m_children[1].m_start],
         m_children[1].m_items * sizeof(unsigned int));
...
void HitKDNode::HitTestBallSse(const Ball* const pball, CollisionEvent& coll) const
{
  ...
  do
  {
    ...
    for (unsigned int i = start; i != end; i += dt)
    {
      ...
      __m128 ex = _mm_add_ps(_mm_max_ps(_mm_sub_ps(pL[i], posx), zero),
                             _mm_max_ps(_mm_sub_ps(posx, pR[i]), zero));
      __m128 ey = _mm_add_ps(_mm_max_ps(_mm_sub_ps(pT[i], posy), zero),
                             _mm_max_ps(_mm_sub_ps(posy, pB[i]), zero));
      __m128 ez = _mm_add_ps(_mm_max_ps(_mm_sub_ps(pZl[i], posz), zero),
                             _mm_max_ps(_mm_sub_ps(posz, pZh[i]), zero));
      ex = _mm_mul_ps(ex, ex);
      ey = _mm_mul_ps(ey, ey);
      ez = _mm_mul_ps(ez, ez);
...
void HitKDNode::HitTestXRay(const Ball* const pball,
