...
void PUPMediaPlayer::Play(const string& szFilename)
{
   ...
   SetThreadName(m_thread, "PUPMediaPlayer.Run("s.append(m_szFilename).append(")"));
}
...
void PUPMediaPlayer::Run()
{
   ...
   while (true)
   {
      ...
      if (rfRet == AVERROR_EOF)
      {
         ...
         // FIXME this is not totally clean as the frame buffer contains old frame that will be seen as new. This may cause (little) glitches.
         // A better way would be to always advance in time and use a modulo against the video length.
         m_startTimestamp = SDL_GetTicks();
         continue;
      }
      else if (rfRet < 0)
...
void PUPMediaPlayer::HandleVideoFrame(AVFrame* frame)
{
   ...
   // Wait for the buffer slot to be outdated (do not overwrite a frame that is waiting to be displayed)
