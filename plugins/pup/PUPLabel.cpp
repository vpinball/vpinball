   ...
   , m_szName(szName)
{
   if (!szFont.empty())
   {
      TTF_Font* pFont = manager->GetFont(szFont);
      if (!pFont)
      {
...

   {
      ...
      if (m_szCaption != szText)
      {
         ...
         if (szExt == "gif" || szExt == "png" || szExt == "apng" || szExt == "bmp" || szExt == "jpg")
         {
            ...
            PUPPlaylist* pPlaylist = m_pScreen->GetPlaylist(fs_path.parent_path().string());
            if (pPlaylist)
            {
...
void PUPLabel::SetSpecial(const string& szSpecial)
{
   ...
   // FIXME will fail with "{'mt':2,'}" (appends in TNA)
...
void PUPLabel::Render(VPXRenderContext2D* const ctx, SDL_Rect& rect, int pagenum)
{
   ...
   // FIXME move to an update thread
   UpdateLabelTexture(rect);
   ...
   if (m_pAnimation)
   {
      if (++m_frame >= m_pAnimation->count)
         m_frame = 0;
      SDL_LockSurface(m_pAnimation->frames[m_frame]);
      UpdateTexture(&m_pTexture, m_pAnimation->frames[m_frame]->w, m_pAnimation->frames[m_frame]->h, VPXTextureFormat::VPXTEXFMT_sRGBA, static_cast<uint8_t*>(m_pAnimation->frames[m_frame]->pixels));
      SDL_UnlockSurface(m_pAnimation->frames[m_frame]);
   }
   ...
   {
      width = (m_width / 100.0f) * static_cast<float>(rect.w);
      height = (m_height / 100.0f) * static_cast<float>(rect.h);
   }
   ...
   GetTextureInfo(m_pTexture, &texWidth, &texHeight);
   ctx->DrawImage(ctx, m_pTexture, 1.f, 1.f, 1.f, 1.f, 0.f, 0.f, static_cast<float>(texWidth), static_cast<float>(texHeight), dest.x, dest.y, dest.w, dest.h);
   // FIXME port SDL_RenderTextureRotated(m_pTexture, NULL, &dest, -m_angle / 10.0, &center, SDL_FLIP_NONE);
}

void PUPLabel::UpdateLabelTexture(SDL_Rect& rect)
{
   if (!m_dirty)
      return;
   m_dirty = false;
   ...
   if (!m_szPath.empty())
   {
      ...
      {
         SDL_Surface* image = IMG_Load(m_szPath.c_str());
         if (image)
         {
         ...
         {
            LOGE("Unable to load image: %s", m_szPath.c_str());
         }
      }
      else if (m_type == PUP_LABEL_TYPE_GIF)
      {
         ...
         m_frame = 0;
         if (m_pAnimation)
            m_pTexture = CreateTexture(m_pAnimation->frames[m_frame]);
         else
            LOGE("Unable to load animation: %s", m_szPath.c_str());
      }
      return;
   }

   float height = ((m_size / 100.0f) * static_cast<float>(rect.h));
   ...
   if (pMergedSurface)
   {
      m_pTexture = CreateTexture(pMergedSurface);
      if (m_pTexture)
      {
         m_width = static_cast<float>(pMergedSurface->w);
         m_height = static_cast<float>(pMergedSurface->h);
      }
