...
static void OnControllerGameStart(const unsigned int eventId, void* userData, void* msgData)
{
   ...
   else if (auto path3 = find_case_insensitive_file_path(tablePath.parent_path() / "pinmame" / "altcolor" / gameId / palFile); !path3.empty())
   {
      palPath = path3;
      if (auto path4 = find_case_insensitive_file_path(tablePath.parent_path() / "pinmame" / "altcolor" / gameId / vniFile); !path4.empty())
         vniPath = path4;
   }
   else if (!vniBasePath.empty())
   {
      if (auto path5 = find_case_insensitive_file_path(vniBasePath / gameId / palFile); !path5.empty())
      {
         palPath = path5;
         if (auto path6 = find_case_insensitive_file_path(vniBasePath / gameId / vniFile); !path6.empty())
            vniPath = path6;
      }
   }
