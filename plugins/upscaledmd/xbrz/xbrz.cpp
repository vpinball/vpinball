...
inline
float distYCbCr(uint32_t pix1, uint32_t pix2, float /*testAttribute*/)
{
    ...
    const float r_diff = (float)(static_cast<int>(getRed  (pix1)) - getRed  (pix2)); //defer division by 255 to after matrix multiplication
    ...
    return sqrtf(square(y) + square(c_b) + square(c_r));
}


inline
float distYCbCrBuffered(uint32_t pix1, uint32_t pix2, float /*testAttribute*/)
{
...
{
    static float dist(uint32_t pix1, uint32_t pix2, float testAttribute)
    {
...
struct ColorDistanceARGB
{
    static float dist(uint32_t pix1, uint32_t pix2, float testAttribute)
    {
        ...
        /*  Requirements for a color distance handling alpha channel: with a1, a2 in [0, 1]
            ...
            alternative? sqrtf(a1 * a2 * square(distYCbCrBuffered(pix1, pix2)) + square(255 * (a1 - a2)));   */

        //=> following code is 15% faster:
        const float d = distYCbCrBuffered(pix1, pix2, testAttribute);
        if (a1 < a2)
...
struct ColorDistanceUnbufferedARGB
{
    static float dist(uint32_t pix1, uint32_t pix2, float testAttribute)
    {
        ...
        const float d = distYCbCr(pix1, pix2, testAttribute);
        if (a1 < a2)
            return a1 * d + 255.f * (a2 - a1);
        else
            return a2 * d + 255.f * (a1 - a2);
...
bool xbrz::equalColorTest2(uint32_t col1, uint32_t col2, ColorFormat colFmt, float equalColorTolerance, float testAttribute)
{
    switch (colFmt)
    {
        case ColorFormat::rgb:
