...
// minor additional tweaks for VPX plugin: replace all doubles by float, constexpr's, and pre-alloc the vector
...
float distYCbCr(uint32_t pix1, uint32_t pix2, float lumaWeight)
{
    ...
    const float r_diff = (float)(static_cast<int>(getRed  (pix1)) - getRed  (pix2)); //we may delay division by 255 to after matrix multiplication
    const float g_diff = (float)(static_cast<int>(getGreen(pix1)) - getGreen(pix2)); //
    const float b_diff = (float)(static_cast<int>(getBlue (pix1)) - getBlue (pix2)); //substraction for int is noticeable faster than for double!
...
inline
float distYCbCrBuffered(uint32_t pix1, uint32_t pix2)
{
    ...
    static const std::vector<float> diffToDist = []
    {
        ...
        {
            const float r_diff = static_cast<float>(getByte<2>(i)) * 2.f;
            const float g_diff = static_cast<float>(getByte<1>(i)) * 2.f;
            const float b_diff = static_cast<float>(getByte<0>(i)) * 2.f;
            ...
            tmp[i] = sqrtf(square(y) + square(c_b) + square(c_r));
        }
        return tmp;
    }();
    ...
    constexpr int debugPixelX = -1;
    constexpr int debugPixelY = 58;
...
BlendResult preProcessCorners(const Kernel_4x4& ker, const xbrz::ScalerCfg& cfg) //result: F, G, J, K corners of "GradientType"
{
    ...
    const float jg = dist(ker.i, ker.f) + dist(ker.f, ker.c) + dist(ker.n, ker.k) + dist(ker.k, ker.h) + cfg.centerDirectionBias * dist(ker.j, ker.g);
    const float fk = dist(ker.e, ker.j) + dist(ker.j, ker.o) + dist(ker.b, ker.g) + dist(ker.g, ker.l) + cfg.centerDirectionBias * dist(ker.f, ker.k);
...
FORCE_INLINE BlendType getTopR   (unsigned char b) { return static_cast<BlendType>(0x3 & (b >> 2)); }
FORCE_INLINE BlendType getBottomR(unsigned char b) { return static_cast<BlendType>(0x3 & (b >> 4)); }
FORCE_INLINE BlendType getBottomL(unsigned char b) { return static_cast<BlendType>(0x3 & (b >> 6)); }

FORCE_INLINE void clearAddTopL(unsigned char& b, BlendType bt) { b = static_cast<unsigned char>(bt); }
FORCE_INLINE void addTopR     (unsigned char& b, BlendType bt) { b |= (bt << 2); } //buffer is assumed to be initialized before preprocessing!
FORCE_INLINE void addBottomR  (unsigned char& b, BlendType bt) { b |= (bt << 4); } //e.g. via clearAddTopL()
FORCE_INLINE void addBottomL  (unsigned char& b, BlendType bt) { b |= (bt << 6); } //

FORCE_INLINE bool blendingNeeded(unsigned char b)
{
    static_assert(BLEND_NONE == 0);
    return b != 0;
}

template <RotationDegree rotDeg> FORCE_INLINE
unsigned char rotateBlendInfo(unsigned char b) { return b; }
template <> FORCE_INLINE unsigned char rotateBlendInfo<ROT_90 >(unsigned char b) { return ((b << 2) | (b >> 6)) & 0xff; }
template <> FORCE_INLINE unsigned char rotateBlendInfo<ROT_180>(unsigned char b) { return ((b << 4) | (b >> 4)) & 0xff; }
template <> FORCE_INLINE unsigned char rotateBlendInfo<ROT_270>(unsigned char b) { return ((b << 6) | (b >> 2)) & 0xff; }
...
struct ColorDistanceARGB
{
    static float dist(uint32_t pix1, uint32_t pix2, float luminanceWeight)
    {
        ...
        */
        ...
        //return std::min(a1, a2) * distYCbCrBuffered(pix1, pix2) + 255.f * abs(a1 - a2);
        //=> following code is 15% faster:
        const float d = distYCbCrBuffered(pix1, pix2);
        if (a1 < a2)
            return a1 * d + 255.f * (a2 - a1);
        else
            return a2 * d + 255.f * (a1 - a2);

        //alternative? return sqrtf(a1 * a2 * square(distYCbCrBuffered(pix1, pix2)) + square(255.f * (a1 - a2)));
    }
};
...
struct ColorDistanceUnbufferedARGB
{
    static float dist(uint32_t pix1, uint32_t pix2, float luminanceWeight)
    {
        ...
        const float d = distYCbCr(pix1, pix2, luminanceWeight);
        if (a1 < a2)
            return a1 * d + 255.f * (a2 - a1);
        else
            return a2 * d + 255.f * (a1 - a2);
...
void bilinearScaleCpu(const uint32_t* src, int srcWidth, int srcHeight,
                      /**/  uint32_t* trg, int trgWidth, int trgHeight)
{
    constexpr int TASK_GRANULARITY = 16;
                      ...
                      /**/  uint32_t* trg, int trgWidth, int trgHeight)
{
    ...
    parallel_for_each(trgView.extent, [=](index<2> idx) restrict(amp) //throw ?
    {
        ...
        const int x = idx[1];
        //Perf notes:
        //    -> float-based calculation is (almost) 2x as fast as double!
