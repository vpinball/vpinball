...
void DispReel::SetDefaults(bool fromMouseClick)
{
  ...
  m_d.m_useImageGrid = fromMouseClick
                           ...
                           : false;
  m_d.m_visible =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\EMReel", "Visible", true) : true;
  m_d.m_imagesPerGridRow =
      fromMouseClick ? LoadValueIntWithDefault("DefaultProps\\EMReel", "ImagesPerRow", 1) : 1;
  m_d.m_transparent = fromMouseClick
                          ? LoadValueBoolWithDefault("DefaultProps\\EMReel", "Transparent", false)
                          : false;
  m_d.m_reelcount =
      fromMouseClick ? LoadValueIntWithDefault("DefaultProps\\EMReel", "ReelCount", 5) : 5;
  m_d.m_width =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\EMReel", "Width", 30.0f) : 30.0f;
  m_d.m_height =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\EMReel", "Height", 40.0f) : 40.0f;
  m_d.m_reelspacing = fromMouseClick
                          ? LoadValueFloatWithDefault("DefaultProps\\EMReel", "ReelSpacing", 4.0f)
                          : 4.0f;
  m_d.m_motorsteps = fromMouseClick
                         ? (int)LoadValueFloatWithDefault("DefaultProps\\EMReel", "MotorSteps", 2.f)
                         : 2;
  m_d.m_digitrange =
      fromMouseClick ? LoadValueIntWithDefault("DefaultProps\\EMReel", "DigitRange", 9) : 9;
  m_d.m_updateinterval =
      fromMouseClick ? LoadValueIntWithDefault("DefaultProps\\EMReel", "UpdateInterval", 50) : 50;
  m_d.m_backcolor =
      fromMouseClick ? LoadValueIntWithDefault("DefaultProps\\EMReel", "BackColor", RGB(64, 64, 64))
                     : RGB(64, 64, 64);
  m_d.m_tdr.m_TimerEnabled =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\EMReel", "TimerEnabled", false)
                     : false;
  m_d.m_tdr.m_TimerInterval =
      fromMouseClick ? LoadValueIntWithDefault("DefaultProps\\EMReel", "TimerInterval", 100) : 100;
}
...
void DispReel::RenderDynamic()
{
  ...
  pd3dDevice->SetRenderState(RenderDevice::ALPHAFUNC,
                             RenderDevice::Z_GREATER); //!! still necessary?
  ...
  for (int r = 0; r < m_d.m_reelcount; ++r) //!! optimize by doing all draws in a single one
  {
    ...
    float Verts[4 * 5] = {1.0f, 1.0f, 0.0f, u1, v1, 0.0f, 1.0f, 0.0f, u0, v1,
                          1.0f, 0.0f, 0.0f, u1, v0, 0.0f, 0.0f, 0.0f, u0, v0};
...
HRESULT DispReel::InitLoad(IStream* pstm,
                           PinTable* ptable,
                           int* pid,
                           int version,
...
bool DispReel::LoadToken(const int id, BiffReader* const pbr)
{
  switch (id)
  {
    case FID(PIID):
      ...
      break;
    case FID(VER1):
      pbr->GetVector2(m_d.m_v1);
      break;
    case FID(VER2):
      pbr->GetVector2(m_d.m_v2);
      break;
    case FID(WDTH):
...
{
  const float height =
      m_d.m_height + m_d.m_reelspacing + m_d.m_reelspacing; // spacing also includes edges
