...
bool ObjLoader::Load(const string& filename, const bool flipTv, const bool convertToLeftHanded)
{
    ...
    else if (strcmp(lineHeader, "f") == 0)
    {
      ...
      do
      {
        ...
        if (matches == 3)
        {
          vi.v--;
          vi.t--;
          vi.n--; // convert to 0-based indices
  ...
  m_verts.reserve(
      m_tmpFaces.size() *
      3); //!! reserve more than needed, but this is necessary to guarantee to not have any re-allocs below, so that all addresses to elements in the vector remain the same during the loop!
  ...
  for (size_t i = 0; i < m_tmpFaces.size(); i++)
  {
    ...
    std::unordered_set<std::pair<const Vertex3D_NoTex2*, const unsigned int>,
                       Vertex3D_NoTex2IdxHashFunctor, Vertex3D_NoTex2IdxComparator>::const_iterator
        idx = m_tmpCombined.find(std::pair<const Vertex3D_NoTex2*, const unsigned int>(
            &tmp, 0)); // idx is ignored when searching via find
    if (idx == m_tmpCombined.end())
    {
      m_verts.push_back(tmp);
      m_tmpCombined.emplace(std::pair<const Vertex3D_NoTex2*, const unsigned int>(
          &m_verts.back(), (unsigned int)(m_verts.size() - 1)));
...
void ObjLoader::WriteFaceInfo(const std::vector<WORD>& faces)
{
  ...
  {
    fprintf_s(m_fHandle, "f %u/%u/%u %u/%u/%u %u/%u/%u\n", faces[i + 2] + 1 + m_faceIndexOffset,
              faces[i + 2] + 1 + m_faceIndexOffset, faces[i + 2] + 1 + m_faceIndexOffset,
              faces[i + 1] + 1 + m_faceIndexOffset, faces[i + 1] + 1 + m_faceIndexOffset,
              faces[i + 1] + 1 + m_faceIndexOffset, faces[i] + 1 + m_faceIndexOffset,
              faces[i] + 1 + m_faceIndexOffset, faces[i] + 1 + m_faceIndexOffset);
  }
}
...
void ObjLoader::WriteMaterial(const string& texelName,
                              const string& texelFilename,
