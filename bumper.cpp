...
{
  m_d.m_radius =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Bumper", "Radius", 45.f) : 45.f;

  SetDefaultPhysics(fromMouseClick);

  m_d.m_heightScale = fromMouseClick
                          ? LoadValueFloatWithDefault("DefaultProps\\Bumper", "HeightScale", 90.0f)
                          : 90.0f;
  m_d.m_ringSpeed =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Bumper", "RingSpeed", 0.5f) : 0.5f;
  m_d.m_orientation = fromMouseClick
                          ? LoadValueFloatWithDefault("DefaultProps\\Bumper", "Orientation", 0.0f)
                          : 0.0f;
  m_d.m_threshold =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Bumper", "Threshold", 1.f) : 1.f;
  ...
  m_d.m_tdr.m_TimerInterval =
      fromMouseClick ? LoadValueIntWithDefault("DefaultProps\\Bumper", "TimerInterval", 100) : 100;
  m_d.m_capVisible =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Bumper", "CapVisible", true) : true;
  m_d.m_baseVisible =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Bumper", "BaseVisible", true) : true;
  m_d.m_ringVisible =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Bumper", "RingVisible", true) : true;
  m_d.m_skirtVisible = fromMouseClick
                           ? LoadValueBoolWithDefault("DefaultProps\\Bumper", "SkirtVisible", true)
                           : true;
  m_d.m_reflectionEnabled =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Bumper", "ReflectionEnabled", true)
                     : true;
  m_d.m_hitEvent =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Bumper", "HasHitEvent", true) : true;
  m_d.m_collidable =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Bumper", "Collidable", true) : true;
...
void Bumper::GetHitShapes(vector<HitObject*>& pvho)
{
  ...
  BumperHitCircle* const phitcircle =
      new BumperHitCircle(m_d.m_vCenter, m_d.m_radius, height, height + m_d.m_heightScale);
...
void Bumper::GetHitShapesDebug(vector<HitObject*>& pvho)
{
  ...
  Hit3DPoly* const pcircle =
      new Hit3DPoly(m_d.m_vCenter.x, m_d.m_vCenter.y, height + m_d.m_heightScale, m_d.m_radius, 32);
...
void Bumper::RenderBase(const Material* const baseMaterial)
{
  ...
  pd3dDevice->DrawIndexedPrimitiveVB(RenderDevice::TRIANGLELIST, MY_D3DFVF_NOTEX2_VERTEX,
                                     m_baseVertexBuffer, 0, bumperBaseNumVertices,
                                     m_baseIndexBuffer, 0, bumperBaseNumIndices);
...
void Bumper::RenderSocket(const Material* const socketMaterial)
{
  ...
  pd3dDevice->basicShader->Begin(0);
  pd3dDevice->DrawIndexedPrimitiveVB(RenderDevice::TRIANGLELIST, MY_D3DFVF_NOTEX2_VERTEX,
                                     m_socketVertexBuffer, 0, bumperSocketNumVertices,
                                     m_socketIndexBuffer, 0, bumperSocketNumIndices);
  pd3dDevice->basicShader->End();
}
...
void Bumper::RenderCap(const Material* const capMaterial)
{
  ...
  pd3dDevice->basicShader->Begin(0);
  pd3dDevice->DrawIndexedPrimitiveVB(RenderDevice::TRIANGLELIST, MY_D3DFVF_NOTEX2_VERTEX,
                                     m_capVertexBuffer, 0, bumperCapNumVertices, m_capIndexBuffer,
                                     0, bumperCapNumIndices);
  pd3dDevice->basicShader->End();
}
...
void Bumper::UpdateSkirt(const bool doCalculation)
{
  ...
  for (int i = 0; i < bumperSocketNumVertices; i++)
  {
    ...
    buf[i].z = vert.z * (m_d.m_heightScale * m_ptable->m_BG_scalez[m_ptable->m_BG_current_set]) +
               (m_baseHeight + 5.0f);
...
void Bumper::RenderDynamic()
{
  ...
  {
    const float limit =
        m_d.m_ringDropOffset +
        (m_d.m_heightScale * 0.5f) * m_ptable->m_BG_scalez[m_ptable->m_BG_current_set];
    ...
    pd3dDevice->basicShader->Begin(0);
    pd3dDevice->DrawIndexedPrimitiveVB(RenderDevice::TRIANGLELIST, MY_D3DFVF_NOTEX2_VERTEX,
                                       m_ringVertexBuffer, 0, bumperRingNumVertices,
                                       m_ringIndexBuffer, 0, bumperRingNumIndices);
    pd3dDevice->basicShader->End();
  }
...
void Bumper::GenerateCapMesh(Vertex3D_NoTex2* buf)
{
  ...
  for (int i = 0; i < bumperCapNumVertices; i++)
  {
    ...
    buf[i].x = vert.x * scalexy + m_d.m_vCenter.x;
    buf[i].y = vert.y * scalexy + m_d.m_vCenter.y;
    buf[i].z = (vert.z * m_d.m_heightScale + m_d.m_heightScale) *
                   m_ptable->m_BG_scalez[m_ptable->m_BG_current_set] +
               m_baseHeight;
...
void Bumper::RenderSetup()
{
  ...
  if (m_d.m_baseVisible)
  {
    ...
    SAFE_BUFFER_RELEASE(m_baseIndexBuffer);
    m_baseIndexBuffer = IndexBuffer::CreateAndFillIndexBuffer(bumperBaseNumIndices,
                                                              bumperBaseIndices, PRIMARY_DEVICE);

    SAFE_BUFFER_RELEASE(m_baseVertexBuffer);
    VertexBuffer::CreateVertexBuffer(bumperBaseNumVertices, 0, MY_D3DFVF_NOTEX2_VERTEX,
                                     &m_baseVertexBuffer, PRIMARY_DEVICE);
  ...
  if (m_d.m_skirtVisible)
  {
    ...
    SAFE_BUFFER_RELEASE(m_socketIndexBuffer);
    m_socketIndexBuffer = IndexBuffer::CreateAndFillIndexBuffer(
        bumperSocketNumIndices, bumperSocketIndices, PRIMARY_DEVICE);

    SAFE_BUFFER_RELEASE(m_socketVertexBuffer);
    VertexBuffer::CreateVertexBuffer(bumperSocketNumVertices, 0, MY_D3DFVF_NOTEX2_VERTEX,
                                     &m_socketVertexBuffer, PRIMARY_DEVICE);
  ...
  if (m_d.m_ringVisible)
  {
    ...
    SAFE_BUFFER_RELEASE(m_ringIndexBuffer);
    m_ringIndexBuffer = IndexBuffer::CreateAndFillIndexBuffer(bumperRingNumIndices,
                                                              bumperRingIndices, PRIMARY_DEVICE);

    SAFE_BUFFER_RELEASE(m_ringVertexBuffer);
    VertexBuffer::CreateVertexBuffer(bumperRingNumVertices, USAGE_DYNAMIC, MY_D3DFVF_NOTEX2_VERTEX,
                                     &m_ringVertexBuffer, PRIMARY_DEVICE);
  ...
  if (m_d.m_capVisible)
  {
    ...
    SAFE_BUFFER_RELEASE(m_capVertexBuffer);
    VertexBuffer::CreateVertexBuffer(bumperCapNumVertices, 0, MY_D3DFVF_NOTEX2_VERTEX,
                                     &m_capVertexBuffer, PRIMARY_DEVICE);
...
HRESULT Bumper::InitLoad(IStream* pstm,
                         PinTable* ptable,
                         int* pid,
                         int version,
...
{
  m_d.m_force =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Bumper", "Force", 15) : 15;
  m_d.m_scatter =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Bumper", "Scatter", 0) : 0;
}
