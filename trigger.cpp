...
{
  m_d.m_radius =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Trigger", "Radius", 25.0f) : 25.0f;
  m_d.m_rotation =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Trigger", "Rotation", 0.f) : 0.f;
  m_d.m_wireThickness =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Trigger", "WireThickness", 0.f)
                     : 0.f;
  m_d.m_scaleX =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Trigger", "ScaleX", 1.f) : 1.f;
  m_d.m_scaleY =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Trigger", "ScaleY", 1.f) : 1.f;
  m_d.m_tdr.m_TimerEnabled =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Trigger", "TimerEnabled", false)
                     : false;
  m_d.m_tdr.m_TimerInterval =
      fromMouseClick ? LoadValueIntWithDefault("DefaultProps\\Trigger", "TimerInterval", 100) : 100;
  m_d.m_enabled =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Trigger", "Enabled", true) : true;
  m_d.m_visible =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Trigger", "Visible", true) : true;
  m_d.m_hit_height =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Trigger", "HitHeight", 50.f) : 50.f;
  m_d.m_shape = fromMouseClick ? (TriggerShape)LoadValueIntWithDefault("DefaultProps\\Trigger",
                                                                       "Shape", TriggerWireA)
                               : TriggerWireA;
  ...
  m_d.m_reflectionEnabled =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Trigger", "ReflectionEnabled", true)
                     : true;
}
...
void Trigger::UIRenderPass2(Sur* const psur)
{
  ...
  else
  {
    ...
    psur->Line(m_d.m_vCenter.x - m_d.m_radius, m_d.m_vCenter.y, m_d.m_vCenter.x + m_d.m_radius,
               m_d.m_vCenter.y);
    psur->Line(m_d.m_vCenter.x, m_d.m_vCenter.y - m_d.m_radius, m_d.m_vCenter.x,
               m_d.m_vCenter.y + m_d.m_radius);

    const float r2 = m_d.m_radius * (float)sin(M_PI / 4.0);

    psur->Line(m_d.m_vCenter.x - r2, m_d.m_vCenter.y - r2, m_d.m_vCenter.x + r2,
               m_d.m_vCenter.y + r2);
    psur->Line(m_d.m_vCenter.x - r2, m_d.m_vCenter.y + r2, m_d.m_vCenter.x + r2,
               m_d.m_vCenter.y - r2);
  }
...
void Trigger::GetHitShapes(vector<HitObject*>& pvho)
{
  ...
  if (m_d.m_shape == TriggerStar || m_d.m_shape == TriggerButton)
  {
    ...
    m_ptriggerhitcircle =
        new TriggerHitCircle(m_d.m_vCenter, m_d.m_radius, height, height + m_d.m_hit_height);
  ...
  switch (m_d.m_shape)
  {
    ...
    case TriggerStar:
    {
      Hit3DPoly* const pcircle =
          new Hit3DPoly(m_d.m_vCenter.x, m_d.m_vCenter.y, height + 10, m_d.m_radius, 32);
...
void Trigger::AddLine(vector<HitObject*>& pvho,
                      const RenderVertex& pv1,
                      const RenderVertex& pv2,
                      const float height)
{
  ...
  plineseg->m_hitBBox.zhigh =
      height + max(m_d.m_hit_height - 8.0f, 0.f); //adjust for same hit height as circular
...
void Trigger::RenderDynamic()
{
  ...
  pd3dDevice->DrawIndexedPrimitiveVB(RenderDevice::TRIANGLELIST, MY_D3DFVF_NOTEX2_VERTEX,
                                     m_vertexBuffer, 0, m_numVertices, m_triggerIndexBuffer, 0,
                                     m_numIndices);
...
void Trigger::GenerateMesh()
{
  ...
  for (int i = 0; i < m_numVertices; i++)
  {
    ...
    if (m_d.m_shape == TriggerButton || m_d.m_shape == TriggerStar)
    {
      ...
      m_triggerVertices[i].z =
          (vert.z * m_d.m_radius * m_ptable->m_BG_scalez[m_ptable->m_BG_current_set]) + baseHeight +
          zoffset;
    }
    else
    {
      ...
      m_triggerVertices[i].z = (vert.z * 1.0f * m_ptable->m_BG_scalez[m_ptable->m_BG_current_set]) +
                               baseHeight + zoffset;
    }
...
void Trigger::RenderSetup()
{
  ...
  SAFE_BUFFER_RELEASE(m_triggerIndexBuffer);
  m_triggerIndexBuffer =
      IndexBuffer::CreateAndFillIndexBuffer(m_numIndices, indices, PRIMARY_DEVICE);
  SAFE_BUFFER_RELEASE(m_vertexBuffer);
...
void Trigger::Scale(const float scalex,
                    const float scaley,
...
HRESULT Trigger::InitLoad(IStream* pstm,
                          PinTable* ptable,
                          int* pid,
                          int version,
  ...
  if (g_pplayer)
  {
    for (size_t i = 0; i < g_pplayer->m_vball.size(); i++)
    {
      ...
      if (pball->m_d.m_vpVolObjs &&
          FindIndexOf(*(pball->m_d.m_vpVolObjs), (IFireEvents*)this) >=
              0) // cast to IFireEvents necessary, as it is stored like this in HitObject.m_obj
      {
...
STDMETHODIMP Trigger::DestroyBall(int* pVal)
{
  ...
  if (g_pplayer)
  {
    for (size_t i = 0; i < g_pplayer->m_vball.size(); i++)
    {
      ...
      int j;
      if (pball->m_d.m_vpVolObjs &&
          (j = FindIndexOf(*(pball->m_d.m_vpVolObjs), (IFireEvents*)this)) >=
              0) // cast to IFireEvents necessary, as it is stored like this in HitObject.m_obj
      {
        ++cnt;
