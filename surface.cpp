...
HRESULT Surface::Init(PinTable* ptable, float x, float y, bool fromMouseClick)
{
  ...
  const float width =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Wall", "Width", 50.f) : 50.f;
  const float length =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Wall", "Length", 50.f) : 50.f;
...
void Surface::SetDefaults(bool fromMouseClick)
{
  ...
  m_d.m_slingshot_threshold =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "SlingshotThreshold", 0.0f) : 0.0f;
  m_d.m_inner = true; //!! Deprecated, do not use anymore
  ...
  m_d.m_flipbook = fromMouseClick ? LoadValueBoolWithDefault(strKeyName, "Flipbook", false) : false;
  m_d.m_isBottomSolid =
      fromMouseClick ? LoadValueBoolWithDefault(strKeyName, "IsBottomSolid", true) : false;
  ...
  m_d.m_slingshotforce =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "SlingshotForce", 80.0f) : 80.0f;
  m_d.m_slingshotAnimation =
      fromMouseClick ? LoadValueBoolWithDefault(strKeyName, "SlingshotAnimation", true) : true;

  SetDefaultPhysics(fromMouseClick);

  m_d.m_topBottomVisible =
      fromMouseClick ? LoadValueBoolWithDefault(strKeyName, "Visible", true) : true;
  m_d.m_sideVisible =
      fromMouseClick ? LoadValueBoolWithDefault(strKeyName, "SideVisible", true) : true;
  m_d.m_collidable =
...
void Surface::GetBoundingVertices(std::vector<Vertex3Ds>& pvvertex3D)
{
  ...
  for (int i = 0; i < 8; i++)
  {
    const Vertex3Ds pv((i & 1) ? m_ptable->m_right : m_ptable->m_left,
...
void Surface::GenerateMesh(std::vector<Vertex3D_NoTex2>& topBuf,
                           ...
                           std::vector<WORD>& sideIndices)
{
  ...
  //if (m_d.m_visible)      // Visible could still be set later if rendered dynamically
  {
    ...
    Vertex3D_NoTex2* const vertsTop[3] = {&topBuf[0], &topBuf[m_numVertices],
                                          &topBuf[m_numVertices * 2]};
...
void Surface::PrepareWallsAtHeight()
{
  ...
  VertexBuffer::CreateVertexBuffer(m_numVertices * 4 +
                                       (!topBottomBuf.empty() ? m_numVertices * 3 : 0),
                                   0, MY_D3DFVF_NOTEX2_VERTEX, &m_VBuffer, PRIMARY_DEVICE);
  ...
  IndexBuffer::CreateIndexBuffer((unsigned int)topBottomIndices.size() +
                                     (unsigned int)sideIndices.size(),
                                 0, IndexBuffer::FMT_INDEX16, &m_IBuffer, PRIMARY_DEVICE);
...
static constexpr WORD rgiSlingshot[24] = {0, 4, 3, 0, 1, 4, 1, 2, 5, 1, 5, 4,
                                          4, 8, 5, 4, 7, 8, 3, 7, 4, 3, 6, 7};
...
void Surface::PrepareSlingshots()
{
  ...
  for (size_t i = 0; i < m_vlinesling.size(); i++, offset += 9)
  {
    ...
    rgv3D[offset + 1].x = (plinesling->v1.x + plinesling->v2.x) * 0.5f +
                          plinesling->normal.x * (m_d.m_slingshotforce * 0.25f); //40;//20;
    rgv3D[offset + 1].y = (plinesling->v1.y + plinesling->v2.y) * 0.5f +
                          plinesling->normal.y * (m_d.m_slingshotforce * 0.25f); //20;
  ...
  VertexBuffer::CreateVertexBuffer((unsigned int)m_vlinesling.size() * 9, 0,
                                   MY_D3DFVF_NOTEX2_VERTEX, &m_slingshotVBuffer, PRIMARY_DEVICE);
...
void Surface::RenderWallsAtHeight(const bool drop)
{
  ...
  if (m_d.m_sideVisible && !drop && (m_numVertices > 0)) // Don't need to render walls if dropped
  {
    ...
    pd3dDevice->basicShader->Begin(0);
    pd3dDevice->DrawIndexedPrimitiveVB(RenderDevice::TRIANGLELIST, MY_D3DFVF_NOTEX2_VERTEX,
                                       m_VBuffer, 0, m_numVertices * 4, m_IBuffer, 0,
                                       m_numVertices * 6);
    pd3dDevice->basicShader->End();
  }
  ...
  if (m_d.m_topBottomVisible && (m_numPolys > 0))
  {
    ...
    pd3dDevice->basicShader->Begin(0);
    pd3dDevice->DrawIndexedPrimitiveVB(RenderDevice::TRIANGLELIST, MY_D3DFVF_NOTEX2_VERTEX,
                                       m_VBuffer, m_numVertices * 4 + (!drop ? 0 : m_numVertices),
                                       m_numVertices, m_IBuffer, m_numVertices * 6, m_numPolys * 3);
    pd3dDevice->basicShader->End();
    ...
    if (m_ptable->m_reflectionEnabled)
    {
      ...
      pd3dDevice->basicShader->Begin(0);
      pd3dDevice->DrawIndexedPrimitiveVB(RenderDevice::TRIANGLELIST, MY_D3DFVF_NOTEX2_VERTEX,
                                         m_VBuffer, m_numVertices * 4 + m_numVertices * 2,
                                         m_numVertices, m_IBuffer, m_numVertices * 6,
                                         m_numPolys * 3);
      pd3dDevice->basicShader->End();
    }
  }
...
void Surface::Scale(const float scalex,
                    const float scaley,
...
HRESULT Surface::InitLoad(IStream* pstm,
                          PinTable* ptable,
                          int* pid,
                          int version,
                          ...
                          HCRYPTKEY hcryptkey)
{
  ...
  if (!m_d.m_inner)
  {
    ...
    {
      pdp->AddRef();
      pdp->Init(this, m_ptable->m_right + 1.0f, m_ptable->m_bottom, 0.f,
                false); //!!! +1 needed for whatever reason (triangulation screwed up)
      m_vdpoint.insert(m_vdpoint.begin() + (cvertex - minyindex - 1), pdp);
    }
    CComObject<DragPoint>::CreateInstance(&pdp);
    if (pdp)
    ...
    {
      pdp->AddRef();
      pdp->Init(this, m_ptable->m_left - 1.0f, m_ptable->m_top, 0.f,
                false); //!!! -1 needed for whatever reason (triangulation screwed up)
      m_vdpoint.insert(m_vdpoint.begin() + (cvertex - minyindex - 1), pdp);
    }
    CComObject<DragPoint>::CreateInstance(&pdp);
...
{
  char tbuf[128];
  sprintf_s(tbuf, "TopHeight: %.03f | BottomHeight: %0.3f",
            m_vpinball->ConvertToUnit(m_d.m_heighttop),
            m_vpinball->ConvertToUnit(m_d.m_heightbottom));
...
void Surface::SetDefaultPhysics(bool fromMouseClick)
{
  ...
  m_d.m_elasticityFalloff =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "ElasticityFallOff", 0.0f) : 0.0f;
  m_d.m_friction = fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "Friction", 0.3f) : 0.3f;
  m_d.m_scatter = fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "Scatter", 0) : 0;
}
