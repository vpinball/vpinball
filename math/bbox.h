...
public:
  ...
  FRect3D(const float x1,
          const float x2,
          const float y1,
          const float y2,
          const float z1,
          const float z2)
    : left(x1), right(x2), top(y1), bottom(y2), zlow(z1), zhigh(z2)
  {
  }
...
public:
  ...
  Vertex2D Center() const { return Vertex2D(0.5f * (left + right), 0.5f * (top + bottom)); }
...
{
  const __m128 rc1128 = _mm_loadu_ps(
      &rc1.left); // this shouldn't use loadu, but doesn't matter anymore nowadays anyhow
...
inline bool fRectIntersect3D(const FRect3D& rc1, const FRect3D& rc2)
{
  return (rc1.right >= rc2.left && rc1.bottom >= rc2.top && rc1.left <= rc2.right &&
          rc1.top <= rc2.bottom && rc1.zlow <= rc2.zhigh && rc1.zhigh >= rc2.zlow);
}
#endif

inline bool fRectIntersect3D(
    const Vertex3Ds& sphere_p,
    const float sphere_rsqr,
    const FRect3D&
        rc) // could also use SSE, but kd and quadtree already have native SSE variants in there
{
