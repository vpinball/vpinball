...
public:
  ...
  Vertex2D operator+(const Vertex2D& v) const { return Vertex2D(x + v.x, y + v.y); }
  Vertex2D operator-(const Vertex2D& v) const { return Vertex2D(x - v.x, y - v.y); }
  Vertex2D operator-() const { return Vertex2D(-x, -y); }
  ...
  Vertex2D operator*(const float s) const { return Vertex2D(s * x, s * y); }
  friend Vertex2D operator*(const float s, const Vertex2D& v) { return Vertex2D(s * v.x, s * v.y); }
  ...
  float Dot(const Vertex2D& pv) const { return x * pv.x + y * pv.y; }
...
public:
  ...
  Vertex3Ds operator+(const Vertex3Ds& v) const { return Vertex3Ds(x + v.x, y + v.y, z + v.z); }
  Vertex3Ds operator-(const Vertex3Ds& v) const { return Vertex3Ds(x - v.x, y - v.y, z - v.z); }
  Vertex3Ds operator-() const { return Vertex3Ds(-x, -y, -z); }
  ...
  Vertex3Ds operator*(const float s) const { return Vertex3Ds(s * x, s * y, s * z); }
  ...
  float Dot(const Vertex3D_NoTex2& pv) const { return x * pv.x + y * pv.y + z * pv.z; }

  float LengthSquared() const { return x * x + y * y + z * z; }

  float Length() const { return sqrtf(x * x + y * y + z * z); }

  bool IsZero() const { return fabsf(x) <= FLT_MIN && fabsf(y) <= FLT_MIN && fabsf(z) <= FLT_MIN; }
...
{
  return Vertex3Ds(pv1.y * pv2.z - pv1.z * pv2.y, pv1.z * pv2.x - pv1.x * pv2.z,
...
inline Vertex3Ds rotate_to_vector_upper(const Vertex3Ds& vec, const Vertex3Ds& normal)
{
  ...
  if (normal.y > -0.99999f)
  {
    ...
    return Vertex3Ds(vec.x * (normal.y + hz * normal.z) + vec.y * normal.x - vec.z * hzx,
...
inline Vertex3Ds rotate_to_vector_full(const Vertex3Ds& vec, const Vertex3Ds& normal)
{
  ...
  if (fabsf(normal.y) <= 0.99999f)
  {
    ...
    return Vertex3Ds(vec.x * (normal.y + h * zz) + vec.y * normal.x - vec.z * hzx,
                     vec.y * normal.y - vec.x * normal.x - vec.z * normal.z,
