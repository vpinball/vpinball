...
static constexpr float vertsTipBottomf[13 * 3] = {
    -0.101425f, 0.786319f,  0.003753f,  -0.097969f, 0.812569f,  0.003753f, -0.087837f, 0.837031f,
    0.003753f,  -0.071718f, 0.858037f,  0.003753f,  -0.050713f, 0.874155f, 0.003753f,  -0.026251f,
    0.884288f,  0.003753f,  -0.000000f, 0.887744f,  0.003753f,  0.026251f, 0.884288f,  0.003753f,
    0.050713f,  0.874155f,  0.003753f,  0.071718f,  0.858037f,  0.003753f, 0.087837f,  0.837031f,
    0.003753f,  0.097969f,  0.812569f,  0.003753f,  0.101425f,  0.786319f, 0.003753f};

static const Vertex3Ds* const vertsTipBottom = (Vertex3Ds*)vertsTipBottomf;

static constexpr float vertsTipTopf[13 * 3] = {
    -0.101425f, 0.786319f,  1.004253f,  -0.097969f, 0.812569f,  1.004253f, -0.087837f, 0.837031f,
    1.004253f,  -0.071718f, 0.858037f,  1.004253f,  -0.050713f, 0.874155f, 1.004253f,  -0.026251f,
    0.884288f,  1.004253f,  -0.000000f, 0.887744f,  1.004253f,  0.026251f, 0.884288f,  1.004253f,
    0.050713f,  0.874155f,  1.004253f,  0.071718f,  0.858037f,  1.004253f, 0.087837f,  0.837031f,
    1.004253f,  0.097969f,  0.812569f,  1.004253f,  0.101425f,  0.786319f, 1.004253f};

static const Vertex3Ds* const vertsTipTop = (Vertex3Ds*)vertsTipTopf;

static constexpr float vertsBaseBottomf[13 * 3] = {
    -0.100762f, -0.000000f, 0.003753f,  -0.097329f, -0.026079f, 0.003753f,  -0.087263f, -0.050381f,
    0.003753f,  -0.071250f, -0.071250f, 0.003753f,  -0.050381f, -0.087263f, 0.003753f,  -0.026079f,
    -0.097329f, 0.003753f,  -0.000000f, -0.100762f, 0.003753f,  0.026079f,  -0.097329f, 0.003753f,
    0.050381f,  -0.087263f, 0.003753f,  0.071250f,  -0.071250f, 0.003753f,  0.087263f,  -0.050381f,
    0.003753f,  0.097329f,  -0.026079f, 0.003753f,  0.100762f,  -0.000000f, 0.003753f};

static const Vertex3Ds* const vertsBaseBottom = (Vertex3Ds*)vertsBaseBottomf;

static constexpr float vertsBaseTopf[13 * 3] = {
    -0.100762f, 0.000000f,  1.004253f,  -0.097329f, -0.026079f, 1.004253f,  -0.087263f, -0.050381f,
    1.004253f,  -0.071250f, -0.071250f, 1.004253f,  -0.050381f, -0.087263f, 1.004253f,  -0.026079f,
    -0.097329f, 1.004253f,  -0.000000f, -0.100762f, 1.004253f,  0.026079f,  -0.097329f, 1.004253f,
    0.050381f,  -0.087263f, 1.004253f,  0.071250f,  -0.071250f, 1.004253f,  0.087263f,  -0.050381f,
    1.004253f,  0.097329f,  -0.026079f, 1.004253f,  0.100762f,  -0.000000f, 1.004253f};
...
void Flipper::SetDefaults(bool fromMouseClick)
{
  ...
  m_d.m_StartAngle =
      fromMouseClick ? LoadValueFloatWithDefault(regKey, "StartAngle", 121.f) : 121.f;
  m_d.m_EndAngle = fromMouseClick ? LoadValueFloatWithDefault(regKey, "EndAngle", 70.f) : 70.f;
  m_d.m_BaseRadius =
      fromMouseClick ? LoadValueFloatWithDefault(regKey, "BaseRadius", 21.5f) : 21.5f; // 15
  m_d.m_EndRadius =
      fromMouseClick ? LoadValueFloatWithDefault(regKey, "EndRadius", 13.f) : 13.f; // 6
  m_d.m_FlipperRadiusMax =
      fromMouseClick ? LoadValueFloatWithDefault(regKey, "Length", 130.f) : 130.f; // 80
  m_d.m_FlipperRadiusMin =
      fromMouseClick ? LoadValueFloatWithDefault(regKey, "MaxDifLength", 0.f) : 0.f;

  m_d.m_FlipperRadius = m_d.m_FlipperRadiusMax;

  m_d.m_tdr.m_TimerEnabled =
      fromMouseClick ? LoadValueBoolWithDefault(regKey, "TimerEnabled", false) : false;
  m_d.m_tdr.m_TimerInterval =
      fromMouseClick ? LoadValueIntWithDefault(regKey, "TimerInterval", 100) : 100;
  m_d.m_color = fromMouseClick ? LoadValueIntWithDefault(regKey, "Color", RGB(255, 255, 255))
                               : RGB(255, 255, 255);
  m_d.m_rubbercolor = fromMouseClick
                          ? LoadValueIntWithDefault(regKey, "RubberColor", RGB(128, 50, 50))
                          : RGB(128, 50, 50);
  ...
  m_d.m_rubberthickness =
      fromMouseClick ? LoadValueFloatWithDefault(regKey, "RubberThickness", 7.f) : 7.f;
  m_d.m_rubberheight =
      fromMouseClick ? LoadValueFloatWithDefault(regKey, "RubberHeight", 19.f) : 19.f;
  m_d.m_rubberwidth =
      fromMouseClick ? LoadValueFloatWithDefault(regKey, "RubberWidth", 24.f) : 24.f;
  ...
  m_d.m_reflectionEnabled =
      fromMouseClick ? LoadValueBoolWithDefault(regKey, "ReflectionEnabled", true) : true;
}
...
void Flipper::UpdatePhysicsSettings()
{
  ...
  {
    const int idx =
        m_d.m_OverridePhysics ? (m_d.m_OverridePhysics - 1) : (m_ptable->m_overridePhysics - 1);
    char tmp[256];
...
void Flipper::GetHitShapes(vector<HitObject*>& pvho)
{
  ...
  if (m_d.m_FlipperRadiusMin > 0.f && m_d.m_FlipperRadiusMax > m_d.m_FlipperRadiusMin)
  {
    m_d.m_FlipperRadius =
        m_d.m_FlipperRadiusMax -
        (m_d.m_FlipperRadiusMax - m_d.m_FlipperRadiusMin) * m_ptable->m_globalDifficulty;
  ...
  HitFlipper* const phf =
      new HitFlipper(m_d.m_Center, max(m_d.m_BaseRadius, 0.01f), max(m_d.m_EndRadius, 0.01f),
                     max(m_d.m_FlipperRadius, 0.01f), ANGTORAD(m_d.m_StartAngle),
...
void Flipper::SetVertices(const float basex,
                          const float basey,
                          const float angle,
                          Vertex2D* const pvEndCenter,
                          Vertex2D* const rgvTangents,
                          const float baseradius,
                          const float endradius) const
{
  const float fradius = m_d.m_FlipperRadius;
  const float fa =
      asinf((baseradius - endradius) / fradius); //face to centerline angle (center to center)

  const float faceNormOffset =
      (float)(M_PI / 2.0) - fa; //angle of normal when flipper center line at angle zero
...
void Flipper::UIRenderPass1(Sur* const psur)
{
  ...
  Vertex2D vendcenter;
  Vertex2D rgv[4];
  SetVertices(m_d.m_Center.x, m_d.m_Center.y, anglerad, &vendcenter, rgv, m_d.m_BaseRadius,
              m_d.m_EndRadius);
  ...
  // rubber
  SetVertices(m_d.m_Center.x, m_d.m_Center.y, anglerad, &vendcenter, rgv, rubBaseRadius,
              rubEndRadius);
...
void Flipper::UIRenderPass2(Sur* const psur)
{
  ...
  psur->Arc(m_d.m_Center.x, m_d.m_Center.y, m_d.m_BaseRadius, rgv[0].x, rgv[0].y, rgv[3].x,
            rgv[3].y);
  ...
  SetVertices(m_d.m_Center.x, m_d.m_Center.y, anglerad2, &vendcenter, rgv, m_d.m_BaseRadius,
              m_d.m_EndRadius);
  ...
  {
    m_d.m_FlipperRadius =
        (m_ptable->m_globalDifficulty > 0.f) ? m_d.m_FlipperRadiusMin : m_d.m_FlipperRadiusMax;
    m_d.m_FlipperRadius = max(m_d.m_FlipperRadius, m_d.m_BaseRadius - m_d.m_EndRadius + 0.05f);
  }
  else
    return;
...
void Flipper::SetDefaultPhysics(bool fromMouseClick)
{
  ...
  m_d.m_torqueDamping =
      fromMouseClick ? LoadValueFloatWithDefault(regKey, "EOSTorque", 0.75f) : 0.75f;
  m_d.m_torqueDampingAngle =
      fromMouseClick ? LoadValueFloatWithDefault(regKey, "EOSTorqueAngle", 6.f) : 6.f;

  //m_d.m_angleEOS = 0;

  m_d.m_return =
      fromMouseClick ? LoadValueFloatWithDefault(regKey, "ReturnStrength", 0.058f) : 0.058f;
  ...
  m_d.m_elasticityFalloff =
      fromMouseClick ? LoadValueFloatWithDefault(regKey, "ElasticityFalloff", 0.43f) : 0.43f;
...
void Flipper::RenderDynamic()
{
  ...
  pd3dDevice->DrawIndexedPrimitiveVB(RenderDevice::TRIANGLELIST, MY_D3DFVF_NOTEX2_VERTEX,
                                     m_vertexBuffer, 0, flipperBaseVertices, m_indexBuffer, 0,
                                     flipperBaseNumIndices);
  ...
  if (m_d.m_rubberthickness > 0.f)
  {
    ...
    pd3dDevice->basicShader->Begin(0);
    pd3dDevice->DrawIndexedPrimitiveVB(RenderDevice::TRIANGLELIST, MY_D3DFVF_NOTEX2_VERTEX,
                                       m_vertexBuffer, flipperBaseVertices, flipperBaseVertices,
                                       m_indexBuffer, 0, flipperBaseNumIndices);
    pd3dDevice->basicShader->End();
  }
...
static void ApplyFix(Vertex3D_NoTex2& vert,
                     const Vertex2D& center,
                     const float midAngle,
                     const float radius,
                     const Vertex2D& newCenter,
...
void Flipper::GenerateBaseMesh(Vertex3D_NoTex2* buf)
{
  ...
  const float sinAngle =
      clamp((m_d.m_BaseRadius - m_d.m_EndRadius) / m_d.m_FlipperRadius, -1.f, 1.f);
  ...
  for (unsigned int i = 0; i < flipperBaseVertices; i++)
  {
    ...
    vert = fullMatrix.MultiplyVectorNoTranslate(
        Vertex3Ds(flipperBaseMesh[i].nx, flipperBaseMesh[i].ny, flipperBaseMesh[i].nz));
    buf[i].nx = vert.x;
    buf[i].ny = vert.y;
    buf[i].nz = vert.z;
  ...
  if (m_d.m_rubberthickness > 0.f)
  {
    ...
    for (unsigned int i = 0; i < flipperBaseVertices; i++)
    {
      ...
      buf[i + flipperBaseVertices].z =
          vert.z * m_d.m_rubberwidth * m_ptable->m_BG_scalez[m_ptable->m_BG_current_set] +
          (height + m_d.m_rubberheight);
...
{
  SAFE_BUFFER_RELEASE(m_indexBuffer);
  m_indexBuffer = IndexBuffer::CreateAndFillIndexBuffer(flipperBaseNumIndices, flipperBaseIndices,
                                                        PRIMARY_DEVICE);

  SAFE_BUFFER_RELEASE(m_vertexBuffer);
  VertexBuffer::CreateVertexBuffer(flipperBaseVertices * 2, 0, MY_D3DFVF_NOTEX2_VERTEX,
...
HRESULT Flipper::InitLoad(IStream* pstm,
                          PinTable* ptable,
                          int* pid,
                          int version,
...
{
  *pVal =
      (m_d.m_OverridePhysics || (m_ptable->m_overridePhysicsFlipper && m_ptable->m_overridePhysics))
          ? m_d.m_OverrideTorqueDamping
          : m_d.m_torqueDamping;
...
STDMETHODIMP Flipper::get_EOSTorqueAngle(float* pVal)
{
  *pVal =
      (m_d.m_OverridePhysics || (m_ptable->m_overridePhysicsFlipper && m_ptable->m_overridePhysics))
          ? m_d.m_OverrideTorqueDampingAngle
          : m_d.m_torqueDampingAngle;
