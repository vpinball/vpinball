...
#define CHECK_EMBREE(dev) \
  { \
    ...
    { \
      ...
      { \
        char error[256]; \
        sprintf_s(error, "%u %s %d", rc, __FILE__, __LINE__); \
        ShowError(error); \
        break; \
      }; \
    } \
...
{
  const HitObject* const ho =
      (*((const std::vector<HitObject*>*)args->geometryUserPtr))[args->primID];
...
void HitQuadtree::Initialize()
{
  ...
  const unsigned int l_geom = rtcAttachGeometry(
      m_scene,
      geom); // no need to rtcDeleteGeometry(l_geom), as we throw away complete scene later-on
...
void HitQuadtree::FillFromVector(vector<HitObject*>& vho)
{
  ...
  for (size_t i = 0; i < vho.size(); ++i)
    vho[i]
        ->CalcHitBBox(); // need to update here, as only done lazily for some objects (i.e. balls!)
...
void HitQuadtree::CreateNextLevel(const FRect& bounds,
                                  const unsigned int level,
                                  unsigned int level_empty)
{
  ...
  for (size_t i = 0; i < m_vho.size(); i++)
  {
    ...
    if (((pho->m_e != 0) ? pho->m_obj : nullptr) !=
        m_unique) // are all objects in current node unique/belong to the same primitive?
      m_unique = nullptr;
...
void HitQuadtree::InitSseArrays()
{
  ...
  if (lefts_rights_tops_bottoms_zlows_zhighs == nullptr && padded > 0)
  {
...
#endif
    lefts_rights_tops_bottoms_zlows_zhighs =
        (float*)_aligned_malloc(padded * (mul * sizeof(float)), 16);
    ...
    if (end != m_vho.size())
    {
      ...
      const FRect3D r1 = end + 1 < m_vho.size()
                             ? m_vho[end + 1]->m_hitBBox
                             : FRect3D(FLT_MAX, -FLT_MAX, FLT_MAX, -FLT_MAX, FLT_MAX, -FLT_MAX);
      const FRect3D r2 = end + 2 < m_vho.size()
                             ? m_vho[end + 2]->m_hitBBox
                             : FRect3D(FLT_MAX, -FLT_MAX, FLT_MAX, -FLT_MAX, FLT_MAX, -FLT_MAX);
...
void HitQuadtree::HitTestBall(const Ball* const pball, CollisionEvent& coll) const
{
  ...
  if (!m_leaf)
  {
    ...
    {
      if (left)
        m_children[2].HitTestBall(pball, coll);
      if (right)
        m_children[3].HitTestBall(pball, coll);
    }
  }
#endif
  //
...
{
  const HitQuadtree* stack
      [128]; //!! should be enough, but better implement test in construction to not exceed this
  ...
  do
  {
    if (current->m_unique == nullptr ||
        ...
        (current->m_ObjType == eHitTarget &&
         ((HitTarget*)current->m_unique)->m_d.m_isDropped ==
             false)) // early out if only one unique primitive/hittarget stored inside all of the subtree/current node that is also not collidable (at the moment)
    {
      if (current->lefts_rights_tops_bottoms_zlows_zhighs != nullptr) // does node contain hitables?
      {
        ...
        for (size_t i = start, i2 = start * mul; i != end; i += dt, i2 += dt2)
        {
          ...
          __m128 ex = _mm_add_ps(_mm_max_ps(_mm_sub_ps(p[i2] /*left*/, posx), zero),
                                 _mm_max_ps(_mm_sub_ps(posx, p[i2 + 1] /*right */), zero));
          __m128 ey = _mm_add_ps(_mm_max_ps(_mm_sub_ps(p[i2 + 2] /*top */, posy), zero),
                                 _mm_max_ps(_mm_sub_ps(posy, p[i2 + 3] /*bottom*/), zero));
#ifndef DISABLE_ZTEST
          __m128 ez = _mm_add_ps(_mm_max_ps(_mm_sub_ps(p[i2 + 4] /*zlow*/, posz), zero),
                                 _mm_max_ps(_mm_sub_ps(posz, p[i2 + 5] /*zhigh */), zero));
#endif
          ex = _mm_mul_ps(ex, ex);
          ey = _mm_mul_ps(ey, ey);
#ifndef DISABLE_ZTEST
          ...
          if ((mask2 & 2) != 0 /*&& (i*4+1)<m_vho.size()*/ &&
              (pball != current->m_vho[i * 4 + 1])) // ball can not hit itself
            DoHitTest(pball, current->m_vho[i * 4 + 1], coll);
          if ((mask2 & 4) != 0 /*&& (i*4+2)<m_vho.size()*/ &&
              (pball != current->m_vho[i * 4 + 2])) // ball can not hit itself
            DoHitTest(pball, current->m_vho[i * 4 + 2], coll);
          if ((mask2 & 8) != 0 /*&& (i*4+3)<m_vho.size()*/ &&
              (pball != current->m_vho[i * 4 + 3])) // ball can not hit itself
      ...
      if (!current->m_leaf)
      {
        ...
        {
          if (left)
            stack[++stackpos] = current->m_children + 2;
          if (right)
            stack[++stackpos] = current->m_children + 3;
        }
      }
    }
...
void HitQuadtree::HitTestXRay(const Ball* const pball,
                              vector<HitObject*>& pvhoHit,
                              CollisionEvent& coll) const
{
#ifdef USE_EMBREE
  ...
  for (size_t i = 0; i < m_vho.size(); i++)
  {
    ...
    g_pplayer->c_tested++;
#endif
    if ((pball != m_vho[i]) && fRectIntersect3D(pball->m_hitBBox, m_vho[i]->m_hitBBox) &&
        fRectIntersect3D(pball->m_d.m_pos, rcHitRadiusSqr, m_vho[i]->m_hitBBox))
    {
#ifdef DEBUGPHYSICS
  ...
  if (!m_leaf)
  {
    ...
    {
      if (left)
        m_children[2].HitTestXRay(pball, pvhoHit, coll);
      if (right)
        m_children[3].HitTestXRay(pball, pvhoHit, coll);
    }
  }
#endif
}
...
void EmbreeCollideBalls(void* const userPtr,
                        RTCCollision* const collisions,
                        const unsigned int num_collisions)
{
