...
public:
  ...
  static void CreateVertexBuffer(const unsigned int vertexCount,
                                 const DWORD usage,
                                 const DWORD fvf,
...
private:
  ...
  GLuint Array = 0;
  GLuint offset =
      0; //unused ATM, but if we want to group multiple IndexBuffers later in one buffer we might need it
