...
BOOL DebuggerDialog::OnInitDialog()
{
  ...
  HANDLE hIcon = ::LoadImage(GetModuleHandle(nullptr), MAKEINTRESOURCE(IDI_PLAY), IMAGE_ICON, 16,
                             16, LR_DEFAULTCOLOR);
  m_playButton.SetIcon((HICON)hIcon);

  hIcon = ::LoadImage(GetModuleHandle(nullptr), MAKEINTRESOURCE(IDI_PAUSE), IMAGE_ICON, 16, 16,
                      LR_DEFAULTCOLOR);
  m_pauseButton.SetIcon((HICON)hIcon);
  hIcon = ::LoadImage(GetModuleHandle(nullptr), MAKEINTRESOURCE(IDI_STEP), IMAGE_ICON, 16, 16,
                      LR_DEFAULTCOLOR);
  ...
  g_pplayer->m_hwndDebugOutput =
      CreateWindowEx(0, "Scintilla", "",
                     WS_CHILD | ES_NOHIDESEL | WS_VISIBLE | ES_SUNKEN | WS_HSCROLL | WS_VSCROLL |
                         ES_MULTILINE | ES_WANTRETURN | WS_BORDER,
                     rcEditSize.left, rcEditSize.top, rcEditSize.right - rcEditSize.left,
                     rcEditSize.bottom - rcEditSize.top, GetHwnd(), nullptr, g_pvp->theInstance, 0);
  ...
  SendMessage(m_hThrowBallsInPlayerCheck, BM_SETCHECK,
              g_pplayer->m_throwBalls ? BST_CHECKED : BST_UNCHECKED, 0);
  SendMessage(m_hBallControlCheck, BM_SETCHECK,
              g_pplayer->m_ballControl ? BST_CHECKED : BST_UNCHECKED, 0);
  ...
  m_resizer.AddChild(g_pplayer->m_hwndDebugOutput, bottomright,
...
INT_PTR DebuggerDialog::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  switch (uMsg)
  {
    ...
    case WM_SIZE:
    {
      ...
      ::SetWindowPos(g_pplayer->m_hwndDebugOutput, nullptr, 0, 0, rc.right - rc.left,
                     rc.bottom - rc.top,
  ...
  switch (LOWORD(wParam))
  {
    ...
    case IDC_COLOR_BUTTON1:
    {
      ...
      if (m_colorDialog.DoModal(GetHwnd()) == IDOK)
      {
        ...
        memcpy(g_pplayer->m_ptable->m_rgcolorcustom, m_colorDialog.GetCustomColors(),
               sizeof(g_pplayer->m_ptable->m_rgcolorcustom));
      }
      return TRUE;
    }
...
void DbgMaterialDialog::OnOK()
{
  ...
  if (pMat != &g_pvp->m_dummyMaterial)
  {
    ...
    pMat->m_fGlossyImageLerp =
        saturate(sz2f(GetDlgItemText(IDC_DBG_MATERIAL_GLOSSY_IMGLERP_EDIT).c_str()));
      ...
      switch (HIWORD(wParam))
      {
        case CBN_SELCHANGE:
        {
          ...
          if (pMat != &g_pvp->m_dummyMaterial)
          {
            ...
            SendMessage(GetDlgItem(IDC_DBG_METAL_MATERIAL_CHECK), BM_SETCHECK,
                        pMat->m_bIsMetal ? BST_CHECKED : BST_UNCHECKED, 0);
            SendMessage(GetDlgItem(IDC_DBG_MATERIAL_OPACITY_ACTIVE_CHECK), BM_SETCHECK,
                        pMat->m_bOpacityActive ? BST_CHECKED : BST_UNCHECKED, 0);
