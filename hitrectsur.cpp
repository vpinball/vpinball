...
#define PTINRECT(x, y) \
  ((x) >= m_rcRect.left && (x) <= m_rcRect.right && (y) > m_rcRect.top && (y) < m_rcRect.bottom)

HitRectSur::HitRectSur(const HDC hdc,
                       const float zoom,
                       const float offx,
                       const float offy,
                       const int width,
                       const int height,
                       const FRect* const prcRect,
                       vector<ISelect*>* const pvsel)
...
void HitRectSur::Ellipse(const float centerx, const float centery, const float radius)
{
  ...
  if (!PTINRECT(centerx - radius, centery - radius) ||
      !PTINRECT(centerx + radius, centery + radius))
  {
    FailObject();
  }
}
...
void HitRectSur::PolygonImage(const std::vector<RenderVertex>& rgv,
                              HBITMAP hbm,
                              const float left,
                              const float top,
                              const float right,
...
void HitRectSur::Arc(const float x,
                     const float y,
                     const float radius,
                     const float pt1x,
                     const float pt1y,
...
void HitRectSur::Image(const float x,
                       const float y,
                       const float x2,
                       const float y2,
                       HDC hdcSrc,
