...
void PinInput::InitOpenPinballDevices()
{
   ...
   if (m_OpenPinDevContext != nullptr) delete m_OpenPinDevContext;
...
void PinInput::ReadOpenPinballDevices(const U32 cur_time_msec)
{
   ...
   // there's nothing more to do if we didn't receive a new report
   if (!hasNewReport)
      return;
   ...
   if (m_lr_axis == 9)
   {
      ...
      int const val = (g_pplayer->m_accelInputIsVelocity ? cr.vxNudge : cr.axNudge) * scaleFactor;
   ...
   if (m_ud_axis == 9)
   {
      ...
      int const val = (g_pplayer->m_accelInputIsVelocity ? cr.vyNudge : cr.ayNudge) * scaleFactor;
   ...
   // Fire events for changs to the generic buttons
   ...
   // Fire events for changes to the pinball-specific buttons
