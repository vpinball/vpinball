...
#if !defined(DEBUG_XXX) && \
    !defined( \
        _CRTDBG_MAP_ALLOC) //&& (!defined(__STDCPP_DEFAULT_NEW_ALIGNMENT__) || (__STDCPP_DEFAULT_NEW_ALIGNMENT__ < 16))
...
public:
  ...
  virtual BOOL InitInstance()
  {
    ...
    m_vpinball.m_logicalNumberOfProcessors =
        sysinfo
            .dwNumberOfProcessors; //!! this ignores processor groups, so if at some point we need extreme multi threading, implement this in addition!
    ...
    for (int i = 0; i < nArgs; ++i)
    {
          ...
          lstrcmpi(szArglist[i], _T("-?")) == 0 || lstrcmpi(szArglist[i], _T("/?")) == 0)
      {
        m_vpinball.MessageBox(
            "-UnregServer  Unregister VP functions\n-RegServer  Register VP "
            "functions\n\n-DisableTrueFullscreen  Force-disable True Fullscreen "
            "setting\n-EnableTrueFullscreen  Force-enable True Fullscreen setting\n-Minimized  "
            "Start VP in the 'invisible' minimized window mode\n-ExtMinimized  Start VP in the "
            "'invisible' minimized window mode, but with enabled Pause Menu\n-Primary  Force VP to "
            "render on the Primary/Pixel(0,0) Monitor\n\n-LessCPUthreads  Limit the amount of "
            "parallel execution\n\n-Edit [filename]  Load file into VP\n-Play [filename]  Load and "
            "play file\n-PovEdit [filename]  Load and run file in camera mode, then export new pov "
            "on exit\n-Pov [filename]  Load, export pov and close\n-ExtractVBS [filename]  Load, "
            "export table script and close\n-c1 [customparam] .. -c9 [customparam]  Custom user "
            "parameters that can be accessed in the script via GetCustomParam(X)",
      ...
      if (lstrcmpi(szArglist[i], _T("-UnregServer")) == 0 ||
          lstrcmpi(szArglist[i], _T("/UnregServer")) == 0)
      {
      ...
      if (lstrcmpi(szArglist[i], _T("-RegServer")) == 0 ||
          lstrcmpi(szArglist[i], _T("/RegServer")) == 0)
      {
      ...
      if (lstrcmpi(szArglist[i], _T("-DisableTrueFullscreen")) == 0 ||
          lstrcmpi(szArglist[i], _T("/DisableTrueFullscreen")) == 0)
      {
        m_vpinball.m_disEnableTrueFullscreen = 0;
        continue;
      }
      if (lstrcmpi(szArglist[i], _T("-EnableTrueFullscreen")) == 0 ||
          lstrcmpi(szArglist[i], _T("/EnableTrueFullscreen")) == 0)
      {
      ...
      if (useCustomParams && (i + 1 < nArgs))
      {
        ...
        MultiByteToWideCharNull(CP_ACP, 0, szArglist[i + 1], -1,
                                m_vpinball.m_customParameters[customIdx - 1], (int)len + 1);
      ...
      const bool minimized = (lstrcmpi(szArglist[i], _T("-Minimized")) == 0 ||
                              lstrcmpi(szArglist[i], _T("/Minimized")) == 0);
      if (minimized)
      {
      ...
      const bool playfile =
          (lstrcmpi(szArglist[i], _T("-Play")) == 0 || lstrcmpi(szArglist[i], _T("/Play")) == 0);
      ...
      const bool extractscript = (lstrcmpi(szArglist[i], _T("-ExtractVBS")) == 0 ||
                                  lstrcmpi(szArglist[i], _T("/ExtractVBS")) == 0);
  ...
  void InitVPX()
  {
...
#else
    const HRESULT hRes = _Module.RegisterClassObjects(CLSCTX_LOCAL_SERVER, REGCLS_MULTIPLEUSE);
#endif
    _ASSERTE(SUCCEEDED(hRes));
    ...
    if (file)
    {
      ...
      else
      {
        ...
        m_vpinball.m_table_played_via_SelectTableOnStart =
            LoadValueBoolWithDefault("Editor", "SelectTableOnPlayerClose", true) ? loadFileResult
                                                                                 : false;
      }
