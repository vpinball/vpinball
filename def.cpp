...
LocalString::LocalString(const int resid)
{
   ...
   if (resid > 0)
      ...
      m_szbuffer[0] = '\0';
#else
   static ankerl::unordered_dense::map<int, const char*> ids_map = {
     { IDS_SCRIPT, "Script" },
   ...
   const ankerl::unordered_dense::map<int, const char*>::iterator it = ids_map.find(resid);
   if (it != ids_map.end())
   {
...
{
   static ankerl::unordered_dense::map<GLuint, const char*> value_map = {
   ...
   const ankerl::unordered_dense::map<GLuint, const char*>::iterator it = value_map.find(value);
   if (it != value_map.end()) {
      return it->second;
   }
