...
void VertexBuffer::CreateVertexBuffer(const unsigned int vertexCount,
                                      const DWORD usage,
                                      const DWORD fvf,
                                      VertexBuffer** vBuffer,
                                      const deviceNumber dN)
{
  ...
  const HRESULT hr = (dN == PRIMARY_DEVICE ? m_pd3dPrimaryDevice : m_pd3dSecondaryDevice)
...
void VertexBuffer::lock(const unsigned int offsetToLock,
                        const unsigned int sizeToLock,
                        void** dataBuffer,
                        const DWORD flags)
{
  m_curLockCalls++;
  ...
  {
    *dataBuffer = malloc(
        this->sizeToLock); //!! does not init the buffer from the VBuffer data if flags is set accordingly (i.e. WRITEONLY, or better: create a new flag like 'PARTIALUPDATE'?)
...
void VertexBuffer::bind()
{
  ...
  if (/*m_curVertexBuffer == nullptr ||*/ m_curVertexBuffer != this)
  {
    ...
    CHECKD3D((m_dN == PRIMARY_DEVICE ? m_pd3dPrimaryDevice : m_pd3dSecondaryDevice)
                 ->SetStreamSource(0, m_vb, 0, vsize));
    m_curVertexBuffer = this;
  }
#endif
}
...
void VertexBuffer::UploadBuffers()
{
  ...
  if (countNT > 0)
  {
    ...
    glBufferData(GL_ARRAY_BUFFER, countNT * fvfToSize(MY_D3DFVF_NOTEX2_VERTEX), nullptr,
                 GL_STATIC_DRAW);
  }
  if (countT > 0)
  {
