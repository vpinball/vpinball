...
{
inline long CompareAndSwap(volatile long* ptr, long comparand, long exchange)
{
...
struct BlackBoxEntry
{
  ...
  bool operator<(const BlackBoxEntry& rhs) const { return ticks < rhs.ticks; }
...
void AddMessageInternal(const char* msg, long i)
{
  ...
  if (threadName != 0)
  {
    ...
    strncat_s(messages[i].message, ": ",
              sizeof(messages[i].message) -
                  strnlen_s(messages[i].message, sizeof(messages[i].message)) - 1);
  }
  strncat_s(messages[i].message, msg,
            sizeof(messages[i].message) -
                strnlen_s(messages[i].message, sizeof(messages[i].message)) - 1);
