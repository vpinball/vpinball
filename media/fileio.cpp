...
{
  const HANDLE hFile = CreateFile(szfilename, GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING,
  ...
  *pszout = new char[*psize + 2];
  ...
  (*pszout)[*psize] = '\0';
  (*pszout)[*psize + 1] =
      '\0'; // In case this is a unicode file, end it with a null character properly
...
BiffReader::BiffReader(IStream* pistream,
...
long __stdcall FastIStorage::CreateStream(
    const WCHAR* wzName, unsigned long, unsigned long, unsigned long, struct IStream** ppstm)
{
...
long __stdcall FastIStorage::OpenStream(
    const WCHAR*, void*, unsigned long, unsigned long, struct IStream**)
{
  return S_OK;
}

long __stdcall FastIStorage::CreateStorage(
    const WCHAR* wzName, unsigned long, unsigned long, unsigned long, struct IStorage** ppstg)
{
...
long __stdcall FastIStorage::OpenStorage(
    const WCHAR*, struct IStorage*, unsigned long, WCHAR**, unsigned long, struct IStorage**)
{
  return S_OK;
}

long __stdcall FastIStorage::CopyTo(unsigned long,
                                    const struct _GUID*,
                                    WCHAR**,
                                    struct IStorage* pstgNew)
{
  ...
  for (size_t i = 0; i < m_vstg.size(); i++)
  {
    ...
    if (SUCCEEDED(hr = pstgNew->CreateStorage(pstgCur->m_wzName,
  ...
  for (size_t i = 0; i < m_vstm.size(); i++)
  {
    ...
    if (SUCCEEDED(hr = pstgNew->CreateStream(pstmCur->m_wzName,
                                             STGM_DIRECT | STGM_READWRITE | STGM_SHARE_EXCLUSIVE |
                                                 STGM_CREATE,
                                             0, 0, &pstmT)))
    {
...
long __stdcall FastIStorage::MoveElementTo(const WCHAR*,
                                           struct IStorage*,
                                           const WCHAR*,
                                           unsigned long)
{
...
long __stdcall FastIStorage::EnumElements(unsigned long,
                                          void*,
                                          unsigned long,
                                          struct IEnumSTATSTG**)
{
...
long __stdcall FastIStorage::SetElementTimes(const WCHAR*,
...
long __stdcall FastIStream::Seek(union _LARGE_INTEGER li,
                                 const unsigned long origin,
                                 union _ULARGE_INTEGER* puiOut)
{
  switch (origin)
  {
...
long __stdcall FastIStream::CopyTo(struct IStream*,
...
long __stdcall FastIStream::UnlockRegion(union _ULARGE_INTEGER,
                                         union _ULARGE_INTEGER,
                                         unsigned long)
{
  return S_OK;
