...
static HRESULT ReadMMIO(HMMIO hmmioIn, MMCKINFO* pckInRIFF, WAVEFORMATEX** ppwfxInfo)
{
  ...
  *ppwfxInfo = nullptr;
  ...
  else
  {
    ...
    if (mmioRead(hmmioIn, (CHAR*)(((BYTE*)&((*ppwfxInfo)->cbSize)) + sizeof(WORD)), cbExtraBytes) !=
        cbExtraBytes)
    {
      delete[] * ppwfxInfo;
      *ppwfxInfo = nullptr;
...
//-----------------------------------------------------------------------------
static HRESULT WaveOpenFile(const string& strFileName,
                            HMMIO* phmmioIn,
...
static HRESULT WaveReadFile(
    HMMIO hmmioIn, UINT cbRead, BYTE* pbDest, MMCKINFO* pckIn, UINT* cbActualRead)
{
