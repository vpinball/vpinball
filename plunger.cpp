...
void Plunger::SetDefaults(bool fromMouseClick)
{
  ...
  m_d.m_height =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Plunger", "Height", 20.f) : 20.f;
  m_d.m_width =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Plunger", "Width", 25.f) : 25.f;
  m_d.m_zAdjust =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Plunger", "ZAdjust", 0) : 0;
  m_d.m_stroke = fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Plunger", "Stroke",
                                                            m_d.m_height * 4)
                                : (m_d.m_height * 4);
  m_d.m_speedPull =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Plunger", "PullSpeed", 5.f) : 5.f;
  m_d.m_type = fromMouseClick ? (PlungerType)LoadValueIntWithDefault(
                                    "DefaultProps\\Plunger", "PlungerType", PlungerTypeModern)
                              : PlungerTypeModern;
  m_d.m_color = fromMouseClick
                    ? LoadValueIntWithDefault("DefaultProps\\Plunger", "Color", RGB(76, 76, 76))
                    : RGB(76, 76, 76);
  ...
  m_d.m_tdr.m_TimerEnabled =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Plunger", "TimerEnabled", false)
                     : false;
  m_d.m_tdr.m_TimerInterval =
      fromMouseClick ? LoadValueIntWithDefault("DefaultProps\\Plunger", "TimerInterval", 100) : 100;
  ...
  m_d.m_mechPlunger = fromMouseClick
                          ...
                          : false; // plungers require selection for mechanical input
  m_d.m_autoPlunger = fromMouseClick
                          ? LoadValueBoolWithDefault("DefaultProps\\Plunger", "AutoPlunger", false)
                          : false;
  m_d.m_visible =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Plunger", "Visible", true) : true;
  ...
  m_d.m_rodDiam = fromMouseClick
                      ...
                      : 0.60f;
  m_d.m_ringGap = fromMouseClick
                      ? LoadValueFloatWithDefault("DefaultProps\\Plunger", "CustomRingGap", 2.0f)
                      : 2.0f;
  m_d.m_ringDiam = fromMouseClick
                       ? LoadValueFloatWithDefault("DefaultProps\\Plunger", "CustomRingDiam", 0.94f)
                       : 0.94f;
  m_d.m_ringWidth =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Plunger", "CustomRingWidth", 3.0f)
                     : 3.0f;
  m_d.m_springDiam =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Plunger", "CustomSpringDiam", 0.77f)
                     : 0.77f;
  m_d.m_springGauge = fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Plunger",
                                                                 "CustomSpringGauge", 1.38f)
                                     : 1.38f;
  m_d.m_springLoops =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Plunger", "CustomSpringLoops", 8.0f)
                     : 8.0f;
  m_d.m_springEndLoops = fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Plunger",
                                                                    "CustomSpringEndLoops", 2.5f)
                                        : 2.5f;
  m_d.m_reflectionEnabled =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Plunger", "ReflectionEnabled", true)
                     : true;
}
...
void Plunger::UIRenderPass2(Sur* const psur)
{
  ...
  psur->Rectangle(m_d.m_v.x - m_d.m_width, m_d.m_v.y - m_d.m_stroke, m_d.m_v.x + m_d.m_width,
                  m_d.m_v.y + m_d.m_height);
  ...
  if (m_d.m_parkPosition > 0.0f && m_d.m_parkPosition < 1.0f)
  {
    ...
    psur->Line(m_d.m_v.x - m_d.m_width, m_d.m_v.y - m_d.m_stroke + park, m_d.m_v.x + m_d.m_width,
               m_d.m_v.y - m_d.m_stroke + park);
  }
}
...
void Plunger::GetHitShapes(vector<HitObject*>& pvho)
{
  ...
  HitPlunger* const php =
      new HitPlunger(m_d.m_v.x - m_d.m_width, m_d.m_v.y + m_d.m_height, m_d.m_v.x + m_d.m_width,
                     zheight, m_d.m_v.y - m_d.m_stroke, m_d.m_v.y, this);
...
{
  m_d.m_speedFire = fromMouseClick
                        ? LoadValueFloatWithDefault("DefaultProps\\Plunger", "ReleaseSpeed", 80.f)
                        : 80.f;
  m_d.m_mechStrength =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Plunger", "MechStrength", 85.f)
                     : 85.f;
  m_d.m_parkPosition =
      fromMouseClick
          ? LoadValueFloatWithDefault("DefaultProps\\Plunger", "ParkPosition", (float)(0.5 / 3.0))
          : (float)(0.5 /
  ...
  m_d.m_scatterVelocity =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Plunger", "ScatterVelocity", 0.f)
                     : 0.f;
  m_d.m_momentumXfer = fromMouseClick
                           ? LoadValueFloatWithDefault("DefaultProps\\Plunger", "MomentumXfer", 1.f)
                           : 1.f;
}
...
void Plunger::RenderDynamic()
{
  ...
  const int frame0 = (int)((pa.m_pos - pa.m_frameStart) / (pa.m_frameEnd - pa.m_frameStart) *
                               (float)(m_cframes - 1) +
                           0.5f);
  ...
  pd3dDevice->DrawIndexedPrimitiveVB(RenderDevice::TRIANGLELIST, MY_D3DFVF_NOTEX2_VERTEX,
                                     m_vertexBuffer, frame * m_vtsPerFrame, m_vtsPerFrame,
...
const static PlungerDesc modernDesc = {sizeof(modernCoords) / sizeof(modernCoords[0]),
                                       (PlungerCoord*)modernCoords};
...
{
  const float zheight =
      m_ptable->GetSurfaceHeight(m_d.m_szSurface, m_d.m_v.x, m_d.m_v.y) + m_d.m_zAdjust;
  const float stroke = m_d.m_stroke;
  const float beginy = m_d.m_v.y;
  const float endy = m_d.m_v.y - stroke;
  m_cframes =
      (int)(stroke * (float)(PLUNGER_FRAME_COUNT / 80.0)) + 1; // 25 frames per 80 units travel
  ...
  SAFE_BUFFER_RELEASE(m_vertexBuffer);
  VertexBuffer::CreateVertexBuffer(m_cframes * m_vtsPerFrame, 0, MY_D3DFVF_NOTEX2_VERTEX,
  ...
  for (int i = 0; i < m_cframes; i++, ptr += m_vtsPerFrame)
  {
    ...
    if (m_d.m_type == PlungerTypeFlat)
    {
      ...
      ptr[0].x = xLt;
      ptr[0].nx = 0.0f;
      ptr[0].tu = tu_local; // left
      ptr[0].y = yBot;
      ptr[0].ny = 0.0f;
      ptr[0].tv = tv_local; // bottom
      ptr[0].z = z;
      ptr[0].nz = -1.0f;
      ptr[1].x = xLt;
      ptr[1].nx = 0.0f;
      ptr[1].tu = tu_local; // left
      ptr[1].y = yTop;
      ptr[1].ny = 0.0f;
      ptr[1].tv = 0.0f; // top
      ptr[1].z = z;
      ...
      ptr[2].x = xRt;
      ptr[2].nx = 0.0f;
      ptr[2].tu = tu_local + cellWid; // right
      ptr[2].y = yTop;
      ptr[2].ny = 0.0f;
      ptr[2].tv = 0.0f; // top
      ptr[2].z = z;
      ...
      ptr[3].x = xRt;
      ptr[3].nx = 0.0f;
      ptr[3].tu = tu_local + cellWid; // right
      ptr[3].y = yBot;
      ptr[3].ny = 0.0f;
      ptr[3].tv = tv_local; // bottom
      ptr[3].z = z;
      ptr[3].nz = -1.0f;
    }
    else
    {
      ...
      Vertex3D_NoTex2* pm = &ptr[offset];
      const float dtheta =
          float(M_PI * 2.0) / (float)(circlePoints - 1) + (float)M_PI / (float)(n - 1);
      for (float theta = (float)M_PI, y = y0; n != 0; --n, theta += dtheta, y += dy)
      {
        ...
        pm->y = y;
        pm->z =
            ((springRadius + springGaugeRel / 1.5f) * (cs * m_d.m_width) + m_d.m_width + zheight) *
            zScale;
        pm->nx = sn;
        pm->ny = 0.0f;
        pm->nz = -cs;
...
HRESULT Plunger::InitLoad(IStream* pstm,
                          PinTable* ptable,
                          int* pid,
                          int version,
...
bool Plunger::LoadToken(const int id, BiffReader* const pbr)
{
  switch (id)
  {
    ...
    case FID(SURF):
      ...
      break;
    case FID(TIPS):
      pbr->GetString(m_d.m_szTipShape, sizeof(m_d.m_szTipShape));
      break;
    case FID(RODD):
...
STDMETHODIMP Plunger::Position(float* pVal) // 0..25
{
  ...
  {
    const float range = (float)JOYRANGEMX * (1.0f - m_d.m_parkPosition) -
                        (float)JOYRANGEMN * m_d.m_parkPosition; // final range limit
    float tmp = (g_pplayer->m_curMechPlungerPos < 0.f)
                    ? g_pplayer->m_curMechPlungerPos * m_d.m_parkPosition
                    : (g_pplayer->m_curMechPlungerPos * (1.0f - m_d.m_parkPosition));
  ...
  else if (g_pplayer->m_pininput.uShockType == USHOCKTYPE_GENERIC)
  {
    ...
    if (g_pplayer->m_pininput.m_linearPlunger)
    {
      ...
      // parameters:
      const float m = (1.0f - m_d.m_parkPosition) * (float)(1.0 / JOYRANGEMX),
                  b = m_d.m_parkPosition;
    ...
    {
      tmp = (g_pplayer->m_curMechPlungerPos < 0.f)
                ? g_pplayer->m_curMechPlungerPos * m_d.m_parkPosition
                : (g_pplayer->m_curMechPlungerPos * (1.0f - m_d.m_parkPosition));
      const float range = (float)JOYRANGEMX * (1.0f - m_d.m_parkPosition) -
                          (float)JOYRANGEMN * m_d.m_parkPosition; // final range limit
  ...
  else // non-mechanical
  {
    ...
    *pVal =
        25.f -
        saturate(frame) *
            25.f; //!! somehow if m_mechPlunger is enabled this will only deliver a value 25 - 0..20??
  }
...
STDMETHODIMP Plunger::CreateBall(IBall** pBallEx)
{
  if (m_phitplunger)
  {
    ...
    const float y =
        m_phitplunger->m_plungerMover.m_pos - (25.0f + 0.01f); //!! assumes ball radius 25
