...
void Plunger::WriteRegDefaults()
        ...
        SetRegValue("DefaultProps\\Plunger","CustomTipShape", REG_SZ, &m_d.m_szTipShape, strlen(m_d.m_szTipShape));
...
void Plunger::PostRenderStatic(const RenderDevice* _pd3dDevice)
{
    ...
    Texture *pin = (m_d.m_type != PlungerTypeOrig ? m_ptable->GetImage(m_d.m_szImage) : 0);
    if (pin)
    {
    ...
    pd3dDevice->DrawIndexedPrimitiveVB( D3DPT_TRIANGLELIST, vertexBuffer,
...
// Original Plunger
const static PlungerCoord origCoords[] =
{
    { 1.00f,   0.0f, 0.00f, 1.0f, 0.0f },
    { 1.00f,  10.0f, 0.10f, 0.8f, 0.6f },
    { 0.35f,  20.0f, 0.20f, 0.0f, 1.0f },
    { 0.35f,  24.0f, 0.24f, 1.0f, 0.0f },
    { 0.35f, 100.0f, 1.00f, 1.0f, 0.0f }
};
const static PlungerDesc origDesc = {
    sizeof(origCoords)/sizeof(origCoords[0]), origCoords
};
...
#else
// evern moderner plunger - mjr
const static PlungerCoord modernCoords[] =
{
    { 0.17f,   0.0f, 0.00f, 0.2f, 0.98f }, // tip
    { 0.30f,   2.0f, 0.02f, 0.7f, 0.7f },
    { 0.32f,   3.0f, 0.02f, 0.7f, 0.7f },
    { 0.35f,   5.0f, 0.04f, 1.0f, 0.0f },
    { 0.40f,   7.0f, 0.05f, 1.0f, 0.0f },
    { 0.42f,   8.0f, 0.05f, 1.0f, 0.0f },
    { 0.43f,   9.0f, 0.07f, 1.0f, 0.0f },
    { 0.44f,  11.0f, 0.08f, 1.0f, 0.0f },
    { 0.44f,  14.0f, 0.08f, 1.0f, 0.0f },
    { 0.40f,  39.0f, 0.22f, 1.0f, 0.0f },
    { 0.40f,  39.0f, 0.22f, 0.0f, 1.0f },
    { 0.30f,  39.0f, 0.23f, 0.0f, 1.0f },
    { 0.30f,  39.0f, 0.52f, 1.0f, 0.0f },  // shaft between tip and ring
    { 0.30f,  42.0f, 0.73f, 1.0f, 0.0f },  // shaft between tip and ring
    { 0.30f,  42.0f, 0.27f, 0.0f, 1.0f },  // ring
    { 0.47f,  42.0f, 0.27f, 0.0f, 1.0f },  // ring
    { 0.47f,  45.0f, 0.44f, 0.0f, 1.0f },  // ring
    { 0.30f,  45.0f, 0.46f, 0.0f, 1.0f },  // ring
    { 0.30f,  45.0f, 0.52f, 1.0f, 0.0f },  // shaft
    { 0.30f, 100.0f, 0.73f, 1.0f, 0.0f }   // shaft
    // translates to
    // 0,.34; 2,.6; 1,.64; 2,.7; 2,.8; 1,.84; 1,.86; 2,.88; 3,.88; 25,.8
};
#endif
...
void Plunger::RenderSetup(const RenderDevice* _pd3dDevice )
{
   ...
   Vertex3D_NoTex2 *ptr = (Vertex3D_NoTex2 *)buf;
   ...
   for ( int i = 0 ; i < cframes ; i++, ptr += vtsPerFrame )
   {
       ...
       for (int l = 0, offset = 0 ; l < circlePoints ; l++, offset += lathePoints, tu += stepU)
       {
           ...
           for (int m = 0 ; m < lathePoints ; m++, ++c, ++pm)
           {
               ...
               pm->y = y;
               pm->z = (r * (cs * m_d.m_width) + m_d.m_width + zheight) * m_ptable->m_zScale;
               pm->nx = c->nx * sn;
               pm->ny = c->ny;
               pm->nz = -c->nx * cs;
               pm->tu = tu;
       ...
       if (m_d.m_type == PlungerTypeFlat)
       {
           ...
           //
           const float z = (zheight + m_d.m_width*1.25f) * m_ptable->m_zScale;
       ...
       else
       {
           ...
           int offset = circlePoints * lathePoints;
           float y0 = ptr[offset-2].y;
           float y1 = rody;
           int n = int((springLoops + springEndLoops) * circlePoints);
           int nEnd = int(springEndLoops * circlePoints);
           int nMain = n - nEnd;
           float yEnd = springEndLoops * springGauge * springMinSpacing;
           float dyMain = (y1 - y0 - yEnd)/(nMain - 1);
           ...
           for (float theta = TWOPI/2.0f, y = y0 ; n != 0 ; --n, theta += dtheta, y += dy)
           {
               ...
               pm->y = y - springGauge;
               pm->z = (springRadius * (cs * m_d.m_width) + m_d.m_width + zheight) * m_ptable->m_zScale;
               pm->nx = 0.0f;
               pm->ny = -1.0f;
               pm->nz = -0.0f;
               pm->tu = (sn+1.0f)/2.0f;
               ...
               pm->y = y;
               pm->z = ((springRadius + springGaugeRel/1.5f) * (cs * m_d.m_width) + m_d.m_width + zheight) * m_ptable->m_zScale;
               pm->nx = sn;
               pm->ny = 0.0f;
               pm->nz = -cs;
               pm->tu = (sn+1.0f)/2.0f;
   ...
   WORD *indices = new WORD[indicesPerFrame];
   int k = 0;
...
STDMETHODIMP Plunger::put_Image(BSTR newVal)
{
        ...
        WideCharToMultiByte(CP_ACP, 0, newVal, -1, m_d.m_szImage, MAXTOKEN, NULL, NULL);
