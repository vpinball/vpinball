         ...
         if ( !renderNewPlunger )
         {
            ...
            for (int l=0;l<16;l++, tu+=stepU)
            {
               ...
               for (int m=0;m<PLUNGEPOINTS1;m++)
               {
                  ...
                  ptr[m + offset].y = y;
                  ptr[m + offset].z = (rgcrossplunger1[m][0] * (cs * m_d.m_width) + m_d.m_width + zheight)*m_ptable->zScale;
      ...
      else if (m_d.m_type == PlungerTypeOrig)
      {
         ...
         for (int l=0;l<16;l++)
         {
            ...
            for (int m=0;m<PLUNGEPOINTS0;m++)
            {
               ...
               ptr[m + offset].z = (rgcrossplunger0[m][0] * (cs * m_d.m_width) + m_d.m_width + zheight)*m_ptable->zScale;
