...
public:
  ...
  float
      m_currentHitThreshold; // while playing and the ball hits the mesh the hit threshold is updated here
};
  ...
  { \
    ...
    { \
      { \
        ...
        WideStrNCopy(wzUniqueName, (WCHAR*)m_wzName, \
                     sizeof(m_wzName) / \
                         sizeof(m_wzName[0])); /*lstrcpyW((WCHAR *)m_wzName, wzUniqueName);*/ \
      } \
    } \
    InitScript(); \
    return S_OK; \
...
#define STANDARD_NOSCRIPT_EDITABLE_DECLARES(T, ItemType, ResName, AllowedViews) \
  _STANDARD_EDITABLE_CONSTANTS(ItemType, ResName, AllowedViews) \
  _STANDARD_DISPATCH_INDEPENDANT_EDITABLE_DECLARES(T, ItemType) \
  virtual EventProxyBase* GetEventProxyBase() \
  { \
    return nullptr; \
  } \
  inline IFireEvents* GetIFireEvents() \
  { \
    return nullptr; \
  } \
  inline IDebugCommands* GetDebugCommands() \
  { \
    return nullptr; \
  } \
...
#define _STANDARD_DISPATCH_EDITABLE_DECLARES(itemType) \
  ...
  { \
    ...
    { \
      WideStrNCopy(newVal, (WCHAR*)m_wzName, \
                   sizeof(m_wzName) / \
                       sizeof(m_wzName[0])); /*lstrcpyW((WCHAR *)m_wzName, newVal);*/ \
      return S_OK; \
    } \
    return E_FAIL; \
  } \
  STDMETHOD(get_TimerInterval)(/*[out, retval]*/ long* pVal) \
  { \
    *pVal = m_d.m_tdr.m_TimerInterval; \
    return S_OK; \
  } \
  STDMETHOD(put_TimerInterval)(/*[in]*/ long newVal) \
  { \
    return IEditable::put_TimerInterval(newVal, &m_d.m_tdr.m_TimerInterval); \
  } \
  STDMETHOD(get_TimerEnabled)(/*[out, retval]*/ VARIANT_BOOL * pVal) \
  { \
    *pVal = FTOVB(m_d.m_tdr.m_TimerEnabled); \
    return S_OK; \
  } \
  STDMETHOD(put_TimerEnabled)(/*[in]*/ VARIANT_BOOL newVal) \
  { \
    BOOL tmp; \
    const HRESULT res = IEditable::put_TimerEnabled(newVal, &tmp); \
    m_d.m_tdr.m_TimerEnabled = (tmp != 0); \
    return res; \
  } \
  STDMETHOD(get_UserValue)(VARIANT * pVal) \
  { \
    return IEditable::get_UserValue(pVal); \
  } \
  STDMETHOD(put_UserValue)(VARIANT * newVal) \
  { \
    return IEditable::put_UserValue(newVal); \
  } \
  virtual IScriptable* GetScriptable() \
  { \
...
#define _STANDARD_DISPATCH_INDEPENDANT_EDITABLE_DECLARES(T, ItemType) \
  ...
  virtual HRESULT InitLoad(IStream* pstm, PinTable* ptable, int* pid, int version, \
                           HCRYPTHASH hcrypthash, HCRYPTKEY hcryptkey); \
  ...
  { \
    return static_cast<const IDispatch*>(this); \
  } \
  ...
  { \
    return static_cast<Hitable*>(this); \
  } \
  virtual const Hitable* GetIHitable() const \
  { \
    return static_cast<const Hitable*>(this); \
  } \
  ...
  { \
    return GetPTable()->GetPredefinedStrings(dispID, pcaStringsOut, pcaCookiesOut, this); \
  } \
  STDMETHOD(GetPredefinedValue)(DISPID dispID, DWORD dwCookie, VARIANT * pVarOut) \
  { \
    return GetPTable()->GetPredefinedValue(dispID, dwCookie, pVarOut, this); \
  } \
...
public:
  ...
  bool
      m_backglass; // if the light/decal (+dispreel/textbox is always true) is on the table (false) or a backglass view
};
