...
public:
  ...
  virtual HRESULT InitLoad(IStream* pstm,
                           PinTable* ptable,
                           int* pid,
                           int version,
  ...
  ISelect* GetSelectedItem() const
  {
    return m_vmultisel.ElementAt(0);
  }
  ...
  HRESULT WriteInfoValue(IStorage* pstg,
                         const WCHAR* const wzName,
                         const string& szValue,
                         HCRYPTHASH hcrypthash);
  HRESULT ReadInfoValue(IStorage* pstg,
                        const WCHAR* const wzName,
                        char** pszValue,
                        HCRYPTHASH hcrypthash);
  ...
  virtual const IDispatch* GetDispatch() const
  {
    return (const IDispatch*)this;
  }
  virtual IFireEvents* GetIFireEvents()
  {
    return (IFireEvents*)this;
  }
  virtual IDebugCommands* GetDebugCommands()
  {
    return nullptr;
  }
  ...
  (DISPID dispID, CALPOLESTR* pcaStringsOut, CADWORD* pcaCookiesOut, IEditable* piedit);
  ...
  void GetUniqueName(const ItemTypeEnum type,
                     WCHAR* const wzUniqueName,
                     const DWORD wzUniqueName_maxlength) const;
  void GetUniqueName(const WCHAR* const prefix,
                     WCHAR* const wzUniqueName,
                     const DWORD wzUniqueName_maxlength) const;
  void GetUniqueNamePasting(const int type,
                            WCHAR* const wzUniqueName,
  ...
  bool RenderSolid() const
  {
    return m_renderSolid;
  }
  ...
  float m_fOverrideGravityConstant, m_fOverrideContactFriction, m_fOverrideElasticity,
      m_fOverrideElasticityFalloff, m_fOverrideScatterAngle;
  ...
  bool
      m_tblMirrorEnabled; // Mirror tables left to right.  This is activated by a cheat during table selection.
...
private:
  ...
  CString m_notesText;
  robin_hood::unordered_map<std::string, Texture*, StringHashFunctor, StringComparator>
      m_textureMap; // hash table to speed up texture lookup by name
  robin_hood::unordered_map<std::string, Material*, StringHashFunctor, StringComparator>
      m_materialMap; // hash table to speed up material lookup by name
  bool m_moving;
};

class ScriptGlobalTable : public CComObjectRootEx<CComSingleThreadModel>,
...
public:
  ...
  STDMETHOD(NudgeSetCalibration)
  (int XMax, int YMax, int XGain, int YGain, int DeadZone, int TiltSensitivty);
...
private:
  bool GetTextFileFromDirectory(const char* const szfilename,
