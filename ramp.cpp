...
void Ramp::UpdateStatusBarInfo()
{
  ...
  sprintf_s(
      tbuf,
      "TopH: %.03f | BottomH: %0.3f | TopW: %.03f | BottomW: %.03f | LeftW: %.03f | RightW: %.03f",
      m_vpinball->ConvertToUnit(m_d.m_heighttop), m_vpinball->ConvertToUnit(m_d.m_heightbottom),
      m_vpinball->ConvertToUnit(m_d.m_widthtop), m_vpinball->ConvertToUnit(m_d.m_widthbottom),
      m_vpinball->ConvertToUnit(m_d.m_leftwallheightvisible),
      m_vpinball->ConvertToUnit(m_d.m_rightwallheightvisible));
...
void Ramp::SetDefaults(bool fromMouseClick)
{
  ...
  m_d.m_heightbottom =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "HeightBottom", 0.0f) : 0.0f;
  m_d.m_heighttop =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "HeightTop", 50.0f) : 50.0f;
  m_d.m_widthbottom =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "WidthBottom", 75.0f) : 75.0f;
  m_d.m_widthtop =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "WidthTop", 60.0f) : 60.0f;
  m_d.m_type = fromMouseClick
                   ? (RampType)LoadValueIntWithDefault(strKeyName, "RampType", RampTypeFlat)
                   : RampTypeFlat;

  m_d.m_tdr.m_TimerEnabled =
      fromMouseClick ? LoadValueBoolWithDefault(strKeyName, "TimerEnabled", false) : false;
  m_d.m_tdr.m_TimerInterval =
      fromMouseClick ? LoadValueIntWithDefault(strKeyName, "TimerInterval", 100) : 100;
  ...
  m_d.m_imagealignment =
      fromMouseClick
          ? (RampImageAlignment)LoadValueIntWithDefault(strKeyName, "ImageMode", ImageModeWorld)
          : ImageModeWorld;
  m_d.m_imageWalls =
      fromMouseClick ? LoadValueBoolWithDefault(strKeyName, "ImageWalls", true) : true;

  m_d.m_leftwallheight =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "LeftWallHeight", 62.0f) : 62.0f;
  m_d.m_rightwallheight =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "RightWallHeight", 62.0f) : 62.0f;
  m_d.m_leftwallheightvisible =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "LeftWallHeightVisible", 30.0f)
                     : 30.0f;
  m_d.m_rightwallheightvisible =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "RightWallHeightVisible", 30.0f)
                     : 30.0f;
  ...
  m_d.m_wireDiameter =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "WireDiameter", 8.0f) : 8.0f;
  m_d.m_wireDistanceX =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "WireDistanceX", 38.0f) : 38.0f;
  m_d.m_wireDistanceY =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "WireDistanceY", 88.0f) : 88.0f;
}
...
void Ramp::UIRenderPass1(Sur* const psur)
{
  ...
  const Vertex2D* const rgvLocal =
      GetRampVertex(cvertex, nullptr, nullptr, nullptr, nullptr, HIT_SHAPE_DETAIL_LEVEL, false);
...
void Ramp::UIRenderPass2(Sur* const psur)
{
  ...
  int cvertex;
  const Vertex2D* const rgvLocal = GetRampVertex(cvertex, nullptr, &pfCross, nullptr, &middlePoints,
                                                 HIT_SHAPE_DETAIL_LEVEL, false);
  psur->Polygon(rgvLocal, cvertex * 2);
  ...
  if (drawDragpoints)
  {
    for (size_t i = 0; i < m_vdpoint.size(); i++)
    {
      ...
      psur->SetFillColor(-1);
      psur->SetBorderColor(pdp->m_dragging ? RGB(0, 255, 0)
...
void Ramp::GetBoundingVertices(std::vector<Vertex3Ds>& pvvertex3D)
{
  ...
  int cvertex;
  const Vertex2D* const rgvLocal =
      GetRampVertex(cvertex, &rgheight1, nullptr, nullptr, nullptr, HIT_SHAPE_DETAIL_LEVEL, true);
  ...
  {
    {
      const Vertex3Ds pv(rgvLocal[i].x, rgvLocal[i].y,
                         rgheight1[i] +
                             (float)(2.0 * PHYS_SKIN)); // leave room for ball //!! use ballsize
    ...
    const Vertex3Ds pv(rgvLocal[cvertex * 2 - i - 1].x, rgvLocal[cvertex * 2 - i - 1].y,
                       rgheight1[i] +
                           (float)(2.0 * PHYS_SKIN)); // leave room for ball //!! use ballsize
    //pvvertex3D.push_back(pv);
    bbox_min.x = min(bbox_min.x, pv.x);
  ...
  for (int i = 0; i < 8; i++)
  {
    const Vertex3Ds pv((i & 1) ? bbox_min.x : bbox_max.x, (i & 2) ? bbox_min.y : bbox_max.y,
 ...
 */
Vertex2D* Ramp::GetRampVertex(int& pcvertex,
                              float** const ppheight,
                              bool** const ppfCross,
                              float** const ppratio,
                              Vertex2D** const pMiddlePoints,
                              const float _accuracy,
                              const bool inc_width)
{
  ...
  for (int i = 0; i < cvertex; i++)
  {
    ...
    Vertex2D vnormal;
    {
      ...
      Vertex2D v1normal(vprev.y - vmiddle.y,
                        vmiddle.x - vprev.x); // vector vmiddle-vprev rotated RIGHT
      Vertex2D v2normal(vmiddle.y - vnext.y,
                        vnext.x - vmiddle.x); // vector vnext-vmiddle rotated RIGHT
...
float Ramp::GetSurfaceHeight(float x, float y) const
{
  ...
  const float len = sqrtf(dx * dx + dy * dy);
  startlength +=
      len; // Add the distance the object is between the two closest polyline segments.  Matters mostly for straight edges. Z does not respect that yet!
...
void Ramp::GetHitShapes(vector<HitObject*>& pvho)
{
  ...
  if (wallheightright > 0.f)
  {
    for (int i = 0; i < (cvertex - 1); i++)
    {
      ...
      AddWallLineSeg(pvho, pv3, pv2, (i < (cvertex - 2)), rgheight1[i], rgheight1[i + 1],
                     wallheightright);
  ...
  if (wallheightleft > 0.f)
  {
    for (int i = 0; i < (cvertex - 1); i++)
    {
      ...
      AddWallLineSeg(pvho, pv2, pv3, (i > 0), rgheight1[cvertex - i - 2],
                     rgheight1[cvertex - i - 1], wallheightleft);
      AddWallLineSeg(pvho, pv3, pv2, (i < (cvertex - 2)), rgheight1[cvertex - i - 2],
                     rgheight1[cvertex - i - 1], wallheightleft);
  ...
  // Add hit triangles for the ramp floor.
  {
    ...
    for (int i = 0; i < (cvertex - 1); i++)
    {
...

      {
        ...
        const Vertex3Ds rgv3D[3] = {Vertex3Ds(pv2->x, pv2->y, rgheight1[i]),
        ...
        HitTriangle* const ph3dpoly = new HitTriangle(
            rgv3D); //!! this is not efficient at all, use native triangle-soup directly somehow
      ...
      const Vertex3Ds rgv3D[3] = {Vertex3Ds(pv3->x, pv3->y, rgheight1[i + 1]),
                                  Vertex3Ds(pv1->x, pv1->y, rgheight1[i]),
  ...
  for (int i = 0; i < (cvertex - 1); i++)
  {
...

    {
      ...
      const Vertex3Ds rgv3D[3] = {Vertex3Ds(pv1.x, pv1.y, rgheight1[i]),
...
void Ramp::CheckJoint(vector<HitObject*>& pvho,
                      const HitTriangle* const ph3d1,
                      const HitTriangle* const ph3d2)
{
...
void Ramp::AddJoint2D(vector<HitObject*>& pvho,
                      const Vertex2D& p,
                      const float zlow,
                      const float zhigh)
{
...
void Ramp::AddWallLineSeg(vector<HitObject*>& pvho,
                          const Vertex2D& pv1,
                          const Vertex2D& pv2,
                          const bool pv3_exists,
                          const float height1,
                          const float height2,
                          const float wallheight)
{
  ...
  {
    AddWallLineSeg(pvho, pv1, (pv1 + pv2) * 0.5f, pv3_exists, height1, (height1 + height2) * 0.5f,
                   wallheight);
    AddWallLineSeg(pvho, (pv1 + pv2) * 0.5f, pv2, true, (height1 + height2) * 0.5f, height2,
                   wallheight);
  }
  else
  {
...
{
  return m_d.m_type == RampType4Wire || m_d.m_type == RampType1Wire ||
         m_d.m_type == RampType2Wire || m_d.m_type == RampType3WireLeft ||
...
void Ramp::RenderStaticHabitrail(const Material* const mat)
{
  ...
  if (m_d.m_type == RampType2Wire)
  {
    ...
    pd3dDevice->DrawIndexedPrimitiveVB(RenderDevice::TRIANGLELIST, MY_D3DFVF_NOTEX2_VERTEX,
                                       m_dynamicVertexBuffer, 0, m_numVertices,
                                       m_dynamicIndexBuffer, 0, m_numIndices);
    pd3dDevice->DrawIndexedPrimitiveVB(RenderDevice::TRIANGLELIST, MY_D3DFVF_NOTEX2_VERTEX,
                                       m_dynamicVertexBuffer2, 0, m_numVertices,
                                       m_dynamicIndexBuffer, 0, m_numIndices);
...
void Ramp::CreateWire(const int numRings,
                      const int numSegments,
                      const Vertex2D* const midPoints,
                      Vertex3D_NoTex2* const rgvbuf)
{
  Vertex3Ds prevB;
  for (int i = 0, index = 0; i < numRings; i++)
  {
    ...
    Vertex3Ds tangent(midPoints[i2].x - midPoints[i].x, midPoints[i2].y - midPoints[i].y,
                      m_rgheightInit[i2] - m_rgheightInit[i]);
    if (i == numRings - 1)
    {
    ...
    if (i == 0)
    {
      Vertex3Ds up(midPoints[i2].x + midPoints[i].x, midPoints[i2].y + midPoints[i].y,
                   m_rgheightInit[i2] - height);
    ...
    for (int j = 0; j < numSegments; j++, index++)
    {
      ...
      rgvbuf[index].tu = u;
      rgvbuf[index].tv = v;
      const Vertex3Ds n(rgvbuf[index].x - midPoints[i].x, rgvbuf[index].y - midPoints[i].y,
...
void Ramp::GenerateWireMesh(Vertex3D_NoTex2** meshBuf1, Vertex3D_NoTex2** meshBuf2)
{
  ...
  int splinePoints;
  const Vertex2D* const rgvLocal =
      GetRampVertex(splinePoints, &m_rgheightInit, nullptr, nullptr,
                    (m_d.m_type != RampType1Wire) ? nullptr : &middlePoints, -1, false);
...
HRESULT Ramp::InitLoad(IStream* pstm,
                       PinTable* ptable,
                       int* pid,
                       int version,
...
void Ramp::AddPoint(int x, int y, const bool smooth)
{
  ...
  if (pdp)
  {
    ...
    pdp->Init(this, vOut.x, vOut.y,
              (vvertex[max(iSeg - 1, 0)].z + vvertex[min(iSeg + 1, (int)vvertex.size() - 1)].z) *
                  0.5f,
              smooth); // Ramps are usually always smooth
    m_vdpoint.insert(
        m_vdpoint.begin() + icp,
        pdp); // push the second point forward, and replace it with this one.  Should work when index2 wraps.
  }
...
void Ramp::Scale(const float scalex,
                 const float scaley,
...
void Ramp::RenderRamp(const Material* const mat)
{
  ...
  else
  {
    ...
    pd3dDevice->SetRenderStateDepthBias(0.0f);
    pd3dDevice->SetRenderState(RenderDevice::ZWRITEENABLE, RenderDevice::RS_TRUE);
    pd3dDevice->SetRenderStateCulling(
        RenderDevice::CULL_NONE); // as both floor and walls are thinwalled
    ...
    if (m_d.m_rightwallheightvisible != 0.f && m_d.m_leftwallheightvisible != 0.f &&
        (!pin || m_d.m_imageWalls))
    {
      // both walls with image and floor
      pd3dDevice->basicShader->Begin(0);
      pd3dDevice->DrawIndexedPrimitiveVB(RenderDevice::TRIANGLELIST, MY_D3DFVF_NOTEX2_VERTEX,
                                         m_dynamicVertexBuffer, 0, m_numVertices * 3,
                                         m_dynamicIndexBuffer, 0, (m_rampVertex - 1) * 6 * 3);
      pd3dDevice->basicShader->End();
    }
    else
    {
...
void Ramp::GenerateRampMesh(Vertex3D_NoTex2** meshBuf)
{
  ...
  const Vertex2D* const rgvLocal = GetRampVertex(
      m_rampVertex, &rgheight, nullptr,
      (m_d.m_imagealignment == ImageModeWorld) ? nullptr : &rgratio, nullptr, -1, true);
  ...
  m_numIndices =
      rgioffset *
      3; // to draw the full ramp in one go (could only use *1, and draw three times with offsets into vertices)
...
void Ramp::GenerateVertexBuffer()
{
  ...
  SAFE_BUFFER_RELEASE(m_dynamicVertexBuffer);
  VertexBuffer::CreateVertexBuffer(
      m_numVertices * 3, 0, MY_D3DFVF_NOTEX2_VERTEX, &m_dynamicVertexBuffer,
      PRIMARY_DEVICE); //!! use USAGE_DYNAMIC if it would actually be "really" dynamic
