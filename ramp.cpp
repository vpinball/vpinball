...
void Ramp::RenderSetup(const RenderDevice* _pd3dDevice)
{
   ...
   Vertex3D_NoTex2 *floorVertices;
   Vertex3D_NoTex2 *leftWallVertices[2];
   Vertex3D_NoTex2 *rightWallVertices[2];

   floorVertices = new Vertex3D_NoTex2[(cvertex-1)*4];
   leftWallVertices[0] = new Vertex3D_NoTex2[(cvertex-1)*4];
   leftWallVertices[1] = new Vertex3D_NoTex2[(cvertex-1)*4];
   rightWallVertices[0] = new Vertex3D_NoTex2[(cvertex-1)*4];
   rightWallVertices[1] = new Vertex3D_NoTex2[(cvertex-1)*4];
   ...
   for (int i=0;i<(cvertex-1);i++)
   {
      Vertex3D_NoTex2 * const rgv3D = floorVertices+i*4;
      rgv3D[0].x = rgv[i].x;
      rgv3D[0].y = rgv[i].y;
      ...
      indices[0][i*6]   = i*4;
      indices[0][i*6+1] = i*4+1;
      indices[0][i*6+2] = i*4+2;
      indices[0][i*6+3] = i*4;
      indices[0][i*6+4] = i*4+2;
      indices[0][i*6+5] = i*4+3;

      indices[1][i*6]   = i*4;
      indices[1][i*6+1] = i*4+3;
      indices[1][i*6+2] = i*4+2;
      indices[1][i*6+3] = i*4;
      indices[1][i*6+4] = i*4+2;
      indices[1][i*6+5] = i*4+1;
   }

   memcpy( buf, floorVertices, dataSize);
   offset+=(cvertex-1)*4;
   memcpy( rightWallVertices[0],floorVertices, dataSize);
   
   for (int i=0;i<(cvertex-1);i++)
   {
      Vertex3D_NoTex2 * const rgv3D = rightWallVertices[0]+i*4;
   ...
   memcpy( &buf[offset], rightWallVertices[0], dataSize);
   offset+=(cvertex-1)*4;

   memcpy( rightWallVertices[1],rightWallVertices[0], dataSize);
   memcpy( leftWallVertices[0],rightWallVertices[0], dataSize);
   ...
   for (int i=0;i<(cvertex-1);i++)
   {
      for(int j = 0; j < 4; ++j) 
      {
         rightWallVertices[1][i*4+j].nx = -rightWallVertices[1][i*4+j].nx;
         rightWallVertices[1][i*4+j].ny = -rightWallVertices[1][i*4+j].ny;
         rightWallVertices[1][i*4+j].nz = -rightWallVertices[1][i*4+j].nz;
      }
   }

   memcpy( &buf[offset], rightWallVertices[1], dataSize);
   offset+=(cvertex-1)*4;

   for (int i=0;i<(cvertex-1);i++)
   {
      Vertex3D_NoTex2 * rgv3D = leftWallVertices[0]+i*4;
   ...
   memcpy( &buf[offset], leftWallVertices[0], dataSize);
   offset+=(cvertex-1)*4;
   memcpy( leftWallVertices[1],leftWallVertices[0], dataSize);

   // Flip Normals and redraw
   for (int i=0;i<(cvertex-1);i++)
   {
      for(int j = 0; j < 4; ++j) {
         leftWallVertices[1][i*4+j].nx = -leftWallVertices[1][i*4+j].nx;
         leftWallVertices[1][i*4+j].ny = -leftWallVertices[1][i*4+j].ny;
         leftWallVertices[1][i*4+j].nz = -leftWallVertices[1][i*4+j].nz;
      }
   }

   memcpy( &buf[offset], leftWallVertices[1], dataSize);
   vertexBuffer->unlock();
   delete(floorVertices);
   delete(rightWallVertices[0]);
   delete(rightWallVertices[1]);
   delete(leftWallVertices[0]);
   delete(leftWallVertices[1]);
}
...
void Ramp::PostRenderStatic(const RenderDevice* _pd3dDevice)
{
   ...
   {		
      ...
      const int rightWallIndex0= ((cvertex-1)*4);
      const int rightWallIndex1= ((cvertex-1)*4)*2;
      const int leftWallIndex0= ((cvertex-1)*4)*3;
      const int leftWallIndex1= ((cvertex-1)*4)*4;

      
      //render floor
      pd3dDevice->renderPrimitive( D3DPT_TRIANGLELIST, vertexBuffer, 0, (cvertex-1)*4, (LPWORD)indices[0], (cvertex-1)*6,0 );
