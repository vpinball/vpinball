...
{
  m_d.m_length =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Spinner", "Length", 80.f) : 80.f;
  m_d.m_rotation =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Spinner", "Rotation", 0.f) : 0.f;
  m_d.m_showBracket = fromMouseClick
                          ? LoadValueBoolWithDefault("DefaultProps\\Spinner", "ShowBracket", true)
                          : true;
  m_d.m_height =
      (float)(fromMouseClick ? LoadValueIntWithDefault("DefaultProps\\Spinner", "Height", 60000)
                             : 60000) /
      1000.0f;
  ...
  m_d.m_angleMax =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Spinner", "AngleMax", 0.f) : 0.f;
  m_d.m_angleMin =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Spinner", "AngleMin", 0.f) : 0.f;
  m_d.m_visible =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Spinner", "Visible", true) : true;
  m_d.m_reflectionEnabled =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Spinner", "ReflectionEnabled", true)
                     : true;
  m_d.m_tdr.m_TimerEnabled =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Spinner", "TimerEnabled", false)
                     : false;
  m_d.m_tdr.m_TimerInterval =
      fromMouseClick ? LoadValueIntWithDefault("DefaultProps\\Spinner", "TimerInterval", 100) : 100;
...
void Spinner::GetHitShapes(vector<HitObject*>& pvho)
{
  ...
  if (m_d.m_showBracket)
  {
    ...
    phitcircle = new HitCircle(
        Vertex2D(m_d.m_vCenter.x + cs * halflength, m_d.m_vCenter.y + sn * halflength),
        m_d.m_length * 0.075f, height + m_d.m_height, height + h);
  ...
  if (m_d.m_showBracket)
  {
    ...
    for (int i = 0; i < spinnerBracketNumVertices; i++)
    {
      ...
      transformedVertices[i].z =
          vert.z * m_d.m_length * m_ptable->m_BG_scalez[m_ptable->m_BG_current_set] + m_posZ;
...
void Spinner::RenderDynamic()
{
  ...
  pd3dDevice->DrawIndexedPrimitiveVB(RenderDevice::TRIANGLELIST, MY_D3DFVF_NOTEX2_VERTEX,
                                     m_plateVertexBuffer, 0, spinnerPlateNumVertices,
                                     m_plateIndexBuffer, 0, spinnerPlateNumFaces);
...
void Spinner::RenderSetup()
{
  ...
  SAFE_BUFFER_RELEASE(m_bracketIndexBuffer);
  m_bracketIndexBuffer = IndexBuffer::CreateAndFillIndexBuffer(
      spinnerBracketNumFaces, spinnerBracketIndices, PRIMARY_DEVICE);

  SAFE_BUFFER_RELEASE(m_bracketVertexBuffer);
  VertexBuffer::CreateVertexBuffer(spinnerBracketNumVertices, 0, MY_D3DFVF_NOTEX2_VERTEX,
  ...
  SAFE_BUFFER_RELEASE(m_plateIndexBuffer);
  m_plateIndexBuffer = IndexBuffer::CreateAndFillIndexBuffer(spinnerPlateNumFaces,
                                                             spinnerPlateIndices, PRIMARY_DEVICE);

  SAFE_BUFFER_RELEASE(m_plateVertexBuffer);
  VertexBuffer::CreateVertexBuffer(spinnerPlateNumVertices, USAGE_DYNAMIC, MY_D3DFVF_NOTEX2_VERTEX,
...
void Spinner::RenderStatic()
{
  ...
  pd3dDevice->basicShader->Begin(0);
  pd3dDevice->DrawIndexedPrimitiveVB(RenderDevice::TRIANGLELIST, MY_D3DFVF_NOTEX2_VERTEX,
                                     m_bracketVertexBuffer, 0, spinnerBracketNumVertices,
                                     m_bracketIndexBuffer, 0, spinnerBracketNumFaces);
  pd3dDevice->basicShader->End();
}
...
{
  m_d.m_elasticity = fromMouseClick
                         ? LoadValueFloatWithDefault("DefaultProps\\Spinner", "Elasticity", 0.3f)
                         : 0.3f;
  m_d.m_damping = fromMouseClick
                      ? LoadValueFloatWithDefault("DefaultProps\\Spinner", "AntiFriction", 0.9879f)
                      : 0.9879f;
}
...
HRESULT Spinner::InitLoad(IStream* pstm,
                          PinTable* ptable,
                          int* pid,
                          int version,
...
STDMETHODIMP Spinner::get_Damping(float* pVal)
{
  *pVal = !g_pplayer ? m_d.m_damping
                     : powf(m_phitspinner->m_spinnerMover.m_damping, (float)(1.0 / PHYS_FACTOR));
