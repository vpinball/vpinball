...
HANDLE connectToIthUSBHIDDevice(DWORD deviceIndex)
{
  ...
  SetupDiGetDeviceInterfaceDetail(hardwareDeviceInfoSet, &deviceInterfaceData,
  ...
  PSP_INTERFACE_DEVICE_DETAIL_DATA deviceDetail =
      (PSP_INTERFACE_DEVICE_DETAIL_DATA)calloc(requiredSize, 1);
  deviceDetail->cbSize = sizeof(SP_INTERFACE_DEVICE_DETAIL_DATA);

  DWORD newRequiredSize;

  //Fill the buffer with the device details
  if (!SetupDiGetDeviceInterfaceDetail(hardwareDeviceInfoSet, &deviceInterfaceData, deviceDetail,
                                       requiredSize, &newRequiredSize, nullptr))
  {
    SetupDiDestroyDeviceInfoList(hardwareDeviceInfoSet);
    free(deviceDetail);
    return INVALID_HANDLE_VALUE;
  ...
  const HANDLE deviceHandle = CreateFile(deviceDetail->DevicePath, GENERIC_READ | GENERIC_WRITE,
...
void hid_init()
{
  ...
  if (hnd != INVALID_HANDLE_VALUE)
  {
    ...
    ReadFile(hnd, inbuffer, Capabilities.InputReportByteLength, &bytes_read, &ol);
