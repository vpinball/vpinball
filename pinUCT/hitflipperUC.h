class FlipperAnimObjectUC : public FlipperAnimObject
	{
public:
	...
	BOOL m_fEnabled;		// The flipper should not be rendered.
	BOOL m_fInvisible;		// The flipper has collision but should not be rendered.
...
public:
	...
	HitFlipperUC(float x, float y, float baser, float endr, float flipr, float angle, float zlow, float zhigh
	...
	FlipperAnimObjectUC m_flipperanim;
	int m_last_hittime;
	};