								...
								,bool direction, bool lateral, bool rigid)
{
	...
	PINFLOAT bnv = ballvx*normal.x + ballvy*normal.y;//ball velocity normal to segment, postive if receeding, zero=parallel

	if (direction &&  bnv > C_LOWNORMVEL)		//direction true and clearly receeding from normal face
	{
#ifndef SHOWNORMAL
	...
	{
		if (bnv * bnd >= 0)										// outside-receeding || inside-approaching
		{
	...
	{phitnormal[1].x = (float)bUnHit;}	// UnHit signal	is receeding from outside target
...
PINFLOAT HitCircle::HitTestBasicRadius(Ball *pball, PINFLOAT dtime, Vertex3D *phitnormal, bool direction,bool lateral, bool rigid)
{
	...
	if (direction && bnv > C_LOWNORMVEL) return -1;				// clearly receeding from radius
	...
	// Kicker is special.. handle ball stalled on kicker, commonly hit while receeding, knocking back into kicker pocket
	...
	{	
		if(!rigid && bnd * bnv > 0) return -1;	// (outside and receeding) or (inside and approaching)
	...
	if (!rigid)											// non rigid body collision? return direction
		phitnormal[1].x = (float)fUnhit;				// UnHit signal	is receeding from target
