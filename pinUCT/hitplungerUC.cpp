...
extern UINT LastPlungerHit;


HitPlungerUC::HitPlungerUC(float x, float y, float x2, float pos, float zheight,Plunger* pPlunger) : HitPlunger(x, y, x2, pos, zheight)
{
...
void PlungerAnimObjectUC::UpdateTimeTemp(PINFLOAT dtime)
{
	m_pos += (float)dtime*m_speed;

	if (m_pos < m_frameEnd)	
	{
		//m_plunger->FireVoidEventParm(DISPID_LimitEvents_EOS, fabs(m_speed));	// send EOS event //TODO
		m_speed = 0;
	...
	{
		//m_plunger->FireVoidEventParm(DISPID_LimitEvents_BOS, fabs(m_speed));	// send Park event //TODO
		m_speed = 0;
...
void PlungerAnimObjectUC::UpdateAcceleration(PINFLOAT dtime)//dtime always 1.0f
{
...
PINFLOAT HitPlungerUC::HitTest(Ball *pball, PINFLOAT dtime, Vertex3D *phitnormal)
{
	int i;
...
void HitPlungerUC::Collide(Ball *pball, Vertex3D *phitnormal)
{
...
void PlungerAnimObjectUC::Check3D()
{
...
ObjFrame *PlungerAnimObjectUC::Draw3D(RECT *prc)
{
	LPDIRECTDRAWSURFACE7 pdds;
