...
public:
  ...
  void UpdateFaceOffset(unsigned int numVertices) { m_faceIndexOffset += numVertices; }
  void WriteObjectName(const string& objname) { fprintf_s(m_fHandle, "o %s\n", objname.c_str()); }
  ...
  void WriteMaterial(const string& texelName,
                     const string& texelFilename,
                     const Material* const mat);

  std::vector<Vertex3D_NoTex2>& GetVertices() { return std::move(m_verts); }
  std::vector<unsigned int>& GetIndices() { return std::move(m_indices); }
...
private:
  ...
  std::unordered_set<std::pair<const Vertex3D_NoTex2*, const unsigned int>,
                     Vertex3D_NoTex2IdxHashFunctor,
                     Vertex3D_NoTex2IdxComparator>
      m_tmpCombined; // only used to find duplicate vertices quickly
