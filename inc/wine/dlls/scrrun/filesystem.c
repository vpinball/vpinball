                                        ...
                                        LCID lcid, DISPID *rgDispId)
{
    ...
    return hr;
#else
    if (!wcsicmp(*rgszNames, L"FolderExists")) {
        *rgDispId = 1000;
        return S_OK;
    }
    else if (!wcsicmp(*rgszNames, L"FileExists")) {
        *rgDispId = 1001;
        return S_OK;
    }
    else if (!wcsicmp(*rgszNames, L"GetFile")) {
        *rgDispId = 1002;
        return S_OK;
    }
                                      ...
                                      EXCEPINFO *pExcepInfo, UINT *puArgErr)
{
    ...
    switch(dispIdMember) {
        case 1000:
            ...
            return filesys_FolderExists(iface, V_BSTR(&pDispParams->rgvarg[0]), &V_BOOL(pVarResult));
        
        case 1001:
            V_VT(pVarResult) = VT_BOOL;
            return filesys_FileExists(iface, V_BSTR(&pDispParams->rgvarg[0]), &V_BOOL(pVarResult));
       
        case 1002:
            V_VT(pVarResult) = VT_DISPATCH;
            return filesys_GetFile(iface, V_BSTR(&pDispParams->rgvarg[0]), (IFile**)&V_DISPATCH(pVarResult));
