...
#include "wine/dlls/scrrun/scrrun_private.h"
...
static IUnknown *create_object(script_ctx_t *ctx, const WCHAR *progid)
{
    ...
    if (!wcsicmp(progid, L"Scripting.FileSystemObject")) {
        ...
        hres = vpinball_create_object(progid, cf, (void**)&obj); 
    }
#endif
    if(FAILED(hres))
        return NULL;
...
static HRESULT Global_Eval(BuiltinDisp *This, VARIANT *arg, unsigned args_cnt, VARIANT *res)
{
    ...
    return E_NOTIMPL;
#else
    named_item_t* item = lookup_named_item(This->ctx, V_BSTR(arg), 0);
    if (item) {
        V_VT(res) = VT_DISPATCH;
        V_DISPATCH(res) = item->disp;
...
#else
    vbscode_t *code;
    HRESULT hres = compile_script(This->ctx, V_BSTR(arg), 0, 0, 0, 0, SCRIPTTEXT_ISVISIBLE, &code);
