...
static HRESULT interp_redim(exec_ctx_t *ctx)
{
    ...
    if(ref.type != REF_VAR) {
        ...
        array_desc = (array_desc_t *)malloc (sizeof(array_desc_t));
        array_desc->dim_cnt = 1;
        array_desc->bounds = (SAFEARRAYBOUND *) malloc(sizeof(SAFEARRAYBOUND));
        to_int(stack_top(ctx, 0), &dim);
        stack_popn(ctx, 1);
        array_desc->bounds->cElements = dim + 1;
        array_desc->bounds->lLbound = 0;
        ctx->func->array_descs = array_desc;
        ctx->instr->arg2.uint = 0;
        VARIANT *new;
        hres = add_dynamic_var(ctx, identifier, FALSE, &new);
        return interp_dim(ctx);
#endif
    }
