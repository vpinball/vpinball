...
inline void ref_count_trigger(const ULONG r,
                              const char* file,
                              const int line) // helper for debugging
{
...
#define SAFE_RELEASE(p) \
  { \
    if (p) \
    { \
      const ULONG rcc = (p)->Release(); \
      if (rcc != 0) \
        ref_count_trigger(rcc, __FILE__, __LINE__); \
      (p) = nullptr; \
    } \
  }
#define SAFE_RELEASE_NO_SET(p) \
...
#define SAFE_RELEASE_NO_RCC(p) \
  ...
  } // use for releasing things like surfaces gotten from GetSurfaceLevel (that seem to "share" the refcount with the underlying texture)
#endif
#define FORCE_RELEASE(p) \
  { \
    if (p) \
    { \
      ULONG rcc = 1; \
  ...
  } // release all references until it is 0
#define SAFE_BUFFER_RELEASE(p) \
  { \
    if (p) \
    { \
      (p)->release(); \
...
class
    Vertex3D_NoTex2 // for rendering, uses MY_D3DFVF_NOTEX2_VERTEX or MY_D3DTRANSFORMED_NOTEX2_VERTEX
{
public:
  // Position
  D3DVALUE x;
...
__forceinline __m128 rsqrtps(const __m128& T) //Newton Raphson
{
  ...
  return _mm_mul_ps(_mm_mul_ps(_mm_set1_ps(0.5f), TRSQRT),
                    _mm_sub_ps(_mm_set1_ps(3.0f), _mm_mul_ps(_mm_mul_ps(TRSQRT, T), TRSQRT)));
}

__forceinline __m128 rsqrtss(const __m128& T) //Newton Raphson
{
  const __m128 TRSQRT = _mm_rsqrt_ss(T);
  return _mm_mul_ss(_mm_mul_ss(_mm_set_ss(0.5f), TRSQRT),
                    _mm_sub_ss(_mm_set_ss(3.0f), _mm_mul_ss(_mm_mul_ss(TRSQRT, T), TRSQRT)));
}
...
__forceinline float rand_mt_01()
{
  return int_as_float(0x3F800000u | (unsigned int)(tinymtu(tinymt64state) >> 41)) - 1.0f;
}
__forceinline float rand_mt_m11()
{
  return int_as_float(0x3F800000u | (unsigned int)(tinymtu(tinymt64state) >> 41)) * 2.0f - 3.0f;
}
...
inline int WideCharToMultiByteNull(const UINT CodePage,
                                   const DWORD dwFlags,
...
// this variant always makes sure that the outgoing string is zero terminated
inline int MultiByteToWideCharNull(const UINT CodePage,
                                   const DWORD dwFlags,
