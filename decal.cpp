...
{
  m_d.m_width =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Decal", "Width", 100.0f) : 100.0f;
  m_d.m_height =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Decal", "Height", 100.0f) : 100.0f;
  m_d.m_rotation =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Decal", "Rotation", 0.f) : 0.f;
  ...
  m_d.m_color = fromMouseClick
                    ? LoadValueIntWithDefault("DefaultProps\\Decal", "Color", RGB(0, 0, 0))
                    : RGB(0, 0, 0);
  m_d.m_verticalText = fromMouseClick
                           ? LoadValueBoolWithDefault("DefaultProps\\Decal", "VerticalText", false)
                           : false;
  ...
  if (!m_pIFont)
  {
    ...
    fd.sWeight = fromMouseClick
                     ? LoadValueIntWithDefault("DefaultProps\\Decal", "FontWeight", FW_NORMAL)
                     : FW_NORMAL;
    fd.sCharset =
        fromMouseClick ? LoadValueIntWithDefault("DefaultProps\\Decal", "FontCharSet", 0) : 0;
    fd.fItalic = fromMouseClick
                     ? LoadValueBoolWithDefault("DefaultProps\\Decal", "FontItalic", false)
                     : false;
    fd.fUnderline = fromMouseClick
                        ? LoadValueBoolWithDefault("DefaultProps\\Decal", "FontUnderline", false)
                        : false;
    fd.fStrikethrough =
        fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Decal", "FontStrikeThrough", false)
                       : false;
...
void Decal::WriteRegDefaults()
{
  ...
  if (m_pIFont)
  {
    ...
    WideCharToMultiByteNull(CP_ACP, 0, fd.lpstrName, -1, strTmp, (int)(2 * charCnt), nullptr,
                            nullptr);
...
void Decal::UIRenderPass1(Sur* const psur)
{
  if (!m_backglass || GetPTable()->GetDecalsEnabled())
  {
    ...
    const Vertex2D rgv[4] = {Vertex2D(m_d.m_vCenter.x + sn * halfheight - cs * halfwidth,
...
void Decal::GetTextSize(int* const px, int* const py)
{
  ...
  if (m_d.m_verticalText)
  {
    ...
    *px = 0;
    for (int i = 0; i < len; i++)
    {
      ...
      rcOut.right = 0x1;
      rcOut.bottom = 0x1;
      clientDC.DrawText(m_d.m_sztext.c_str() + i, 1, rcOut,
                        alignment | DT_NOCLIP | DT_NOPREFIX | DT_WORDBREAK | DT_CALCRECT);
  ...
  else
  {
    *py = tm.tmAscent;
    ...
    rcOut.right = 0x1;
    rcOut.bottom = 0x1;
    clientDC.DrawText(m_d.m_sztext.c_str(), len, rcOut,
                      alignment | DT_NOCLIP | DT_NOPREFIX | DT_WORDBREAK | DT_CALCRECT);
...
void Decal::PreRenderText()
{
  ...
  if (m_d.m_verticalText)
  {
    for (int i = 0; i < len; i++)
    {
      ...
      dc.DrawText(m_d.m_sztext.c_str() + i, 1, rcOut,
...
void Decal::RenderSetup()
{
  ...
  VertexBuffer::CreateVertexBuffer(4, 0, vertexType, &m_vertexBuffer,
  ...
  else
  {
    ...
    for (int i = 0; i < 4; ++i)
    {
      ...
      vertices[i].nx =
          1.0f; //!! as this is the w component due to MY_D3DTRANSFORMED_NOTEX2_VERTEX usage
      vertices[i].ny = 0.0f;
      vertices[i].nz = 0.0f;
    }
  }
...
float Decal::GetDepth(const Vertex3Ds& viewDir) const
{
  const float height =
      ...
      m_ptable->m_BG_scalez[m_ptable->m_BG_current_set];
  return !m_backglass
             ? (viewDir.x * m_d.m_vCenter.x + viewDir.y * m_d.m_vCenter.y + viewDir.z * height)
             : 0.f;
}
...
void Decal::RenderObject()
{
  ...
  pd3dDevice->DrawPrimitiveVB(RenderDevice::TRIANGLEFAN,
                              m_backglass ? MY_D3DTRANSFORMED_NOTEX2_VERTEX
                                          : MY_D3DFVF_NOTEX2_VERTEX,
                              m_vertexBuffer, 0, 4, true);
...
HRESULT Decal::InitLoad(IStream* pstm,
                        PinTable* ptable,
                        int* pid,
                        int version,
...
{
  if (((m_d.m_sizingtype != AutoSize) || (m_d.m_decaltype == DecalImage)) &&
          (m_d.m_sizingtype != AutoWidth) ||
