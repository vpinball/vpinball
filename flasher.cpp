...
void Flasher::PostRenderStatic(RenderDevice* pd3dDevice)
{
   ...
   const D3DXVECTOR4 ab((float)m_d.m_fFilterAmount/100.0f, min(max(m_d.m_modulate_vs_add,0.00001f),0.999f), 0.f,0.f); // avoid 0, as it disables the blend and avoid 1 as it looks not good with day->night changes
