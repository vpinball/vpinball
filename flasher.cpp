...
{
  m_d.m_height =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Flasher", "Height", 50.f) : 50.f;
  m_d.m_rotX =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Flasher", "RotX", 0.f) : 0.f;
  m_d.m_rotY =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Flasher", "RotY", 0.f) : 0.f;
  m_d.m_rotZ =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Flasher", "RotZ", 0.f) : 0.f;
  m_d.m_color = fromMouseClick
                    ? LoadValueIntWithDefault("DefaultProps\\Flasher", "Color", RGB(50, 200, 50))
                    : RGB(50, 200, 50);
  m_d.m_tdr.m_TimerEnabled =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Flasher", "TimerEnabled", false)
                     : false;
  m_d.m_tdr.m_TimerInterval =
      fromMouseClick ? LoadValueIntWithDefault("DefaultProps\\Flasher", "TimerInterval", 100) : 100;
  ...
  m_d.m_modulate_vs_add =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Flasher", "ModulateVsAdd", 0.9f)
                     : 0.9f;
  m_d.m_filterAmount =
      fromMouseClick ? LoadValueIntWithDefault("DefaultProps\\Flasher", "FilterAmount", 100) : 100;
  m_d.m_isVisible =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Flasher", "Visible", true) : true;
  m_inPlayState = m_d.m_isVisible;
  m_d.m_addBlend =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Flasher", "AddBlend", false) : false;
  m_d.m_isDMD =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Flasher", "DMD", false) : false;
  m_d.m_displayTexture =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Flasher", "DisplayTexture", false)
                     : false;
  m_d.m_imagealignment = fromMouseClick ? (RampImageAlignment)LoadValueIntWithDefault(
                                              "DefaultProps\\Flasher", "ImageMode", ImageModeWrap)
                                        : ImageModeWrap;
  m_d.m_filter = fromMouseClick ? (Filters)LoadValueIntWithDefault("DefaultProps\\Flasher",
...
{
  const float height =
      (m_d.m_height + m_ptable->m_tableheight) * m_ptable->m_BG_scalez[m_ptable->m_BG_current_set];
  ...
  TMatrix.SetTranslation(-movx, //-m_d.m_vCenter.x,
...
void Flasher::RenderSetup()
{
  ...
  SAFE_BUFFER_RELEASE(m_dynamicVertexBuffer);
  VertexBuffer::CreateVertexBuffer(m_numVertices, USAGE_DYNAMIC, MY_D3DFVF_TEX,
                                   &m_dynamicVertexBuffer, PRIMARY_DEVICE);
...
void Flasher::Scale(const float scalex,
                    const float scaley,
...
HRESULT Flasher::InitLoad(IStream* pstm,
                          PinTable* ptable,
                          int* pid,
                          int version,
...
STDMETHODIMP Flasher::put_VideoCapUpdate(BSTR cWinTitle)
{
  ...
  m_isVideoCap = StretchBlt(hdcMemDC, 0, 0, pWidth, pHeight, hdcWindow, 0, 0,
                            m_videoSourceRect.right - m_videoSourceRect.left,
                            m_videoSourceRect.bottom - m_videoSourceRect.top, SRCCOPY);
  if (m_isVideoCap)
  {
    ...
    GetDIBits(hdcWindow, hbmScreen, 0, (UINT)bmpScreen.bmHeight, lpbitmap, (BITMAPINFO*)&bi,
              DIB_RGB_COLORS);
...
void Flasher::RenderDynamic()
{
  ...
  if (m_d.m_isDMD && (((g_pplayer->m_dmdstate == 1) && alphadmd) || // render alpha DMD
                      ((g_pplayer->m_dmdstate == 2) && !alphadmd))) // render normal DMD
  {
    ...
    pd3dDevice->DMDShader->SetTechnique(
        SHADER_TECHNIQUE_basic_DMD_world); //!! DMD_UPSCALE ?? -> should just work

    pd3dDevice->DMDShader->SetVector(SHADER_vColor_Intensity, &color);

#ifdef DMD_UPSCALE
    const vec4 r((float)(g_pplayer->m_dmd.x * 3), (float)(g_pplayer->m_dmd.y * 3),
                 m_d.m_modulate_vs_add,
                 (float)(g_pplayer->m_overall_frames %
                         2048)); //(float)(0.5 / m_width), (float)(0.5 / m_height));
#else
    const vec4 r((float)g_pplayer->m_dmd.x, (float)g_pplayer->m_dmd.y, m_d.m_modulate_vs_add,
                 (float)(g_pplayer->m_overall_frames %
                         2048)); //(float)(0.5 / m_width), (float)(0.5 / m_height));
#endif
    pd3dDevice->DMDShader->SetVector(SHADER_vRes_Alpha_time, &r);

    pd3dDevice->DMDShader->SetTexture(SHADER_Texture0,
                                      g_pplayer->m_pin3d.m_pd3dPrimaryDevice->m_texMan.LoadTexture(
                                          g_pplayer->m_texdmd, false, true),
                                      false);

    pd3dDevice->DMDShader->Begin(0);
    pd3dDevice->DrawIndexedPrimitiveVB(RenderDevice::TRIANGLELIST, MY_D3DFVF_TEX,
                                       m_dynamicVertexBuffer, 0, m_numVertices,
                                       m_dynamicIndexBuffer, 0, m_numPolys * 3);
  ...
  else if (g_pplayer->m_dmdstate == 0)
  {
    ...
    const vec4 ab(
        ...
        min(max(m_d.m_modulate_vs_add, 0.00001f),
            0.9999f), // avoid 0, as it disables the blend and avoid 1 as it looks not good with day->night changes
    ...
    pd3dDevice->SetRenderState(RenderDevice::DESTBLEND, m_d.m_addBlend
                                                            ? RenderDevice::INVSRC_COLOR
                                                            : RenderDevice::INVSRC_ALPHA);
    pd3dDevice->SetRenderState(RenderDevice::BLENDOP, m_d.m_addBlend
                                                          ? RenderDevice::BLENDOP_REVSUBTRACT
                                                          : RenderDevice::BLENDOP_ADD);
