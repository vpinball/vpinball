...
void LightVisualsProperty::UpdateVisuals(const int dispid/*=-1*/)
{
    for (int i = 0; i < m_pvsel->size(); i++)
    {
        ...
        if (dispid == IDC_FADE_SPEED_UP || dispid == -1)
            PropertyDialog::SetFloatTextbox(m_fadeSpeedUpEdit, min(100000.0f, light->m_d.m_intensity / light->m_d.m_fadeSpeedUp));
        if (dispid == IDC_FADE_SPEED_DOWN || dispid == -1)
            PropertyDialog::SetFloatTextbox(m_fadeSpeedDownEdit, min(100000.0f, light->m_d.m_intensity / light->m_d.m_fadeSpeedDown));
...
void LightVisualsProperty::UpdateProperties(const int dispid)
{
    for (int i = 0; i < m_pvsel->size(); i++)
    {
        ...
        switch (dispid)
        {
            ...
            case IDC_INTENSITY:
                 {
                     if (light->m_d.m_intensity != PropertyDialog::GetFloatTextbox(m_intensityEdit))
                     {
                         ...
                         float speedUpMs = PropertyDialog::GetFloatTextbox(m_fadeSpeedUpEdit);
                         light->m_d.m_fadeSpeedUp = speedUpMs < 0.1f ? 100000.0f/*(light->m_d.m_intensity * 1000.0f)*/ : (light->m_d.m_intensity / speedUpMs);
                         float speedDownMs = PropertyDialog::GetFloatTextbox(m_fadeSpeedDownEdit);
                         light->m_d.m_fadeSpeedDown = speedDownMs < 0.1f ? 100000.0f/*(light->m_d.m_intensity * 1000.0f)*/ : (light->m_d.m_intensity / speedDownMs);
            ...
            {
                float speedUpMs = PropertyDialog::GetFloatTextbox(m_fadeSpeedUpEdit);
                CHECK_UPDATE_ITEM(light->m_d.m_fadeSpeedUp, speedUpMs < 0.1f ? 100000.0f/*(light->m_d.m_intensity * 1000.0f)*/ : (light->m_d.m_intensity / speedUpMs), light);
                break;
            }
            case IDC_FADE_SPEED_DOWN:
            {
                float speedDownMs = PropertyDialog::GetFloatTextbox(m_fadeSpeedDownEdit);
                CHECK_UPDATE_ITEM(light->m_d.m_fadeSpeedDown, speedDownMs < 0.1f ? 100000.0f/*(light->m_d.m_intensity * 1000.0f)*/ : (light->m_d.m_intensity / speedDownMs), light);
                break;
            }
