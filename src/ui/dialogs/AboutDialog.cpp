...
INT_PTR AboutDialog::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
   switch (uMsg)
   {
      case WM_INITDIALOG:
      {
         ...
         {
            std::ifstream file(g_pvp->m_myPath + "Changelog.txt");
            if (!file.is_open())
               file = std::ifstream(g_pvp->m_myPath + "Doc"  + PATH_SEPARATOR_CHAR + "Changelog.txt");
            if (!file.is_open())
               file = std::ifstream(g_pvp->m_myPath + "docs" + PATH_SEPARATOR_CHAR + "Changelog.txt");
            string line, text;
