...
void PlumbOverlay::Update()
{
   ...
   if (m_lastTiltIndex != m_player->m_physics->GetPlumlbTiltIndex())
   {
      m_lastTiltIndex = m_player->m_physics->GetPlumlbTiltIndex();
      m_tiltFade = 1.f;
   ...
   ImGui::PushStyleColor(ImGuiCol_WindowBg, IM_COL32(m_tiltFade * 255, 0, 0, plumbFade * 64));
   ImGui::SetNextWindowSize(fullSize);
   ImGui::Begin("PlumbOverlay", nullptr, window_flags);
   const ImVec2 &pos = ImGui::GetWindowPos();
   ImU32 alphaCol = IM_COL32(255, 0, 0, plumbFade * 255);
   // Tilt circle
