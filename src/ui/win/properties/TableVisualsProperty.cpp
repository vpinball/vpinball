...
TableVisualsProperty::TableVisualsProperty(const VectorProtected<ISelect> *pvsel)
...
void TableVisualsProperty::UpdateProperties(const int dispid)
{
   ...
   case IDC_MATERIAL_COMBO: CHECK_UPDATE_COMBO_TEXT_STRING(table->m_playfieldMaterial, m_materialCombo, table); break;
   case IDC_REFLECTION_PLAYFIELD:
      ...
      break;
   case IDC_BALL_SPHERICAL_MAP: CHECK_UPDATE_ITEM(table->m_ballSphericalMapping, PropertyDialog::GetCheckboxState(m_hSphericalMapCheck), table); break;
   case IDC_BALL_DECAL_MODE: CHECK_UPDATE_ITEM(table->m_BallDecalMode, PropertyDialog::GetCheckboxState(m_hLogoModeCheck), table); break;
   case DISPID_Image3: CHECK_UPDATE_COMBO_TEXT_STRING(table->m_ballImage, m_ballImageCombo, table); break;
   case DISPID_Image4: CHECK_UPDATE_COMBO_TEXT_STRING(table->m_ballImageDecal, m_ballDecalCombo, table); break;
   case IDC_BALLPLAYFIELD_REFLECTION: CHECK_UPDATE_ITEM(table->m_ballPlayfieldReflectionStrength, PropertyDialog::GetFloatTextbox(m_ballReflectPlayfieldEdit), table); break;
   case IDC_BULBINTENSITYSCALE: CHECK_UPDATE_ITEM(table->m_defaultBulbIntensityScaleOnBall, PropertyDialog::GetFloatTextbox(m_ballDefaultBulbIntensScaleEdit), table); break;
   case IDC_ENABLE_AO: CHECK_UPDATE_ITEM(table->m_enableAO, PropertyDialog::GetCheckboxState(m_hEnableAOCheck), table); break;
   case IDC_AOSCALE: CHECK_UPDATE_ITEM(table->m_AOScale, PropertyDialog::GetFloatTextbox(m_ambientOcclusionScaleEdit), table); break;
   case IDC_ENABLE_SSR: CHECK_UPDATE_ITEM(table->m_enableSSR, PropertyDialog::GetCheckboxState(m_hEnableSSRCheck), table); break;
   case IDC_SSR_STRENGTH: CHECK_UPDATE_ITEM(table->m_SSRScale, PropertyDialog::GetFloatTextbox(m_screenSpaceReflEdit), table); break;
   case IDC_BLOOM_STRENGTH: CHECK_UPDATE_ITEM(table->m_bloom_strength, PropertyDialog::GetFloatTextbox(m_bloomStrengthEdit), table); break;
   case IDC_EXPOSURE: CHECK_UPDATE_VALUE_SETTER(table->SetExposure, table->GetExposure, PropertyDialog::GetFloatTextbox, m_exposureEdit, table); break;
   case IDC_TONEMAPPER:
