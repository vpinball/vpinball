...
void TablePhysicsProperty::UpdateProperties(const int dispid)
{
   ...
   case IDC_DEFAULT_SCATTER_EDIT: CHECK_UPDATE_ITEM(table->m_defaultScatter, PropertyDialog::GetFloatTextbox(m_defaultScatterEdit), table); break;
   case IDC_NUDGE_TIME_EDIT: CHECK_UPDATE_ITEM(table->m_nudgeTime, PropertyDialog::GetFloatTextbox(m_nudgeTimeEdit), table); break;
   case IDC_PHYSICS_MAX_LOOPS_EDIT: CHECK_UPDATE_ITEM(table->m_PhysicsMaxLoops, PropertyDialog::GetIntTextbox(m_physicsLoopEdit), table); break;
   case IDC_TABLE_WIDTH_EDIT: CHECK_UPDATE_VALUE_SETTER(table->SetTableWidth, table->GetTableWidth, PropertyDialog::GetFloatTextboxInchesToVPU, m_tableWidthEdit, table); break;
   case IDC_TABLE_HEIGHT_EDIT: CHECK_UPDATE_VALUE_SETTER(table->SetHeight, table->GetHeight, PropertyDialog::GetFloatTextboxInchesToVPU, m_tableHeightEdit, table); break;
   case IDC_TABLE_GLASS_BOTTOM_HEIGHT_EDIT: CHECK_UPDATE_ITEM(table->m_glassBottomHeight, PropertyDialog::GetFloatTextboxInchesToVPU(m_bottomGlassHeightEdit), table); break;
   case IDC_TABLE_GLASS_TOP_HEIGHT_EDIT: CHECK_UPDATE_ITEM(table->m_glassTopHeight, PropertyDialog::GetFloatTextboxInchesToVPU(m_topGlassHeightEdit), table); break;
   case IDC_TABLE_GROUND_TO_LOCKBAR_HEIGHT_EDIT: CHECK_UPDATE_ITEM(table->m_groundToLockbarHeight, PropertyDialog::GetFloatTextboxInchesToVPU(m_groundToLockbarHeightEdit), table); break;
   case IDC_MIN_DIFFICULTY_EDIT: CHECK_UPDATE_ITEM(table->m_angletiltMin, PropertyDialog::GetFloatTextbox(m_minSlopeEdit), table); break;
   case IDC_MAX_DIFFICULTY_EDIT: CHECK_UPDATE_ITEM(table->m_angletiltMax, PropertyDialog::GetFloatTextbox(m_maxSlopeEdit), table); break;
   case IDC_GAME_DIFFICULTY_EDIT: CHECK_UPDATE_ITEM(table->m_difficulty, PropertyDialog::GetFloatTextbox(m_gameplayDifficultyEdit) / 100.f, table); break;
   case IDC_OVERWRITE_PHYSICS_COMBO: CHECK_UPDATE_ITEM(table->m_overridePhysics, (PropertyDialog::GetComboBoxIndex(m_overwritePhysicsSetCombo, m_physicSetList)), table); break;
   case IDC_OVERRIDEPHYSICS_FLIPPERS: CHECK_UPDATE_ITEM(table->m_overridePhysicsFlipper, PropertyDialog::GetCheckboxState(m_hOverwriteFlipperCheck), table); break;
   default: break;
   }
...
BOOL TablePhysicsProperty::OnCommand(WPARAM wParam, LPARAM lParam)
{
   ...
   {
      ((PinTable *)SelAt(0)->GetEditable())->m_tableEditor->ImportPhysics();
      return TRUE;
   }
   case IDC_EXPORT_PHYSICS_BUTTON:
   {
      ((PinTable *)SelAt(0)->GetEditable())->m_tableEditor->ExportPhysics();
      return TRUE;
   }
   default: break;
   }
