...
BOOL EditorOptionsDialog::OnInitDialog()
{
    ...
    m_defaultMaterialColor = g_app->GetSettings().GetEditor_DefaultMaterialColor(); 
    ...
    const std::filesystem::path iniPath = g_app->GetSettings().GetIniPath();
    if (iniPath == appPath)
    ...
    const int units = g_app->GetSettings().GetEditor_Units();
...
BOOL EditorOptionsDialog::OnCommand(WPARAM wParam, LPARAM lParam)
{
    ...
    switch (LOWORD(wParam))
    {
       ...
       case IDC_SET_DEFAULTS_BUTTON:
       {
          ...
          constexpr int x = 0;
          constexpr int y = 0;
          g_app->GetSettings().SetEditor_CodeViewPosX(x, false);
          g_app->GetSettings().SetEditor_CodeViewPosY(y, false);
          constexpr int width = 640;
          constexpr int height = 490;
          g_app->GetSettings().SetEditor_CodeViewPosWidth(width, false);
          g_app->GetSettings().SetEditor_CodeViewPosHeight(height, false);
...
void EditorOptionsDialog::OnOK()
{
    ...
    if (iniPath == prefPath || iniPath == appPath) // Not available when running from a custom ini specified on the commandline
    {
       ...
       g_app->GetSettings().SetIniPath(checked ? appPath : prefPath);
    }
    ...
    g_app->GetSettings().SetEditor_DefaultMaterialColor(m_defaultMaterialColor, false);
    g_app->GetSettings().SetEditor_ElementSelectColor((int)m_vpxEditor->m_elemSelectColor, false);
    g_app->GetSettings().SetEditor_ElementSelectLockedColor((int)m_vpxEditor->m_elemSelectLockedColor, false);
    g_app->GetSettings().SetEditor_BackGroundColor((int)m_vpxEditor->m_backgroundColor, false);
    g_app->GetSettings().SetEditor_FillColor((int)m_vpxEditor->m_fillColor, false);

    checked = (IsDlgButtonChecked(IDC_START_VP_FILE_DIALOG) == BST_CHECKED);
    g_app->GetSettings().SetEditor_SelectTableOnStart(checked, false);

    checked = (IsDlgButtonChecked(IDC_START_VP_FILE_DIALOG2) == BST_CHECKED);
    g_app->GetSettings().SetEditor_SelectTableOnPlayerClose(checked, false);

    LRESULT units = SendDlgItemMessage(IDC_UNIT_LIST_COMBO, CB_GETCURSEL, 0, 0);
    if (units == LB_ERR)
        units = 0;
    g_app->GetSettings().SetEditor_Units((int)units, false);
