...
BOOL PhysicsOptionsDialog::OnInitDialog()
{
    ...
    {
        physicsoptions[i] = g_app->GetSettings().GetPlayer_PhysicsSetName(i);
    ...
    const float FlipperPhysicsMass = g_app->GetSettings().GetPlayer_FlipperPhysicsMass(physicsselection);
    SetItemText(DISPID_Flipper_Speed, FlipperPhysicsMass);
    const float FlipperPhysicsStrength = g_app->GetSettings().GetPlayer_FlipperPhysicsStrength(physicsselection);
    SetItemText(19, FlipperPhysicsStrength);
    const float FlipperPhysicsElasticity = g_app->GetSettings().GetPlayer_FlipperPhysicsElasticity(physicsselection);
    SetItemText(21, FlipperPhysicsElasticity);
    const float FlipperPhysicsScatter = g_app->GetSettings().GetPlayer_FlipperPhysicsScatter(physicsselection);
    SetItemText(112, FlipperPhysicsScatter);
    const float FlipperPhysicsTorqueDamping = g_app->GetSettings().GetPlayer_FlipperPhysicsEOSTorque(physicsselection);
    SetItemText(113, FlipperPhysicsTorqueDamping);
    const float FlipperPhysicsTorqueDampingAngle = g_app->GetSettings().GetPlayer_FlipperPhysicsEOSTorqueAngle(physicsselection);
    SetItemText(189, FlipperPhysicsTorqueDampingAngle);
    const float FlipperPhysicsReturnStrength = g_app->GetSettings().GetPlayer_FlipperPhysicsReturnStrength(physicsselection);
    SetItemText(23, FlipperPhysicsReturnStrength);
    const float FlipperPhysicsElasticityFalloff = g_app->GetSettings().GetPlayer_FlipperPhysicsElasticityFalloff(physicsselection);
    SetItemText(22, FlipperPhysicsElasticityFalloff);
    const float FlipperPhysicsFriction = g_app->GetSettings().GetPlayer_FlipperPhysicsFriction(physicsselection);
    SetItemText(109, FlipperPhysicsFriction);
    const float FlipperPhysicsCoilRampUp = g_app->GetSettings().GetPlayer_FlipperPhysicsCoilRampUp(physicsselection);
    SetItemText(110, FlipperPhysicsCoilRampUp);
    const float TablePhysicsGravityConstant = g_app->GetSettings().GetPlayer_TablePhysicsGravityConstant(physicsselection);
    SetItemText(1100, TablePhysicsGravityConstant);
    const float TablePhysicsContactFriction = g_app->GetSettings().GetPlayer_TablePhysicsContactFriction(physicsselection);
    SetItemText(1101, TablePhysicsContactFriction);
    const float TablePhysicsElasticity = g_app->GetSettings().GetPlayer_TablePhysicsElasticity(physicsselection);
    SetItemText(1708, TablePhysicsElasticity);
    const float TablePhysicsElasticityFalloff = g_app->GetSettings().GetPlayer_TablePhysicsElasticityFalloff(physicsselection);
    SetItemText(1709, TablePhysicsElasticityFalloff);
    const float TablePhysicsScatterAngle = g_app->GetSettings().GetPlayer_TablePhysicsScatterAngle(physicsselection);
    SetItemText(1710, TablePhysicsScatterAngle);
    const float TablePhysicsContactScatterAngle = g_app->GetSettings().GetPlayer_TablePhysicsContactScatterAngle(physicsselection);
    SetItemText(1102, TablePhysicsContactScatterAngle);
    const float TablePhysicsMinSlope = g_app->GetSettings().GetPlayer_TablePhysicsMinSlope(physicsselection);
    SetItemText(1103, TablePhysicsMinSlope);
    const float TablePhysicsMaxSlope = g_app->GetSettings().GetPlayer_TablePhysicsMaxSlope(physicsselection);
...
BOOL PhysicsOptionsDialog::OnCommand(WPARAM wParam, LPARAM lParam)
{
    ...
    switch(LOWORD(wParam))
    {
        ...
        case 1112:
        {
            ...
            string szInitialDir = g_app->GetSettings().GetRecentDir_PhysicsDir();
...
{
    const string& szInitialDir = g_app->GetSettings().GetRecentDir_PhysicsDir();
    ...
    if (index != string::npos)
        g_app->GetSettings().SetRecentDir_PhysicsDir(szFileName[0].substr(0, index), false);
...
{
    g_app->GetSettings().SetPlayer_FlipperPhysicsMass(physicsselection, sz2f(GetItemText(DISPID_Flipper_Speed)), false);
    g_app->GetSettings().SetPlayer_FlipperPhysicsStrength(physicsselection, sz2f(GetItemText(19)), false);
    g_app->GetSettings().SetPlayer_FlipperPhysicsElasticity(physicsselection, sz2f(GetItemText(21)), false);
    g_app->GetSettings().SetPlayer_FlipperPhysicsScatter(physicsselection, sz2f(GetItemText(112)), false);
    g_app->GetSettings().SetPlayer_FlipperPhysicsEOSTorque(physicsselection, sz2f(GetItemText(113)), false);
    g_app->GetSettings().SetPlayer_FlipperPhysicsEOSTorqueAngle(physicsselection, sz2f(GetItemText(189)), false);
    g_app->GetSettings().SetPlayer_FlipperPhysicsReturnStrength(physicsselection, sz2f(GetItemText(23)), false);
    g_app->GetSettings().SetPlayer_FlipperPhysicsElasticityFalloff(physicsselection, sz2f(GetItemText(22)), false);
    g_app->GetSettings().SetPlayer_FlipperPhysicsFriction(physicsselection, sz2f(GetItemText(109)), false);
    g_app->GetSettings().SetPlayer_FlipperPhysicsCoilRampUp(physicsselection, sz2f(GetItemText(110)), false);
    g_app->GetSettings().SetPlayer_TablePhysicsGravityConstant(physicsselection, sz2f(GetItemText(1100)), false);
    g_app->GetSettings().SetPlayer_TablePhysicsContactFriction(physicsselection, sz2f(GetItemText(1101)), false);
    g_app->GetSettings().SetPlayer_TablePhysicsElasticity(physicsselection, sz2f(GetItemText(1708)), false);
    g_app->GetSettings().SetPlayer_TablePhysicsElasticityFalloff(physicsselection, sz2f(GetItemText(1709)), false);
    g_app->GetSettings().SetPlayer_TablePhysicsScatterAngle(physicsselection, sz2f(GetItemText(1710)), false);
    g_app->GetSettings().SetPlayer_TablePhysicsContactScatterAngle(physicsselection, sz2f(GetItemText(1102)), false);
    g_app->GetSettings().SetPlayer_TablePhysicsMinSlope(physicsselection, sz2f(GetItemText(1103)), false);
    g_app->GetSettings().SetPlayer_TablePhysicsMaxSlope(physicsselection, sz2f(GetItemText(1104)), false);
    g_app->GetSettings().SetPlayer_PhysicsSetName(physicsselection, GetItemText(1110), false);
}
