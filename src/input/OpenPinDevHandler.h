...
class OpenPinDevHandler : public InputHandler
{
public:
   explicit OpenPinDevHandler(PinInput& pininput);
   ~OpenPinDevHandler() final;
   void Update() final;

private:
   static inline uint64_t GetJoyId(const int index) { return static_cast<uint64_t>(0x400000000) | static_cast<uint64_t>(index); }
