...
void Player::NudgeUpdate()      // called on every integral physics frame
{
   ...
   for (int j = 0; j < m_pininput.m_num_joy; ++j)
   {
      ...
      m_Nudge.y += m_ptable->m_tblAccelNormalMount ? nugY : -nugY;               // add as left or right hand coordinate system
   }
}
...
void Player::FilterNudge(U64 timeStamp)
{
   m_NudgeFilterX.sample(m_Nudge.x, timeStamp);
   m_NudgeFilterY.sample(m_Nudge.y, timeStamp);
}
