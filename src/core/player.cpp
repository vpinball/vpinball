...
#ifdef __STANDALONE__
      int sx = g_pvp->m_settings.LoadValueWithDefault(Settings::Player, "WindowPosX"s, -1);
      int sy = g_pvp->m_settings.LoadValueWithDefault(Settings::Player, "WindowPosY"s, -1);
      int sw = g_pvp->m_settings.LoadValueWithDefault(Settings::Player, "Width"s, -1);
      int sh = g_pvp->m_settings.LoadValueWithDefault(Settings::Player, "Height"s, -1);
      if (display == -1 && sx != -1 && sy != -1 && sw != -1 && sh != -1) {
        x = sx;
        y = sy;
        m_wnd_width = sw;
        m_wnd_height = sh;

        PLOGI.printf("Window resolution override: %dx%d+%d,%d", m_wnd_width, m_wnd_height, x, y);
      }
#endif
   }
...
#else
      for (vector<DisplayConfig>::iterator dispConf = displays.begin(); dispConf != displays.end(); ++dispConf) {
         if (dispConf->left == cs.x && dispConf->top == cs.y && dispConf->width == cs.cx && dispConf->height == cs.cy) {
            flags |= SDL_WINDOW_FULLSCREEN_DESKTOP;
            break;
         }
      }
...
void Player::OnClose()
{
   ...
   if ((m_ptable->m_settings.LoadValueWithDefault(Settings::Player, "CacheMode"s, 1) > 0) && FileExists(m_ptable->m_szFileName))
   {
      ...
      ankerl::unordered_dense::map<string, bool> prevPreRenderOnly;
   ...
   if (stereo)
   {
      ...
      if (m_stereo3D == STEREO_VR)
      {
         ...
         if (m_liveUI->IsTweakMode()) // Render as an overlay in VR (not in preview) at the eye resolution, without depth
            m_pin3d.m_pd3dPrimaryDevice->RenderLiveUI(0);
         ...
         if (m_liveUI->IsTweakMode()) // Render as an overlay in VR (not in preview) at the eye resolution, without depth
            m_pin3d.m_pd3dPrimaryDevice->RenderLiveUI(1);
