...
{
public:
   MeshBuffer(const wstring& name, std::shared_ptr<VertexBuffer> vb, std::shared_ptr<IndexBuffer> = nullptr, const bool applyVertexBufferOffsetToIndexBuffer = false);
   ...
   std::unique_ptr<MeshBuffer> CreateSharedVertexMeshBuffer(std::shared_ptr<IndexBuffer> ib) const;
