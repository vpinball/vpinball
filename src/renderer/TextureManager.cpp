...
Sampler* TextureManager::LoadTexture(BaseTexture* memtex, const SamplerFilter filter, const SamplerAddressMode clampU, const SamplerAddressMode clampV, const bool force_linear_rgb)
{
   ...
   if (it == m_map.end())
   {
      ...
      entry.preRenderOnly = g_pplayer->m_renderer && g_pplayer->m_renderer->IsRenderPass(Pin3D::STATIC_ONLY);
      m_map[memtex] = entry;
   ...
   else
   {
      ...
      entry.filter = filter;
      entry.forceLinearRGB = force_linear_rgb;
      entry.preRenderOnly &= g_pplayer->m_renderer->IsRenderPass(Pin3D::STATIC_ONLY);
      return entry.sampler;
   }
}
