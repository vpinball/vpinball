   ...
   , m_renderFrame(this)
{
   ...
   init.resolution.width = wnd->GetDrawableWidth();
   init.resolution.height = wnd->GetDrawableHeight();
...
void RenderDevice::AddWindow(VPX::Window* wnd)
{
   ...
   return nullptr;
#endif // BX_PLATFORM_
   bgfx::FrameBufferHandle fbh = bgfx::createFrameBuffer(nwh, uint16_t(wnd->GetDrawableWidth()), uint16_t(wnd->GetDrawableHeight()));
   m_outputWnd[m_nOutputWnd] = wnd;
   m_nOutputWnd++;
   wnd->SetBackBuffer(new RenderTarget(this, SurfaceType::RT_DEFAULT, fbh, BGFX_INVALID_HANDLE, BGFX_INVALID_HANDLE, "BackBuffer #" + std::to_string(m_nOutputWnd), wnd->GetDrawableWidth(), wnd->GetDrawableHeight(), fmt));
#endif
}
