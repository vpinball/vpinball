...
{
   ankerl::unordered_dense::map<string,DisplayConfig>* data = reinterpret_cast<ankerl::unordered_dense::map<string,DisplayConfig>*>(dwData);
   MONITORINFOEX info;
...
int getDisplayList(vector<DisplayConfig>& displays)
{
   ...
   for (int i = 0; i < SDL_GetNumVideoDisplays(); ++i)
   {
      ...
      {
         for (ankerl::unordered_dense::map<string, DisplayConfig>::iterator display = displayMap.begin(); display != displayMap.end(); ++display)
         {
