...
void RenderDevice::RenderThread(RenderDevice* rd, const bgfx::Init& initReq)
{
   ...
   if ((bgfx::getCaps()->supported & BGFX_CAPS_HDR10) && (g_pplayer->m_vrDevice == nullptr) && (g_app->m_commandLineProcessor.m_captureAttract == 0))
   {
   ...
   #endif
   {
      ...
      while (rd->m_renderDeviceAlive)
      {
         ...
         if (!rd->m_framePending)
            continue;
         const bool useVSync = (g_pplayer->GetVideoSyncMode() == VideoSyncMode::VSM_VSYNC) && (g_app->m_commandLineProcessor.m_captureAttract == 0);
