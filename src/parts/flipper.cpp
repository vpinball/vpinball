...
#define LinkProp(field, prop) field = fromMouseClick ? g_app->GetSettings().GetDefaultPropsFlipper_##prop() : Settings::GetDefaultPropsFlipper_##prop##_Default()
...
{
#define LinkProp(field, prop) g_app->GetSettings().SetDefaultPropsFlipper_##prop(field, false)
   LinkProp(m_d.m_scatter, Scatter);
   LinkProp(m_d.m_strength, Strength);
   LinkProp(m_d.m_torqueDamping, EOSTorque);
   LinkProp(m_d.m_torqueDampingAngle, EOSTorqueAngle);
...
void Flipper::UpdatePhysicsSettings()
{
   if (m_d.m_OverridePhysics || (m_ptable->m_overridePhysicsFlipper && m_ptable->m_overridePhysics))
   {
      ...
      m_d.m_OverrideMass = g_app->GetSettings().GetPlayer_FlipperPhysicsMass(idx);
      if (m_d.m_OverrideMass < 0.0f)
         m_d.m_OverrideMass = m_d.m_mass;

      m_d.m_OverrideStrength = g_app->GetSettings().GetPlayer_FlipperPhysicsStrength(idx);
      if (m_d.m_OverrideStrength < 0.0f)
         m_d.m_OverrideStrength = m_d.m_strength;

      m_d.m_OverrideElasticity = g_app->GetSettings().GetPlayer_FlipperPhysicsElasticity(idx);
      if (m_d.m_OverrideElasticity < 0.0f)
         m_d.m_OverrideElasticity = m_d.m_elasticity;

      m_d.m_OverrideScatterAngle = g_app->GetSettings().GetPlayer_FlipperPhysicsScatter(idx);
      if (m_d.m_OverrideScatterAngle < 0.0f)
         m_d.m_OverrideScatterAngle = m_d.m_scatter;

      m_d.m_OverrideReturnStrength = g_app->GetSettings().GetPlayer_FlipperPhysicsReturnStrength(idx);
      if (m_d.m_OverrideReturnStrength < 0.0f)
         m_d.m_OverrideReturnStrength = m_d.m_return;

      m_d.m_OverrideElasticityFalloff = g_app->GetSettings().GetPlayer_FlipperPhysicsElasticityFalloff(idx);
      if (m_d.m_OverrideElasticityFalloff < 0.0f)
         m_d.m_OverrideElasticityFalloff = m_d.m_elasticityFalloff;

      m_d.m_OverrideFriction = g_app->GetSettings().GetPlayer_FlipperPhysicsFriction(idx);
      if (m_d.m_OverrideFriction < 0.0f)
         m_d.m_OverrideFriction = m_d.m_friction;

      m_d.m_OverrideCoilRampUp = g_app->GetSettings().GetPlayer_FlipperPhysicsCoilRampUp(idx);
      if (m_d.m_OverrideCoilRampUp < 0.0f)
         m_d.m_OverrideCoilRampUp = m_d.m_rampUp;

      m_d.m_OverrideTorqueDamping = g_app->GetSettings().GetPlayer_FlipperPhysicsEOSTorque(idx);
      if (m_d.m_OverrideTorqueDamping < 0.0f)
         m_d.m_OverrideTorqueDamping = m_d.m_torqueDamping;

      m_d.m_OverrideTorqueDampingAngle = g_app->GetSettings().GetPlayer_FlipperPhysicsEOSTorqueAngle(idx);
