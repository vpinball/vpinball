...
{
#define LinkProp(field, prop) field = fromMouseClick ? g_app->GetSettings().GetDefaultPropsDispReel_##prop() : Settings::GetDefaultPropsDispReel_##prop##_Default()
...
{
#define LinkProp(field, prop) g_app->GetSettings().SetDefaultPropsDispReel_##prop(field, false)
   LinkProp(m_d.m_szImage, Image);
   LinkProp(m_d.m_szSound, Sound);
   LinkProp(m_d.m_useImageGrid, UseImageGrid);
   LinkProp(m_d.m_visible, Visible);
