...
bool mixer_init(const HWND wnd)
{
  ...
  if (::mixerOpen(&m_hMixer, 0, reinterpret_cast<size_t>(wnd), 0,
...
void mixer_update()
{
  ...
  mxcd.cbDetails = sizeof(MIXERCONTROLDETAILS_UNSIGNED);
  mxcd.paDetails = &mxcdVolume;
  if (::mixerSetControlDetails(reinterpret_cast<HMIXEROBJ>(m_hMixer), &mxcd,
...
void mixer_draw()
{
  ...
  const bool cabMode =
      fmodf(g_pplayer->m_ptable->m_BG_rotation[g_pplayer->m_ptable->m_BG_current_set], 360.f) !=
      0.f;
  ...
  for (F32 vol = 0.f, y = -volume_adjustment_bar_ysize * 0.5f; vol < 1.0f;
       vol += yoff / volume_adjustment_bar_ysize, y += yoff)
  {
    ...
    // Set the color.
    // Draw the tick mark.  (Reversed x and y to match coordinate system of front end.)
    g_pplayer->Spritedraw(cabMode ? fX : fY, cabMode ? fY : fX, cabMode ? size[0] : size[1],
                          cabMode ? size[1] : size[0], color, (Texture*)nullptr, fade);
  }
