...
public:
  ...
  void lock(const unsigned int offsetToLock,
  ...
  static void CreateIndexBuffer(const unsigned int numIndices,
                                const DWORD usage,
                                const IndexBuffer::Format format,
                                IndexBuffer** idxBuffer,
                                const deviceNumber dN);

  static IndexBuffer* CreateAndFillIndexBuffer(const unsigned int numIndices,
...
public:
  ...
  enum LockFlags
  {
    ...
    NOOVERWRITE =
        D3DLOCK_NOOVERWRITE, // meaning: no recently drawn vertices are overwritten. only works with dynamic VBs.
  ...
  static void bindNull() { m_curIndexBuffer = nullptr; }
  static void setD3DDevice(IDirect3DDevice9* primary, IDirect3DDevice9* secondary)
  {
    m_pd3dPrimaryDevice = primary;
    m_pd3dSecondaryDevice = secondary;
  }
