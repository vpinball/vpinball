...
#ifdef __STANDALONE__
   // if that fails, try the path where the table was loaded from
   if (!success)
      success = GetTextFileFromDirectory(m_vpinball->m_currentTablePath + szFileName, string(), pContents);
#endif
...
STDMETHODIMP ScriptGlobalTable::SaveValue(BSTR TableName, BSTR ValueName, VARIANT Value)
{
   ...
   pstgRoot->Release();
#else
   string szIniPath = LoadValueWithDefault(regKey[RegName::Standalone], "VPRegPath"s, ""s);
