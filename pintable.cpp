...
STDMETHODIMP ScriptGlobalTable::NudgeGetCalibration(VARIANT* XMax,
                                                    VARIANT* YMax,
                                                    VARIANT* XGain,
                                                    VARIANT* YGain,
                                                    VARIANT* DeadZone,
...
STDMETHODIMP ScriptGlobalTable::NudgeSetCalibration(
    int XMax, int YMax, int XGain, int YGain, int DeadZone, int TiltSensitivity)
{
  int newvalue;

  newvalue = XGain;
  if ((SSIZE_T)newvalue < 0)
  {
    newvalue = 0;
  }
...
STDMETHODIMP ScriptGlobalTable::PlaySound(BSTR bstr,
                                          long LoopCount,
                                          float volume,
                                          float pan,
                                          float randompitch,
                                          ...
                                          float front_rear_fade)
{
  ...
  {
    m_pt->PlaySound(bstr, LoopCount, volume, pan, randompitch, pitch, usesame, restart,
                    front_rear_fade);
  }
...
STDMETHODIMP ScriptGlobalTable::PlayMusic(BSTR str, float volume)
{
  if (g_pplayer && g_pplayer->m_PlayMusic)
  {
    ...
    const float MusicVolume =
        max(min((float)g_pplayer->m_MusicVolume * m_pt->m_TableMusicVolume * volume, 100.0f),
            0.0f) *
        (float)(1.0 / 100.0);
    ...
    if (!g_pplayer->m_audio->MusicInit(m_vpinball->m_szMyPath + "Music\\" + szT,
                                       string("C:\\Visual Pinball\\Music\\") + szT, MusicVolume))
    {
...
bool ScriptGlobalTable::GetTextFileFromDirectory(const char* const szfilename,
...
STDMETHODIMP ScriptGlobalTable::GetTextFile(BSTR FileName, BSTR* pContents)
{
  ...
  if (!success)
  {
    success = GetTextFileFromDirectory((string("C:\\Visual Pinball\\User\\") + szFileName).c_str(),
                                       nullptr, pContents);
  }
  if (!success)
  {
    success = GetTextFileFromDirectory(
        (string("C:\\Visual Pinball\\Scripts\\") + szFileName).c_str(), nullptr, pContents);
  }
  if (!success)
  {
    success = GetTextFileFromDirectory(
        (string("C:\\Visual Pinball\\Tables\\") + szFileName).c_str(), nullptr, pContents);
  }
...
STDMETHODIMP ScriptGlobalTable::SaveValue(BSTR TableName, BSTR ValueName, VARIANT Value)
{
  ...
  IStorage* pstgRoot;
  if (FAILED(hr = StgOpenStorage(wzPath.c_str(), nullptr,
                                 STGM_TRANSACTED | STGM_READWRITE | STGM_SHARE_EXCLUSIVE, nullptr,
                                 0, &pstgRoot)))
  {
    // Registry file does not exist - create it
    if (FAILED(hr = StgCreateDocfile(wzPath.c_str(),
  ...
  IStorage* pstgTable;
  if (FAILED(hr = pstgRoot->OpenStorage(TableName, nullptr,
                                        STGM_TRANSACTED | STGM_READWRITE | STGM_SHARE_EXCLUSIVE,
                                        nullptr, 0, &pstgTable)))
  {
    // Table file does not exist
    if (FAILED(hr = pstgRoot->CreateStorage(
                   TableName, STGM_TRANSACTED | STGM_READWRITE | STGM_SHARE_EXCLUSIVE | STGM_CREATE,
                   0, 0, &pstgTable)))
    {
      pstgRoot->Release();
      return hr;
  ...
  IStream* pstmValue;
  if (FAILED(hr = pstgTable->CreateStream(
                 ValueName, STGM_DIRECT | STGM_READWRITE | STGM_SHARE_EXCLUSIVE | STGM_CREATE, 0, 0,
                 &pstmValue)))
  {
    pstgTable->Release();
    pstgRoot->Release();
...
STDMETHODIMP ScriptGlobalTable::LoadValue(BSTR TableName, BSTR ValueName, VARIANT* Value)
{
  ...
  IStream* pstmValue;
  if (FAILED(hr = pstgTable->OpenStream(
                 ValueName, 0, STGM_DIRECT | STGM_READ | STGM_SHARE_EXCLUSIVE, 0, &pstmValue)))
  {
    SetVarBstr(Value, SysAllocString(L""));
    pstgTable->Release();
    pstgRoot->Release();
...
STDMETHODIMP ScriptGlobalTable::get_ShowDT(VARIANT_BOOL* pVal)
{
  ...
  {
    *pVal =
        FTOVB(m_pt->m_BG_current_set == BG_DESKTOP || m_pt->m_BG_current_set == BG_FSS); // DT & FSS
  }
  return S_OK;
}
...
STDMETHODIMP ScriptGlobalTable::UpdateMaterial(BSTR pVal,
                                               float wrapLighting,
                                               float roughness,
...
STDMETHODIMP ScriptGlobalTable::GetMaterial(BSTR pVal,
...
STDMETHODIMP ScriptGlobalTable::UpdateMaterialPhysics(
    BSTR pVal, float elasticity, float elasticityFalloff, float friction, float scatterAngle)
{
  if (!g_pplayer)
  {
    return E_POINTER;
...
STDMETHODIMP ScriptGlobalTable::GetMaterialPhysics(BSTR pVal,
                                                   VARIANT* elasticity,
                                                   VARIANT* elasticityFalloff,
                                                   VARIANT* friction,
                                                   VARIANT* scatterAngle)
...
STDMETHODIMP ScriptGlobalTable::put_DMDPixels(
    VARIANT pVal) // assumes VT_UI1 as input //!! use 64bit instead of 8bit to reduce overhead??
{
...
#ifdef DMD_UPSCALE
      g_pplayer->m_texdmd =
          new BaseTexture(g_pplayer->m_dmd.x * 3, g_pplayer->m_dmd.y * 3, BaseTexture::RGBA, false);
#else
      g_pplayer->m_texdmd =
          new BaseTexture(g_pplayer->m_dmd.x, g_pplayer->m_dmd.y, BaseTexture::RGBA, false);
#endif
    }

    DWORD* const data =
        (DWORD*)g_pplayer->m_texdmd->data(); //!! assumes tex data to be always 32bit
...
STDMETHODIMP ScriptGlobalTable::put_DMDColoredPixels(
    VARIANT pVal) //!! assumes VT_UI4 as input //!! use 64bit instead of 32bit to reduce overhead??
{
  SAFEARRAY* psa = pVal.parray;
  ...
  if (psa && g_pplayer && g_pplayer->m_dmd.x > 0 && g_pplayer->m_dmd.y > 0)
  {
    ...
    for (int ofs = 0; ofs < size; ++ofs)
    {
      data[ofs] =
          p[ofs].uintVal |
          0xFF000000u; // store RGB values and let shader do the rest (set alpha to let shader know that this is RGB and not just brightness)
    }
    SafeArrayUnaccessData(psa);
...
STDMETHODIMP ScriptGlobalTable::SetupSerial(
    int baud, int bits, int parity, int stopbit, VARIANT_BOOL rts, VARIANT_BOOL dtr)
{
  Serial.setup(serial::get_baud(baud), serial::get_bits(bits),
               parity == 0 ? SERIAL_PARITY_NONE
                           : (parity == 1 ? SERIAL_PARITY_EVEN : SERIAL_PARITY_ODD),
               serial::get_stopbit(stopbit));
...
{
  *pVal = ((g_pplayer->m_stereo3D != 0) && g_pplayer->m_stereo3Denabled)
              ? 1
              : 0; // 0 = Normal 2D, 1 = Stereo 3D, 2 = VR
  return S_OK;
}
...
PinTable::PinTable()
{
  ...
  nudge_set_sensitivity((float)LoadValueIntWithDefault("Player", "NudgeSensitivity", 500) *
                        (float)(1.0 / 1000.0));

  m_globalDifficulty = dequantizeUnsignedPercent(
      LoadValueIntWithDefault("Player", "GlobalDifficulty", 20)); // easy by default
...
{
  m_tblAccelerometer = LoadValueBoolWithDefault("Player", "PBWEnabled",
                                                true); // true if electronic accelerometer enabled
  m_tblAccelNormalMount = LoadValueBoolWithDefault(
      "Player", "PBWNormalMount", true); // true is normal mounting (left hand coordinates)
  ...
  m_tblAccelAmp.x =
      dequantizeUnsignedPercentNoClamp(LoadValueIntWithDefault("Player", "PBWAccelGainX", 150));
  m_tblAccelAmp.y =
      dequantizeUnsignedPercentNoClamp(LoadValueIntWithDefault("Player", "PBWAccelGainY", 150));
  ...
  plumb_set_sensitivity((float)LoadValueIntWithDefault("Player", "TiltSensitivity", 400) *
                        (float)(1.0 / 1000.0));
...
void PinTable::FVerifySaveToClose()
{
  ...
  {
    /*const DWORD wait =*/WaitForMultipleObjects((DWORD)m_vAsyncHandles.size(),
                                                 m_vAsyncHandles.data(), TRUE, INFINITE);
...
void PinTable::UpdatePropertyImageList()
{
  ...
  g_pvp->GetPropertiesDocker()->GetContainProperties()->GetPropertyDialog()->UpdateTabs(
      m_vmultisel);
}
...
void PinTable::InitBuiltinTable(const size_t tableId)
{
  ...
  ILockBytes* pilb;
  CreateILockBytesOnHGlobal(hcopiedmem, TRUE,
                            &pilb); // "TRUE" parm gives ownership of hcopiedmem to Global Object

  IStorage* pis;
  StgOpenStorageOnILockBytes(pilb, nullptr, STGM_TRANSACTED | STGM_READWRITE | STGM_SHARE_EXCLUSIVE,
                             nullptr, 0, &pis);
...
#define CLEAN_MATERIAL(pEditMaterial) \
  { \
    robin_hood::unordered_map<std::string, Material*, StringHashFunctor, \
                              StringComparator>::const_iterator it = \
        m_materialMap.find(pEditMaterial); \
...
#define CLEAN_IMAGE(pEditImage) \
  { \
    robin_hood::unordered_map<std::string, Texture*, StringHashFunctor, \
                              StringComparator>::const_iterator it = \
        m_textureMap.find(pEditImage); \
...
void PinTable::InitTablePostLoad()
{
  ...
  for (unsigned int i = 1; i < NUM_BG_SETS; ++i)
  {
    if (m_BG_FOV[i] == FLT_MAX) // old table, copy FS and/or FSS settings over from old DT setting
    {
      ...
      if (m_BG_image[i].empty() && i == BG_FSS)
      { // copy image over for FSS mode
...
void PinTable::GetUniqueName(const ItemTypeEnum type,
                             WCHAR* const wzUniqueName,
                             const DWORD wzUniqueName_maxlength) const
{
  WCHAR wzRoot[256];
...
void PinTable::GetUniqueName(const WCHAR* const wzRoot,
                             WCHAR* const wzUniqueName,
                             const DWORD wzUniqueName_maxlength) const
{
  int suffix = 1;
...
void PinTable::GetUniqueNamePasting(const int type,
                                    WCHAR* const wzUniqueName,
                                    const DWORD wzUniqueName_maxlength)
{
...
void PinTable::UIRenderPass2(Sur* const psur)
{
  ...
  {
    Texture* const ppi =
        GetImage((!m_vpinball->m_backglassView) ? m_image : m_BG_image[m_BG_current_set]);
    ...
    if (ppi)
    {
      ...
      if (ppi->m_hbmGDIVersion)
      {
        ...
        psur->Image(frect.left, frect.top, frect.right, frect.bottom, hdcNew, ppi->m_width,
                    ppi->m_height);
...
void PinTable::Render3DProjection(Sur* const psur)
{
  ...
  const float FOV =
      (m_BG_FOV[m_BG_current_set] < 1.0f)
          ? 1.0f
          : m_BG_FOV
                [m_BG_current_set]; // Can't have a real zero FOV, but this will look almost the same
  ...
  const bool fullscreen =
      LoadValueBoolWithDefault("Player", "FullScreen", IsWindows10_1803orAbove());
  const int renderWidth = LoadValueIntWithDefault(
      "Player", "Width", fullscreen ? DEFAULT_PLAYER_FS_WIDTH : DEFAULT_PLAYER_WIDTH);
  ...
  pinproj.FitCameraToVertices(vvertex3D, aspect, rotation, inclination, FOV,
                              m_BG_xlatez[m_BG_current_set], m_BG_layback[m_BG_current_set]);
  pinproj.m_matView.RotateXMatrix((float)M_PI); // convert Z=out to Z=in (D3D coordinate system)
  pinproj.m_matWorld.SetIdentity();
  Matrix3D proj =
      Matrix3D::MatrixPerspectiveFovLH(ANGTORAD(FOV), aspect, pinproj.m_rznear, pinproj.m_rzfar);
  ...
  pinproj.TranslateView(m_BG_xlatex[m_BG_current_set] - pinproj.m_vertexcamera.x,
                        m_BG_xlatey[m_BG_current_set] - pinproj.m_vertexcamera.y,
                        -pinproj.m_vertexcamera.z);
  ...
  rgvIn[1].x = m_left;
  rgvIn[1].y = m_top;
  rgvIn[1].z = m_glassheight;
  rgvIn[2].x = m_right;
  rgvIn[2].y = m_top;
  rgvIn[2].z = m_glassheight;
  rgvIn[3].x = m_right;
  rgvIn[3].y = m_top;
  rgvIn[3].z = 50.0f;
  rgvIn[4].x = m_right;
...
void PinTable::Paint(HDC hdc)
{
  ...
  if (m_dirtyDraw)
  {
    Sur* const psur = new PaintSur(dc.GetHDC(), m_zoom, m_offset.x, m_offset.y, rc.right - rc.left,
                                   rc.bottom - rc.top, GetSelectedItem());
    UIRenderPass2(psur);
...
ISelect* PinTable::HitTest(const int x, const int y)
{
  ...
  HitSur phs(dc.GetHDC(), m_zoom, m_offset.x, m_offset.y, rc.right - rc.left, rc.bottom - rc.top, x,
             y, this);
  HitSur phs2(dc.GetHDC(), m_zoom, m_offset.x, m_offset.y, rc.right - rc.left, rc.bottom - rc.top,
              x, y, this);
...
void PinTable::Play(const bool cameraMode)
{
  ...
  if (!m_pcv->m_scriptError)
  {
    ...
    if (m_overridePhysics)
    {
      ...
      m_fOverrideContactFriction =
          LoadValueFloatWithDefault("Player", tmp, DEFAULT_TABLE_CONTACTFRICTION);
      ...
      m_fOverrideElasticityFalloff =
          LoadValueFloatWithDefault("Player", tmp, DEFAULT_TABLE_ELASTICITY_FALLOFF);

      sprintf_s(tmp, 256, "TablePhysicsScatterAngle%d", m_overridePhysics - 1);
      m_fOverrideScatterAngle =
          LoadValueFloatWithDefault("Player", tmp, DEFAULT_TABLE_PFSCATTERANGLE);

      sprintf_s(tmp, 256, "TablePhysicsContactScatterAngle%d", m_overridePhysics - 1);
      fOverrideContactScatterAngle =
          LoadValueFloatWithDefault("Player", tmp, DEFAULT_TABLE_SCATTERANGLE);
    ...
    g_pplayer->Attach(g_pplayer->m_pin3d.m_pd3dPrimaryDevice
                          ? g_pplayer->m_pin3d.m_pd3dPrimaryDevice->getHwnd()
                          : 0);
#else
    g_pplayer->Create();
#endif
...
HRESULT PinTable::SaveToStorage(IStorage* pstgRoot)
{
  ...
  HWND hwndProgressBar =
      CreateWindowEx(0, PROGRESS_CLASS, (LPSTR) nullptr, WS_CHILD | WS_VISIBLE, rc.left, rc.top,
                     rc.right - rc.left, rc.bottom - rc.top, m_vpinball->m_hwndStatusBar, (HMENU)0,
                     m_vpinball->theInstance, nullptr);
  ...
  foo = CryptAcquireContext(&hcp, nullptr, nullptr, PROV_RSA_FULL,
  ...
  const int ctotalitems = (int)(m_vedit.size() + m_vsound.size() + m_vimage.size() +
                                m_vfont.size() + m_vcollection.size());
  int csaveditems = 0;
  ...
  IStorage* pstgData;
  HRESULT hr;
  if (SUCCEEDED(hr = pstgRoot->CreateStorage(L"GameStg",
                                             STGM_DIRECT /*STGM_TRANSACTED*/ | STGM_READWRITE |
                                                 STGM_SHARE_EXCLUSIVE | STGM_CREATE,
                                             0, 0, &pstgData)))
  {
    IStream* pstmGame;
    if (SUCCEEDED(hr = pstgData->CreateStream(L"GameData",
                                              STGM_DIRECT | STGM_READWRITE | STGM_SHARE_EXCLUSIVE |
                                                  STGM_CREATE,
                                              0, 0, &pstmGame)))
    {
      IStream* pstmItem;
      if (SUCCEEDED(hr = pstgData->CreateStream(L"Version",
      ...
      IStorage* pstgInfo;
      if (SUCCEEDED(hr = pstgRoot->CreateStorage(L"TableInfo",
                                                 ...
                                                 0, 0, &pstgInfo)))
      {
        ...
        if (SUCCEEDED(hr = pstgData->CreateStream(L"CustomInfoTags",
                                                  STGM_DIRECT | STGM_READWRITE |
                                                      STGM_SHARE_EXCLUSIVE | STGM_CREATE,
                                                  0, 0, &pstmItem)))
        {
      ...
      if (SUCCEEDED(hr = SaveData(pstmGame, hch, false)))
      {
        for (size_t i = 0; i < m_vedit.size(); i++)
        {
          ...
          if (SUCCEEDED(hr = pstgData->CreateStream(wszStmName,
                                                    STGM_DIRECT | STGM_READWRITE |
                                                        STGM_SHARE_EXCLUSIVE | STGM_CREATE,
                                                    0, 0, &pstmItem)))
          {
    ...
    IStream* pstmItem;
    if (SUCCEEDED(hr = pstgData->CreateStream(
                      L"MAC", STGM_DIRECT | STGM_READWRITE | STGM_SHARE_EXCLUSIVE | STGM_CREATE, 0,
                      0, &pstmItem)))
    {
      ULONG writ;
      //int version = CURRENT_FILE_FORMAT_VERSION;
...
HRESULT PinTable::SaveSoundToStream(const PinSound* const pps, IStream* pstm)
{
  ...
  if (pps->IsWav2())
  { // only use old code if playing wav's
...
#ifdef ONLY_USE_BASS
  if (FAILED(
          hr = pstm->Write(pps->IsWav2() ? &pps->m_cdata_org : &pps->m_cdata, sizeof(int), &writ)))
#else
...
#ifdef ONLY_USE_BASS
  if (FAILED(hr =
                 pstm->Write(pps->IsWav2() ? pps->m_pdata_org : pps->m_pdata, pps->m_cdata, &writ)))
#else
...
HRESULT PinTable::LoadSoundFromStream(IStream* pstm, const int LoadFileVersion)
{
  ...
  if (pps->IsWav2())
  {
    ...
    waveFileSize = sizeof(WAVEHEADER) + sizeof(WAVEFORMATEX) + pps->m_wfx.cbSize +
                   sizeof(WaveData) + sizeof(DWORD) + pps->m_cdata;
  ...
  else
  {
    ...
    pps->m_outputTarget =
        (StrStrI(pps->m_szName.c_str(), "bgout_") != nullptr) ||
                (_stricmp(pps->m_szPath.c_str(), "* Backglass Output *") ==
                 0) // legacy behavior, where the BG selection was encoded into the strings directly
                || toBackglassOutput
            ? SNDOUT_BACKGLASS
            : SNDOUT_TABLE;
  }
...
HRESULT PinTable::WriteInfoValue(IStorage* pstg,
                                 const WCHAR* const wzName,
                                 ...
                                 HCRYPTHASH hcrypthash)
{
  ...
  if (!szValue.empty() &&
      SUCCEEDED(hr = pstg->CreateStream(
                    wzName, STGM_DIRECT | STGM_READWRITE | STGM_SHARE_EXCLUSIVE | STGM_CREATE, 0, 0,
                    &pstm)))
  {
    ULONG writ;
...
HRESULT PinTable::SaveInfo(IStorage* pstg, HCRYPTHASH hcrypthash)
{
  ...
  if (pin != nullptr && pin->m_ppb != nullptr)
  {
    ...
    if (SUCCEEDED(hr = pstg->CreateStream(L"Screenshot",
                                          STGM_DIRECT | STGM_READWRITE | STGM_SHARE_EXCLUSIVE |
                                              STGM_CREATE,
                                          0, 0, &pstm)))
    {
...
HRESULT PinTable::ReadInfoValue(IStorage* pstg,
                                const WCHAR* const wzName,
                                char** pszValue,
                                HCRYPTHASH hcrypthash)
{
  ...
  if (SUCCEEDED(hr = pstg->OpenStream(wzName, nullptr,
                                      STGM_DIRECT | STGM_READ | STGM_SHARE_EXCLUSIVE, 0, &pstm)))
  {
    STATSTG ss;
...
HRESULT PinTable::LoadInfo(IStorage* pstg, HCRYPTHASH hcrypthash, int version)
{
  ...
  if (SUCCEEDED(hr = pstg->OpenStream(L"Screenshot", nullptr,
                                      STGM_DIRECT | STGM_READ | STGM_SHARE_EXCLUSIVE, 0, &pstm)))
  {
    STATSTG ss;
    pstm->Stat(&ss, STATFLAG_NONAME);
...
HRESULT PinTable::LoadCustomInfo(IStorage* pstg,
                                 IStream* pstmTags,
                                 HCRYPTHASH hcrypthash,
                                 int version)
{
...
HRESULT PinTable::SaveData(IStream* pstm, HCRYPTHASH hcrypthash, const bool backupForPlay)
{
  ...
  if (!m_materials.empty())
  {
    ...
    for (size_t i = 0; i < m_materials.size(); i++)
    {
      ...
      mats[i].fGlossyImageLerp =
          255 -
          quantizeUnsigned<8>(clamp(m->m_fGlossyImageLerp, 0.f,
                                    1.f)); // '255 -' to be compatible with previous table versions
      mats[i].fThickness = quantizeUnsigned<8>(
          clamp(m->m_fThickness, 0.05f,
                1.f)); // clamp with 0.05f to be compatible with previous table versions
      ...
      mats[i].bOpacityActive_fEdgeAlpha |= quantizeUnsigned<7>(clamp(m->m_fEdgeAlpha, 0.f, 1.f))
                                           << 1;
      strncpy_s(mats[i].szName, m->m_szName.c_str(), sizeof(mats[i].szName) - 1);
      for (size_t c = strnlen_s(mats[i].szName, sizeof(mats[i].szName)); c < sizeof(mats[i].szName);
           ++c)
      { // to avoid garbage after 0
        mats[i].szName[c] = 0;
      }
    }
    ...
    for (size_t i = 0; i < m_materials.size(); i++)
    {
      ...
      for (size_t c = strnlen_s(phymats[i].szName, sizeof(phymats[i].szName));
           c < sizeof(phymats[i].szName); ++c)
      { // to avoid garbage after 0
    ...
    bw.WriteStruct(FID(PHMA), phymats.data(),
                   (int)(sizeof(SavePhysicsMaterial) * m_materials.size()));
  }
...
HRESULT PinTable::LoadGameFromFilename(const string& szFileName)
{
  ...
  HRESULT hr;
  IStorage* pstgRoot;
  if (FAILED(hr = StgOpenStorage(wszCodeFile, nullptr, STGM_TRANSACTED | STGM_READ, nullptr, 0,
                                 &pstgRoot)))
  {
...
HRESULT PinTable::LoadGameFromStorage(IStorage* pstgRoot)
{
  ...
  IStorage* pstgData;
  HRESULT hr;
  if (SUCCEEDED(hr = pstgRoot->OpenStorage(L"GameStg", nullptr,
                                           STGM_DIRECT | STGM_READ | STGM_SHARE_EXCLUSIVE, nullptr,
                                           0, &pstgData)))
  {
    IStream* pstmGame;
    if (SUCCEEDED(hr = pstgData->OpenStream(L"GameData", nullptr,
                                            STGM_DIRECT | STGM_READ | STGM_SHARE_EXCLUSIVE, 0,
                                            &pstmGame)))
    {
      IStream* pstmVersion;
      if (SUCCEEDED(hr = pstgData->OpenStream(L"Version", nullptr,
                                              STGM_DIRECT | STGM_READ | STGM_SHARE_EXCLUSIVE, 0,
                                              &pstmVersion)))
      {
        ULONG read;
        ...
        if (loadfileversion > CURRENT_FILE_FORMAT_VERSION)
        {
          ...
          sprintf_s(errorMsg,
                    "This table was saved with version %i.%02i and is newer than the supported "
                    "version %i.%02i! You might get problems loading/playing it!",
                    loadfileversion / 100, loadfileversion % 100, CURRENT_FILE_FORMAT_VERSION / 100,
                    CURRENT_FILE_FORMAT_VERSION % 100);
          ShowError(errorMsg);
          /*
                              pstgRoot->Release();
                              pstmGame->Release();
        ...
        CryptDeriveKey(
            hcp, CALG_RC2, hchkey,
            (loadfileversion == 600) ? CRYPT_EXPORTABLE : (CRYPT_EXPORTABLE | 0x00280000), &hkey);
      }

      IStorage* pstgInfo;
      if (SUCCEEDED(hr = pstgRoot->OpenStorage(L"TableInfo", nullptr,
                                               STGM_DIRECT | STGM_READ | STGM_SHARE_EXCLUSIVE,
                                               nullptr, 0, &pstgInfo)))
      {
        LoadInfo(pstgInfo, hch, loadfileversion);
        IStream* pstmItem;
        if (SUCCEEDED(hr = pstgData->OpenStream(L"CustomInfoTags", nullptr,
                                                STGM_DIRECT | STGM_READ | STGM_SHARE_EXCLUSIVE, 0,
                                                &pstmItem)))
        {
      ...
      if (SUCCEEDED(hr = LoadData(pstmGame, csubobj, csounds, ctextures, cfonts, ccollection,
                                  loadfileversion, hch,
                                  (loadfileversion < NO_ENCRYPTION_FORMAT_VERSION) ? hkey : NULL)))
      {
        ProfileLog("LoadData");
        ...
        for (int i = 0; i < csubobj; i++)
        {
          ...
          IStream* pstmItem;
          if (SUCCEEDED(hr = pstgData->OpenStream(wszStmName, nullptr,
                                                  STGM_DIRECT | STGM_READ | STGM_SHARE_EXCLUSIVE, 0,
                                                  &pstmItem)))
          {
            ULONG read;
            ...
            hr = piedit->InitLoad(
                pstmItem, this, &id, loadfileversion, (loadfileversion < 1000) ? hch : NULL,
                (loadfileversion < 1000) ? hkey : NULL); // 1000 (VP10 beta) removed the encryption
        ...
        m_vimage.resize(ctextures); // due to multithreaded loading do pre-allocation
        {
          ThreadPool pool(
              g_pvp
                  ->m_logicalNumberOfProcessors); //!! Note that this dramatically increases the amount of temporary memory needed, especially if Max Texture Dimension is set (as then all the additional conversion/rescale mem is also needed 'in parallel')

          for (int i = 0; i < ctextures; i++)
          {
            pool.enqueue(
                [i, loadfileversion, pstgData, this]
                {
                  const string szStmName = "Image" + std::to_string(i);
                  MAKE_WIDEPTR_FROMANSI(wszStmName, szStmName.c_str());
                  ...
                  IStream* pstmItem;
                  HRESULT hr;
                  if (SUCCEEDED(hr = pstgData->OpenStream(
                                    wszStmName, nullptr,
                                    STGM_DIRECT | STGM_READ | STGM_SHARE_EXCLUSIVE, 0, &pstmItem)))
                  {
        ...
        for (size_t i = 0; i < m_vimage.size() - 1; ++i)
        {
          ...
          {
            if (m_vimage[i]->m_szName == m_vimage[i2]->m_szName &&
                m_vimage[i]->m_szPath == m_vimage[i2]->m_szPath)
            {
        ...
        for (int i = 0; i < ccollection; i++)
        {
                                                  ...
                                                  &pstmItem)))
          {
            ...
            pcol->AddRef();
            pcol->LoadData(pstmItem, this, loadfileversion, hch,
                           (loadfileversion < NO_ENCRYPTION_FORMAT_VERSION) ? hkey : NULL);
      ...
      {
        if (SUCCEEDED(hr = pstgData->OpenStream(L"MAC", nullptr,
                                                STGM_DIRECT | STGM_READ | STGM_SHARE_EXCLUSIVE, 0,
                                                &pstmVersion)))
        {
          BYTE hashvalOld[256];
      ...
      { // the m_fGlossyImageLerp part was included first with 10.3, so set all previously saved materials to the old default
        for (size_t i = 0; i < m_materials.size(); ++i)
        {
      ...
      { // the m_fThickness part was included first with 10.4, so set all previously saved materials to the old default
        for (size_t i = 0; i < m_materials.size(); ++i)
        {
...
HRESULT PinTable::LoadData(IStream* pstm,
                           int& csubobj,
                           int& csounds,
                           int& ctextures,
                           int& cfonts,
...
bool PinTable::LoadToken(const int id, BiffReader* const pbr)
{
  switch (id)
  {
    ...
    case FID(TBLH):
      ...
      break;
    case FID(IMAG):
      pbr->GetString(m_image);
      break;
    case FID(BLIM):
      pbr->GetString(m_ballImage);
      break;
    case FID(BLIF):
      pbr->GetString(m_ballImageDecal);
      break;
    case FID(SSHT):
    ...
    case FID(NAME):
      ...
      break;
    case FID(BIMG):
      pbr->GetString(m_BG_image[0]);
      break;
    case FID(BIMF):
      pbr->GetString(m_BG_image[1]);
      break;
    case FID(BIMS):
      pbr->GetString(m_BG_image[2]);
      break;
    case FID(BIMN):
      pbr->GetBool(m_ImageBackdropNightDay);
      break;
    case FID(IMCG):
      pbr->GetString(m_imageColorGrade);
      break;
    case FID(EIMG):
      pbr->GetString(m_envImage);
      break;
    case FID(PLMA):
      pbr->GetString(m_playfieldMaterial);
      break;
    case FID(NOTX):
    {
      std::string txt;
      pbr->GetString(txt);
      m_notesText = CString(txt.c_str());
      break;
    }
    case FID(LZAM):
    ...
    case FID(CODE):
    {
      ...
      const bool script_protected =
          (((m_protectionData.flags & DISABLE_EVERYTHING) == DISABLE_EVERYTHING) ||
           ((m_protectionData.flags & DISABLE_SCRIPT_EDITING) == DISABLE_SCRIPT_EDITING));

      m_pcv->LoadFromStream(pbr->m_pistream, pbr->m_hcrypthash,
                            script_protected ? pbr->m_hcryptkey : NULL);
      break;
    }
    case FID(CCUS):
    ...
    case FID(MATE):
    {
      ...
      for (int i = 0; i < m_numMaterials; i++)
      {
        ...
        pmat->m_fGlossyImageLerp =
            1.0f -
            dequantizeUnsigned<8>(
                mats[i]
                    .fGlossyImageLerp); //!! '1.0f -' to be compatible with previous table versions
        pmat->m_fThickness =
            (mats[i].fThickness == 0)
                ? 0.05f
                : dequantizeUnsigned<8>(
                      mats[i]
                          .fThickness); //!! 0 -> 0.05f to be compatible with previous table versions
...
bool PinTable::ExportSound(PinSound* const pps, const char* const szfilename)
{
  ...
  const HMMIO hmmio = mmioOpen((LPSTR)szfilename, &mmio,
                               MMIO_ALLOCBUF | MMIO_CREATE | MMIO_EXCLUSIVE | MMIO_READWRITE);
  ...
  if (hmmio != nullptr)
  {
    ...
    i = pps->m_cdata;
    mmioWrite(hmmio, (char*)&i, 4); // data size bytes
...
void PinTable::NewCollection(const HWND hwndListView, const bool fromSelection)
{
  ...
  if (fromSelection && !MultiSelIsEmpty())
  {
    for (int i = 0; i < m_vmultisel.size(); i++)
    {
      ...
      if (piedit)
      {
        if (piedit->GetISelect() ==
            pisel) // Do this check so we don't put walls in a collection when we only have the control point selected
        {
          if (piedit
                  ->GetScriptable()) // check for scriptable because can't add decals to a collection - they have no name
          {
...
void PinTable::SetMyScrollInfo()
{
  ...
  const HitSur phs(nullptr, m_zoom, m_offset.x, m_offset.y, rc.right - rc.left, rc.bottom - rc.top,
                   0, 0, nullptr);
...
void PinTable::OnRightButtonDown(int x, int y)
{
  OnLeftButtonUp(
      x,
      y); //corrects issue with left mouse button being in 'stuck down' position on a control point or object - BDS
...
void PinTable::FillCollectionContextMenu(CMenu& mainMenu, CMenu& colSubMenu, ISelect* psel)
{
  ...
  for (int i = maxItems; i >= 0; i--)
  {
    ...
    if ((maxItems - i) % 32 == 0)
    { // add new column each 32 entries
      flags |= MF_MENUBREAK;
    }
...
void PinTable::DoContextMenu(int x, int y, const int menuid, ISelect* psel)
{
  ...
  const int icmd =
      newMenu.TrackPopupMenuEx(TPM_RETURNCMD, pt.x, pt.y, m_mdiTable->GetHwnd(), nullptr);
...
void PinTable::DoCommand(int icmd, int x, int y)
{
  ...
  switch (icmd)
  {
    ...
    case ID_WALLMENU_ROTATE:
      DialogBoxParam(m_vpinball->theInstance, MAKEINTRESOURCE(IDD_ROTATE), m_vpinball->GetHwnd(),
                     RotateProc, (size_t)(ISelect*)this);
      break;
    case ID_WALLMENU_SCALE:
      DialogBoxParam(m_vpinball->theInstance, MAKEINTRESOURCE(IDD_SCALE), m_vpinball->GetHwnd(),
                     ScaleProc, (size_t)(ISelect*)this);
      break;
    case ID_WALLMENU_TRANSLATE:
      DialogBoxParam(m_vpinball->theInstance, MAKEINTRESOURCE(IDD_TRANSLATE), m_vpinball->GetHwnd(),
                     TranslateProc, (size_t)(ISelect*)this);
      break;
  }
}
...
bool PinTable::GetCollectionIndex(const ISelect* const element,
                                  int& collectionIndex,
                                  int& elementIndex)
{
  for (int i = 0; i < m_vcollection.size(); i++)
...
void PinTable::Scale(const float scalex,
                     const float scaley,
...
void PinTable::ExportBlueprint()
{
  ...
  ofn.hInstance = m_vpinball->theInstance;
  ofn.hwndOwner = m_vpinball->GetHwnd();
  ofn.lpstrFilter = "PNG (.png)\0*.png;\0Bitmap (.bmp)\0*.bmp;\0TGA (.tga)\0*.tga;\0TIFF "
                    "(.tiff/.tif)\0*.tiff;*.tif;\0WEBP (.webp)\0*.webp;\0";
  ...
  const int result =
      m_vpinball->MessageBox("Do you want a solid blueprint?", "Export As Solid?", MB_YESNO);
  ...
  {
    PaintSur psur(hdc2, (float)bmwidth / tablewidth, tablewidth * 0.5f, tableheight * 0.5f, bmwidth,
                  bmheight, nullptr);
  ...
  if (!FreeImage_Save(FreeImage_GetFIFFromFilename(szBlueprintFileName), dib, szBlueprintFileName,
                      PNG_Z_BEST_COMPRESSION | BMP_SAVE_RLE))
  {
...
void PinTable::ExportMesh(ObjLoader& loader)
{
  ...
  rgv[0].x = m_left;
  rgv[0].y = m_top;
  rgv[0].z = m_tableheight;
  rgv[1].x = m_right;
  rgv[1].y = m_top;
  rgv[1].z = m_tableheight;
  rgv[2].x = m_right;
  rgv[2].y = m_bottom;
  rgv[2].z = m_tableheight;
  rgv[3].x = m_left;
  rgv[3].y = m_bottom;
  rgv[3].z = m_tableheight;

  // These next 4 vertices are used just to set the extents
  rgv[4].x = m_left;
  rgv[4].y = m_top;
  rgv[4].z = m_tableheight + 50.0f;
  rgv[5].x = m_left;
  rgv[5].y = m_bottom;
  rgv[5].z = m_tableheight + 50.0f;
  rgv[6].x = m_right;
  rgv[6].y = m_bottom;
  rgv[6].z = m_tableheight + 50.0f;
...
void PinTable::ImportBackdropPOV(const string& filename)
{
  ...
  if (filename.empty())
  {
    ...
    if (!m_vpinball->OpenFileDialog(szInitialDir, szFileName,
                                    "POV file (*.pov)\0*.pov\0Old POV file(*.xml)\0*.xml\0", "pov",
                                    0))
    {
      return;
    }
  ...
  try
  {
    ...
    sscanf_s(fullscreen->first_node("inclination")->value(), "%f",
             &m_BG_inclination[BG_FULLSCREEN]);
    ...
    if (custom)
    {
      ...
      if (node)
      {
        sscanf_s(node->value(), "%i", &m_useFXAA);
      }
      node = custom->first_node("ingameAO");
      if (node)
      {
        sscanf_s(node->value(), "%i", &m_useAO);
      }
      node = custom->first_node("ScSpReflect");
      if (node)
      {
        sscanf_s(node->value(), "%i", &m_useSSR);
      }
      node = custom->first_node("FPSLimiter");
      if (node)
      {
        sscanf_s(node->value(), "%i", &m_TableAdaptiveVSync);
      }
      ...
      if (node)
      {
        sscanf_s(node->value(), "%i", &m_userDetailLevel);
      }
      node = custom->first_node("BallReflection");
      if (node)
      {
        sscanf_s(node->value(), "%i", &m_useReflectionForBalls);
      }
      node = custom->first_node("BallTrail");
      if (node)
      {
        sscanf_s(node->value(), "%i", &m_useTrailForBalls);
      }
      node = custom->first_node("BallTrailStrength");
      if (node)
      {
        sscanf_s(node->value(), "%f", &m_ballTrailStrength);
      }
      ...
      if (node)
      {
        sscanf_s(node->value(), "%i", &m_overridePhysics);
      }
...
void PinTable::ExportBackdropPOV(const string& filename)
{
  ...
  try
  {
    ...
    xml_node<>* userOverwriteDetail =
        xmlDoc.allocate_node(node_element, "OverwriteDetailsLevel", codl.c_str());
    ...
    xml_node<>* userReflectBall =
        xmlDoc.allocate_node(node_element, "BallReflection", curb.c_str());
    ...
    xml_node<>* userTrailStrength =
        xmlDoc.allocate_node(node_element, "BallTrailStrength", cubs.c_str());
    custom->append_node(userTrailStrength);
    const std::string codn = std::to_string(m_overwriteGlobalDayNight);
    xml_node<>* userOverwriteDayNight =
        xmlDoc.allocate_node(node_element, "OverwriteNightDay", codn.c_str());
    ...
    xml_node<>* userDifficutly =
        xmlDoc.allocate_node(node_element, "GameplayDifficulty", cgd.c_str());
    ...
    xml_node<>* userFlipperPhysics =
        xmlDoc.allocate_node(node_element, "IncludeFlipperPhysics", copf.c_str());
...
{
  const SaveDirtyState sdsNewDirtyState = (SaveDirtyState)max(
      max((int)m_sdsDirtyProp, (int)m_sdsDirtyScript), (int)m_sdsNonUndoableDirty);
    ...
    else
    {
      ...
      if (type != eItemDecal)
      {
        GetUniqueNamePasting(type, peditNew->GetScriptable()->m_wzName,
                             sizeof(peditNew->GetScriptable()->m_wzName) /
                                 sizeof(peditNew->GetScriptable()->m_wzName[0]));
      ...
      peditNew->GetISelect()->m_layerName =
          m_vpinball->GetLayersListDialog()->GetCurrentSelectedLayerName();
      m_vpinball->GetLayersListDialog()->AddLayer(
          m_vpinball->GetLayersListDialog()->GetCurrentSelectedLayerName(), peditNew);

      m_vedit.push_back(peditNew);

      AddMultiSel(peditNew->GetISelect(), (i != m_vpinball->m_vstmclipboard.size() - 1), true,
                  false);
      cpasted++;
    }
  }
...
HRESULT PinTable::InitLoad(IStream* pstm,
                           PinTable* ptable,
                           int* pid,
                           int version,
                           HCRYPTHASH hcrypthash,
...
void PinTable::AddMultiSel(ISelect* psel,
                           const bool add,
                           const bool update,
                           const bool contextClick)
{
  ...
  else if (m_vmultisel.ElementAt(0) !=
           psel) // Object already in list - no change to selection, only to primary
  {
    int colIndex = -1;
    int elemIndex = -1;
    if (!GetCollectionIndex(psel, colIndex, elemIndex))
  ...
  if (piSelect && piSelect->GetIEditable() && piSelect->GetIEditable()->GetScriptable())
  {
    ...
    if (piSelect->GetItemType() == eItemPrimitive)
    {
      ...
      {
        info += " (animated " +
                std::to_string((unsigned long long)prim->m_mesh.m_animationFrames.size() - 1) +
                " frames)";
      }
    }
...
void PinTable::OnDelete()
{
  ...
  if (inCollection)
  {
    ...
    const int ans = m_mdiTable->MessageBox(
        ls.m_szbuffer /*"Selected elements are part of one or more collections.\nDo you really want to delete them?"*/
        ,
...
void PinTable::OnKeyDown(int key)
{
  ...
  switch (key)
  {
    ...
    case VK_DOWN:
    {
      ...
      {
        ISelect* const pisel = m_vmultisel.ElementAt(i);
        if (!pisel->GetIEditable()
                 ->GetISelect()
                 ->m_locked) // control points get lock info from parent - UNDONE - make this code snippet be in one place
        {
          switch (key)
          {
            case VK_LEFT:
...
void PinTable::UseTool(int x, int y, int tool)
{
  ...
  if (pie)
  {
    ...
    m_vpinball->GetLayersListDialog()->AddLayer(
        m_vpinball->GetLayersListDialog()->GetCurrentSelectedLayerName(), pie);
    BeginUndo();
...
void PinTable::OnLButtonUp(int x, int y)
{
  if (m_dragging)
  {
    ...
    if ((m_rcDragRect.left != m_rcDragRect.right) || (m_rcDragRect.top != m_rcDragRect.bottom))
    {
      ...
      HitRectSur* const phrs =
          new HitRectSur(dc.GetHDC(), m_zoom, m_offset.x, m_offset.y, rc.right - rc.left,
                         rc.bottom - rc.top, &m_rcDragRect, &vsel);
      ...
      if (!vsel.empty())
      {
        ...
        for (size_t i = 0; i < vsel.size(); i++)
        {
          if (vsel[i]->GetSelectLevel() == minlevel)
          {
            AddMultiSel(vsel[i], true, (i == lastItemForUpdate),
                        false); //last item updates the (multi-)selection in the editor
          }
        }
      }
...
STDMETHODIMP PinTable::PlaySound(BSTR bstr,
                                 int loopcount,
                                 float volume,
                                 float pan,
                                 float randompitch,
                                 ...
                                 float front_rear_fade)
{
  ...
  if (i == m_vsound.size()) // did not find it
  {
    if (szName[0] && m_pcv && g_pplayer && g_pplayer->m_hwndDebugOutput)
    {
      const std::string logmsg =
          std::string("Request to play \"") + szName + "\", but sound not found.";
  ...
  m_vpinball->m_ps.Play(pps, volume * m_TableSoundVolume * (float)g_pplayer->m_SoundVolume,
                        randompitch, pitch, pan, front_rear_fade, loopcount, VBTOb(usesame),
                        VBTOb(restart));
...
Texture* PinTable::GetImage(const std::string& szName) const
{
  ...
  {
    const robin_hood::unordered_map<std::string, Texture*, StringHashFunctor,
                                    StringComparator>::const_iterator it =
        m_textureMap.find(szName);
    if (it != m_textureMap.end())
    {
      return it->second;
    }
...
bool PinTable::ExportImage(const Texture* const ppi, const char* const szfilename)
{
  ...
  else if (ppi->m_pdsBuffer != nullptr)
  {
    ...
    const int pitch = ppi->m_pdsBuffer->pitch();
    const BYTE* spch =
        ppi->m_pdsBuffer->data() +
        (ppi->m_height * pitch); // just past the end of the Texture part of DD surface
    ...
    if (!FreeImage_Save(FreeImage_GetFIFFromFilename(szfilename), dib, szfilename,
                        PNG_Z_BEST_COMPRESSION | JPEG_QUALITYGOOD | BMP_SAVE_RLE))
    {
    ...
    for (size_t i = 0; i < m_vedit.size(); i++)
    {
      ...
      switch (pEdit->GetItemType())
      {
        ...
        case eItemPrimitive:
        {
          ...
          if ((_stricmp(pPrim->m_d.m_szImage.c_str(), ppi->m_szName.c_str()) == 0) ||
              (_stricmp(pPrim->m_d.m_szNormalMap.c_str(), ppi->m_szName.c_str()) == 0))
          {
            inUse = true;
          }
          break;
        ...
        case eItemSurface:
        {
          ...
          if ((_stricmp(pSurf->m_d.m_szImage.c_str(), ppi->m_szName.c_str()) == 0) ||
              (_stricmp(pSurf->m_d.m_szSideImage.c_str(), ppi->m_szName.c_str()) == 0))
          {
            inUse = true;
          }
          break;
        ...
        case eItemFlasher:
        {
          ...
          if ((_stricmp(pFlash->m_d.m_szImageA.c_str(), ppi->m_szName.c_str()) == 0) ||
              (_stricmp(pFlash->m_d.m_szImageB.c_str(), ppi->m_szName.c_str()) == 0))
          {
            inUse = true;
          }
          break;
...
Material* PinTable::GetMaterial(const std::string& szName) const
{
  ...
  {
    const robin_hood::unordered_map<std::string, Material*, StringHashFunctor,
                                    StringComparator>::const_iterator it =
        m_materialMap.find(szName);
    if (it != m_materialMap.end())
    {
      return it->second;
    }
  ...
  else
  {
    for (size_t i = 0; i < m_vedit.size(); i++)
    {
      ...
      switch (pEdit->GetItemType())
      {
        ...
        {
          const Primitive* const pPrim = (Primitive*)pEdit;
          if ((_stricmp(pPrim->m_d.m_szMaterial.c_str(), pmat->m_szName.c_str()) == 0) ||
              (_stricmp(pPrim->m_d.m_szPhysicsMaterial.c_str(), pmat->m_szName.c_str()) == 0))
          {
            inUse = true;
          }
          break;
        ...
        {
          const Ramp* const pRamp = (Ramp*)pEdit;
          if ((_stricmp(pRamp->m_d.m_szMaterial.c_str(), pmat->m_szName.c_str()) == 0) ||
              (_stricmp(pRamp->m_d.m_szPhysicsMaterial.c_str(), pmat->m_szName.c_str()) == 0))
          {
            inUse = true;
          }
          break;
        ...
        {
          const Surface* const pSurf = (Surface*)pEdit;
          if ((_stricmp(pSurf->m_d.m_szPhysicsMaterial.c_str(), pmat->m_szName.c_str()) == 0) ||
              (_stricmp(pSurf->m_d.m_szSideMaterial.c_str(), pmat->m_szName.c_str()) == 0) ||
              (_stricmp(pSurf->m_d.m_szTopMaterial.c_str(), pmat->m_szName.c_str()) == 0) ||
              (_stricmp(pSurf->m_d.m_szSlingShotMaterial.c_str(), pmat->m_szName.c_str()) == 0))
          {
            inUse = true;
          }
          break;
        ...
        {
          const Flipper* const pFlip = (Flipper*)pEdit;
          if ((_stricmp(pFlip->m_d.m_szRubberMaterial.c_str(), pmat->m_szName.c_str()) == 0) ||
              (_stricmp(pFlip->m_d.m_szMaterial.c_str(), pmat->m_szName.c_str()) == 0))
          {
            inUse = true;
          }
          break;
        ...
        {
          const HitTarget* const pHit = (HitTarget*)pEdit;
          if ((_stricmp(pHit->m_d.m_szMaterial.c_str(), pmat->m_szName.c_str()) == 0) ||
              (_stricmp(pHit->m_d.m_szPhysicsMaterial.c_str(), pmat->m_szName.c_str()) == 0))
          {
            inUse = true;
          }
          break;
        ...
        {
          const Rubber* const pRub = (Rubber*)pEdit;
          if ((_stricmp(pRub->m_d.m_szMaterial.c_str(), pmat->m_szName.c_str()) == 0) ||
              (_stricmp(pRub->m_d.m_szPhysicsMaterial.c_str(), pmat->m_szName.c_str()) == 0))
          {
            inUse = true;
          }
          break;
        ...
        case eItemBumper:
        {
          ...
          if ((_stricmp(pBump->m_d.m_szCapMaterial.c_str(), pmat->m_szName.c_str()) == 0) ||
              (_stricmp(pBump->m_d.m_szBaseMaterial.c_str(), pmat->m_szName.c_str()) == 0) ||
              (_stricmp(pBump->m_d.m_szSkirtMaterial.c_str(), pmat->m_szName.c_str()) == 0) ||
              (_stricmp(pBump->m_d.m_szRingMaterial.c_str(), pmat->m_szName.c_str()) == 0))
          {
            inUse = true;
          }
          break;
...
int PinTable::AddListItem(HWND hwndListView,
                          const string& szName,
                          const string& szValue1,
                          LPARAM lparam)
{
  LVITEM lvitem;
  lvitem.mask = LVIF_DI_SETITEM | LVIF_TEXT | LVIF_PARAM;
...
STDMETHODIMP PinTable::GetPredefinedStrings(DISPID dispID,
                                            ...
                                            IEditable* piedit)
{
  ...
  switch (dispID)
  {
    ...
    case DISPID_Collection:
    {
      ...
      for (size_t ivar = 0; ivar < cvar; ivar++)
      {
        constexpr DWORD cwch =
            sizeof(m_vcollection[(int)ivar].m_wzName) + sizeof(DWORD); //!! +DWORD?
...
STDMETHODIMP PinTable::GetPredefinedValue(DISPID dispID,
                                          DWORD dwCookie,
                                          VARIANT* pVarOut,
                                          IEditable* piedit)
{
  ...
  switch (dispID)
  {
    ...
    case IDC_MATERIAL_COMBO4:
    {
      ...
      else
      {
        ...
        MultiByteToWideCharNull(CP_ACP, 0, m_materials[dwCookie]->m_szName.c_str(), -1, wzDst,
                                cwch);
      }
      break;
    }
    ...
    case DISPID_Collection:
    {
      ...
      {
        constexpr size_t cwch =
            sizeof(m_vcollection[dwCookie].m_wzName) + sizeof(DWORD); //!! +DWORD?
        wzDst = (WCHAR*)CoTaskMemAlloc(cwch);
        if (wzDst == nullptr)
        {
...
STDMETHODIMP PinTable::ImportPhysics()
{
  ...
  if (!m_vpinball->OpenFileDialog(szInitialDir, szFileName,
                                  "Visual Pinball Physics (*.vpp)\0*.vpp\0", "vpp", 0))
  {
    return S_OK;
  }
...
{
  xml_document<> xmlDoc;
  float FlipperPhysicsMass, FlipperPhysicsStrength, FlipperPhysicsElasticity, FlipperPhysicsScatter,
      FlipperPhysicsTorqueDamping, FlipperPhysicsTorqueDampingAngle, FlipperPhysicsReturnStrength,
      FlipperPhysicsElasticityFalloff, FlipperPhysicsFriction, FlipperPhysicsCoilRampUp;
  try
  {
    std::stringstream buffer;
...
STDMETHODIMP PinTable::ExportPhysics()
{
  ...
  float val;
  char fspeed[16], fstrength[16], felasticity[16], fscatter[16], ftorquedamping[16],
      ftorquedampingangle[16], freturn[16], felasticityFalloff[16], fFriction[16], fRampup[16];
  ...
  xml_node<>* flipTorqueDampingAngle =
      xmlDoc.allocate_node(node_element, "eosTorqueAngle", ftorquedampingangle);
  ...
  xml_node<>* flipElasticityFalloff =
      xmlDoc.allocate_node(node_element, "elasticityFalloff", felasticityFalloff);
  ...
  char tgravity[16], tFriction[16], tDefaultScatter[16], telasticity[16], telasticityFallOff[16],
      tcontactScatter[16];
  ...
  xml_node<>* tabElasticityFalloff =
      xmlDoc.allocate_node(node_element, "elasticityFalloff", telasticityFallOff);
  physTab->append_node(tabElasticityFalloff);

  get_DefaultScatter(&val);
  sprintf_s(tDefaultScatter, "%f", val);
  xml_node<>* tabScatterAngle =
      xmlDoc.allocate_node(node_element, "defaultElementScatter", tDefaultScatter);
  physTab->append_node(tabScatterAngle);

  get_Scatter(&val);
  sprintf_s(tcontactScatter, "%f", val);
  xml_node<>* tabContactScatterAngle =
      xmlDoc.allocate_node(node_element, "playfieldScatter", tcontactScatter);
...
void PinTable::InvokeBallBallCollisionCallback(const Ball* b1, const Ball* b2, float hitVelocity)
{
  if (g_pplayer)
  {
    ...
    if (SUCCEEDED(hr)) // did we find the collision callback function?
    {
      ...
      CComVariant rgvar[3] = {CComVariant(hitVelocity),
...
{
  const bool middleMouseButtonPressed =
      ((GetKeyState(VK_MBUTTON) & 0x100) != 0); //((GetKeyState(VK_MENU) & 0x80000000) != 0);
...
bool PinTableMDI::CanClose() const
{
  if (m_table != nullptr && m_table->FDirty() && !g_pvp->m_povEdit)
  {
    ...
    const string szText = ls1.m_szbuffer /*"Do you want to save the changes you made to '"*/ +
                          m_table->m_szTitle + ls2.m_szbuffer;
    const int result = MessageBox(szText.c_str(), "Visual Pinball",
