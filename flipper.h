...
public:
  ...
  {
    return (m_d.m_OverridePhysics ||
            (m_ptable->m_overridePhysicsFlipper && m_ptable->m_overridePhysics))
               ? m_d.m_OverrideCoilRampUp
               : m_d.m_rampUp;
  }
  ...
  void SetFlipperRadiusMin(const float value) { m_d.m_FlipperRadiusMin = max(value, 0.0f); }

  FlipperData m_d;
  PinTable* m_ptable;

private:
  void SetVertices(const float basex,
                   const float basey,
                   const float angle,
