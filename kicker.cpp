...
{
  m_d.m_radius =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Kicker", "Radius", 25.f) : 25.f;
  m_d.m_tdr.m_TimerEnabled =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Kicker", "TimerEnabled", false)
                     : false;
  m_d.m_tdr.m_TimerInterval =
      fromMouseClick ? LoadValueIntWithDefault("DefaultProps\\Kicker", "TimerInterval", 100) : 100;
  m_d.m_enabled =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Kicker", "Enabled", true) : true;
  m_d.m_hitAccuracy = fromMouseClick
                          ? LoadValueFloatWithDefault("DefaultProps\\Kicker", "HitAccuracy", 0.5f)
                          : 0.5f;
  m_d.m_hit_height = fromMouseClick
                         ? LoadValueFloatWithDefault("DefaultProps\\Kicker", "HitHeight", 35.0f)
                         : 35.0f;
  m_d.m_orientation =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Kicker", "Orientation", 0.f) : 0.f;
  ...
  m_d.m_fallThrough = fromMouseClick
                          ...
                          : false;
  m_d.m_legacyMode =
      fromMouseClick ? LoadValueBoolWithDefault("DefaultProps\\Kicker", "Legacy", true) : true;
}
...
void Kicker::GenerateMesh(Vertex3D_NoTex2* const buf)
{
  ...
  for (int i = 0; i < num_vertices; i++)
  {
    ...
    buf[i].z =
        vert.z * m_d.m_radius * m_ptable->m_BG_scalez[m_ptable->m_BG_current_set] + m_baseHeight;
...
void Kicker::RenderSetup()
{
  ...
  if (m_d.m_kickertype == KickerCup || m_d.m_kickertype == KickerHole ||
      ...
      m_d.m_kickertype == KickerGottlieb || m_d.m_kickertype == KickerCup2)
  {
    ...
    for (unsigned int i = 0; i < kickerPlateNumVertices; i++)
    {
      ...
      buf[i].z =
          kickerPlate[i].z * rad * m_ptable->m_BG_scalez[m_ptable->m_BG_current_set] + m_baseHeight;
    ...
    SAFE_BUFFER_RELEASE(m_plateIndexBuffer);
    m_plateIndexBuffer = IndexBuffer::CreateAndFillIndexBuffer(kickerPlateNumIndices,
                                                               kickerPlateIndices, PRIMARY_DEVICE);

    SAFE_BUFFER_RELEASE(m_plateVertexBuffer);
    VertexBuffer::CreateVertexBuffer(kickerPlateNumVertices, 0, MY_D3DFVF_NOTEX2_VERTEX,
                                     &m_plateVertexBuffer, PRIMARY_DEVICE);
  ...
  SAFE_BUFFER_RELEASE(m_vertexBuffer);
  VertexBuffer::CreateVertexBuffer(m_numVertices, 0, MY_D3DFVF_NOTEX2_VERTEX, &m_vertexBuffer,
                                   PRIMARY_DEVICE);
...
{
  m_d.m_scatter =
      fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\Kicker", "Scatter", 0.f) : 0.f;
}
...
void Kicker::RenderDynamic()
{
      ...
      m_d.m_kickertype == KickerGottlieb || m_d.m_kickertype == KickerCup2)
  {
    ...
    pd3dDevice->basicShader->SetFloat(SHADER_fKickerScale,
                                      m_ptable->m_BG_scalez[m_ptable->m_BG_current_set]);
    ...
    pd3dDevice->DrawIndexedPrimitiveVB(RenderDevice::TRIANGLELIST, MY_D3DFVF_NOTEX2_VERTEX,
                                       m_plateVertexBuffer, 0, kickerPlateNumVertices,
                                       m_plateIndexBuffer, 0, kickerPlateNumIndices);
...
HRESULT Kicker::InitLoad(IStream* pstm,
                         PinTable* ptable,
                         int* pid,
                         int version,
...
STDMETHODIMP Kicker::CreateSizedBallWithMass(/*[in]*/ float radius,
                                             /*[in]*/ float mass,
                                             /*out, retval]*/ IBall** pBallEx)
{
  if (m_phitkickercircle)
  {
    ...
    Ball* const pball =
        g_pplayer->CreateBall(m_phitkickercircle->center.x, m_phitkickercircle->center.y, height,
                              0.1f, 0, 0, radius, mass);
...
STDMETHODIMP Kicker::CreateSizedBall(/*[in]*/ float radius, /*out, retval]*/ IBall** pBallEx)
{
  if (m_phitkickercircle)
  {
    ...
    Ball* const pball = g_pplayer->CreateBall(
        m_phitkickercircle->center.x, m_phitkickercircle->center.y, height, 0.1f, 0, 0, radius);
...
{
  return HitTestBasicRadius(ball, dtime, coll, false, false,
                            false); //any face, not-lateral, non-rigid
}
...
void KickerHitCircle::DoChangeBallVelocity(Ball* const pball, const Vertex3Ds& hitnormal) const
{
  ...
  if (idx != ~0u)
  {
    ...
    if (tangentSpSq > 1e-6f)
    {
      ...
      const float kt =
          1.0f / pball->m_d.m_mass + tangent.Dot(CrossProduct(cross / pball->Inertia(), surfP));
...
void KickerHitCircle::DoCollide(Ball* const pball,
                                ...
                                const bool newBall)
{
  ...
  if (newBall || ((!hitbit) == (i < 0))) // New or (Hit && !Vol || UnHit && Vol)
  {
    ...
    {
      const float grabHeight =
          (m_hitBBox.zlow + pball->m_d.m_radius) * m_pkicker->m_d.m_hitAccuracy;
      // early out here if the ball is slow and we are near the kicker center
      const bool hitEvent =
          (pball->m_d.m_pos.z < grabHeight || m_pkicker->m_d.m_legacyMode || newBall);
    ...
    {
      pball->m_d.m_vpVolObjs->erase(pball->m_d.m_vpVolObjs->begin() +
                                    i); // remove kicker to ball's volume set
