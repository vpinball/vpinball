...
public:
  ...
  {
    m_d.m_reelcount =
        min(max(1, reels), MAX_REELS); // must have at least 1 reel and a max of MAX_REELS
  ...
  void SetUpdateInterval(const int interval) { m_d.m_updateinterval = max((int)5, interval); }
  ...
  public:
    ...
    {
      m_pDispReel->Animate();
    } // this function is called every frame to animate the object/reels animation
