...
public:
  ...
  virtual void PolygonImage(const std::vector<RenderVertex>& rgv,
                            HBITMAP hbm,
                            const float left,
                            const float top,
                            const float right,
  ...
  virtual void Arc(const float x,
                   const float y,
                   const float radius,
                   const float pt1x,
                   const float pt1y,
                   const float pt2x,
                   const float pt2y) = 0;
  virtual void Image(const float x,
                     const float y,
                     const float x2,
                     const float y2,
                     HDC hdcSrc,
  ...
  inline Vertex2D ScreenToSurface(const int screenx, const int screeny) const
  {
    ...
    return Vertex2D((float)screenx * inv_zoom + m_offx, (float)screeny * inv_zoom + m_offy);
  }
