...
void LightSeq::SetDefaults(bool fromMouseClick)
{
  m_d.m_updateinterval =
      fromMouseClick ? LoadValueIntWithDefault("DefaultProps\\LightSequence", "UpdateInterval", 25)
                     : 25;
  ...
  m_d.m_vCenter.x = fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\LightSequence",
                                                               "CenterX", EDITOR_BG_WIDTH / 2)
                                   : (EDITOR_BG_WIDTH / 2);
  m_d.m_vCenter.y = fromMouseClick ? LoadValueFloatWithDefault("DefaultProps\\LightSequence",
                                                               "CenterY", (2 * EDITOR_BG_WIDTH) / 2)
                                   : ((2 * EDITOR_BG_WIDTH) / 2);
  m_d.m_tdr.m_TimerEnabled =
      fromMouseClick
          ? LoadValueBoolWithDefault("DefaultProps\\LightSequence", "TimerEnabled", false)
          : false;
  m_d.m_tdr.m_TimerInterval =
      fromMouseClick ? LoadValueIntWithDefault("DefaultProps\\LightSequence", "TimerInterval", 100)
                     : 100;
}
...
void LightSeq::WriteRegDefaults()
{
  ...
  WideCharToMultiByteNull(CP_ACP, 0, m_d.m_wzCollection.c_str(), -1, strTmp, MAXSTRING, nullptr,
                          nullptr);
...
void LightSeq::RenderSetup()
{
  ...
  ZeroMemory((void*)m_pgridData,
             (size_t)((m_lightSeqGridHeight * m_lightSeqGridWidth) * sizeof(short)));
...
void LightSeq::Animate()
{
  ...
  else
  {
    ...
    if (m_queue.Head != m_queue.Tail)
    {
      ...
      SetupTracers(m_queue.Data[Tail].Animation, m_queue.Data[Tail].TailLength,
                   m_queue.Data[Tail].Repeat, m_queue.Data[Tail].Pause);
    }
  }
}
...
HRESULT LightSeq::InitLoad(IStream* pstm,
                           PinTable* ptable,
                           int* pid,
                           int version,
...
void LightSeq::SetupTracers(const SequencerState Animation,
                            long TailLength,
                            long Repeat,
                            long Pause)
{
  ...
  switch (Animation)
  {
    ...
    case SeqCircleOutOn:
      ...
      m_th1.frameCount = m_lightSeqGridWidth / 2 + m_GridXCenterAdjust + m_lightSeqGridHeight / 2 +
                         m_GridYCenterAdjust;
      break;
    ...
    case SeqCircleInOn:
      m_th1.type = eSeqCircle;
      m_th1.radius = (float)(m_lightSeqGridWidth / 2 + m_GridXCenterAdjust +
                             m_lightSeqGridHeight / 2 + m_GridYCenterAdjust);
      m_th1.stepRadius = -1.0f;
      m_th1.x = m_GridXCenter;
      m_th1.y = m_GridYCenter;
      m_th1.frameCount = m_lightSeqGridWidth / 2 + m_GridXCenterAdjust + m_lightSeqGridHeight / 2 +
    ...
    case SeqClockRightOn:
      ...
      m_th1.x = m_GridXCenter;
      m_th1.y = m_GridYCenter;
      m_th1.length = m_lightSeqGridWidth / 2 + m_GridXCenterAdjust + m_lightSeqGridHeight / 2 +
                     m_GridYCenterAdjust;
    ...
    case SeqRadarRightOn:
      ...
      m_th1.y = (float)(m_lightSeqGridHeight - 1);
      m_th1.length =
          m_lightSeqGridHeight + m_lightSeqGridWidth + m_GridYCenterAdjust + m_GridXCenterAdjust;
    ...
    case SeqScrewRightOn:
      ...
      m_th1.x = m_GridXCenter;
      m_th1.y = m_GridYCenter - 1.0f;
      m_th1.length = m_lightSeqGridHeight / 2 + m_GridYCenterAdjust + m_lightSeqGridWidth / 2 +
                     m_GridXCenterAdjust;
      m_th1.frameCount = 180;
      ...
      m_th2.x = m_GridXCenter;
      m_th2.y = m_GridYCenter;
      m_th2.length = m_lightSeqGridHeight / 2 + m_GridYCenterAdjust + m_lightSeqGridWidth / 2 +
                     m_GridXCenterAdjust;
  ...
  if (pTracer->delay == 0)
  {
    switch (pTracer->type)
    {
      ...
      case eSeqRadar:
      {
        ...
        const float angle2 =
            (pTracer->angle + pTracer->stepAngle * 0.25f) * (float)(M_PI * 2.0 / 360.0);
        const float sn2 = sinf(angle2);
        const float cs2 = cosf(angle2);
        const float angle3 =
            (pTracer->angle + pTracer->stepAngle * 0.5f) * (float)(M_PI * 2.0 / 360.0);
        const float sn3 = sinf(angle3);
        const float cs3 = cosf(angle3);
        const float angle4 =
            (pTracer->angle + pTracer->stepAngle * 0.75f) * (float)(M_PI * 2.0 / 360.0);
...
{
  if (((x >= 0) && (x < m_lightSeqGridWidth)) && ((y >= 0) && (y < m_lightSeqGridHeight)))
  {
