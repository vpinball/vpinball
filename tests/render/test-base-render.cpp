...
static void TestRender(const string& tableFile, const string& imageName, VPXBackends backends)
{
   ...
   if ((int)backends & (int)VPXBackends::DX9)
      SUBCASE("Direct3D9")
      {
   ...
   if ((int)backends & (int)VPXBackends::GL)
      SUBCASE("OpenGL")
      {
   ...
   if ((int)backends & (int)VPXBackends::BGFX_VULKAN)
      SUBCASE("BGFX-Vulkan")
      {
         g_app->m_settings.SetPlayer_GfxBackend("Vulkan"s, false);
   ...
   if ((int)backends & (int)VPXBackends::BGFX_GL)
      SUBCASE("BGFX-OpenGL")
      {
         g_app->m_settings.SetPlayer_GfxBackend("OpenGL"s, false);
   ...
   {
      g_app->m_settings.SetPlayer_GfxBackend("OpenGLES"s, false);
   ...
   if ((int)backends & (int)VPXBackends::BGFX_DX11)
      ...
      {
         g_app->m_settings.SetPlayer_GfxBackend("Direct3D11"s, false);
