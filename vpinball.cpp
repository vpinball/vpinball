...
void VPinball::GetMyPrefPath()
{
   ...
   if (!DirExists(m_szMyPrefPath)) {
      std::error_code ec;
      if (std::filesystem::create_directory(m_szMyPrefPath, ec)) {
         PLOGI.printf("Pref path created: %s", m_szMyPrefPath.c_str());
      }
      else {
         PLOGE.printf("Unable to create pref path: %s", m_szMyPrefPath.c_str());
      }
   }
}
