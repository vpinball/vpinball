...
{
   if (!m_pSDLTexture) {
      m_pSDLTexture = SDL_CreateTexture(m_pRenderer, SDL_PIXELFORMAT_ABGR8888, SDL_TEXTUREACCESS_TARGET, GetPixelWidth(), GetPixelHeight());
      if (!m_pSDLTexture)
         return;
   }

   if (!m_pTexture) {
      m_pTexture = new BaseTexture(GetPixelWidth(), GetPixelHeight(), BaseTexture::RGBA);
      if (!m_pTexture)
         return;
   } 
