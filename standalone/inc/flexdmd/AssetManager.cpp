...
AssetSrc* AssetManager::ResolveSrc(const string& src, AssetSrc* pBaseSrc)
{
   if (src.find("|") != string::npos) {
      PLOGW.printf("'|' is not allowed inside file names as it is the separator for image sequences: %s", src.c_str());
      return NULL;
   }
   ...
   if (parts[0].starts_with("FlexDMD.Resources.")) {
      ...
      if (path.starts_with("dmds."))
         pAssetSrc->SetPath(path.substr(5));
      else 
      ...
      for (Texture* texturePtr : g_pvp->m_ptableActive->m_vimage) {
         if (case_insensitive_compare(texturePtr->m_szName, pAssetSrc->GetPath())) {
            pTexture = texturePtr;
            break;
         }
      }
   ...
   if (pAssetSrc->GetAssetType() == AssetType_Image) {
      ...
      PLOGE.printf("Failed to resolve asset: %s", src.c_str());
   }
...
void* AssetManager::Open(AssetSrc* pSrc)
{
   ...
   switch(pSrc->GetSrcType()) {
      ...
      {
         for (Texture* texturePtr : g_pvp->m_ptableActive->m_vimage) {
            if (case_insensitive_compare(texturePtr->m_szName, pSrc->GetPath())) {
...
Bitmap* AssetManager::GetBitmap(AssetSrc* pSrc)
{
   ...
   if (it != m_cachedBitmaps.end()) {
      ...
      Bitmap* pCachedBitmap = new Bitmap(Open(pSrc), pSrc->GetAssetType());
      pCachedBitmap->AddRef();
