...
void Font::DrawCharacter(Graphics* graphics, char character, char previousCharacter, float& x, float& y)
{
   if (character == '\n') {
      x = 0;
         ...
         if (graphics) {
            ...
            if (pSource) {
               ...
               graphics->DrawImage(pSource, 
                  ...
                  (int)(x + offset.x + kerning), (int)(y + offset.y), bounds.w, bounds.h);
            }
         }
...
void Font::DrawText_(Graphics* graphics, float x, float y, const string& text)
{
