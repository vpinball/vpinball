...
void FlexDMD::RenderLoop()
{
   ...
   m_pGraphics = new VP::Graphics(m_width, m_height);
         ...
         if (m_pDmd) {
            switch (m_renderMode) {
               case RenderMode_DMD_GRAY_2:
                  m_pDmd->SetPixelData(m_pGraphics->GetPixels(), m_dmdColor, 2);
                  break;
               case RenderMode_DMD_GRAY_4:
                  m_pDmd->SetPixelData(m_pGraphics->GetPixels(), m_dmdColor, 4);
                  break;
               case RenderMode_DMD_RGB:
                  m_pDmd->SetRGB24PixelData(m_pGraphics->GetPixels());
                  break;
               case RenderMode_SEG_2x16Alpha:
               case RenderMode_SEG_2x20Alpha:
               case RenderMode_SEG_2x7Alpha_2x7Num:
