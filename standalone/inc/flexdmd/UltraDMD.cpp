...
Actor* UltraDMD::ResolveImage(const string& filename, bool useFrame)
{
   ...
   if (try_parse_int(filename, key) && m_preloads.find(key) != m_preloads.end()) {
      ...
      if (pVideoDef) {
         Video* pActor = new Video(m_pFlexDMD, pVideoDef->GetVideoFilename(), "", pVideoDef->GetLoop());
         ...
         if (pImageSequenceDef) {
            ImageSequence* pVideo = new ImageSequence(m_pFlexDMD, m_pFlexDMD->GetAssetManager(), pImageSequenceDef->GetImages(), "", pImageSequenceDef->GetFPS(), pImageSequenceDef->GetLoop());
       ...
       if (path.find("|") != string::npos)
          return new ImageSequence(m_pFlexDMD, m_pFlexDMD->GetAssetManager(), path, "", 30, true);
       else {
          ...
          if (assetType == AssetType_Image)
             return new Image(m_pFlexDMD, m_pFlexDMD->GetAssetManager(), path, "");
