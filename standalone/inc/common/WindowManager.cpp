...
vector<VPX::Window*> WindowManager::GetVPXWindows() const
{
   vector<VPX::Window*> result;
   for (VP::Window* pWnd : m_windows) {
      if (VPX::Window* pVPXWnd = pWnd->GetWindow())
         result.push_back(pVPXWnd);
   }
   return result;
}
