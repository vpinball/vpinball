   ...
   if (index == displayCount - 1) {
      for (int i = 0; i < m_displays.size(); i++) {
         if ((m_displays[i].type & DMD) == DMD) {
            ...
            m_dmdHeight = m_displays[i].height;
            m_dmdWidth = m_displays[i].width;
            ...
            generate_palette(m_dmdPalette, m_displays[i].depth, m_dmdColor);
      ...
      if (m_dmdIndex == -1) {
         switch(PinmameGetHardwareGen()) {
            case PINMAME_HARDWARE_GEN::S3:
            case PINMAME_HARDWARE_GEN::S3C:
            case PINMAME_HARDWARE_GEN::S4:
            case PINMAME_HARDWARE_GEN::S6:
               m_alphaNumericLayout = NumericalLayout::__2x6Num_2x6Num_4x1Num;
               break;
            case PINMAME_HARDWARE_GEN::S7:
               m_alphaNumericLayout = NumericalLayout::__2x7Num_2x7Num_4x1Num_gen7;
               break;
            case PINMAME_HARDWARE_GEN::S9:
               m_alphaNumericLayout = NumericalLayout::__2x7Num10_2x7Num10_4x1Num;
               break;
            ...
            case PINMAME_HARDWARE_GEN::S11B2:
               m_alphaNumericLayout = NumericalLayout::__2x16Alpha;
               break;
            case PINMAME_HARDWARE_GEN::S11:
               m_alphaNumericLayout = NumericalLayout::__6x4Num_4x1Num;
               break;
            case PINMAME_HARDWARE_GEN::S11X:
               switch(displayCount) {
                  case 2:
                     ...
                     break;
                  case 3:
                     m_alphaNumericLayout = NumericalLayout::__1x16Alpha_1x16Num_1x7Num;
                     break;
                  case 4:
                     m_alphaNumericLayout = NumericalLayout::__2x7Alpha_2x7Num;
                     break;
                  case 8:
                     m_alphaNumericLayout = NumericalLayout::__2x7Alpha_2x7Num_4x1Num;
                     break;
               }
               break;
            ...
            case PINMAME_HARDWARE_GEN::GTS80:
               switch(m_displays[0].length) {
                  case 6:
                     m_alphaNumericLayout = NumericalLayout::__2x6Num10_2x6Num10_4x1Num;
                     break;
                  case 7:
                     m_alphaNumericLayout = NumericalLayout::__2x7Num10_2x7Num10_4x1Num;
                     break;
            ...
            case PINMAME_HARDWARE_GEN::GTS3:
               m_alphaNumericLayout = NumericalLayout::__2x20Alpha;
               break;
            ...
            case PINMAME_HARDWARE_GEN::STMPU200:
               switch(m_displays[0].length) {
                  case 6:
                     ...
                     break;
                  case 7:
                     m_alphaNumericLayout = NumericalLayout::__2x7Num_2x7Num_4x1Num;
                     break;
               }
               break;
            ...
            case PINMAME_HARDWARE_GEN::BY35:
               switch(m_displays[0].length) {
                  ...
                  case 7:
                     if (displayCount == 9)
                       m_alphaNumericLayout = NumericalLayout::__2x7Num_2x7Num_10x1Num;
                     else
                       m_alphaNumericLayout = NumericalLayout::__2x7Num_2x7Num_4x1Num;
                     break;
               }
            ...
            case PINMAME_HARDWARE_GEN::BY6803A:
               m_alphaNumericLayout = NumericalLayout::__4x7Num10;
               break;
         ...
         if (m_alphaNumericLayout != NumericalLayout::None) {
            m_dmdWidth = 128;
            m_dmdHeight = 32;

            m_pDmdLevels = (UINT8*)LEVELS_WPC;
            generate_palette(m_dmdPalette, 2, m_dmdColor);
         }
      }

      if (m_dmdIndex != -1 || m_alphaNumericLayout != NumericalLayout::None) {
          m_pDmdData = (UINT8*)malloc(m_dmdWidth * m_dmdHeight);
          memset(m_pDmdData, 0,  m_dmdWidth * m_dmdHeight);

          m_serum.Load(m_szPath + "altcolor", string(m_pPinmameGame->name));

          if (m_pZeDMDUtil) {
             m_pZeDMDUtil->Open(m_dmdWidth, m_dmdHeight);
             m_pZeDMDUtil->SetPalette(m_dmdPalette);
          }
      }
   }
}
   ...
   if (m_dmdIndex != -1) {
      if (m_dmdIndex == index) {
          ...
          if (!m_serum.IsLoaded()) {
             if (p_displayData) {
                ...
                if (m_pZeDMDUtil) {
                   if (p_displayLayout->depth == 2)
                      m_pZeDMDUtil->RenderGray2(m_pDmdData);
                   else
                      m_pZeDMDUtil->RenderGray4(m_pDmdData);
                }
             }
          }
             ...
             if (m_pZeDMDUtil)
                m_pZeDMDUtil->RenderColoredGray6(m_pDmdData, m_dmdPalette, NULL);
          }

          lock.unlock();
      }
   }
   else if (m_alphaNumericLayout != NumericalLayout::None) {
      if (p_displayData)
      ...
      if (index == m_displays.size() - 1) {
         UINT8* pBuffer = AlphaNumeric::Render(m_alphaNumericLayout, m_segmentData);

         std::unique_lock<std::mutex> lock(m_dmdMutex);

         if (!m_serum.IsLoaded()) {
            if (pBuffer) {
               memcpy(m_pDmdData, pBuffer, m_dmdWidth * m_dmdHeight);
               m_dmdUpdate = true;
         ...
         else if (m_serum.Convert((UINT8*)pBuffer, m_pDmdData, m_dmdPalette)) {
             m_dmdUpdate = true;
...
VPinMAMEController::VPinMAMEController()
{
   ...
   m_szPath = LoadValueWithDefault(regKey[RegName::Standalone], "PinMAMEPath"s, ""s);

   if (!m_szPath.empty()) {
      if (m_szPath == string(".") + PATH_SEPARATOR_CHAR)
         m_szPath = g_pvp->m_currentTablePath + "pinmame" + PATH_SEPARATOR_CHAR;
      else if (!m_szPath.ends_with(PATH_SEPARATOR_CHAR))
   ...
   m_szIniPath = LoadValueWithDefault(regKey[RegName::Standalone], "PinMAMEIniPath"s, ""s);

   if (!m_szIniPath.empty()) {
      if (m_szIniPath == string(".") + PATH_SEPARATOR_CHAR)
         m_szIniPath = g_pvp->m_currentTablePath + "pinmame" + PATH_SEPARATOR_CHAR;
      else if (!m_szIniPath.ends_with(PATH_SEPARATOR_CHAR))
         m_szIniPath += PATH_SEPARATOR_CHAR;
   }
   else {
      m_szIniPath = g_pvp->m_szMyPrefPath + "pinmame" + PATH_SEPARATOR_CHAR;
   ...
   m_dmdIndex = -1;
   m_alphaNumericLayout = NumericalLayout::None;
...
STDMETHODIMP VPinMAMEController::get_RawDmdColoredPixels(VARIANT *pVal)
{
   ...
   for (UINT32 i = 0; i < end; i++) {
      pos = m_pDmdData[i] * 3;
      r = m_dmdPalette[pos];
      g = m_dmdPalette[pos + 1];
      b = m_dmdPalette[pos + 2];
      V_VT(&pData[i]) = VT_UI4;
