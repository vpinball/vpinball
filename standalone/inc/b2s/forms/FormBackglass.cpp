...
FormBackglass::FormBackglass() : Control()
{
   ...
   m_selectedLEDType = eLEDType_Undefined;
   ...
   m_pB2SScreen = new B2SScreen(m_pB2SData);
   ...
   m_pStartupTimer = new B2S::Timer(2000, &FormBackglass::StartupTimerTick, this);
   m_pStartupTimer->Start();

   // create rotation timer
   m_pRotateTimer = new B2S::Timer(m_rotateTimerInterval, &FormBackglass::RotateTimerTick, this);
}
...
void FormBackglass::OnPaint(SDL_Surface* pSurface)
{
   if (m_pBackgroundImage) {
      // draw background image
      SDL_BlitScaled(m_pBackgroundImage, NULL, pSurface, NULL);
      ...
      if (m_pB2SData->GetIlluminations()->size() > 0) {
         if (!m_pB2SData->IsUseZOrder()) {
            ...
            for(const auto& [key, pPicbox] : *m_pB2SData->GetZOrderImages()) {
               for (int i = 0; i < pPicbox.size(); i++)
                  DrawImage(pSurface, pPicbox[i]);
            }
         }
      }
   }

   Control::OnPaint(pSurface);
}

void FormBackglass::DrawImage(SDL_Surface* pSurface, B2SPictureBox* pPicbox)
{
   if (pPicbox) {
      SDL_FRect rectf = pPicbox->GetRectangleF();
      SDL_Rect rect = {(int)roundf(rectf.x), (int)roundf(rectf.y), (int)roundf(rectf.w), (int)roundf(rectf.h)};
      ...
      if (drawme) {
         if (m_pB2SData->IsOnAndOffImage()) {
            if (m_pB2SData->IsOffImageVisible() && pPicbox->GetOffImage())
               SDL_BlitScaled(pPicbox->GetOffImage(), NULL, pSurface, &rect);
            else
               SDL_BlitScaled(pPicbox->GetBackgroundImage(), NULL, pSurface, &rect);
         }
         else
            SDL_BlitScaled(pPicbox->GetBackgroundImage(), NULL, pSurface, &rect);
      }
...
void FormBackglass::StartupTimerTick(void* param)
{
   FormBackglass* pFormBackglass = static_cast<FormBackglass*>(param);
   pFormBackglass->m_pStartupTimer->Stop();

   // maybe show some 'startup on' images
   pFormBackglass->ShowStartupImages();

   // start autostarted animations
   // B2SAnimation.AutoStart()
}

void FormBackglass::RotateTimerTick(void* param)
{
   FormBackglass* pFormBackglass = static_cast<FormBackglass*>(param);
   ...
   auto rotatingImages = pFormBackglass->m_pB2SData->GetRotatingImages();
   if (rotatingImages && (*rotatingImages)[0].find(currentAngle) != (*rotatingImages)[0].end()) {
      auto rotatingPictureBox = pFormBackglass->m_pB2SData->GetRotatingPictureBox();
      ...
      currentAngleS += pFormBackglass->m_rotateAngle;
      if (currentAngleS >= 360.0f)
         currentAngleS = 0;
      currentAngle = (int)currentAngleS;
   }
   else {
      pFormBackglass->m_pRotateTimer->Stop();
   }

   if (pFormBackglass->m_rotateSlowDownSteps > 0) {
       int lastStep = 25;
       int add2IntervalPerStep = 3;

       if (pFormBackglass->m_rotateSlowDownSteps >= lastStep) {
          pFormBackglass->m_pRotateTimer->Stop();
          pFormBackglass->m_rotateSlowDownSteps = 0;
          pFormBackglass->m_pRotateTimer->SetInterval(pFormBackglass->m_rotateTimerInterval);
       }
       else {
          pFormBackglass->m_rotateSlowDownSteps += 1;
          pFormBackglass->m_pRotateTimer->SetInterval(pFormBackglass->m_pRotateTimer->GetInterval() + add2IntervalPerStep);
       }
   }
   else if (pFormBackglass->m_rotateRunTillEnd) {
      if (currentAngleS + pFormBackglass->m_rotateAngle >= 360)
         pFormBackglass->m_pRotateTimer->Stop();
   }
   else if (pFormBackglass->m_rotateRunToFirstStep) {
      if (currentAngleS == 0)
         pFormBackglass->m_pRotateTimer->Stop();
   }
}
...
void FormBackglass::LoadB2SData()
{
   ...
   try {
      ...
      if (!m_pB2SData->IsDualBackglass())
         m_pB2SSettings->SetCurrentDualMode(eDualMode_2_Authentic);
      ...
      if (topnode->FirstChildElement("Scores")) {
         ...
         for (auto innerNode = topnode->FirstChildElement("Scores")->FirstChildElement("Score"); innerNode != nullptr; innerNode = innerNode->NextSiblingElement("Score")) {
            ...
            // set some tmp vars
            bool isOnBackglass = (parent == "Backglass");
            bool isDream7LEDs = case_insensitive_compare(reeltype, "dream7");
            bool isRenderedLEDs = (reeltype.length() > 7 && case_insensitive_compare(reeltype.substr(0, 7), "rendered"));
            ...
            if (isDream7LEDs || isRenderedLEDs) {
               ...
               // color settings
               pLed->SetGlassColor(reellitcolor);
               pLed->SetLightColor(RGB(min(GetRValue(reellitcolor) + 35, 255), min(GetGValue(reellitcolor) + 35, 255), min(GetBValue(reellitcolor) + 25, 255)));
               pLed->SetGlassColorCenter(RGB(min(GetRValue(reellitcolor) + 50, 255), min(GetGValue(reellitcolor) + 50, 255), min(GetBValue(reellitcolor) + 50, 255)));
      ...
      if (topnode->FirstChildElement("Images")) {
        ...
        m_pBackgroundImage = GetDarkImage();
      ...
      for(const auto& [key, pPicbox] : *m_pB2SData->GetIlluminations()) {
         if (pPicbox->GetInitialState() == 1 && !pPicbox->IsImageSnippit()) {
            if (GetTopRomID() > 0 && pPicbox->GetRomID() == GetTopRomID() && pPicbox->GetRomIDType() == GetTopRomIDType() && pPicbox->IsRomInverted() == IsTopRomInverted()) {
               topIsOn = true;
               if (GetTopLightImage() && GetTopLightImage() != m_pBackgroundImage)
                  m_pBackgroundImage = GetTopLightImage();
            }
               ...
               if (GetSecondLightImage() && GetSecondLightImage() != m_pBackgroundImage)
                  m_pBackgroundImage = GetSecondLightImage();
            }
            else
               pPicbox->SetVisible(true);
         }
...
void FormBackglass::CheckBulbs(int romid, eRomIDType romidtype, bool rominverted, eDualMode dualmode)
{
   if (romid > 0 && romidtype != eRomIDType_NotDefined) {
      ...
      if (pUsedRomIDs->find(romid) != pUsedRomIDs->end()) {
         ...
         for(const auto& [key, pPicbox] : *m_pB2SData->GetIlluminations()) {
            if (pPicbox->GetRomID() && pPicbox->GetRomIDType() == romidtype && pPicbox->IsRomInverted() != rominverted && (pPicbox->GetDualMode() == eDualMode_Both || pPicbox->GetDualMode() == dualmode))
...
SDL_Surface* FormBackglass::RotateSurface(SDL_Surface* source, double angle)
{
   ...
   int center_x = source->w / 2;
   int center_y = source->h / 2;

   for (int y = 0; y < source->h; ++y) {
      for (int x = 0; x < source->w; ++x) {
         int new_x = static_cast<int>((x - center_x) * cosine - (y - center_y) * sine + center_x);
         int new_y = static_cast<int>((x - center_x) * sine + (y - center_y) * cosine + center_y);
