...
FormBackglass::~FormBackglass()
{
   ...
   if (m_pDarkImage4Fantasy)
      SDL_FreeSurface(m_pDarkImage4Fantasy);
...
SDL_Rect FormBackglass::GetBoundingRectangle(SDL_Surface* pImage)
{
   ...
   for (int y = 0; y < pImage->h; y++) {
      for (int x = 0; x < pImage->w; x++) {;
         SDL_GetRGBA(pixels[y * pitch + x], pImage->format, &r, &g, &b, &alpha);
...
SDL_Surface* FormBackglass::CropImageToTransparency(SDL_Surface* pImage, SDL_Surface* pOffImage, SDL_Point& loc, SDL_Rect& size)
{
   ...
   SDL_Surface* pCroppedImage = SDL_CreateRGBSurface(0, boundingRect.w, boundingRect.h, pImage->format->BitsPerPixel,
   ...
   if (pOffImage) {
      ...
      if (offboundingRect.w == 0 && offboundingRect.h == 0) {
         SDL_FreeSurface(pCroppedImage);
         return NULL;
      }

      SDL_Rect unionRect;
      SDL_UnionRect(&boundingRect, &offboundingRect, &unionRect);
      boundingRect = unionRect;

      SDL_Rect srcRect = boundingRect;
      SDL_Rect dstRect = { 0, 0, boundingRect.w, boundingRect.h };
      SDL_BlitScaled(pOffImage, &srcRect, pCroppedImage, &dstRect);
   }

   SDL_Rect srcRect = boundingRect;
   SDL_Rect dstRect = { 0, 0, boundingRect.w, boundingRect.h };
   SDL_BlitScaled(pImage, &srcRect, pCroppedImage, &dstRect);
