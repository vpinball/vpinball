...
void B2SScreen::ReadB2SSettingsFromFile()
{
   ...
   m_backglassSize = { 0, 0,
      pSettings->LoadValueWithDefault(Settings::Standalone, "B2SBackglassWidth"s, 0),
      pSettings->LoadValueWithDefault(Settings::Standalone, "B2SBackglassHeight"s, 0) };

   m_backglassLocation = { 
      pSettings->LoadValueWithDefault(Settings::Standalone, "B2SBackglassX"s, 0),
      pSettings->LoadValueWithDefault(Settings::Standalone, "B2SBackglassY"s, 0) };
  
   m_dmdSize = { 0, 0,
      pSettings->LoadValueWithDefault(Settings::Standalone, "B2SDMDWidth"s, 0),
      pSettings->LoadValueWithDefault(Settings::Standalone, "B2SDMDHeight"s, 0) };

   m_dmdLocation = {
      pSettings->LoadValueWithDefault(Settings::Standalone, "B2SDMDX"s, 0),
      pSettings->LoadValueWithDefault(Settings::Standalone, "B2SDMDY"s, 0) };
...
void B2SScreen::RenderBackglass(VP::Graphics* pGraphics)
{
   m_pFormBackglass->Render(pGraphics);
}

void B2SScreen::RenderDMD(VP::Graphics* pGraphics)
{
   m_pFormDMD->Render(pGraphics);
}
