...
{
   m_backglassSize = { 0, 0,
      LoadValueWithDefault(regKey[RegName::Standalone], "B2SBackglassWidth"s, 0),
      LoadValueWithDefault(regKey[RegName::Standalone], "B2SBackglassHeight"s, 0) };

   m_backglassLocation = { 
      LoadValueWithDefault(regKey[RegName::Standalone], "B2SBackglassX"s, 0),
      LoadValueWithDefault(regKey[RegName::Standalone], "B2SBackglassY"s, 0) };
  
   m_dmdSize = { 0, 0,
      LoadValueWithDefault(regKey[RegName::Standalone], "B2SDMDWidth"s, 0),
      LoadValueWithDefault(regKey[RegName::Standalone], "B2SDMDHeight"s, 0) };

   m_dmdLocation = {
      LoadValueWithDefault(regKey[RegName::Standalone], "B2SDMDX"s, 0),
      LoadValueWithDefault(regKey[RegName::Standalone], "B2SDMDY"s, 0) };

   m_dmdFlipY = LoadValueWithDefault(regKey[RegName::Standalone], "B2SDMDFlipY"s, false);
}
...
void B2SScreen::GetB2SSettings(SDL_Point defaultDMDLocation, eDMDViewMode dmdViewMode, int backglassGrillHeight, int backglassSmallGrillHeight)
{
   ...
   if (showTheGrill) {
      ...
      if (backglassSmallGrillHeight > 0 && pFormBackglass->GetDarkImage()) {
         ...
         pFormBackglass->SetSize( { 0, 0, pFormBackglass->GetBackgroundImage()->w, pFormBackglass->GetBackgroundImage()->h});
      }
   }
   else {
...
void B2SScreen::Show()
{
   ...
   B2SWindows* pWindows = B2SWindows::GetInstance();

   pWindows->EnableBackglass(true);
   ...
   if (m_dmdToBeShown) {
      ...
      pWindows->EnableDMD(true);
   }

   pWindows->SetB2SScreen(this);
}

void B2SScreen::RenderBackglass(SDL_Renderer* pRenderer)
{
   m_pFormBackglass->Render(pRenderer);
}

void B2SScreen::RenderDMD(SDL_Renderer* pRenderer)
{
   m_pFormDMD->Render(pRenderer);
}
