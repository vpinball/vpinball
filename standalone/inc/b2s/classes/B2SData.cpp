...
B2SData::B2SData()
{
   ...
   m_lampsInfoDirty = true;
   m_solenoidsInfoDirty = true;
   m_giStringsInfoDirty = true;
   m_ledInfoDirty = true;
}
...
{
   return (B2SSettings::GetInstance()->GetCurrentDualMode() == eDualMode_2_Fantasy) ? &m_usedRomLampIDs4Fantasy : &m_usedRomLampIDs4Authentic;
}

std::map<int, vector<B2SBaseBox*>>* B2SData::GetUsedRomSolenoidIDs()
{
   return (B2SSettings::GetInstance()->GetCurrentDualMode() == eDualMode_2_Fantasy) ? &m_usedRomSolenoidIDs4Fantasy : &m_usedRomSolenoidIDs4Authentic;
}

std::map<int, vector<B2SBaseBox*>>* B2SData::GetUsedRomGIStringIDs()
{
   return (B2SSettings::GetInstance()->GetCurrentDualMode() == eDualMode_2_Fantasy) ? &m_usedRomGIStringIDs4Fantasy : &m_usedRomGIStringIDs4Authentic;
}

std::map<int, vector<B2SBaseBox*>>* B2SData::GetUsedRomMechIDs()
{
   return (B2SSettings::GetInstance()->GetCurrentDualMode() == eDualMode_2_Fantasy) ? &m_usedRomMechIDs4Fantasy : &m_usedRomMechIDs4Authentic;
}

bool B2SData::GetLampsData()
{
   static bool ret = false;
   if (m_lampsInfoDirty) {
       m_lampsInfoDirty = false;
       ret = GetUseRomLamps() || GetUseAnimationLamps();
   }
   return ret;
}

bool B2SData::GetSolenoidsData()
{
   static bool ret = false;
   if (m_solenoidsInfoDirty) {
       m_solenoidsInfoDirty = false;
       ret = GetUseRomSolenoids() || GetUseAnimationSolenoids();
   }
   return ret;
}

bool B2SData::GetGIStringsData()
{
   static bool ret = false;
   if (m_giStringsInfoDirty) {
       m_giStringsInfoDirty = false;
       ret = GetUseRomGIStrings() || GetUseAnimationGIStrings();
   }
   return ret;
}

bool B2SData::GetLEDsData()
{
   static bool ret = false;
   if (m_ledInfoDirty) {
       m_ledInfoDirty = false;
       ret = GetUseLEDs() || GetUseLEDDisplays() || GetUseReels();
   }
   return ret;
}

bool B2SData::GetUseRomLamps()
{
   if (B2SSettings::GetInstance()->GetCurrentDualMode() == eDualMode_2_Fantasy)
...
bool B2SData::GetUseRomSolenoids()
{
   if (B2SSettings::GetInstance()->GetCurrentDualMode() == eDualMode_2_Fantasy)
...
bool B2SData::GetUseRomGIStrings()
{
   if (B2SSettings::GetInstance()->GetCurrentDualMode() == eDualMode_2_Fantasy)
