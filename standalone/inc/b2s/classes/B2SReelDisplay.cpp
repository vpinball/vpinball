...
B2SReelDisplay::B2SReelDisplay()
{
   ...
   m_pTimerRR = new B2S::Timer(17, &B2SReelDisplay::TimerRRTick, this);
   m_pTimerIA = new B2S::Timer(17, &B2SReelDisplay::TimerIATick, this);
}
...
void B2SReelDisplay::SetScore_(int score, int startAtIndex)
{
   if (m_reels.size() > 0) {
      ...
      std::string scoreAsStringX = oss.str();
...
void B2SReelDisplay::TimerRRTick(void* param)
{
   static_cast<B2SReelDisplay*>(param)->TimerRRTick();
}

void B2SReelDisplay::TimerRRTick()
{
...
void B2SReelDisplay::TimerIATick(void* param)
{
   static_cast<B2SReelDisplay*>(param)->TimerIATick();
}

void B2SReelDisplay::TimerIATick()
{
   if (!IsInAction()) {
      m_pTimerIA->Stop();
