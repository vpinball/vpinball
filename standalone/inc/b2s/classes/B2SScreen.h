...
public:
   ...
   void SetBackglassCutOff(const SDL_Rect& backglassCutOff) { m_backglassCutOff = backglassCutOff; }
   bool IsDMDToBeShown() const { return m_dmdToBeShown; }
   void SetDMDToBeShown(const bool dmdToBeShown) { m_dmdToBeShown = dmdToBeShown; }
   bool IsStartBackground() const { return m_startBackground; }
   void SetStartBackground(const bool startBackground) { m_startBackground = startBackground; }

   void ReadB2SSettingsFromFile();
   void Start(FormBackglass* pFormBackglass, int backglassGrillHeight, int smallBackglassGrillHeight);
   void GetB2SSettings();
   void Show();
