...
B2SReelBox::B2SReelBox() : B2SBaseBox()
{
   ...
   m_pTimer = new B2S::Timer((m_rollingInterval / (((m_intermediates == -1) ? 3 : m_intermediates) + 2)), &B2SReelBox::ReelAnimationTimerTick, this);
}
...
void B2SReelBox::OnPaint(SDL_Surface* pSurface)
{
   ...
   if (!m_szReelIndex.empty()) {
      ...
      if (m_intermediates == -1 && m_pTimer->IsEnabled()) {
         static int firstintermediatecount = 1;
         name = m_szReelType + "_" + m_szReelIndex + (m_setID > 0 && m_illuminated ? "_" + std::to_string(m_setID) : "") + "_" + std::to_string(firstintermediatecount);
         if (pIntImages->contains(name)) {
            SDL_BlitScaled((*pIntImages)[name], NULL, pSurface, &rect);
            firstintermediatecount++;
            ...
            if (pImages->contains(name))
               SDL_BlitScaled((*pImages)[name], NULL, pSurface, &rect);
            m_intermediates = firstintermediatecount - 1;
...
void B2SReelBox::ReelAnimationTimerTick(void* param)
{
   static_cast<B2SReelBox*>(param)->ReelAnimationTimerTick();
}

void B2SReelBox::ReelAnimationTimerTick()
{
...
void B2SReelBox::SetReelType(string szReelType)
{
   ...
   string lcReelType = szReelType;
   std::transform(lcReelType.begin(), lcReelType.end(), lcReelType.begin(), [](unsigned char c){ return std::tolower(c); });
   ...
   if (lcReelType.compare(0, 3, "led", 3) == 0 || lcReelType.compare(0, 11, "importedled", 11) == 0) {
      m_led = true;
      m_szReelIndex = "Empty";
      m_initValue = "Empty";
      SetText(-1);
