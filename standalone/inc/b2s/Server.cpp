...
STDMETHODIMP Server::get_ChangedGIStrings(VARIANT *pRetVal)
{
   ...
   if (m_pB2SData->GetGIStringsData())
...
void Server::CheckGIStrings(SAFEARRAY* psa)
{
   ...
   if (psa) {
      ...
      for (ix[0] = 0; ix[0] < uCount; ix[0]++) {
         ...
         if (m_pB2SData->GetUseRomGIStrings() || m_pB2SData->GetUseAnimationGIStrings()) {
            // collect illumination data
            ...
            else if (m_pFormBackglass->GetSecondRomIDType() == eRomIDType_Solenoid && m_pFormBackglass->GetSecondRomID() == gistringId)
...
void Server::CheckLEDs(SAFEARRAY* psa)
{
   ...
   if (psa) {
      ...
      for (ix[0] = 0; ix[0] < uCount; ix[0]++) {
         ...
         if (m_pB2SData->GetUseLEDs() || m_pB2SData->GetUseLEDDisplays() || m_pB2SData->GetUseReels())
   ...
   if (m_pCollectLEDsData->ShowData()) {
      bool useLEDs = m_pB2SData->GetUseLEDs() && m_pB2SSettings->GetUsedLEDType() == eLEDTypes_Rendered;
      bool useLEDDisplays = m_pB2SData->GetUseLEDDisplays() && m_pB2SSettings->GetUsedLEDType() == eLEDTypes_Dream7;
      bool useReels = m_pB2SData->GetUseReels();
