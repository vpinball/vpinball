...
void DMDUtil::Render(VP::Graphics* pGraphics)
{
   if (!pGraphics)
      return;

   if (!m_pTexture) {
      m_pTexture = SDL_CreateTexture(pGraphics->GetRenderer(), SDL_PIXELFORMAT_RGB24, SDL_TEXTUREACCESS_STREAMING, m_width, m_height);
      if (!m_pTexture) {
   ...
   if (m_rgb) {
      ...
      for (y = 0; y < m_height; ++y) {
         for (x = 0; x < m_width; ++x) {
            i = m_pRenderData[y * m_width + x] * 3;
            pos = y * pitch + x * 3;
            pPixels[pos] = m_renderPalette[i];
            pPixels[pos + 1] = m_renderPalette[i + 1];
            pPixels[pos + 2] = m_renderPalette[i + 2];
         }
      }
      SDL_UnlockTexture(m_pTexture);
   }
   ...
   pGraphics->Clear();
   pGraphics->DrawTexture(m_pTexture, NULL, NULL);
   pGraphics->Present();
}
