...
void ZeDMDUtil::Open(int width, int height)
{
   ...
   new std::thread([this, width, height]() {
      ...
      if (pSettings->LoadValueWithDefault(Settings::Standalone, "ZeDMDDebug", false)) {
         m_zedmd.EnableDebug();
         PLOGI.printf("ZeDMD debug enabled");
      }

      const int rgbOrder = (int)pSettings->LoadValueWithDefault(Settings::Standalone, "ZeDMDRGBOrder"s, -1);
      if (rgbOrder != -1) {
         m_zedmd.SetRGBOrder(rgbOrder);
         PLOGI.printf("ZeDMD RGB order override: rgbOrder=%d", rgbOrder);
      }

      const int brightness = (int)pSettings->LoadValueWithDefault(Settings::Standalone, "ZeDMDBrightness"s, -1);
      if (brightness != -1) {
         m_zedmd.SetBrightness(brightness);
         PLOGI.printf("ZeDMD brightness override: brightness=%d", brightness);
      }

      if (pSettings->LoadValueWithDefault(Settings::Standalone, "ZeDMDSave", false)) {
