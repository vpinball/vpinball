...
#else
  USAGE_STATIC =
      D3DUSAGE_WRITEONLY, // to be used for vertex/index buffers which are uploaded once and never touched again
  USAGE_DYNAMIC =
      D3DUSAGE_DYNAMIC // to be used for vertex/index buffers which are locked every frame/very often
#endif
};
...
public:
  ...
  void Clear(const DWORD numRects,
             const D3DRECT* rects,
             const DWORD flags,
  ...
  D3DTexture* GetBackBufferTexture() const
  {
    return m_pOffscreenBackBufferTexture;
  }
  D3DTexture* GetBackBufferTmpTexture() const
  {
    return m_pOffscreenBackBufferTmpTexture;
  } // stereo/FXAA only
  D3DTexture* GetBackBufferTmpTexture2() const
  {
    return m_pOffscreenBackBufferTmpTexture2;
  } // SMAA only
#ifdef ENABLE_SDL
  D3DTexture* GetNonMSAABlitTexture(int m_MSAASamples) const
  {
    return m_MSAASamples == 1 ? m_pOffscreenBackBufferTexture : m_pOffscreenNonMSAABlitTexture;
  }
  D3DTexture* GetOffscreenVR(int eye) const
  {
    return eye == 0 ? m_pOffscreenVRLeft : m_pOffscreenVRRight;
  }
#endif
  D3DTexture* GetMirrorTmpBufferTexture() const
  {
    return m_pMirrorTmpBufferTexture;
  }
  D3DTexture* GetReflectionBufferTexture() const
  {
    return m_pReflectionBufferTexture;
  }
  RenderTarget* GetOutputBackBuffer() const
  {
    return m_pBackBuffer;
  }

  D3DTexture* GetBloomBufferTexture() const
  {
    return m_pBloomBufferTexture;
  }
  D3DTexture* GetBloomTmpBufferTexture() const
  {
    return m_pBloomTmpBufferTexture;
  }
  ...
  D3DTexture* UploadTexture(BaseTexture* const surf,
                            int* const pTexWidth,
  ...
  void SetRenderStateAlphaTestFunction(const DWORD testValue,
...
#ifndef ENABLE_SDL
  void SetTextureStageState(const DWORD stage,
  ...
  void DrawPrimitiveVB(const PrimitiveTypes type,
                       const DWORD fvf,
                       VertexBuffer* vb,
                       const DWORD startVertex,
                       const DWORD vertexCount,
                       const bool stereo);
  void DrawIndexedPrimitiveVB(const PrimitiveTypes type,
                              const DWORD fvf,
                              VertexBuffer* vb,
                              const DWORD startVertex,
                              const DWORD vertexCount,
  ...
  unsigned int Perf_GetNumDrawCalls() const
  {
  ...
  inline void CreateVertexDeclaration(const VertexElement* const element,
...
private:
  void DrawPrimitive(const PrimitiveTypes type,
                     const DWORD fvf,
                     const void* vertices,
                     const DWORD vertexCount);
...
public:
  ...
  static VertexBuffer*
      m_quadVertexBuffer; // internal vb for rendering quads //!! only on primary device for now!
