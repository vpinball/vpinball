...
void HitTarget::SetDefaults(bool fromMouseClick)
{
  ...
  m_d.m_isDropped =
      fromMouseClick ? LoadValueBoolWithDefault(strKeyName, "IsDropped", false) : false;
...
static const Vertex3Ds dropTargetHitPlaneVertices[num_dropTargetHitPlaneVertices] = {
    Vertex3Ds(-0.300000f, 0.001737f, -0.160074f), Vertex3Ds(-0.300000f, 0.001738f, 0.439926f),
    Vertex3Ds(0.300000f, 0.001738f, 0.439926f),   Vertex3Ds(0.300000f, 0.001737f, -0.160074f),
    Vertex3Ds(-0.500000f, 0.001738f, 0.439926f),  Vertex3Ds(-0.500000f, 0.001738f, 1.789926f),
    Vertex3Ds(0.500000f, 0.001738f, 1.789926f),   Vertex3Ds(0.500000f, 0.001738f, 0.439926f),
    Vertex3Ds(-0.535355f, 0.001738f, 0.454570f),  Vertex3Ds(-0.535355f, 0.001738f, 1.775281f),
    Vertex3Ds(-0.550000f, 0.001738f, 0.489926f),  Vertex3Ds(-0.550000f, 0.001738f, 1.739926f),
    Vertex3Ds(0.535355f, 0.001738f, 0.454570f),   Vertex3Ds(0.535355f, 0.001738f, 1.775281f),
    Vertex3Ds(0.550000f, 0.001738f, 0.489926f),   Vertex3Ds(0.550000f, 0.001738f, 1.739926f)};
...
constexpr WORD dropTargetHitPlaneIndices[num_dropTargetHitPlaneIndices] = {
    0, 1, 2, 2, 3,  0,  1,  4, 5, 6, 7,  2, 5,  6, 1,  2,  1,  6,  4,  8,  9,
    9, 5, 4, 8, 10, 11, 11, 9, 8, 6, 12, 7, 12, 6, 13, 12, 13, 14, 13, 15, 14};
...
void HitTarget::GetHitShapes(vector<HitObject*>& pvho)
{
  ...
  if (m_d.m_targetType == DropTargetBeveled || m_d.m_targetType == DropTargetFlatSimple ||
      m_d.m_targetType == DropTargetSimple)
  {
    ...
    if (!m_d.m_legacy)
    {
      ...
      {
        Vertex3Ds vert(dropTargetHitPlaneVertices[i].x,
                       dropTargetHitPlaneVertices[i].y + hitShapeOffset,
                       dropTargetHitPlaneVertices[i].z);
        ...
        rgv3D[i].z = vert.z * m_ptable->m_BG_scalez[m_ptable->m_BG_current_set] +
...
void HitTarget::AddHitEdge(vector<HitObject*>& pvho,
                           std::set<std::pair<unsigned, unsigned>>& addedEdges,
                           const unsigned i,
                           const unsigned j,
                           const Vertex3Ds& vi,
                           const Vertex3Ds& vj,
                           const bool setHitObject)
{
...
void HitTarget::GenerateMesh(std::vector<Vertex3D_NoTex2>& buf)
{
  ...
  for (unsigned int i = 0; i < m_numVertices; i++)
  {
    ...
    buf[i].z = vert.z * m_ptable->m_BG_scalez[m_ptable->m_BG_current_set] + m_d.m_vPosition.z +
...
void HitTarget::TransformVertices()
{
  ...
  for (unsigned i = 0; i < m_numVertices; i++)
  {
    ...
    m_hitUIVertices[i].z = vert.z * m_ptable->m_BG_scalez[m_ptable->m_BG_current_set] +
                           (m_d.m_vPosition.z + m_ptable->m_tableheight);
  }
}
...
void HitTarget::RenderObject()
{
  ...
  pd3dDevice->SetRenderStateCulling(mat->m_bOpacityActive ? RenderDevice::CULL_CW
  ...
  pd3dDevice->DrawIndexedPrimitiveVB(RenderDevice::TRIANGLELIST, MY_D3DFVF_NOTEX2_VERTEX,
                                     m_vertexBuffer, 0, m_numVertices, m_indexBuffer, 0,
                                     m_numIndices);
...
void HitTarget::UpdateTarget()
{
  ...
  else
  {
    ...
    tempMatrix.SetTranslation(m_d.m_vPosition.x, m_d.m_vPosition.y,
                              m_d.m_vPosition.z + m_ptable->m_tableheight);
    tempMatrix.Multiply(vertMatrix, vertMatrix);
...
void HitTarget::RenderSetup()
{
  SAFE_BUFFER_RELEASE(m_vertexBuffer);
  SetMeshType(m_d.m_targetType);
  VertexBuffer::CreateVertexBuffer((unsigned int)m_numVertices, USAGE_DYNAMIC,
                                   MY_D3DFVF_NOTEX2_VERTEX, &m_vertexBuffer, PRIMARY_DEVICE);
      ...
      m_d.m_targetType == DropTargetFlatSimple)
  {
    ...
    {
      m_moveDown = false;
      m_moveAnimationOffset =
          -DROP_TARGET_LIMIT * m_ptable->m_BG_scalez[m_ptable->m_BG_current_set];
      UpdateTarget();
      return;
    }
  }
...
HRESULT HitTarget::InitLoad(IStream* pstm,
                            PinTable* ptable,
                            int* pid,
                            int version,
...
void HitTarget::SetDefaultPhysics(bool fromMouseClick)
{
  ...
  m_d.m_elasticity =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "Elasticity", 0.35f) : 0.35f;
...
{
  *pVal =
      (VARIANT_BOOL)(m_d.m_isDropped); //don't use FTOVB here because it converts a TRUE to -1 instead to 1
