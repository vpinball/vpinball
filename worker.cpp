...
VOID CALLBACK HangSnoopProc(HWND hwnd, UINT uMsg, UINT_PTR idEvent, DWORD dwTime)
{
  ...
  if (!g_pplayer->m_pause && newvalue == lasthangsnoopvalue && !g_pplayer->m_ModalRefCount)
  {
    ...
    delete[] wzError;
    /*const HRESULT hr =*/g_pplayer->m_ptable->m_pcv->m_pScript->InterruptScriptThread(
        SCRIPTTHREADID_BASE /*SCRIPTTHREADID_ALL*/, &eiInterrupt,
        /*SCRIPTINTERRUPT_DEBUG*/ SCRIPTINTERRUPT_RAISEEXCEPTION);
  }
...
void CompleteAutoSave(HANDLE hEvent, LPARAM lParam)
{
  ...
  const std::wstring wzT =
      g_pvp->m_wzMyPath + L"AutoSave" + std::to_wstring(pasp->tableindex) + L".vpx";
