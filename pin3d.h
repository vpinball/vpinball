...
public:
  ...
  void FitCameraToVerticesFS(const std::vector<Vertex3Ds>& pvvertex3D,
                             float aspect,
                             float rotation,
                             float inclination,
                             float FOV,
                             float xlatez,
                             float layback);
  void FitCameraToVertices(const std::vector<Vertex3Ds>& pvvertex3D,
                           float aspect,
                           float rotation,
                           float inclination,
                           float FOV,
  ...
  void TransformVertices(const Vertex3Ds* const rgv,
...
public:
  ...
  HRESULT InitPin3D(const bool fullScreen,
                    const int width,
                    const int height,
  ...
  void TransformVertices(const Vertex3D_NoTex2* const __restrict rgv,
...
#ifdef ENABLE_SDL
  void SetRenderTarget(RenderDevice* const pd3dDevice,
  ...
  void EnableAlphaBlend(const bool additiveBlending,
                        const bool set_dest_blend = true,
                        const bool set_blend_op = true) const;

  void DrawBackground();
  void RenderPlayfieldGraphics(const bool depth_only);

  const Matrix3D& GetWorldTransform() const
  {
    return m_proj.m_matWorld;
  }
  const Matrix3D& GetViewTransform() const
  {
    return m_proj.m_matView;
  }
...
private:
  ...
  HRESULT InitPrimary(const bool fullScreen,
                      const int colordepth,
                      int& refreshrate,
                      const int VSync,
                      const bool useAA,
...
public:
  void
  UpdateBAMHeadTracking(); // #ravarcade: UpdateBAMHeadTracking will set proj/view matrix to add BAM view and head tracking
#endif
};
