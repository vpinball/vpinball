...
class DebuggerModule : public CComObjectRootEx<CComSingleThreadModel>,
...
public:
  ...
  STDMETHOD(OnScriptTerminate)(const VARIANT* pvr, const EXCEPINFO* pei) { return S_OK; }
  ...
  void LoadFromStream(IStream* pistream,
                      HCRYPTHASH const hcrypthash,
                      const HCRYPTKEY hcryptkey); // incl. table protection
...
private:
  ...
  bool ParseStructureName(vector<UserData>& ListIn,
                          UserData ud,
                          const string& UCline,
                          const string& line,
                          const int Lineno);
...
class OMCollectionEnum : public CComObjectRootEx<CComSingleThreadModel>, public IEnumVARIANT
{
public:
