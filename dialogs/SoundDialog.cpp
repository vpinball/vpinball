...
void SoundDialog::Import()
{
  ...
  if (g_pvp->OpenFileDialog(szInitialDir, szFileName,
                            "Sound Files (.wav/.ogg/.mp3)\0*.wav;*.ogg;*.mp3\0", "mp3",
...
void SoundDialog::ReImportFrom()
{
  ...
  if (sel != -1)
  {
    const LocalString ls(IDS_REPLACESOUND);
    const int ans =
        MessageBox(ls.m_szbuffer /*"Are you sure you want to replace this sound with a new one?"*/,
                   "Confirm Reimport", MB_YESNO | MB_DEFBUTTON2);
    if (ans == IDYES)
    {
      string szInitialDir;
      ...
      if (g_pvp->OpenFileDialog(szInitialDir, szFileName,
                                "Sound Files (.wav/.ogg/.mp3)\0*.wav;*.ogg;*.mp3\0", "mp3", 0))
      {
        LVITEM lvitem;
        lvitem.mask = LVIF_PARAM;
        lvitem.iItem = sel;
...
void SoundDialog::Export()
{
  ...
  if (selectedItemsCount)
  {
    ...
    if (sel != -1)
    {
      ...
      {
        strncat_s(filename, pps->m_szName.c_str(),
                  sizeof(filename) - strnlen_s(filename, sizeof(filename)) - 1);
        const size_t idx = pps->m_szPath.find_last_of('.');
        strncat_s(filename, pps->m_szPath.c_str() + idx,
                  sizeof(filename) - strnlen_s(filename, sizeof(filename)) - 1);
      }
        ...
        while (sel != -1)
        {
          if (selectedItemsCount > 1)
          {
            ...
            if (!renameOnExport)
            {
              ...
              strncat_s(filename, pps->m_szPath.c_str() + begin,
                        sizeof(filename) - strnlen_s(filename, sizeof(filename)) - 1);
            }
            else
            {
...
void SoundDialog::SoundToBG()
{
  ...
  if (ListView_GetSelectedCount(hSoundList))
  {
    ...
    while (sel != -1)
    {
      ...
      pps->m_outputTarget =
          (pps->m_outputTarget != SNDOUT_BACKGLASS) ? SNDOUT_BACKGLASS : SNDOUT_TABLE;
