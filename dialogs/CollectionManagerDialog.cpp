...
BOOL CollectionManagerDialog::OnCommand(WPARAM wParam, LPARAM lParam)
{
  ...
  switch (LOWORD(wParam))
  {
    ...
    case IDC_DELETE_COLLECTION:
    {
      ...
      {
        // TEXT
        const int ans = MessageBox("Are you sure you want to remove this collection?",
...
CollectionDialog::CollectionDialog(CollectionDialogStruct& pcds)
  : CDialog(IDD_COLLECTION), pCurCollection(pcds)
{
}
...
BOOL CollectionDialog::OnInitDialog()
{
  ...
  ::SendMessage(GetDlgItem(IDC_FIRE).GetHwnd(), BM_SETCHECK,
                pcol->m_fireEvents ? BST_CHECKED : BST_UNCHECKED, 0);
  ::SendMessage(GetDlgItem(IDC_SUPPRESS).GetHwnd(), BM_SETCHECK,
                pcol->m_stopSingleEvents ? BST_CHECKED : BST_UNCHECKED, 0);
  ::SendMessage(GetDlgItem(IDC_GROUP_CHECK).GetHwnd(), BM_SETCHECK,
                pcol->m_groupElements ? BST_CHECKED : BST_UNCHECKED, 0);
  ...
  for (int i = 0; i < pcol->m_visel.size(); i++)
  {
    ...
    if (piscript)
    {
      WideCharToMultiByteNull(CP_ACP, 0, piscript->m_wzName, -1, szT, sizeof(szT), nullptr,
                              nullptr);
...
BOOL CollectionDialog::OnCommand(WPARAM wParam, LPARAM lParam)
{
  ...
  switch (LOWORD(wParam))
  {
    ...
    case IDC_DOWN:
    {
      ...
      //for (i=count-1;i>=0;i--)
      {
        ...
        const size_t newindex =
            (LOWORD(wParam) == IDC_UP)
                ? max(rgsel[i] - 1, (int)i)
                : min(rgsel[i] + 2, (int)(listsize - (count - 1) + i)); //!! see above
    ...
    {
      const HWND hwndOut =
          GetDlgItem((LOWORD(wParam) == IDC_IN) ? IDC_OUTLIST : IDC_INLIST).GetHwnd();
      const HWND hwndIn =
          GetDlgItem((LOWORD(wParam) == IDC_IN) ? IDC_INLIST : IDC_OUTLIST).GetHwnd();
...
void CollectionDialog::OnOK()
{
  ...
  pCurCollection.ppt->SetCollectionName(pcol, GetDlgItem(IDC_NAME).GetWindowText().c_str(), nullptr,
                                        0);
