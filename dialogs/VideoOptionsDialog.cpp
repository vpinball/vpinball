...
void VideoOptionsDialog::AddToolTip(const char* const text,
                                    HWND parentHwnd,
                                    HWND toolTipHwnd,
                                    HWND controlHwnd)
{
    ...
    const unsigned int profile) // 0 = default, 1 = lowend PC, 2 = highend PC
{
  if (profile == 0)
  {
    ...
    SendMessage(GetDlgItem(IDC_FULLSCREEN).GetHwnd(), BM_SETCHECK,
                fullscreen ? BST_CHECKED : BST_UNCHECKED, 0);
    SendMessage(GetHwnd(), fullscreen ? GET_FULLSCREENMODES : GET_WINDOW_MODES, 0, 0);
  }
  if (profile < 2)
  {
    SendMessage(GetDlgItem(IDC_10BIT_VIDEO).GetHwnd(), BM_SETCHECK,
                false ? BST_CHECKED : BST_UNCHECKED, 0);
  }
  ...
  SendMessage(GetDlgItem(IDC_Tex2048).GetHwnd(), BM_SETCHECK,
              profile == 1 ? BST_CHECKED : BST_UNCHECKED, 0);
  SendMessage(GetDlgItem(IDC_TexUnlimited).GetHwnd(), BM_SETCHECK,
              profile != 1 ? BST_CHECKED : BST_UNCHECKED, 0);
  SendMessage(GetDlgItem(IDC_GLOBAL_REFLECTION_CHECK).GetHwnd(), BM_SETCHECK,
              profile != 1 ? BST_CHECKED : BST_UNCHECKED, 0);
  SendMessage(GetDlgItem(IDC_GLOBAL_TRAIL_CHECK).GetHwnd(), BM_SETCHECK,
              true ? BST_CHECKED : BST_UNCHECKED, 0);
  SendMessage(GetDlgItem(IDC_GLOBAL_DISABLE_LIGHTING_BALLS).GetHwnd(), BM_SETCHECK,
              false ? BST_CHECKED : BST_UNCHECKED, 0);
  ...
  SendMessage(GetDlgItem(IDC_AA_ALL_TABLES).GetHwnd(), BM_SETCHECK,
              false ? BST_CHECKED : BST_UNCHECKED, 0);
  SendMessage(GetDlgItem(IDC_DYNAMIC_DN).GetHwnd(), BM_SETCHECK,
              false ? BST_CHECKED : BST_UNCHECKED, 0);
  SendMessage(GetDlgItem(IDC_DYNAMIC_AO).GetHwnd(), BM_SETCHECK,
              profile == 2 ? BST_CHECKED : BST_UNCHECKED, 0);
  SendMessage(GetDlgItem(IDC_ENABLE_AO).GetHwnd(), BM_SETCHECK, true ? BST_CHECKED : BST_UNCHECKED,
              0);
  SendMessage(GetDlgItem(IDC_GLOBAL_SSREFLECTION_CHECK).GetHwnd(), BM_SETCHECK,
              profile == 2 ? BST_CHECKED : BST_UNCHECKED, 0);
  SendMessage(GetDlgItem(IDC_GLOBAL_PFREFLECTION_CHECK).GetHwnd(), BM_SETCHECK,
              profile != 1 ? BST_CHECKED : BST_UNCHECKED, 0);

  if (profile == 0)
  {
    SendMessage(GetDlgItem(IDC_OVERWRITE_BALL_IMAGE_CHECK).GetHwnd(), BM_SETCHECK,
                false ? BST_CHECKED : BST_UNCHECKED, 0);
  ...
  SendMessage(GetDlgItem(IDC_FXAACB).GetHwnd(), CB_SETCURSEL,
              profile == 1 ? Disabled : (profile == 2 ? Quality_FXAA : Standard_FXAA), 0);
  SendMessage(GetDlgItem(IDC_SHARPENCB).GetHwnd(), CB_SETCURSEL, profile != 2 ? 0 : 2, 0);
  SendMessage(GetDlgItem(IDC_SCALE_FX_DMD).GetHwnd(), BM_SETCHECK,
              false ? BST_CHECKED : BST_UNCHECKED, 0);

  if (profile == 0)
  {
    SendMessage(GetDlgItem(IDC_BG_SET).GetHwnd(), BM_SETCHECK, false ? BST_CHECKED : BST_UNCHECKED,
                0);
    SendMessage(GetDlgItem(IDC_3D_STEREO).GetHwnd(), CB_SETCURSEL, 0, 0);
    SendMessage(GetDlgItem(IDC_3D_STEREO_Y).GetHwnd(), BM_SETCHECK,
                false ? BST_CHECKED : BST_UNCHECKED, 0);
    ...
    SendMessage(GetDlgItem(IDC_USE_NVIDIA_API_CHECK).GetHwnd(), BM_SETCHECK,
                false ? BST_CHECKED : BST_UNCHECKED, 0);
  }

  SendMessage(GetDlgItem(IDC_BLOOM_OFF).GetHwnd(), BM_SETCHECK, false ? BST_CHECKED : BST_UNCHECKED,
              0);

  if (profile == 0)
  {
    SendMessage(GetDlgItem(IDC_DISABLE_DWM).GetHwnd(), BM_SETCHECK,
                false ? BST_CHECKED : BST_UNCHECKED, 0);
  }

  SendMessage(GetDlgItem(IDC_FORCE_ANISO).GetHwnd(), BM_SETCHECK,
              profile != 1 ? BST_CHECKED : BST_UNCHECKED, 0);
  SendMessage(GetDlgItem(IDC_TEX_COMPRESS).GetHwnd(), BM_SETCHECK,
              false ? BST_CHECKED : BST_UNCHECKED, 0);
  SendMessage(GetDlgItem(IDC_SOFTWARE_VP).GetHwnd(), BM_SETCHECK,
              false ? BST_CHECKED : BST_UNCHECKED, 0);
...
void VideoOptionsDialog::FillVideoModesList(const std::vector<VideoMode>& modes,
                                            const VideoMode* curSelMode)
{
...
BOOL VideoOptionsDialog::OnInitDialog()
{
  ...
  const HWND toolTipHwnd = CreateWindowEx(
      NULL, TOOLTIPS_CLASS, NULL, WS_POPUP | TTS_ALWAYSTIP | TTS_BALLOON, CW_USEDEFAULT,
      CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, hwndDlg, NULL, g_pvp->theInstance, NULL);
  if (toolTipHwnd)
  {
    SendMessage(toolTipHwnd, TTM_SETMAXTIPWIDTH, 0, 180);
    AddToolTip(
        "Disable lighting and reflection effects on balls, e.g. to help the visually handicapped.",
        hwndDlg, toolTipHwnd, GetDlgItem(IDC_GLOBAL_DISABLE_LIGHTING_BALLS).GetHwnd());
    AddToolTip("Activate this if you get the corresponding error message on table start, or if you "
               "experience rendering problems.",
               hwndDlg, toolTipHwnd, GetDlgItem(IDC_USE_NVIDIA_API_CHECK).GetHwnd());
    AddToolTip("Forces the bloom filter to be always off. Only for very low-end graphics cards.",
               hwndDlg, toolTipHwnd, GetDlgItem(IDC_BLOOM_OFF).GetHwnd());
    AddToolTip("This saves memory on your graphics card but harms quality of the textures.",
               hwndDlg, toolTipHwnd, GetDlgItem(IDC_TEX_COMPRESS).GetHwnd());
    AddToolTip("Disable Windows Desktop Composition (only works on Windows Vista and Windows 7 "
               "systems).\r\nMay reduce lag and improve performance on some setups.",
               hwndDlg, toolTipHwnd, GetDlgItem(IDC_DISABLE_DWM).GetHwnd());
    AddToolTip("Activate this if you have issues using an Intel graphics chip.", hwndDlg,
               toolTipHwnd, GetDlgItem(IDC_SOFTWARE_VP).GetHwnd());
    AddToolTip("1-activates VSYNC for every frame (avoids tearing)\r\n2-adaptive VSYNC, waits only "
               "for fast frames (e.g. over 60fps)\r\nor set it to e.g. 60 or 120 to limit the fps "
               "to that value (energy saving/less heat)",
               hwndDlg, toolTipHwnd, GetDlgItem(IDC_ADAPTIVE_VSYNC).GetHwnd());
    AddToolTip("Leave at 0 if you have enabled 'Low Latency' or 'Anti Lag' settings in the "
               "graphics driver.\r\nOtherwise experiment with 1 or 2 for a chance of lag reduction "
               "at the price of a bit of framerate.",
               hwndDlg, toolTipHwnd, GetDlgItem(IDC_MAX_PRE_FRAMES).GetHwnd());
    AddToolTip("If played in cabinet mode and you get an egg shaped ball activate this.\r\nFor "
               "screen ratios other than 16:9 you may have to adjust the offsets.\r\nNormally you "
               "have to set the Y offset (around 1.5) but you have to experiment.",
               hwndDlg, toolTipHwnd, GetDlgItem(IDC_StretchMonitor).GetHwnd());
    AddToolTip("Changes the visual effect/screen shaking when nudging the table.", hwndDlg,
               toolTipHwnd, GetDlgItem(IDC_NUDGE_STRENGTH).GetHwnd());
    AddToolTip("Activate this to switch the table brightness automatically based on your PC "
               "date,clock and location.\r\nThis requires to fill in geographic coordinates for "
               "your PCs location to work correctly.\r\nYou may use openstreetmap.org for example "
               "to get these in the correct format.",
               hwndDlg, toolTipHwnd, GetDlgItem(IDC_DYNAMIC_DN).GetHwnd());
    AddToolTip("In decimal degrees (-90..90, North positive)", hwndDlg, toolTipHwnd,
               GetDlgItem(IDC_DN_LATITUDE).GetHwnd());
    AddToolTip("In decimal degrees (-180..180, East positive)", hwndDlg, toolTipHwnd,
               GetDlgItem(IDC_DN_LONGITUDE).GetHwnd());
    AddToolTip("Activate this to enable dynamic Ambient Occlusion.\r\nThis slows down performance, "
               "but enables contact shadows for dynamic objects.",
               hwndDlg, toolTipHwnd, GetDlgItem(IDC_DYNAMIC_AO).GetHwnd());
    AddToolTip("Activate this to enhance the texture filtering.\r\nThis slows down performance "
               "only a bit (on most systems), but increases quality tremendously.",
               hwndDlg, toolTipHwnd, GetDlgItem(IDC_FORCE_ANISO).GetHwnd());
    AddToolTip("Activate this to enable Ambient Occlusion.\r\nThis enables contact shadows between "
               "objects.",
               hwndDlg, toolTipHwnd, GetDlgItem(IDC_ENABLE_AO).GetHwnd());
    AddToolTip("Activate this to enable 3D Stereo output using the requested format.\r\nSwitch "
               "on/off during play with the F10 key.\r\nThis requires that your TV can display 3D "
               "Stereo, and respective 3D glasses.",
               hwndDlg, toolTipHwnd, GetDlgItem(IDC_3D_STEREO).GetHwnd());
    AddToolTip("Switches 3D Stereo effect to use the Y Axis.\r\nThis should usually be selected "
               "for Cabinets/rotated displays.",
               hwndDlg, toolTipHwnd, GetDlgItem(IDC_3D_STEREO_Y).GetHwnd());
    ...
    AddToolTip(
        ...
        hwndDlg, toolTipHwnd, GetDlgItem(IDC_10BIT_VIDEO).GetHwnd());
    AddToolTip("Switches all tables to use the respective Cabinet display setup.\r\nAlso useful if "
               "a 270 degree rotated Desktop monitor is used.",
               hwndDlg, toolTipHwnd, GetDlgItem(IDC_BG_SET).GetHwnd());
    AddToolTip("Enables post-processed Anti-Aliasing.\r\nThis delivers smoother images, at the "
               "cost of slight blurring.\r\n'Quality FXAA' and 'Quality SMAA' are recommended and "
               "lead to less artifacts,\nbut will harm performance on low-end graphics cards.",
               hwndDlg, toolTipHwnd, GetDlgItem(IDC_FXAACB).GetHwnd());
    AddToolTip("Enables post-processed Sharpening of the image.\r\n'Bilateral CAS' is "
               "recommended,\nbut will harm performance on low-end graphics cards.\r\n'CAS' is "
               "less aggressive and faster, but also rather subtle.",
               hwndDlg, toolTipHwnd, GetDlgItem(IDC_SHARPENCB).GetHwnd());
    AddToolTip("Enables brute-force 4x Anti-Aliasing (similar to DSR).\r\nThis delivers very good "
               "quality, but slows down performance significantly.",
               hwndDlg, toolTipHwnd, GetDlgItem(IDC_AA_ALL_TABLES).GetHwnd());
    AddToolTip("When checked it overwrites the ball image/decal image(s) for every table.", hwndDlg,
               toolTipHwnd, GetDlgItem(IDC_OVERWRITE_BALL_IMAGE_CHECK).GetHwnd());
    AddToolTip("Select Display for Video output.", hwndDlg, toolTipHwnd,
               GetDlgItem(IDC_DISPLAY_ID).GetHwnd());
  }

  const int maxTexDim =
      LoadValueIntWithDefault("Player", "MaxTexDimension", 0); // default: Don't resize textures
  switch (maxTexDim)
  {
    case 3072:
      SendMessage(GetDlgItem(IDC_Tex3072).GetHwnd(), BM_SETCHECK, BST_CHECKED, 0);
      break;
    case 512: // legacy, map to 1024 nowadays
    case 1024:
      SendMessage(GetDlgItem(IDC_Tex1024).GetHwnd(), BM_SETCHECK, BST_CHECKED, 0);
      break;
    case 2048:
      SendMessage(GetDlgItem(IDC_Tex2048).GetHwnd(), BM_SETCHECK, BST_CHECKED, 0);
      break;
    default:
  ...
  SendMessage(GetDlgItem(IDC_SCALE_FX_DMD).GetHwnd(), BM_SETCHECK,
              scaleFX_DMD ? BST_CHECKED : BST_UNCHECKED, 0);

  const int bgset = LoadValueIntWithDefault("Player", "BGSet", 0);
  SendMessage(GetDlgItem(IDC_BG_SET).GetHwnd(), BM_SETCHECK,
              (bgset != 0) ? BST_CHECKED : BST_UNCHECKED, 0);
  ...
  SendMessage(GetDlgItem(IDC_3D_STEREO_Y).GetHwnd(), BM_SETCHECK,
              stereo3DY ? BST_CHECKED : BST_UNCHECKED, 0);
  ...
  switch (ballStretchMode)
  {
    ...
    case 0:
      ...
      break;
    case 1:
      SendMessage(GetDlgItem(IDC_StretchYes).GetHwnd(), BM_SETCHECK, BST_CHECKED, 0);
      break;
    case 2:
      SendMessage(GetDlgItem(IDC_StretchMonitor).GetHwnd(), BM_SETCHECK, BST_CHECKED, 0);
      break;
  }
...
INT_PTR VideoOptionsDialog::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  switch (uMsg)
  {
    case GET_WINDOW_MODES:
    {
      ...
      const int display =
          (int)SendMessage(GetDlgItem(IDC_DISPLAY_ID).GetHwnd(), CB_GETCURSEL, 0, 0);
      int screenwidth, screenheight;
      int x, y;
      getDisplaySetupByID(display, x, y, screenwidth, screenheight);
      ...
      static constexpr int portrait_modes_width[num_portrait_modes] = {
          431,  505,  720,  768,  606,  864,  600,  720,  768,  960,  1024,
          768,  768,  800,  1050, 808,  900,  900,  1050, 1200, 1050, 1080,
          1200, 1536, 1212, 1080, 1440, 1440, 1600, 1920, 2048, 1600, 2160};
      static constexpr int portrait_modes_height[num_portrait_modes] = {
          768,  900,  1024, 1024, 1080, 1152, 1280, 1280, 1280, 1280, 1280,
          1360, 1366, 1280, 1400, 1440, 1440, 1600, 1600, 1600, 1680, 1920,
      ...
      for (unsigned int i = 0; i < num_portrait_modes; ++i)
      {
        if ((portrait_modes_width[i] <= screenwidth) &&
            ...
            ((portrait_modes_width[i] != screenwidth) ||
             (portrait_modes_height[i] !=
              screenheight))) // windowed fullscreen is added to the end separately
        {
          VideoMode mode;
      ...
      static constexpr int rgwindowsize[] = {640,  720,  800,  912,  1024, 1152,  1280, 1360, 1366,
                                             1400, 1440, 1600, 1680, 1920, 2048,  2560, 3440, 3840,
                                             4096, 5120, 6400, 7680, 8192, 11520, 15360};
...
BOOL VideoOptionsDialog::OnCommand(WPARAM wParam, LPARAM lParam)
{
  ...
  switch (LOWORD(wParam))
  {
    ...
    {
      const BOOL overwriteEnabled =
          (IsDlgButtonChecked(IDC_OVERWRITE_BALL_IMAGE_CHECK) == BST_CHECKED) ? TRUE : FALSE;
    ...
    case IDC_BROWSE_BALL_IMAGE:
    {
      ...
      // TEXT
      ofn.lpstrFilter = "Bitmap, JPEG, PNG, TGA, WEBP, EXR, HDR Files "
                        "(.bmp/.jpg/.png/.tga/.webp/.exr/"
                        ".hdr)\0*.bmp;*.jpg;*.jpeg;*.png;*.tga;*.webp;*.exr;*.hdr\0";
...
void VideoOptionsDialog::OnOK()
{
  ...
  const bool fullscreen =
      (SendMessage(GetDlgItem(IDC_FULLSCREEN).GetHwnd(), BM_GETCHECK, 0, 0) != 0);
  ...
  const bool video10bit =
      (SendMessage(GetDlgItem(IDC_10BIT_VIDEO).GetHwnd(), BM_GETCHECK, 0, 0) != 0);
  ...
  const bool reflection =
      (SendMessage(GetDlgItem(IDC_GLOBAL_REFLECTION_CHECK).GetHwnd(), BM_GETCHECK, 0, 0) != 0);
  SaveValueBool("Player", "BallReflection", reflection);

  const bool trail =
      (SendMessage(GetDlgItem(IDC_GLOBAL_TRAIL_CHECK).GetHwnd(), BM_GETCHECK, 0, 0) != 0);
  SaveValueBool("Player", "BallTrail", trail);

  const bool disableLighting = (SendMessage(GetDlgItem(IDC_GLOBAL_DISABLE_LIGHTING_BALLS).GetHwnd(),
                                            BM_GETCHECK, 0, 0) != 0);
  ...
  const bool scaleFX_DMD =
      (SendMessage(GetDlgItem(IDC_SCALE_FX_DMD).GetHwnd(), BM_GETCHECK, 0, 0) != 0);
  ...
  useAO = SendMessage(GetDlgItem(IDC_ENABLE_AO).GetHwnd(), BM_GETCHECK, 0, 0)
              ? false
              : true; // inverted logic
  SaveValueBool("Player", "DisableAO", useAO);

  const bool ssreflection =
      (SendMessage(GetDlgItem(IDC_GLOBAL_SSREFLECTION_CHECK).GetHwnd(), BM_GETCHECK, 0, 0) != 0);
  SaveValueBool("Player", "SSRefl", ssreflection);

  const bool pfreflection =
      (SendMessage(GetDlgItem(IDC_GLOBAL_PFREFLECTION_CHECK).GetHwnd(), BM_GETCHECK, 0, 0) != 0);
  ...
  const bool stereo3DY =
      (SendMessage(GetDlgItem(IDC_3D_STEREO_Y).GetHwnd(), BM_GETCHECK, 0, 0) != 0);
  SaveValueBool("Player", "Stereo3DYAxis", stereo3DY);

  const bool forceAniso =
      (SendMessage(GetDlgItem(IDC_FORCE_ANISO).GetHwnd(), BM_GETCHECK, 0, 0) != 0);
  SaveValueBool("Player", "ForceAnisotropicFiltering", forceAniso);

  const bool texCompress =
      (SendMessage(GetDlgItem(IDC_TEX_COMPRESS).GetHwnd(), BM_GETCHECK, 0, 0) != 0);
  SaveValueBool("Player", "CompressTextures", texCompress);

  const bool softwareVP =
      (SendMessage(GetDlgItem(IDC_SOFTWARE_VP).GetHwnd(), BM_GETCHECK, 0, 0) != 0);
  SaveValueBool("Player", "SoftwareVertexProcessing", softwareVP);

  const size_t alphaRampsAccuracy =
      SendMessage(GetDlgItem(IDC_ARASlider).GetHwnd(), TBM_GETPOS, 0, 0);
  ...
  const bool disableDWM =
      (SendMessage(GetDlgItem(IDC_DISABLE_DWM).GetHwnd(), BM_GETCHECK, 0, 0) != 0);
  SaveValueBool("Player", "DisableDWM", disableDWM);

  const bool nvidiaApi =
      (SendMessage(GetDlgItem(IDC_USE_NVIDIA_API_CHECK).GetHwnd(), BM_GETCHECK, 0, 0) != 0);
