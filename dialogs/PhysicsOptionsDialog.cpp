...
static char* physicsoptions[num_physicsoptions] = {nullptr, nullptr, nullptr, nullptr,
...
BOOL PhysicsOptionsDialog::OnInitDialog()
{
  ...
  const float TablePhysicsGravityConstant =
      LoadValueFloatWithDefault("Player", tmp, DEFAULT_TABLE_GRAVITY);
  SetItemText(1100, TablePhysicsGravityConstant);

  sprintf_s(tmp, 256, "TablePhysicsContactFriction%u", physicsselection);
  const float TablePhysicsContactFriction =
  ...
  const float TablePhysicsElasticity =
      LoadValueFloatWithDefault("Player", tmp, DEFAULT_TABLE_ELASTICITY);
  SetItemText(1708, TablePhysicsElasticity);

  sprintf_s(tmp, 256, "TablePhysicsElasticityFalloff%u", physicsselection);
  const float TablePhysicsElasticityFalloff =
  ...
  const float TablePhysicsScatterAngle =
  ...
  const float TablePhysicsContactScatterAngle =
  ...
  const float TablePhysicsMinSlope =
      LoadValueFloatWithDefault("Player", tmp, DEFAULT_TABLE_MIN_SLOPE);
  SetItemText(1103, TablePhysicsMinSlope);

  sprintf_s(tmp, 256, "TablePhysicsMaxSlope%u", physicsselection);
  const float TablePhysicsMaxSlope =
      LoadValueFloatWithDefault("Player", tmp, DEFAULT_TABLE_MAX_SLOPE);
  ...
  switch (LOWORD(wParam))
  {
    ...
    case 1112:
    {
      ...
      xml_node<>* flipTorqueDampingAngle =
          xmlDoc.allocate_node(node_element, "eosTorqueAngle", ftda.c_str());
      flipper->append_node(flipTorqueDampingAngle);

      const string frs(GetItemText(23));
      xml_node<>* flipReturnStrength =
          xmlDoc.allocate_node(node_element, "returnStrength", frs.c_str());
      flipper->append_node(flipReturnStrength);

      const string fef(GetItemText(22));
      xml_node<>* flipElasticityFalloff =
          xmlDoc.allocate_node(node_element, "elasticityFalloff", fef.c_str());
      ...
      xml_node<>* tabGravityConst =
          xmlDoc.allocate_node(node_element, "gravityConstant", tgc.c_str());
      table->append_node(tabGravityConst);

      const string tcf(GetItemText(1101));
      xml_node<>* tabContactFriction =
          xmlDoc.allocate_node(node_element, "contactFriction", tcf.c_str());
      ...
      xml_node<>* tabElasticityFalloff =
          xmlDoc.allocate_node(node_element, "elasticityFalloff", tef.c_str());
      table->append_node(tabElasticityFalloff);

      const string tsa(GetItemText(1710));
      xml_node<>* tabScatterAngle =
          xmlDoc.allocate_node(node_element, "playfieldScatter", tsa.c_str());
      table->append_node(tabScatterAngle);

      const string tcsa(GetItemText(1102));
      xml_node<>* tabContactScatterAngle =
          xmlDoc.allocate_node(node_element, "defaultElementScatter", tcsa.c_str());
      table->append_node(tabContactScatterAngle);

      const string tmns(GetItemText(1103));
      xml_node<>* tabMinSlope =
          xmlDoc.allocate_node(node_element, "playfieldminslope", tmns.c_str());
      table->append_node(tabMinSlope);

      const string tmxs(GetItemText(1104));
      xml_node<>* tabMaxSlope =
          xmlDoc.allocate_node(node_element, "playfieldmaxslope", tmxs.c_str());
...
bool PhysicsOptionsDialog::LoadSetting()
{
  ...
  try
  {
    ...
    strncpy_s(loadValues.gravityConstant, table->first_node("gravityConstant")->value(),
              sizeof(loadValues.gravityConstant) - 1);
    strncpy_s(loadValues.contactFriction, table->first_node("contactFriction")->value(),
              sizeof(loadValues.contactFriction) - 1);
    strncpy_s(loadValues.tableElasticity, table->first_node("elasticity")->value(),
              sizeof(loadValues.tableElasticity) - 1);
    strncpy_s(loadValues.tableElasticityFalloff, table->first_node("elasticityFalloff")->value(),
              sizeof(loadValues.tableElasticityFalloff) - 1);
    strncpy_s(loadValues.playfieldScatter, table->first_node("playfieldScatter")->value(),
              sizeof(loadValues.playfieldScatter) - 1);
    strncpy_s(loadValues.defaultElementScatter, table->first_node("defaultElementScatter")->value(),
    ...
    strncpy_s(loadValues.strength, flipper->first_node("strength")->value(),
              sizeof(loadValues.strength) - 1);
    strncpy_s(loadValues.elasticity, flipper->first_node("elasticity")->value(),
              sizeof(loadValues.elasticity) - 1);
    strncpy_s(loadValues.scatter, flipper->first_node("scatter")->value(),
              sizeof(loadValues.scatter) - 1);
    strncpy_s(loadValues.eosTorque, flipper->first_node("eosTorque")->value(),
              sizeof(loadValues.eosTorque) - 1);
    strncpy_s(loadValues.eosTorqueAngle, flipper->first_node("eosTorqueAngle")->value(),
              sizeof(loadValues.eosTorqueAngle) - 1);
    strncpy_s(loadValues.returnStrength, flipper->first_node("returnStrength")->value(),
              sizeof(loadValues.returnStrength) - 1);
    strncpy_s(loadValues.elasticityFalloff, flipper->first_node("elasticityFalloff")->value(),
              sizeof(loadValues.elasticityFalloff) - 1);
    strncpy_s(loadValues.friction, flipper->first_node("friction")->value(),
              sizeof(loadValues.friction) - 1);
    strncpy_s(loadValues.coilRampup, flipper->first_node("coilRampUp")->value(),
              sizeof(loadValues.coilRampup) - 1);
