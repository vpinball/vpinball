...
RubberVisualsProperty::RubberVisualsProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPRUBBER_VISUALS, pvsel)
{
  ...
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    switch (dispid)
    {
      case IDC_STATIC_RENDERING_CHECK:
        CHECK_UPDATE_ITEM(rubber->m_d.m_staticRendering,
                          PropertyDialog::GetCheckboxState(m_hStaticRenderingCheck), rubber);
        break;
      case IDC_SHOW_IN_EDITOR_CHECK:
        CHECK_UPDATE_ITEM(rubber->m_d.m_showInEditor,
                          PropertyDialog::GetCheckboxState(m_hShowInEditorCheck), rubber);
        break;
      case IDC_RUBBER_HEIGHT_EDIT:
        CHECK_UPDATE_ITEM(rubber->m_d.m_height, PropertyDialog::GetFloatTextbox(m_heightEdit),
                          rubber);
        break;
      case IDC_RUBBER_THICKNESS_EDIT:
        CHECK_UPDATE_ITEM(rubber->m_d.m_thickness, PropertyDialog::GetIntTextbox(m_thicknessEdit),
                          rubber);
        break;
