...
PlungerVisualsProperty::PlungerVisualsProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPPLUNGER_VISUALS, pvsel)
{
  ...
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    switch (dispid)
    {
      case IDC_PLUNGER_TYPE_COMBO:
        CHECK_UPDATE_ITEM(
            plunger->m_d.m_type,
            (PlungerType)(PropertyDialog::GetComboBoxIndex(m_typeCombo, m_typeList) + 1), plunger);
        break;
      case DISPID_PluFrames:
        CHECK_UPDATE_ITEM(plunger->m_d.m_animFrames,
                          PropertyDialog::GetIntTextbox(m_flatFramesEdit), plunger);
        break;
      case DISPID_Width:
        CHECK_UPDATE_ITEM(plunger->m_d.m_width, PropertyDialog::GetFloatTextbox(m_widthEdit),
                          plunger);
        break;
      case DISPID_ZAdjust:
        CHECK_UPDATE_ITEM(plunger->m_d.m_zAdjust,
                          PropertyDialog::GetFloatTextbox(m_zAdjustmentEdit), plunger);
        break;
      case DISPID_RodDiam:
        CHECK_UPDATE_ITEM(plunger->m_d.m_rodDiam,
                          PropertyDialog::GetFloatTextbox(m_rodDiameterEdit), plunger);
        break;
      case DISPID_TipShape:
        PropertyDialog::StartUndo(plunger);
        strncpy_s(plunger->m_d.m_szTipShape, m_tipShapeEdit.GetWindowText().c_str(),
                  sizeof(plunger->m_d.m_szTipShape) - 1);
        PropertyDialog::EndUndo(plunger);
        break;
      case DISPID_RingGap:
        CHECK_UPDATE_ITEM(plunger->m_d.m_ringGap, PropertyDialog::GetFloatTextbox(m_ringGapEdit),
                          plunger);
        break;
      case DISPID_RingDiam:
        CHECK_UPDATE_ITEM(plunger->m_d.m_ringDiam, PropertyDialog::GetFloatTextbox(m_ringDiamEdit),
                          plunger);
        break;
      case DISPID_RingThickness:
        CHECK_UPDATE_ITEM(plunger->m_d.m_ringWidth,
                          PropertyDialog::GetFloatTextbox(m_ringWidthEdit), plunger);
        break;
      case DISPID_SpringDiam:
        CHECK_UPDATE_ITEM(plunger->m_d.m_springDiam,
                          PropertyDialog::GetFloatTextbox(m_springDiamEdit), plunger);
        break;
      case DISPID_SpringGauge:
        CHECK_UPDATE_ITEM(plunger->m_d.m_springGauge,
                          PropertyDialog::GetFloatTextbox(m_springGaugeEdit), plunger);
        break;
      case DISPID_SpringLoops:
        CHECK_UPDATE_ITEM(plunger->m_d.m_springLoops,
                          PropertyDialog::GetFloatTextbox(m_springLoopsEdit), plunger);
        break;
      case DISPID_SpringEndLoops:
        CHECK_UPDATE_ITEM(plunger->m_d.m_springEndLoops,
                          PropertyDialog::GetFloatTextbox(m_endLoopsEdit), plunger);
        break;
      case 902:
...
BOOL PlungerVisualsProperty::OnInitDialog()
{
  ...
  m_widthEdit.AttachItem(DISPID_Width), m_zAdjustmentEdit.AttachItem(DISPID_ZAdjust);
  ...
  m_resizer.AddChild(GetDlgItem(IDC_STATIC17), topleft, RD_STRETCH_WIDTH);
  m_resizer.AddChild(GetDlgItem(IDC_STATIC18), topleft, 0);
  m_resizer.AddChild(GetDlgItem(IDC_STATIC19), topleft, 0);
  m_resizer.AddChild(GetDlgItem(IDC_STATIC20), topleft, 0);
  ...
  m_resizer.AddChild(m_flatFramesEdit, topleft, RD_STRETCH_WIDTH);
  m_resizer.AddChild(m_widthEdit, topleft, RD_STRETCH_WIDTH);
  m_resizer.AddChild(m_zAdjustmentEdit, topleft, RD_STRETCH_WIDTH);
  m_resizer.AddChild(m_rodDiameterEdit, topleft, RD_STRETCH_WIDTH);
  m_resizer.AddChild(m_tipShapeEdit, topleft, RD_STRETCH_WIDTH);
  m_resizer.AddChild(m_ringGapEdit, topleft, RD_STRETCH_WIDTH);
  m_resizer.AddChild(m_ringDiamEdit, topleft, RD_STRETCH_WIDTH);
  m_resizer.AddChild(m_ringWidthEdit, topleft, RD_STRETCH_WIDTH);
  m_resizer.AddChild(m_springDiamEdit, topleft, RD_STRETCH_WIDTH);
  m_resizer.AddChild(m_springGaugeEdit, topleft, RD_STRETCH_WIDTH);
  m_resizer.AddChild(m_springLoopsEdit, topleft, RD_STRETCH_WIDTH);
  m_resizer.AddChild(m_endLoopsEdit, topleft, RD_STRETCH_WIDTH);
