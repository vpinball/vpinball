...
SpinnerPhysicsProperty::SpinnerPhysicsProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPSPINNER_PHYSICS, pvsel)
{
  ...
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    switch (dispid)
    {
      case IDC_DAMPING_EDIT:
        CHECK_UPDATE_ITEM(spinner->m_d.m_damping, PropertyDialog::GetFloatTextbox(m_dampingEdit),
                          spinner);
        break;
