...
RubberPhysicsProperty::RubberPhysicsProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPRUBBER_PHYSICS, pvsel)
{
...
void RubberPhysicsProperty::UpdateProperties(const int dispid)
{
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    {
      case 120:
        CHECK_UPDATE_ITEM(rubber->m_d.m_elasticityFalloff,
                          PropertyDialog::GetFloatTextbox(m_elasticityFallOffEdit), rubber);
        break;
      case IDC_HIT_HEIGHT_EDIT:
        CHECK_UPDATE_ITEM(rubber->m_d.m_hitHeight, PropertyDialog::GetFloatTextbox(m_hitHeightEdit),
                          rubber);
        break;
      default:
