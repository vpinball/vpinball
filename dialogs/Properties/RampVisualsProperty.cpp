...
RampVisualsProperty::RampVisualsProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPRAMP_VISUALS, pvsel)
{
...
void RampVisualsProperty::UpdateProperties(const int dispid)
{
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    {
      case 1:
        CHECK_UPDATE_ITEM(ramp->m_d.m_heightbottom,
                          PropertyDialog::GetFloatTextbox(m_bottomHeightEdit), ramp);
        break;
      case 2:
        CHECK_UPDATE_ITEM(ramp->m_d.m_heighttop, PropertyDialog::GetFloatTextbox(m_topHeightEdit),
                          ramp);
        break;
      case 3:
        CHECK_UPDATE_ITEM(ramp->m_d.m_widthbottom,
                          PropertyDialog::GetFloatTextbox(m_bottomWidthEdit), ramp);
        break;
      case 4:
        CHECK_UPDATE_ITEM(ramp->m_d.m_widthtop, PropertyDialog::GetFloatTextbox(m_topWidthEdit),
                          ramp);
        break;
      case 6:
        CHECK_UPDATE_ITEM(ramp->m_d.m_type,
                          (RampType)(PropertyDialog::GetComboBoxIndex(m_typeCombo, m_typeList)),
                          ramp);
        break;
      case 8:
        CHECK_UPDATE_ITEM(
            ramp->m_d.m_imagealignment,
            (RampImageAlignment)(PropertyDialog::GetComboBoxIndex(m_modeCombo, m_imageModeList)),
            ramp);
        break;
      case 9:
        CHECK_UPDATE_ITEM(ramp->m_d.m_imageWalls,
                          PropertyDialog::GetCheckboxState(::GetDlgItem(GetHwnd(), dispid)), ramp);
        break;
      case 108:
        CHECK_UPDATE_ITEM(ramp->m_d.m_leftwallheightvisible,
                          PropertyDialog::GetFloatTextbox(m_leftWallEdit), ramp);
        break;
      case 109:
        CHECK_UPDATE_ITEM(ramp->m_d.m_rightwallheightvisible,
                          PropertyDialog::GetFloatTextbox(m_rightWallEdit), ramp);
        break;
      case 112:
        CHECK_UPDATE_ITEM(ramp->m_d.m_visible,
                          PropertyDialog::GetCheckboxState(::GetDlgItem(GetHwnd(), dispid)), ramp);
        break;
      case IDC_REFLECT_ENABLED_CHECK:
        CHECK_UPDATE_ITEM(ramp->m_d.m_reflectionEnabled,
                          PropertyDialog::GetCheckboxState(::GetDlgItem(GetHwnd(), dispid)), ramp);
        break;
      case IDC_DEPTH_BIAS:
        CHECK_UPDATE_ITEM(ramp->m_d.m_depthBias, PropertyDialog::GetFloatTextbox(m_depthBiasEdit),
                          ramp);
        break;
      case IDC_WIRE_DIAMETER:
        CHECK_UPDATE_ITEM(ramp->m_d.m_wireDiameter, PropertyDialog::GetFloatTextbox(m_diameterEdit),
                          ramp);
        break;
      case IDC_WIRE_DISTX:
        CHECK_UPDATE_ITEM(ramp->m_d.m_wireDistanceX,
                          PropertyDialog::GetFloatTextbox(m_distanceXEdit), ramp);
        break;
      case IDC_WIRE_DISTY:
        CHECK_UPDATE_ITEM(ramp->m_d.m_wireDistanceY,
                          PropertyDialog::GetFloatTextbox(m_distanceYEdit), ramp);
        break;
      default:
