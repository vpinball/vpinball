...
KickerPhysicsProperty::KickerPhysicsProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPKICKER_STATE, pvsel)
{
...
void KickerPhysicsProperty::UpdateProperties(const int dispid)
{
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    {
      case DISPID_Enabled:
        CHECK_UPDATE_ITEM(kicker->m_d.m_enabled, PropertyDialog::GetCheckboxState(m_hEnableCheck),
                          kicker);
        break;
      case IDC_FALL_THROUGH_HOLE:
        CHECK_UPDATE_ITEM(kicker->m_d.m_fallThrough,
                          PropertyDialog::GetCheckboxState(m_hFallThroughCheck), kicker);
        break;
      case IDC_LEGACYMODE:
        CHECK_UPDATE_ITEM(kicker->m_d.m_legacyMode,
                          PropertyDialog::GetCheckboxState(m_hLegacyCheck), kicker);
        break;
      case IDC_KICKER_HIT_HEIGHT_EDIT:
        CHECK_UPDATE_ITEM(kicker->m_d.m_hit_height,
                          PropertyDialog::GetFloatTextbox(m_hitHeightEdit), kicker);
        break;
      case IDC_HIT_ACC_EDIT:
        CHECK_UPDATE_ITEM(kicker->m_d.m_hitAccuracy,
                          PropertyDialog::GetFloatTextbox(m_hitAccuracyEdit), kicker);
        break;
      default:
