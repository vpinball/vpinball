...
BackglassVisualsProperty::BackglassVisualsProperty(const VectorProtected<ISelect>* pvsel)
...
void BackglassVisualsProperty::UpdateProperties(const int dispid)
{
  ...
  switch (dispid)
  {
    case IDC_BG_NIGHT_DAY:
      CHECK_UPDATE_ITEM(table->m_ImageBackdropNightDay,
                        PropertyDialog::GetCheckboxState(m_hApplyNightDayCheck), table);
      break;
    case DISPID_Image2:
    ...
    case IDC_ENABLE_EMREEL_CHECK:
      CHECK_UPDATE_ITEM(table->m_renderEMReels,
                        PropertyDialog::GetCheckboxState(m_hEnableEMReelCheck), table);
      break;
    case IDC_ENABLE_DECAL_CHECK:
      CHECK_UPDATE_ITEM(table->m_renderDecals, PropertyDialog::GetCheckboxState(m_hEnableDecal),
                        table);
      break;
    case IDC_GLOBAL_3DSTEREO:
      CHECK_UPDATE_ITEM(table->m_overwriteGlobalStereo3D,
                        PropertyDialog::GetCheckboxState(m_hOverwriteGlobalStereoSettingsCheck),
                        table);
      break;
    case IDC_3D_STEREO_OFFSET_EDIT:
      CHECK_UPDATE_VALUE_SETTER(table->Set3DOffset, table->Get3DOffset,
                                PropertyDialog::GetFloatTextbox, m_3dStereoOffsetEdit, table);
      break;
    case IDC_3D_STEREO_SEPARATION_EDIT:
      CHECK_UPDATE_VALUE_SETTER(table->SetMaxSeparation, table->GetMaxSeparation,
                                PropertyDialog::GetFloatTextbox, m_3dStereoSeparationEdit, table);
      break;
    case IDC_3D_STEREO_ZPD_EDIT:
      CHECK_UPDATE_VALUE_SETTER(table->SetZPD, table->GetZPD, PropertyDialog::GetFloatTextbox,
                                m_3dSteroZPDEdit, table);
      break;
    case IDC_COLOR_BUTTON1:
    {
      ...
      if (m_colorDialog.DoModal(GetHwnd()) == IDOK)
      {
        ...
        m_colorButton1.SetColor(table->m_colorbackdrop);
        memcpy(table->m_rgcolorcustom, m_colorDialog.GetCustomColors(),
               sizeof(table->m_rgcolorcustom));
      }
      break;
    }
    default:
