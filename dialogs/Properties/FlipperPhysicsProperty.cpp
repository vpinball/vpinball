...
FlipperPhysicsProperty::FlipperPhysicsProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPFLIPPER_PHYSICS, pvsel)
{
...
void FlipperPhysicsProperty::UpdateProperties(const int dispid)
{
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    {
      case 19:
        CHECK_UPDATE_ITEM(flipper->m_d.m_strength, PropertyDialog::GetFloatTextbox(m_strengthEdit),
                          flipper);
        break;
      case 23:
        CHECK_UPDATE_VALUE_SETTER(flipper->SetReturn, flipper->GetReturn,
                                  PropertyDialog::GetFloatTextbox, m_returnStrengthEdit, flipper);
        break;
      case 27:
        CHECK_UPDATE_VALUE_SETTER(flipper->SetRampUp, flipper->GetRampUp,
                                  PropertyDialog::GetFloatTextbox, m_coilUpRampEdit, flipper);
        break;
      case 28:
        CHECK_UPDATE_VALUE_SETTER(flipper->SetElastacityFalloff, flipper->GetElastacityFalloff,
                                  PropertyDialog::GetFloatTextbox, m_elasticityFalloffEdit,
                                  flipper);
        break;
      case 113:
        CHECK_UPDATE_ITEM(flipper->m_d.m_torqueDamping,
                          PropertyDialog::GetFloatTextbox(m_eosTorqueEdit), flipper);
        break;
      case 189:
        CHECK_UPDATE_ITEM(flipper->m_d.m_torqueDampingAngle,
                          PropertyDialog::GetFloatTextbox(m_eosTorqueAngleEdit), flipper);
        break;
      case DISPID_Flipper_Speed:
        CHECK_UPDATE_ITEM(flipper->m_d.m_mass, PropertyDialog::GetFloatTextbox(m_massEdit),
                          flipper);
        break;
      case 1044:
        CHECK_UPDATE_ITEM(flipper->m_d.m_OverridePhysics,
                          (PhysicsSet)(PropertyDialog::GetComboBoxIndex(m_overwriteSettingsCombo,
                                                                        m_physicSetList)),
                          flipper);
        break;
      default:
