...
GatePhysicsProperty::GatePhysicsProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPGATE_PHYSICS, pvsel)
{
...
void GatePhysicsProperty::UpdateProperties(const int dispid)
{
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    {
      case 13:
        CHECK_UPDATE_ITEM(gate->m_d.m_damping, PropertyDialog::GetFloatTextbox(m_dampingEdit),
                          gate);
        break;
      case 17:
        CHECK_UPDATE_ITEM(gate->m_d.m_gravityfactor,
                          PropertyDialog::GetFloatTextbox(m_gravityFactorEdit), gate);
        break;
      case IDC_TWO_WAY_CHECK:
        CHECK_UPDATE_ITEM(
            gate->m_d.m_twoWay,
            PropertyDialog::GetCheckboxState(::GetDlgItem(GetHwnd(), IDC_TWO_WAY_CHECK)), gate);
        break;
