...
RampPhysicsProperty::RampPhysicsProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPRAMP_PHYSICS, pvsel)
{
...
void RampPhysicsProperty::UpdateProperties(const int dispid)
{
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    {
      case 10:
        CHECK_UPDATE_ITEM(ramp->m_d.m_leftwallheight,
                          PropertyDialog::GetFloatTextbox(m_leftWallEdit), ramp);
        break;
      case 11:
        CHECK_UPDATE_ITEM(ramp->m_d.m_rightwallheight,
