...
TableCustomProperty::TableCustomProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPTABLE_USER, pvsel)
{
...
void TableCustomProperty::UpdateProperties(const int dispid)
{
  ...
  {
    case IDC_ENABLE_AA:
      CHECK_UPDATE_ITEM(table->m_useAA,
                        (PropertyDialog::GetComboBoxIndex(m_SSAACombo, m_userList) - 1), table);
      break;
    case IDC_ENABLE_FXAA:
      CHECK_UPDATE_ITEM(table->m_useFXAA,
                        (PropertyDialog::GetComboBoxIndex(m_postProcAACombo, m_postAAList) - 1),
                        table);
      break;
    case IDC_ENABLE_AO:
      CHECK_UPDATE_ITEM(table->m_useAO,
                        (PropertyDialog::GetComboBoxIndex(m_inGameAOCombo, m_userList) - 1), table);
      break;
    case IDC_ENABLE_SSR:
      CHECK_UPDATE_ITEM(table->m_useSSR,
                        (PropertyDialog::GetComboBoxIndex(m_ScreenReflectionCombo, m_userList) - 1),
                        table);
      break;
    case IDC_GLOBAL_ALPHA_ACC:
      CHECK_UPDATE_ITEM(table->m_overwriteGlobalDetailLevel,
                        PropertyDialog::GetCheckboxState(m_hOverwriteDetailsCheck), table);
      break;
    case IDC_BALL_REFLECTION:
      CHECK_UPDATE_ITEM(table->m_useReflectionForBalls,
                        (PropertyDialog::GetComboBoxIndex(m_ballReflectionCombo, m_userList) - 1),
                        table);
      break;
    case IDC_BALL_TRAIL:
      CHECK_UPDATE_ITEM(table->m_useTrailForBalls,
                        (PropertyDialog::GetComboBoxIndex(m_ballTrailCombo, m_userList) - 1),
                        table);
      break;
    case IDC_TRAIL_EDIT:
      CHECK_UPDATE_VALUE_SETTER(table->SetBallTrailStrength, table->GetBallTrailStrength,
                                PropertyDialog::GetIntTextbox, m_ballTrailStrengthEdit, table);
      break;
    case IDC_GLOBAL_DAYNIGHT:
      CHECK_UPDATE_ITEM(table->m_overwriteGlobalDayNight,
                        PropertyDialog::GetCheckboxState(m_hOverwriteNightDayCheck), table);
      break;
    case IDC_GAME_DIFFICULTY_EDIT:
      CHECK_UPDATE_VALUE_SETTER(table->SetGlobalDifficulty, table->GetGlobalDifficulty,
                                PropertyDialog::GetFloatTextbox, m_gameplayDifficultEdit, table);
      break;
    case IDC_OVERWRITE_PHYSICS_COMBO:
      CHECK_UPDATE_ITEM(
          table->m_overridePhysics,
          (PropertyDialog::GetComboBoxIndex(m_overwritePhysicsSetCombo, m_physicSetList)), table);
      break;
    case IDC_OVERRIDEPHYSICS_FLIPPERS:
      CHECK_UPDATE_ITEM(table->m_overridePhysicsFlipper,
                        PropertyDialog::GetCheckboxState(m_hOverwriteFlipperCheck), table);
      break;
    case IDC_TABLESOUNDVOLUME:
      CHECK_UPDATE_VALUE_SETTER(table->SetTableSoundVolume, table->GetTableSoundVolume,
                                PropertyDialog::GetIntTextbox, m_soundEffectVolEdit, table);
      break;
    case IDC_TABLEMUSICVOLUME:
      CHECK_UPDATE_VALUE_SETTER(table->SetTableMusicVolume, table->GetTableMusicVolume,
                                PropertyDialog::GetIntTextbox, m_musicVolEdit, table);
      break;
    case IDC_TABLEAVSYNC:
      CHECK_UPDATE_ITEM(table->m_TableAdaptiveVSync,
                        PropertyDialog::GetIntTextbox(m_fpsLimiterEdit), table);
      break;
