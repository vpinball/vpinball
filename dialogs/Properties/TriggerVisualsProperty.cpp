...
TriggerVisualsProperty::TriggerVisualsProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPTRIGGER_VISUALS, pvsel)
{
  ...
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    switch (dispid)
    {
      case 1503:
        CHECK_UPDATE_ITEM(
            trigger->m_d.m_shape,
            (TriggerShape)(PropertyDialog::GetComboBoxIndex(m_shapeCombo, m_shapeList)), trigger);
        break;
      case 902:
        CHECK_UPDATE_ITEM(trigger->m_d.m_vCenter.x, PropertyDialog::GetFloatTextbox(m_posXEdit),
                          trigger);
        break;
      case 903:
        CHECK_UPDATE_ITEM(trigger->m_d.m_vCenter.y, PropertyDialog::GetFloatTextbox(m_posYEdit),
                          trigger);
        break;
      ...
      case IDC_STAR_THICKNESS_EDIT:
        CHECK_UPDATE_ITEM(trigger->m_d.m_wireThickness,
                          PropertyDialog::GetFloatTextbox(m_wireThicknessEdit), trigger);
        break;
      case IDC_STAR_RADIUS_EDIT:
        CHECK_UPDATE_ITEM(trigger->m_d.m_radius, PropertyDialog::GetFloatTextbox(m_starRadiusEdit),
                          trigger);
        break;
      case IDC_ROTATION_EDIT:
        CHECK_UPDATE_ITEM(trigger->m_d.m_rotation, PropertyDialog::GetFloatTextbox(m_rotationEdit),
                          trigger);
        break;
      case IDC_RINGSPEED_EDIT:
        CHECK_UPDATE_ITEM(trigger->m_d.m_animSpeed,
                          PropertyDialog::GetFloatTextbox(m_animationSpeedEdit), trigger);
        break;
      default:
