...
SpinnerVisualsProperty::SpinnerVisualsProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPSPINNER_VISUALS, pvsel)
{
  ...
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    switch (dispid)
    {
      case IDC_SHOW_BRACKET_CHECK:
        CHECK_UPDATE_ITEM(spinner->m_d.m_showBracket,
                          PropertyDialog::GetCheckboxState(m_hShowBracketCheck), spinner);
        break;
      case IDC_SURFACE_COMBO:
        CHECK_UPDATE_COMBO_TEXT_STRING(spinner->m_d.m_szSurface, m_surfaceCombo, spinner);
        break;
      case 11:
        CHECK_UPDATE_ITEM(spinner->m_d.m_vCenter.x, PropertyDialog::GetFloatTextbox(m_posXEdit),
                          spinner);
        break;
      case 12:
        CHECK_UPDATE_ITEM(spinner->m_d.m_vCenter.y, PropertyDialog::GetFloatTextbox(m_posYEdit),
                          spinner);
        break;
      case DISPID_Spinner_Length:
        CHECK_UPDATE_ITEM(spinner->m_d.m_length, PropertyDialog::GetFloatTextbox(m_lengthEdit),
                          spinner);
        break;
      case 5:
        CHECK_UPDATE_ITEM(spinner->m_d.m_height, PropertyDialog::GetFloatTextbox(m_heightEdit),
                          spinner);
        break;
      case 4:
        CHECK_UPDATE_ITEM(spinner->m_d.m_rotation, PropertyDialog::GetFloatTextbox(m_rotationEdit),
                          spinner);
        break;
      case 13:
        CHECK_UPDATE_VALUE_SETTER(spinner->SetAngleMax, spinner->GetAngleMax,
                                  PropertyDialog::GetFloatTextbox, m_angleMaxEdit, spinner);
        break;
      case 14:
        CHECK_UPDATE_VALUE_SETTER(spinner->SetAngleMin, spinner->GetAngleMin,
                                  PropertyDialog::GetFloatTextbox, m_angleMinEdit, spinner);
        break;
      default:
...
BOOL SpinnerVisualsProperty::OnInitDialog()
{
  ...
  m_resizer.AddChild(m_hShowBracketCheck, topleft, 0);
  ...
  m_resizer.AddChild(m_angleMaxEdit, topleft, RD_STRETCH_WIDTH);
  m_resizer.AddChild(m_angleMinEdit, topleft, RD_STRETCH_WIDTH);
