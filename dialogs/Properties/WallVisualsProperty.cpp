...
WallVisualsProperty::WallVisualsProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPWALL_VISUALS, pvsel)
{
...
void WallVisualsProperty::UpdateVisuals(const int dispid /*=-1*/)
{
  ...
  {
    PropertyDialog::UpdateTextureComboBox(wall->GetPTable()->GetImageList(), m_sideImageCombo,
                                          wall->m_d.m_szSideImage);
  }
  if (dispid == IDC_MATERIAL_COMBO || dispid == -1)
  {
    PropertyDialog::UpdateMaterialComboBox(wall->GetPTable()->GetMaterialList(), m_topMaterialCombo,
                                           wall->m_d.m_szTopMaterial);
  }
  if (dispid == IDC_MATERIAL_COMBO2 || dispid == -1)
  {
    PropertyDialog::UpdateMaterialComboBox(wall->GetPTable()->GetMaterialList(),
                                           m_sideMaterialCombo, wall->m_d.m_szSideMaterial);
  }
  if (dispid == IDC_MATERIAL_COMBO3 || dispid == -1)
  {
    PropertyDialog::UpdateMaterialComboBox(wall->GetPTable()->GetMaterialList(),
                                           m_slingshotMaterialCombo,
                                           wall->m_d.m_szSlingShotMaterial);
  }
...
void WallVisualsProperty::UpdateProperties(const int dispid)
{
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    if ((m_pvsel->ElementAt(i) == nullptr) ||
        (m_pvsel->ElementAt(i)->GetItemType() != eItemSurface))
    {
      continue;
    }
    ...
    {
      case 9:
        CHECK_UPDATE_ITEM(wall->m_d.m_heighttop, PropertyDialog::GetFloatTextbox(m_topHeightEdit),
                          wall);
        break;
      case 8:
        CHECK_UPDATE_ITEM(wall->m_d.m_heightbottom,
                          PropertyDialog::GetFloatTextbox(m_bottomHeightEdit), wall);
        break;
      case DISPID_Image2:
      ...
      case IDC_MATERIAL_COMBO3:
        CHECK_UPDATE_COMBO_TEXT_STRING(wall->m_d.m_szSlingShotMaterial, m_slingshotMaterialCombo,
                                       wall);
        break;
      case IDC_BLEND_DISABLE_LIGHTING:
        CHECK_UPDATE_ITEM(wall->m_d.m_disableLightingTop,
                          PropertyDialog::GetFloatTextbox(m_disableLightingEdit), wall);
        break;
      case IDC_BLEND_DISABLE_LIGHTING_FROM_BELOW:
        CHECK_UPDATE_ITEM(wall->m_d.m_disableLightingBelow,
                          PropertyDialog::GetFloatTextbox(m_disableLightFromBelowEdit), wall);
        break;
      case 16:
        CHECK_UPDATE_ITEM(wall->m_d.m_topBottomVisible,
                          PropertyDialog::GetCheckboxState(::GetDlgItem(GetHwnd(), dispid)), wall);
        break;
      case 13:
        CHECK_UPDATE_ITEM(wall->m_d.m_displayTexture,
                          PropertyDialog::GetCheckboxState(::GetDlgItem(GetHwnd(), dispid)), wall);
        break;
      case 109:
        CHECK_UPDATE_ITEM(wall->m_d.m_sideVisible,
                          PropertyDialog::GetCheckboxState(::GetDlgItem(GetHwnd(), dispid)), wall);
        break;
      case 112:
        CHECK_UPDATE_ITEM(wall->m_d.m_slingshotAnimation,
                          PropertyDialog::GetCheckboxState(::GetDlgItem(GetHwnd(), dispid)), wall);
        break;
      case 113:
        CHECK_UPDATE_ITEM(wall->m_d.m_flipbook,
                          PropertyDialog::GetCheckboxState(::GetDlgItem(GetHwnd(), dispid)), wall);
        break;
      default:
