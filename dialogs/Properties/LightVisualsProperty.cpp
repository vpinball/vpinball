...
LightVisualsProperty::LightVisualsProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPLIGHT_VISUALS, pvsel)
{
...
void LightVisualsProperty::UpdateProperties(const int dispid)
{
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    {
      case 1:
        CHECK_UPDATE_ITEM(light->m_d.m_falloff, PropertyDialog::GetFloatTextbox(m_falloffEdit),
                          light);
        break;
      case IDC_LIGHT_FALLOFF_POWER:
        CHECK_UPDATE_ITEM(light->m_d.m_falloff_power,
                          PropertyDialog::GetFloatTextbox(m_falloffPowerEdit), light);
        break;
      case 12:
        CHECK_UPDATE_ITEM(light->m_d.m_intensity, PropertyDialog::GetFloatTextbox(m_intensityEdit),
                          light);
        break;
      case IDC_FADE_SPEED_UP:
        CHECK_UPDATE_ITEM(light->m_d.m_fadeSpeedUp,
                          PropertyDialog::GetFloatTextbox(m_fadeSpeedUpEdit), light);
        break;
      case IDC_FADE_SPEED_DOWN:
        CHECK_UPDATE_ITEM(light->m_d.m_fadeSpeedDown,
                          PropertyDialog::GetFloatTextbox(m_fadeSpeedDownEdit), light);
        break;
      case IDC_IMAGE_MODE:
        CHECK_UPDATE_ITEM(light->m_d.m_imageMode,
                          PropertyDialog::GetCheckboxState(m_hPassThroughCheck), light);
        break;
      case IDC_DEPTH_BIAS:
        CHECK_UPDATE_ITEM(light->m_d.m_depthBias, PropertyDialog::GetFloatTextbox(m_depthBiasEdit),
                          light);
        break;
      case IDC_LIGHT_TYPE_CHECKBOX:
        CHECK_UPDATE_ITEM(light->m_d.m_BulbLight, PropertyDialog::GetCheckboxState(m_hEnableCheck),
                          light);
        break;
      case IDC_SHOW_BULB_MESH:
        CHECK_UPDATE_ITEM(light->m_d.m_showBulbMesh,
                          PropertyDialog::GetCheckboxState(m_hShowMeshCheck), light);
        break;
      case IDC_STATIC_BULB_MESH:
        CHECK_UPDATE_ITEM(light->m_d.m_staticBulbMesh,
                          PropertyDialog::GetCheckboxState(m_hStaticMeshCheck), light);
        break;
      case IDC_REFLECT_ON_BALLS:
        CHECK_UPDATE_ITEM(light->m_d.m_showReflectionOnBall,
                          PropertyDialog::GetCheckboxState(m_hRelectOnBalls), light);
        break;
      case IDC_HALO_EDIT:
        CHECK_UPDATE_ITEM(light->m_d.m_bulbHaloHeight,
                          PropertyDialog::GetFloatTextbox(m_haloHeightEdit), light);
        break;
      case IDC_SCALE_BULB_MESH:
        CHECK_UPDATE_ITEM(light->m_d.m_meshRadius, PropertyDialog::GetFloatTextbox(m_scaleMeshEdit),
                          light);
        break;
      case IDC_BULB_MODULATE_VS_ADD:
        CHECK_UPDATE_ITEM(light->m_d.m_modulate_vs_add,
                          PropertyDialog::GetFloatTextbox(m_modulateEdit), light);
        break;
      case IDC_TRANSMISSION_SCALE:
        CHECK_UPDATE_ITEM(light->m_d.m_transmissionScale,
                          PropertyDialog::GetFloatTextbox(m_transmitEdit), light);
        break;
      case 902:
        CHECK_UPDATE_ITEM(light->m_d.m_vCenter.x, PropertyDialog::GetFloatTextbox(m_posXEdit),
                          light);
        break;
      case 903:
        CHECK_UPDATE_ITEM(light->m_d.m_vCenter.y, PropertyDialog::GetFloatTextbox(m_posYEdit),
                          light);
        break;
