...
TextboxVisualsProperty::TextboxVisualsProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPTEXTBOX_VISUALS, pvsel)
{
  ...
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    switch (dispid)
    {
      case IDC_TEXTBOX_TRANSP_CHECK:
        CHECK_UPDATE_ITEM(text->m_d.m_transparent,
                          PropertyDialog::GetCheckboxState(m_hTransparentCheck), text);
        break;
      case IDC_TEXTBOX_ALIGN_COMBO:
        CHECK_UPDATE_ITEM(
            text->m_d.m_talign,
            (TextAlignment)PropertyDialog::GetComboBoxIndex(m_alignmentCombo, m_alignList), text);
        break;
      case IDC_USE_SCRIPT_DMD_CHECK:
        CHECK_UPDATE_ITEM(text->m_d.m_isDMD, PropertyDialog::GetCheckboxState(m_hUseScriptDMDCheck),
                          text);
        break;
      case IDC_TEXT_INTENSITY:
        CHECK_UPDATE_ITEM(text->m_d.m_intensity_scale,
                          PropertyDialog::GetFloatTextbox(m_textIntensityEdit), text);
        break;
