...
PrimitiveVisualsProperty::PrimitiveVisualsProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPPRIMITIVE_VISUALS, pvsel)
{
...
void PrimitiveVisualsProperty::UpdateProperties(const int dispid)
{
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    {
      case IDC_ADDBLEND:
        CHECK_UPDATE_ITEM(prim->m_d.m_addBlend,
                          PropertyDialog::GetCheckboxState(m_hAdditiveBlendCheck), prim);
        break;
      case IDC_DISPLAY_TEXTURE_CHECKBOX:
        CHECK_UPDATE_ITEM(prim->m_d.m_displayTexture,
                          PropertyDialog::GetCheckboxState(m_hDisplayImageCheck), prim);
        break;
      case IDC_OBJECT_SPACE_NORMALMAP:
        CHECK_UPDATE_ITEM(prim->m_d.m_objectSpaceNormalMap,
                          PropertyDialog::GetCheckboxState(m_hObjectSpaceCheck), prim);
        break;
      case IDC_PRIMITIVE_ENABLE_BACKFACES:
        CHECK_UPDATE_ITEM(prim->m_d.m_backfacesEnabled,
                          PropertyDialog::GetCheckboxState(m_hRenderBackfacingCheck), prim);
        break;
      case IDC_STATIC_RENDERING_CHECK:
        CHECK_UPDATE_ITEM(prim->m_d.m_staticRendering,
                          PropertyDialog::GetCheckboxState(m_hStaticRenderingCheck), prim);
        break;
      case IDC_DRAW_TEXTURES_SIDES_CHECK:
        CHECK_UPDATE_ITEM(prim->m_d.m_drawTexturesInside,
                          PropertyDialog::GetCheckboxState(m_hDrawTexturesInsideCheck), prim);
        break;
      ...
      case IDC_EXPORT_MESH_BUTTON:
        ...
        break;
      case IDC_ALPHA_EDIT:
        CHECK_UPDATE_VALUE_SETTER(prim->SetAlpha, prim->GetAlpha, PropertyDialog::GetFloatTextbox,
                                  m_opacityAmountEdit, prim);
        break;
      case IDC_DEPTH_BIAS:
        CHECK_UPDATE_ITEM(prim->m_d.m_depthBias, PropertyDialog::GetFloatTextbox(m_depthBiasEdit),
                          prim);
        break;
      case IDC_BLEND_DISABLE_LIGHTING:
        CHECK_UPDATE_ITEM(prim->m_d.m_disableLightingTop,
                          PropertyDialog::GetFloatTextbox(m_disableLightingEdit), prim);
        break;
      case IDC_BLEND_DISABLE_LIGHTING_FROM_BELOW:
        CHECK_UPDATE_ITEM(prim->m_d.m_disableLightingBelow,
                          PropertyDialog::GetFloatTextbox(m_disableLightFromBelowEdit), prim);
        break;
      case IDC_PRIMITIVE_LEGACY_SIDES_EDIT:
        CHECK_UPDATE_ITEM(prim->m_d.m_Sides, PropertyDialog::GetIntTextbox(m_legacySidesEdit),
                          prim);
        break;
      case IDC_EDGE_FACTOR_UI:
        CHECK_UPDATE_ITEM(prim->m_d.m_edgeFactorUI,
                          PropertyDialog::GetFloatTextbox(m_edgeFactorUIEdit), prim);
        break;
      case DISPID_Image2:
