...
BumperPhysicsProperty::BumperPhysicsProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPBUMPER_PHYSICS, pvsel)
{
...
void BumperPhysicsProperty::UpdateProperties(const int dispid)
{
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    {
      case IDC_FORCE_EDIT:
        CHECK_UPDATE_ITEM(bumper->m_d.m_force, PropertyDialog::GetFloatTextbox(m_forceEdit),
                          bumper);
        break;
      default:
