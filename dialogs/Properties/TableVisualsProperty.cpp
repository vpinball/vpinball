...
void TableVisualsProperty::UpdateProperties(const int dispid)
{
  ...
  switch (dispid)
  {
    ...
    case IDC_REFLECT_ELEMENTS_CHECK:
      CHECK_UPDATE_ITEM(table->m_reflectElementsOnPlayfield,
                        PropertyDialog::GetCheckboxState(m_hReflectElementsCheck), table);
      break;
    case IDC_REFLECTION_PLAYFIELD:
      CHECK_UPDATE_VALUE_SETTER(table->SetPlayfieldReflectionStrength,
                                table->GetPlayfieldReflectionStrength,
                                PropertyDialog::GetIntTextbox, m_reflectionStrengthEdit, table);
      break;
    case IDC_BALL_DECAL_MODE:
      CHECK_UPDATE_ITEM(table->m_BallDecalMode, PropertyDialog::GetCheckboxState(m_hLogoModeCheck),
                        table);
      break;
    case 1505:
    ...
    case IDC_BALLPLAYFIELD_REFLECTION:
      CHECK_UPDATE_ITEM(table->m_ballPlayfieldReflectionStrength,
                        PropertyDialog::GetFloatTextbox(m_ballReflectPlayfieldEdit), table);
      break;
    case IDC_BULBINTENSITYSCALE:
      CHECK_UPDATE_ITEM(table->m_defaultBulbIntensityScaleOnBall,
                        PropertyDialog::GetFloatTextbox(m_ballDefaultBulbIntensScaleEdit), table);
      break;
    default:
      break;
  }
