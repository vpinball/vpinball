...
GateVisualsProperty::GateVisualsProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPGATE_VISUALS, pvsel)
{
...
void GateVisualsProperty::UpdateProperties(const int dispid)
{
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    {
      case 9:
        CHECK_UPDATE_ITEM(gate->m_d.m_type,
                          (GateType)(PropertyDialog::GetComboBoxIndex(m_typeCombo, m_typeList) + 1),
                          gate);
        break;
      ...
      case 6:
        ...
        break;
      case 15:
        CHECK_UPDATE_ITEM(gate->m_d.m_showBracket,
                          PropertyDialog::GetCheckboxState(::GetDlgItem(GetHwnd(), dispid)), gate);
        break;
      ...
      case DISPID_Gate_Rotation:
        CHECK_UPDATE_ITEM(gate->m_d.m_rotation, PropertyDialog::GetFloatTextbox(m_rotationEdit),
                          gate);
        break;
      case 2145:
        CHECK_UPDATE_VALUE_SETTER(gate->SetOpenAngle, gate->GetOpenAngle,
                                  PropertyDialog::GetFloatTextbox, m_openAngleEdit, gate);
        break;
      case 2144:
        CHECK_UPDATE_VALUE_SETTER(gate->SetCloseAngle, gate->GetCloseAngle,
                                  PropertyDialog::GetFloatTextbox, m_closeAngleEdit, gate);
        break;
      default:
