...
HitTargetPhysicsProperty::HitTargetPhysicsProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPHITTARGET_PHYSICS, pvsel)
{
  ...
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    switch (dispid)
    {
      case IDC_TARGET_ISDROPPED_CHECK:
        CHECK_UPDATE_ITEM(target->m_d.m_isDropped,
                          PropertyDialog::GetCheckboxState(m_hIsDroppedCheck), target);
        break;
      case IDC_TARGET_LEGACY_MODE_CHECK:
        CHECK_UPDATE_ITEM(target->m_d.m_legacy,
                          PropertyDialog::GetCheckboxState(m_hLegacyModeCheck), target);
        break;
      case 112:
        CHECK_UPDATE_ITEM(target->m_d.m_elasticityFalloff,
                          PropertyDialog::GetFloatTextbox(m_elasticityFalloffEdit), target);
        break;
      default:
