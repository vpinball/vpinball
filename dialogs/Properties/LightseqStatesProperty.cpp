...
LightseqStatesProperty::LightseqStatesProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPLIGHTSEQ_STATE, pvsel)
{
...
void LightseqStatesProperty::UpdateVisuals(const int dispid /*=-1*/)
{
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    {
      char szT[MAXSTRING];
      WideCharToMultiByteNull(CP_ACP, 0, lightseq->m_d.m_wzCollection.c_str(), -1, szT, sizeof(szT),
                              nullptr, nullptr);
...
void LightseqStatesProperty::UpdateProperties(const int dispid)
{
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    {
      case 9:
        CHECK_UPDATE_VALUE_SETTER(lightseq->SetX, lightseq->GetX, PropertyDialog::GetFloatTextbox,
                                  m_posXEdit, lightseq);
        break;
      case 10:
        CHECK_UPDATE_VALUE_SETTER(lightseq->SetY, lightseq->GetY, PropertyDialog::GetFloatTextbox,
                                  m_posYEdit, lightseq);
        break;
      case IDC_LIGHTSEQ_UPDATE_INTERVAL_EDIT:
        CHECK_UPDATE_VALUE_SETTER(lightseq->SetUpdateInterval, lightseq->GetUpdateInterval,
                                  PropertyDialog::GetIntTextbox, m_updateIntervalEdit, lightseq);
        break;
