...
DispreelStateProperty::DispreelStateProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPDISPREEL_STATE, pvsel)
{
  ...
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    switch (dispid)
    {
      case IDC_MOTOR_STEPS_EDIT:
        CHECK_UPDATE_VALUE_SETTER(reel->SetMotorSteps, reel->GetMotorSteps,
                                  PropertyDialog::GetIntTextbox, m_motorStepsEdit, reel);
        break;
      case IDC_UPDATE_INTERVAL_EDIT:
        CHECK_UPDATE_VALUE_SETTER(reel->SetUpdateInterval, reel->GetUpdateInterval,
                                  PropertyDialog::GetIntTextbox, m_updateIntervalEdit, reel);
        break;
      case DISPID_Sound:
