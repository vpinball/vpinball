...
LightStatesProperty::LightStatesProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPLIGHT_STATE, pvsel)
{
...
void LightStatesProperty::UpdateVisuals(const int dispid /*=-1*/)
{
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    {
      PropertyDialog::UpdateComboBox(m_stateList, m_stateCombo,
                                     m_stateList[(int)light->getLightState()]);
    }
  ...
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    switch (dispid)
    {
      case DISPID_Light_State:
        CHECK_UPDATE_COMBO_VALUE_SETTER(
            light->setLightState, light->getLightState,
            (LightState)(PropertyDialog::GetComboBoxIndex(m_stateCombo, m_stateList)), light);
        break;
      ...
      case DISPID_Light_BlinkInterval:
        CHECK_UPDATE_ITEM(light->m_blinkinterval,
                          PropertyDialog::GetIntTextbox(m_blinkIntervalEdit), light);
        break;
      default:
        break;
    }
