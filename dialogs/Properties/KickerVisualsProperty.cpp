...
KickerVisualsProperty::KickerVisualsProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPKICKER_VISUALS, pvsel)
{
...
void KickerVisualsProperty::UpdateVisuals(const int dispid /*=-1*/)
{
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    if (dispid == IDC_KICKER_DISPLAY_COMBO || dispid == -1)
      PropertyDialog::UpdateComboBox(m_typeList, m_displayCombo,
                                     m_typeList[kicker->m_d.m_kickertype]);
    ...
    if (dispid == IDC_SURFACE_COMBO || dispid == -1)
      PropertyDialog::UpdateSurfaceComboBox(kicker->GetPTable(), m_surfaceCombo,
                                            kicker->m_d.m_szSurface);
  ...
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    switch (dispid)
    {
      case IDC_KICKER_DISPLAY_COMBO:
        ...
        kicker->m_d.m_kickertype =
            (KickerType)(PropertyDialog::GetComboBoxIndex(m_displayCombo, m_typeList));
        PropertyDialog::EndUndo(kicker);
        CHECK_UPDATE_ITEM(
            kicker->m_d.m_kickertype,
            (KickerType)(PropertyDialog::GetComboBoxIndex(m_displayCombo, m_typeList)), kicker);
        break;
      case IDC_KICKER_RADIUS_EDIT:
        CHECK_UPDATE_ITEM(kicker->m_d.m_radius, PropertyDialog::GetFloatTextbox(m_radiusEdit),
                          kicker);
        break;
      case IDC_KICKER_ORIENTATION_EDIT:
        CHECK_UPDATE_ITEM(kicker->m_d.m_orientation,
                          PropertyDialog::GetFloatTextbox(m_orientationEdit), kicker);
        break;
      case 902:
        CHECK_UPDATE_ITEM(kicker->m_d.m_vCenter.x, PropertyDialog::GetFloatTextbox(m_posXEdit),
                          kicker);
        break;
      case 903:
        CHECK_UPDATE_ITEM(kicker->m_d.m_vCenter.y, PropertyDialog::GetFloatTextbox(m_posYEdit),
                          kicker);
        break;
