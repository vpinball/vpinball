...
TriggerPhysicsProperty::TriggerPhysicsProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPTRIGGER_STATE, pvsel)
{
...
void TriggerPhysicsProperty::UpdateProperties(const int dispid)
{
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    {
      case 904:
        CHECK_UPDATE_ITEM(trigger->m_d.m_enabled, PropertyDialog::GetCheckboxState(m_hEnableCheck),
                          trigger);
        break;
      case IDC_HIT_HEIGHT_EDIT:
        CHECK_UPDATE_ITEM(trigger->m_d.m_hit_height,
                          PropertyDialog::GetFloatTextbox(m_hitHeightEdit), trigger);
        break;
      default:
        break;
    }
