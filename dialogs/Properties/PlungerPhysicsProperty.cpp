...
PlungerPhysicsProperty::PlungerPhysicsProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPPLUNGER_PHYSICS, pvsel)
{
  ...
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    switch (dispid)
    {
      case IDC_PULL_SPEED_EDIT:
        CHECK_UPDATE_ITEM(plunger->m_d.m_speedPull,
                          PropertyDialog::GetFloatTextbox(m_pullSpeedEdit), plunger);
        break;
      case IDC_RELEASE_SPEED_EDIT:
        CHECK_UPDATE_ITEM(plunger->m_d.m_speedFire,
                          PropertyDialog::GetFloatTextbox(m_releaseSpeedEdit), plunger);
        break;
      case IDC_STROKE_LENGTH_EDIT:
        CHECK_UPDATE_ITEM(plunger->m_d.m_stroke,
                          PropertyDialog::GetFloatTextbox(m_strokeLengthEdit), plunger);
        break;
      case IDC_SCATTER_VELOCITY_EDIT:
        CHECK_UPDATE_ITEM(plunger->m_d.m_scatterVelocity,
                          PropertyDialog::GetFloatTextbox(m_scatterVelocityEdit), plunger);
        break;
      case IDC_MECH_STRENGTH_EDIT:
        CHECK_UPDATE_ITEM(plunger->m_d.m_mechStrength,
                          PropertyDialog::GetFloatTextbox(m_mechStrengthEdit), plunger);
        break;
      case IDC_MOMENTUM_XFER_EDIT:
        CHECK_UPDATE_ITEM(plunger->m_d.m_momentumXfer,
                          PropertyDialog::GetFloatTextbox(m_momentumXferEdit), plunger);
        break;
      case IDC_PARK_POSITION_EDIT:
        CHECK_UPDATE_ITEM(plunger->m_d.m_parkPosition,
                          PropertyDialog::GetFloatTextbox(m_parkPositionEdit), plunger);
        break;
      case IDC_AUTO_PLUNGER_CHECK:
        CHECK_UPDATE_ITEM(plunger->m_d.m_autoPlunger,
                          PropertyDialog::GetCheckboxState(m_hAutoPlungerCheck), plunger);
        break;
      case IDC_ENABLE_MECH_PLUNGER_CHECK:
        CHECK_UPDATE_ITEM(plunger->m_d.m_mechPlunger,
                          PropertyDialog::GetCheckboxState(m_hEnableMechPlungerCheck), plunger);
        break;
      default:
