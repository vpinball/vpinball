...
DecalVisualsProperty::DecalVisualsProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPDECAL_VISUALS, pvsel), m_font(nullptr)
{
  ...
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    switch (dispid)
    {
      case IDC_FONT_TYPE_COMBO:
        CHECK_UPDATE_ITEM(decal->m_d.m_decaltype,
                          (DecalType)PropertyDialog::GetComboBoxIndex(m_typeCombo, m_typeList),
                          decal);
        break;
      ...
      case IDC_DECAL_VERTICAL_TEXT_CHECK:
        CHECK_UPDATE_ITEM(decal->m_d.m_verticalText,
                          PropertyDialog::GetCheckboxState(m_hVerticalTextCheck), decal);
        break;
      ...
      case DISPID_Decal_SizingType:
        CHECK_UPDATE_ITEM(decal->m_d.m_sizingtype,
                          (SizingType)PropertyDialog::GetComboBoxIndex(m_sizingCombo, m_sizingList),
                          decal);
        break;
      case 5:
        CHECK_UPDATE_ITEM(decal->m_d.m_vCenter.x, PropertyDialog::GetFloatTextbox(m_posXEdit),
                          decal);
        break;
      case 6:
        CHECK_UPDATE_ITEM(decal->m_d.m_vCenter.y, PropertyDialog::GetFloatTextbox(m_posYEdit),
                          decal);
        break;
      case 3:
        ...
        break;
      case 4:
        CHECK_UPDATE_ITEM(decal->m_d.m_height, PropertyDialog::GetFloatTextbox(m_heigthEdit),
                          decal);
        break;
      case 1:
        CHECK_UPDATE_ITEM(decal->m_d.m_rotation, PropertyDialog::GetFloatTextbox(m_rotationEdit),
                          decal);
        break;
