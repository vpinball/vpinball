...
BumperVisualsProperty::BumperVisualsProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPBUMPER_VISUALS, pvsel)
{
  ...
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    switch (dispid)
    {
      ...
      case IDC_MATERIAL_COMBO2:
        CHECK_UPDATE_COMBO_TEXT_STRING(bumper->m_d.m_szBaseMaterial, m_bumpBaseMaterialCombo,
                                       bumper);
        break;
      ...
      case IDC_BUMPER_RADIUS_EDIT:
        CHECK_UPDATE_ITEM(bumper->m_d.m_radius, PropertyDialog::GetFloatTextbox(m_radiusEdit),
                          bumper);
        break;
      case IDC_BUMPER_HEIGHT_SCALE_EDIT:
        CHECK_UPDATE_ITEM(bumper->m_d.m_heightScale,
                          PropertyDialog::GetFloatTextbox(m_heightScaleEdit), bumper);
        break;
      case IDC_ORIENTATION_EDIT:
        CHECK_UPDATE_ITEM(bumper->m_d.m_orientation,
                          PropertyDialog::GetFloatTextbox(m_orientationEdit), bumper);
        break;
      case IDC_RINGSPEED_EDIT:
        CHECK_UPDATE_ITEM(bumper->m_d.m_ringSpeed, PropertyDialog::GetFloatTextbox(m_ringSpeedEdit),
                          bumper);
        break;
      case IDC_RINGDROPOFFSET_EDIT:
        CHECK_UPDATE_ITEM(bumper->m_d.m_ringDropOffset,
                          PropertyDialog::GetFloatTextbox(m_ringDropOffsetEdit), bumper);
        break;
      case IDC_CAP_VISIBLE_CHECK:
        CHECK_UPDATE_ITEM(bumper->m_d.m_capVisible,
                          PropertyDialog::GetCheckboxState(m_hCapVisibleCheck), bumper);
        break;
      case IDC_BASE_VISIBLE_CHECK:
        CHECK_UPDATE_ITEM(bumper->m_d.m_baseVisible,
                          PropertyDialog::GetCheckboxState(m_hBaseVisibleCheck), bumper);
        break;
      case IDC_RING_VISIBLE:
        CHECK_UPDATE_ITEM(bumper->m_d.m_ringVisible,
                          PropertyDialog::GetCheckboxState(m_hRingVisibleCheck), bumper);
        break;
      case IDC_SKIRT_VISIBLE:
        CHECK_UPDATE_ITEM(bumper->m_d.m_skirtVisible,
                          PropertyDialog::GetCheckboxState(m_hSkirtVisibleCheck), bumper);
        break;
      case 902:
        CHECK_UPDATE_ITEM(bumper->m_d.m_vCenter.x, PropertyDialog::GetFloatTextbox(m_posXEdit),
                          bumper);
        break;
      case 903:
        CHECK_UPDATE_ITEM(bumper->m_d.m_vCenter.y, PropertyDialog::GetFloatTextbox(m_posYEdit),
                          bumper);
        break;
...
BOOL BumperVisualsProperty::OnInitDialog()
{
  ...
  m_resizer.AddChild(m_capMaterialCombo, topleft, RD_STRETCH_WIDTH);
  m_resizer.AddChild(m_bumpBaseMaterialCombo, topleft, RD_STRETCH_WIDTH);
  m_resizer.AddChild(m_skirtMaterialCombo, topleft, RD_STRETCH_WIDTH);
  m_resizer.AddChild(m_ringMaterialCombo, topleft, RD_STRETCH_WIDTH);
  m_resizer.AddChild(m_radiusEdit, topleft, RD_STRETCH_WIDTH);
  m_resizer.AddChild(m_heightScaleEdit, topleft, RD_STRETCH_WIDTH);
  m_resizer.AddChild(m_orientationEdit, topleft, RD_STRETCH_WIDTH);
  m_resizer.AddChild(m_ringSpeedEdit, topleft, RD_STRETCH_WIDTH);
  m_resizer.AddChild(m_ringDropOffsetEdit, topleft, RD_STRETCH_WIDTH);
  m_resizer.AddChild(m_hCapVisibleCheck, topleft, 0);
  m_resizer.AddChild(m_hBaseVisibleCheck, topleft, 0);
  m_resizer.AddChild(m_hRingVisibleCheck, topleft, 0);
  m_resizer.AddChild(m_hSkirtVisibleCheck, topleft, 0);
