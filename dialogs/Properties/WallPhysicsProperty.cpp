...
WallPhysicsProperty::WallPhysicsProperty(const VectorProtected<ISelect>* pvsel)
  : BasePropertyDialog(IDD_PROPWALL_PHYSICS, pvsel)
{
...
void WallPhysicsProperty::UpdateProperties(const int dispid)
{
  for (int i = 0; i < m_pvsel->size(); i++)
  {
    ...
    {
      case 11:
        CHECK_UPDATE_ITEM(wall->m_d.m_droppable,
                          PropertyDialog::GetCheckboxState(::GetDlgItem(GetHwnd(), dispid)), wall);
        break;
      case 14:
        CHECK_UPDATE_VALUE_SETTER(wall->SetSlingshotStrength, wall->GetSlingshotStrength,
                                  PropertyDialog::GetFloatTextbox, m_slingshotForceEdit, wall);
        break;
      case 427:
        CHECK_UPDATE_ITEM(wall->m_d.m_slingshot_threshold,
                          PropertyDialog::GetFloatTextbox(m_slingshotThresholdEdit), wall);
        break;
      case 116:
        PropertyDialog::StartUndo(wall);
        wall->m_d.m_isBottomSolid =
            PropertyDialog::GetCheckboxState(::GetDlgItem(GetHwnd(), dispid));
        PropertyDialog::EndUndo(wall);
        CHECK_UPDATE_ITEM(wall->m_d.m_isBottomSolid,
                          PropertyDialog::GetCheckboxState(::GetDlgItem(GetHwnd(), dispid)), wall);
        break;
      case 120:
        CHECK_UPDATE_ITEM(wall->m_d.m_elasticityFalloff,
                          PropertyDialog::GetFloatTextbox(m_elasticityFallOffEdit), wall);
        break;
      default:
