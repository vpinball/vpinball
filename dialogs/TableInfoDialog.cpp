...
*/
  ...
  sprintf_s(buffer, "%s Revision %u",
            !pt->m_szDateSaved.empty() ? pt->m_szDateSaved.c_str() : "N.A.", pt->m_numTimesSaved);
...
BOOL TableInfoDialog::OnCommand(WPARAM wParam, LPARAM lParam)
{
  ...
  switch (LOWORD(wParam))
  {
    ...
    case IDC_SENDMAIL:
    {
      const string url = string("mailto:") + string(m_emailEdit.GetWindowText()) +
                         string("?subject=") + string(m_tableNameEdit.GetWindowText());
      OpenURL(url);
      break;
    }
    default:
