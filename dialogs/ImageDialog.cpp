    ...
    case WM_DRAWITEM:
    {
      ...
      if (sel != -1)
      {
        ...
        if (ppi != nullptr)
        {
          ...
          StretchBlt(pdis->hDC, x, y, width, height, hdcDD, 0, 0, ppi->m_width, ppi->m_height,
                     SRCCOPY);
      ...
      else
      {
        ...
        Rectangle(pdis->hDC, rcClient.left + 5, rcClient.top + 5, rcClient.right - 5,
                  rcClient.bottom - 5);
...
void ImageDialog::Import()
{
  ...
  if (g_pvp->OpenFileDialog(szInitialDir, szFileName,
                            ...
                            "png", OFN_EXPLORER | OFN_ALLOWMULTISELECT))
  {
    CCO(PinTable)* const pt = g_pvp->GetActiveTable();
    const HWND hSoundList = GetDlgItem(IDC_SOUNDLIST).GetHwnd();
  ...
  if (selectedItemsCount) // if some items are selected???
  {
    ...
    if (sel != -1)
    {
      ...
      if (ppi != nullptr)
      {
        ...
        const size_t renameOnExport =
            SendMessage(GetDlgItem(IDC_CHECK_RENAME_ON_EXPORT).GetHwnd(), BM_GETCHECK, 0, 0);
        ...
        {
          strncat_s(g_filename, ppi->m_szName.c_str(),
                    sizeof(g_filename) - strnlen_s(g_filename, sizeof(g_filename)) - 1);
          const size_t idx = ppi->m_szPath.find_last_of('.');
          strncat_s(g_filename, ppi->m_szPath.c_str() + idx,
                    sizeof(g_filename) - strnlen_s(g_filename, sizeof(g_filename)) - 1);
        }
        ...
        ofn.lpstrFilter =
            "PNG (.png)\0*.png;\0Bitmap (.bmp)\0*.bmp;\0JPEG (.jpg/.jpeg)\0*.jpg;*.jpeg;\0IFF "
            "(.iff)\0*.IFF;\0PCX (.pcx)\0*.PCX;\0PICT (.pict)\0*.PICT;\0Photoshop "
          ...
          while (sel != -1 && ppi != nullptr)
          {
            if (selectedItemsCount > 1)
            {
              ...
              if (!renameOnExport)
              {
                ...
                strncat_s(g_filename, ppi->m_szPath.c_str() + begin,
                          sizeof(g_filename) - strnlen_s(g_filename, sizeof(g_filename)) - 1);
              }
              else
              {
...
void ImageDialog::DeleteImage()
{
  ...
  if (count > 0)
  {
    ...
    const int ans = MessageBox(ls.m_szbuffer /*"Are you sure you want to remove this image?"*/,
...
void ImageDialog::Reimport()
{
  ...
  if (count > 0)
  {
    ...
    const int ans = MessageBox(ls.m_szbuffer /*"Are you sure you want to replace this image?"*/,
                               "Confirm Reimport", MB_YESNO | MB_DEFBUTTON2);
    if (ans == IDYES)
    {
      int sel = ListView_GetNextItem(hSoundList, -1, LVNI_SELECTED);
      while (sel != -1)
...
void ImageDialog::UpdateAll()
{
  ...
  for (int sel = 0; sel < count; sel++)
  {
    ...
    if (ppi != nullptr)
    {
      const HANDLE hFile = CreateFile(ppi->m_szPath.c_str(), GENERIC_READ, FILE_SHARE_READ, nullptr,
...
void ImageDialog::ReimportFrom()
{
  ...
  if (sel != -1)
  {
    const LocalString ls(IDS_REPLACEIMAGE);
    const int ans =
        MessageBox(ls.m_szbuffer /*"Are you sure you want to replace this image with a new one?"*/,
                   "Confirm Reimport", MB_YESNO | MB_DEFBUTTON2);
    if (ans == IDYES)
    {
      std::vector<std::string> szFileName;
