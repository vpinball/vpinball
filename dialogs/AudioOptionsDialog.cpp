...
BOOL AudioOptionsDialog::OnInitDialog()
{
  ...
  SendMessage(GetDlgItem(IDC_PLAY_MUSIC).GetHwnd(), BM_SETCHECK,
              fsound ? BST_CHECKED : BST_UNCHECKED, 0);
  if (!fsound)
  {
  ...
  SendMessage(GetDlgItem(IDC_PLAY_SOUND).GetHwnd(), BM_SETCHECK,
              fsound ? BST_CHECKED : BST_UNCHECKED, 0);
  if (!fsound)
  {
  ...
  switch (uMsg)
  {
    case GET_SOUNDDEVICES:
    {
      ...
      if (!FAILED(DirectSoundEnumerate(DSEnumCallBack, &DSads)))
      {
        ...
        {
          const size_t index =
              SendMessage(hwndList, LB_ADDSTRING, 0, (size_t)DSads[i]->description.c_str());
          SendMessage(hwndList, LB_SETITEMDATA, index, (LPARAM)i);
          const size_t indexbg =
              SendMessage(hwndListBG, LB_ADDSTRING, 0, (size_t)DSads[i]->description.c_str());
