...
INT_PTR AboutDialog::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  switch (uMsg)
  {
    case WM_INITDIALOG:
    {
      ...
      sprintf_s(versionString, "Version %i.%i.%i Beta (Revision %i (%s), %ubit)", VP_VERSION_MAJOR,
                VP_VERSION_MINOR, VP_VERSION_REV, GIT_REVISION, GIT_SHA,
#ifdef _WIN64
                64u
#else
                32u
...

      {
        ...
        HFONT hFont = CreateFont(14, 0, 0, 0, FW_DONTCARE, FALSE, FALSE, FALSE, DEFAULT_CHARSET,
                                 OUT_OUTLINE_PRECIS, CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY,
                                 VARIABLE_PITCH, TEXT("Courier New"));

        ::SendMessage(::GetDlgItem(hwndDlg, IDC_CHANGELOG), WM_SETFONT, (WPARAM)hFont,
