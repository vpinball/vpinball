...
void LayersListDialog::AddToolTip(const char* const text,
                                  HWND parentHwnd,
                                  HWND toolTipHwnd,
                                  HWND controlHwnd)
{
...
BOOL LayersListDialog::OnInitDialog()
{
  ...
  HANDLE hIcon = ::LoadImage(GetModuleHandle(nullptr), MAKEINTRESOURCE(IDI_ASSIGN), IMAGE_ICON,
                             iconSize, iconSize, LR_DEFAULTCOLOR);
  m_assignButton.SetIcon((HICON)hIcon);
  hIcon = ::LoadImage(GetModuleHandle(nullptr), MAKEINTRESOURCE(IDI_ADD), IMAGE_ICON, iconSize,
                      iconSize, LR_DEFAULTCOLOR);
  m_addLayerButton.SetIcon((HICON)hIcon);
  hIcon = ::LoadImage(GetModuleHandle(nullptr), MAKEINTRESOURCE(IDI_REMOVE), IMAGE_ICON, iconSize,
                      iconSize, LR_DEFAULTCOLOR);
  m_deleteLayerButton.SetIcon((HICON)hIcon);
  hIcon = ::LoadImage(GetModuleHandle(nullptr), MAKEINTRESOURCE(IDI_EXPANDCOLLAPSE), IMAGE_ICON,
                      iconSize, iconSize, LR_DEFAULTCOLOR);
  m_expandCollapseButton.SetIcon((HICON)hIcon);

  AddToolTip("Assign selected elements to selected layer", GetHwnd(), toolTipHwnd,
             m_assignButton.GetHwnd());
  ...
  AddToolTip("Filter tree. Only elements that match the filter string will be shown!", GetHwnd(),
             toolTipHwnd, m_layerFilterEditBox.GetHwnd());
  AddToolTip("Enable case sensitive filtering", GetHwnd(), toolTipHwnd,
             m_layerFilterCaseButton.GetHwnd());
...
BOOL LayersListDialog::OnCommand(WPARAM wParam, LPARAM lParam)
{
  ...
  switch (id)
  {
    ...
    case IDC_LAYER_FILTER_CASE_BUTTON:
      ...
      Button_SetCheck(m_layerFilterCaseButton,
                      GetCaseSensitiveFilter() ? BST_CHECKED : BST_UNCHECKED);
...
LRESULT LayerTreeView::OnNotifyReflect(WPARAM wparam, LPARAM lparam)
{
  ...
  switch (lpnmh->code)
  {
    ...
    case TVN_SELCHANGED:
      return OnTVNSelChanged((LPNMTREEVIEW)lparam);
    case NM_CLICK:
...
bool LayerTreeView::AddElementToLayer(const HTREEITEM hLayerItem,
                                      const string& name,
                                      IEditable* const pedit)
{
