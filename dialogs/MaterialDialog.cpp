  ...
  switch (LOWORD(wParam))
  {
    ...
    case IDC_IMPORT:
    {
                                ...
                                0))
      {
        ...
        for (int i = 0; i < materialCount; i++)
        {
          ...
          Material* const pmat = new Material(
              mat.fWrapLighting, mat.fRoughness, dequantizeUnsigned<8>(mat.fGlossyImageLerp),
              dequantizeUnsigned<8>(mat.fThickness), mat.fEdge,
              dequantizeUnsigned<7>(mat.bOpacityActive_fEdgeAlpha >> 1), mat.fOpacity, mat.cBase,
              mat.cGlossy, mat.cClearcoat, mat.bIsMetal, !!(mat.bOpacityActive_fEdgeAlpha & 1),
              elasticity, elasticityFalloff, friction, scatterAngle);
      ...
      if (ListView_GetSelectedCount(m_hMaterialList)) // if some items are selected???
      {
        ...
        if (GetSaveFileName(&ofn)) //Get filename from user
        {
          ...
          while (sel != -1)
          {
            ...
            mat.bOpacityActive_fEdgeAlpha |=
                quantizeUnsigned<7>(clamp(pmat->m_fEdgeAlpha, 0.f, 1.f)) << 1;
    ...
    case IDC_DELETE_MATERIAL:
    {
      ...
      if (count > 0)
      {
        ...
        const int ans =
            MessageBox(ls.m_szbuffer /*"Are you sure you want to remove this material?"*/,
      ...
      switch (pnmhdr->code)
      {
        case LVN_ENDLABELEDIT:
        {
          ...
          else
          {
            ...
            ListView_SetItemText(m_hMaterialList, pinfo->item.iItem, 0,
                                 (LPSTR)pmat->m_szName.c_str());
          }
          pt->SetNonUndoableDirty(eSaveDirty);
          return TRUE;
        }
        case LVN_ITEMCHANGING:
        {
          ...
          if ((plistview->uNewState & LVIS_SELECTED) != (plistview->uOldState & LVIS_SELECTED))
          {
            if ((plistview->uNewState & LVIS_SELECTED) == LVIS_SELECTED)
            {
              ...
              SendMessage(GetDlgItem(IDC_DIFFUSE_CHECK).GetHwnd(), BM_SETCHECK,
                          pmat->m_bIsMetal ? BST_CHECKED : BST_UNCHECKED, 0);
              SendMessage(GetDlgItem(IDC_OPACITY_CHECK).GetHwnd(), BM_SETCHECK,
