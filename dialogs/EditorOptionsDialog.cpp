...
void EditorOptionsDialog::AddToolTip(const char* const text,
                                     HWND parentHwnd,
                                     HWND toolTipHwnd,
                                     HWND controlHwnd)
{
...
BOOL EditorOptionsDialog::OnInitDialog()
{
  ...
  const HWND toolTipHwnd = ::CreateWindowEx(
      0, TOOLTIPS_CLASS, nullptr, WS_POPUP | TTS_ALWAYSTIP | TTS_BALLOON, CW_USEDEFAULT,
      CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, GetHwnd(), nullptr, g_pvp->theInstance, nullptr);
  if (toolTipHwnd)
  {
    SendMessage(toolTipHwnd, TTM_SETMAXTIPWIDTH, 0, 180);
    AddToolTip("If checked, the 'Throw Balls in Player' option is always active. You don't need to "
               "activate it in the debug menu again.",
               GetHwnd(), toolTipHwnd, GetDlgItem(IDC_THROW_BALLS_ALWAYS_ON_CHECK));
    AddToolTip("Defines the default size of the ball when dropped onto the table.", GetHwnd(),
               toolTipHwnd, GetDlgItem(IDC_THROW_BALLS_SIZE_EDIT));
  }
  ...
  SendMessage(GetDlgItem(IDC_DRAW_DRAGPOINTS).GetHwnd(), BM_SETCHECK,
              fdrawpoints ? BST_CHECKED : BST_UNCHECKED, 0);

  // light centers
  const bool fdrawcenters = LoadValueBoolWithDefault("Editor", "DrawLightCenters", false);
  SendMessage(GetDlgItem(IDC_DRAW_LIGHTCENTERS).GetHwnd(), BM_SETCHECK,
              fdrawcenters ? BST_CHECKED : BST_UNCHECKED, 0);
  ...
  const int fautosavetime =
      LoadValueIntWithDefault("Editor", "AutoSaveTime", AUTOSAVE_DEFAULT_TIME);
  ...
  SendDlgItemMessage(IDC_THROW_BALLS_ALWAYS_ON_CHECK, BM_SETCHECK,
                     throwBallsAlwaysOn ? BST_CHECKED : BST_UNCHECKED, 0);

  const bool ballControlAlwaysOn = LoadValueBoolWithDefault("Editor", "BallControlAlwaysOn", false);
  SendDlgItemMessage(IDC_BALL_CONTROL_ALWAYS_ON_CHECK, BM_SETCHECK,
                     ballControlAlwaysOn ? BST_CHECKED : BST_UNCHECKED, 0);

  const bool groupElementsCollection =
      LoadValueBoolWithDefault("Editor", "GroupElementsInCollection", true);
  SendDlgItemMessage(IDC_DEFAULT_GROUP_COLLECTION_CHECK, BM_SETCHECK,
                     groupElementsCollection ? BST_CHECKED : BST_UNCHECKED, 0);

  const bool alwaysViewScript = LoadValueBoolWithDefault("Editor", "AlwaysViewScript", false);
  SendDlgItemMessage(IDC_ALWAYSVIEWSCRIPT, BM_SETCHECK,
                     alwaysViewScript ? BST_CHECKED : BST_UNCHECKED, 0);
  ...
  SendDlgItemMessage(IDC_START_VP_FILE_DIALOG, BM_SETCHECK,
                     startVPfileDialog ? BST_CHECKED : BST_UNCHECKED, 0);

  const bool startVPfileDialogPlayerClose =
      LoadValueBoolWithDefault("Editor", "SelectTableOnPlayerClose", true);
  SendDlgItemMessage(IDC_START_VP_FILE_DIALOG2, BM_SETCHECK,
                     startVPfileDialogPlayerClose ? BST_CHECKED : BST_UNCHECKED, 0);
...
void EditorOptionsDialog::OnOK()
{
  ...
  checked =
      (SendDlgItemMessage(IDC_BALL_CONTROL_ALWAYS_ON_CHECK, BM_GETCHECK, 0, 0) == BST_CHECKED);
  ...
  checked =
      (SendDlgItemMessage(IDC_DEFAULT_GROUP_COLLECTION_CHECK, BM_GETCHECK, 0, 0) == BST_CHECKED);
