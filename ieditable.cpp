...
void IEditable::SetName(const std::string& name)
{
  ...
  MultiByteToWideCharNull(CP_ACP, 0, name.c_str(), -1, newName,
                          sizeof(GetScriptable()->m_wzName) / sizeof(GetScriptable()->m_wzName[0]));
  ...
  lstrcpynW(GetScriptable()->m_wzName, namePtr,
            sizeof(GetScriptable()->m_wzName) / sizeof(GetScriptable()->m_wzName[0]));
