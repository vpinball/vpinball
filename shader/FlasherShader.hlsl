...
const float4 amount_blend_modulate_vs_add_hdrTexture01; // last two are bools
...
float4 ps_main_noLight(const in VS_OUTPUT_2D IN) : COLOR
{
   ...
   [branch] if ((flasherMode < 2.) && (amount_blend_modulate_vs_add_hdrTexture01.z == 0.)) // Mode 0 & 1
   ...
   {
      [branch] if (amount_blend_modulate_vs_add_hdrTexture01.w == 0.)
