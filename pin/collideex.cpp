...
void BumperHitCircle::Collide(const CollisionEvent& coll)
{
  ...
  coll.m_ball->Collide3DWall(coll.m_hitnormal, m_elasticity, m_elasticityFalloff, m_friction,
                             m_scatter); // reflect ball from wall

  if (m_pbumper->m_d.m_hitEvent &&
      (dot <= -m_pbumper->m_d.m_threshold)) // if velocity greater than threshold level
  {
    coll.m_ball->m_d.m_vel +=
        coll.m_hitnormal * m_pbumper->m_d.m_force; // add a chunk of velocity to drive ball away
...
void LineSegSlingshot::Collide(const CollisionEvent& coll)
{
  ...
  const bool threshold =
      (dot <= -m_psurface->m_d.m_slingshot_threshold); // normal greater than threshold?

  if (!m_psurface->m_disabled && threshold) // enabled and if velocity greater than threshold level
  {
    const float len =
        (v2.x - v1.x) * hitnormal.y -
        (v2.y - v1.y) * hitnormal.x; // length of segment, Unit TAN points from V1 to V2

    const Vertex2D vhitpoint(pball->m_d.m_pos.x -
                                 hitnormal.x * pball->m_d.m_radius, //project ball radius along norm
    ...
    const float btd = (vhitpoint.x - v1.x) * hitnormal.y -
                      (vhitpoint.y - v1.y) * hitnormal.x; // distance to vhit from V1
    float force = (fabsf(len) > 1.0e-6f) ? ((btd + btd) / len - 1.0f) : -1.0f; // -1..+1
    force = 0.5f *
            (1.0f -
             force * force); //!! maximum value 0.5 ...I think this should have been 1.0...oh well
    // will match the previous physics
    force *= m_force; //-80;

    pball->m_d.m_vel -=
        hitnormal *
        force; // boost velocity, drive into slingshot (counter normal), allow CollideWall to handle the remainder
  }
...
HitGate::HitGate(Gate* const pgate, const float height)
{
  ...
  m_lineseg[0].v2.x =
      pgate->m_d.m_vCenter.x + cs * (halflength + (float)PHYS_SKIN); //oversize by the ball radius
  m_lineseg[0].v2.y =
      pgate->m_d.m_vCenter.y + sn * (halflength + (float)PHYS_SKIN); //this will prevent clipping
  m_lineseg[0].v1.x =
      pgate->m_d.m_vCenter.x - cs * (halflength + (float)PHYS_SKIN); //through the edge of the
...
float HitGate::HitTest(const BallS& ball, const float dtime, CollisionEvent& coll) const
{
  ...
  {
    const float hittime = m_lineseg[i].HitTestBasic(ball, dtime, coll, false, true,
                                                    false); // any face, lateral, non-rigid
    if (hittime >= 0.f)
    {
...
void GateMoverObject::UpdateDisplacements(const float dtime)
{
  ...
  else
  {
    ...
    if (direction * m_angle < m_angleMin)
    {
      ...
      m_pgate->FireVoidEventParm(DISPID_LimitEvents_BOS,
...
void GateMoverObject::UpdateVelocities()
{
  if (!m_open)
  {
    ...
    {
      m_anglespeed -=
          sinf(m_angle) * m_gravityfactor *
          (float)(PHYS_FACTOR /
                  100.0); // Center of gravity towards bottom of object, makes it stop vertical
...
HitSpinner::HitSpinner(Spinner* const pspinner, const float height)
{
  ...
  m_lineseg[0].v2.x = pspinner->m_d.m_vCenter.x +
                      cs * (halflength + (float)PHYS_SKIN); //oversize by the ball radius
  m_lineseg[0].v2.y =
      pspinner->m_d.m_vCenter.y + sn * (halflength + (float)PHYS_SKIN); //this will prevent clipping
  m_lineseg[0].v1.x =
      pspinner->m_d.m_vCenter.x - cs * (halflength + (float)PHYS_SKIN); //through the edge of the
...
{
  if (m_pspinner->m_d.m_angleMin !=
      m_pspinner->m_d.m_angleMax) //blocked spinner, limited motion spinner
  {
    m_angle += m_anglespeed * dtime;
    ...
    {
      m_angle = m_angleMax;
      m_pspinner->FireVoidEventParm(DISPID_LimitEvents_EOS,
                                    fabsf(RADTOANG(m_anglespeed))); // send EOS event
    ...
    if (m_angle < m_angleMin)
    {
      ...
      m_pspinner->FireVoidEventParm(DISPID_LimitEvents_BOS,
                                    fabsf(RADTOANG(m_anglespeed))); // send Park event
...
void SpinnerMoverObject::UpdateVelocities()
{
  m_anglespeed -=
      sinf(m_angle) *
      (float)(0.0025 *
              PHYS_FACTOR); // Center of gravity towards bottom of object, makes it stop vertical
...
void Hit3DPoly::Init(Vertex3Ds* const rgv, const int count)
{
  ...
  const float inv_len = (sqr_len > 0.0f)
                            ? -1.0f / sqrtf(sqr_len)
                            : 0.0f; // NOTE: normal is flipped! Thus we need vertices in CCW order
  m_normal.x *= inv_len;
  m_normal.y *= inv_len;
  m_normal.z *= inv_len;
...
Hit3DPoly::Hit3DPoly(const float x,
                     const float y,
                     const float z,
                     const float r,
...
float Hit3DPoly::HitTest(const BallS& ball, const float dtime, CollisionEvent& coll) const
{
  ...
  Vertex3Ds hitPos =
      ball.m_pos -
      ball.m_radius * m_normal; // nearest point on ball ... projected radius along norm
