...
void Ball::Collide3DWall(const Vertex3Ds& hitNormal,
                         float elasticity,
                         const float elastFalloff,
                         ...
                         float scatter_angle)
{
  ...
  const Vertex3Ds surfP =
      -m_d.m_radius * hitNormal; // surface contact point relative to center of mass
  ...
  {
    scatter_angle = c_hardScatter; // if < 0 use global value
  }
  ...
  if (dot > 1.0f && scatter_angle > 1.0e-5f) //no scatter at low velocity
  {
    ...
    scatter *= (1.0f - scatter * scatter) * 2.59808f *
               scatter_angle; // shape quadratic distribution and scale
    const float radsin = sinf(scatter); // Green's transform matrix... rotate angle delta
    const float radcos =
        cosf(scatter); // rotational transform from current position to position at time t
...
float Ball::HitTest(const BallS& ball, const float dtime, CollisionEvent& coll) const
{
  ...
  if (bnv > C_LOWNORMVEL)
  {
    return -1.0f; // dot of delta velocity and delta displacement, positive if receding no collison
  }
...
#else
      hittime =
          bnd * (float)(1.0 / (2.0 * PHYS_TOUCH)) + 0.5f; // don't compete for fast zero time events
#endif
    }
  ...
  else
  {
    ...
    hittime = (time1 * time2 < 0.f) ? std::max(time1, time2)
                                    : std::min(time1, time2); // find smallest nonnegative solution
  }
  ...
  if (fabsf(hitnormal.x) <= FLT_MIN && fabsf(hitnormal.y) <= FLT_MIN &&
      fabsf(hitnormal.z) <= FLT_MIN)
  {
    return -1.f;
  }
...
void Ball::Collide(const CollisionEvent& coll)
{
  ...
  if (dot < -0.25f)
  { // only collisions with at least some small true impact velocity (no contacts)
...
{
  const float normVel =
      m_d.m_vel.Dot(coll.m_hitnormal); // this should be zero, but only up to +/- C_CONTACTVEL

  // If some collision has changed the ball's velocity, we may not have to do anything.
  if (normVel <= C_CONTACTVEL)
  {
    const Vertex3Ds fe =
        m_d.m_mass * g_pplayer->m_gravity; // external forces (only gravity for now)
    const float dot = fe.Dot(coll.m_hitnormal);
    const float normalForce = std::max(
        0.0f, -(dot * dtime + coll.m_hit_org_normalvelocity)); // normal force is always nonnegative
    ...
    {
      m_d.m_vel +=
          coll.m_hitnormal * max(min(C_EMBEDVELLIMIT, -coll.m_hitdistance), (float)PHYS_TOUCH);
    }
#endif
    ...
    if (m_vel.Length() < 1.f) //!! 1.f=magic, also see below
    {
      ...
      const float damp =
          (1.0f - friction * clamp(-coll.m_hit_org_normalvelocity / C_CONTACTVEL, 0.0f, 1.0f)) *
              vell +
          (1.0f -
           vell); // do not kill spin completely, otherwise stuck balls will happen during regular gameplay
...
void Ball::ApplyFriction(const Vertex3Ds& hitnormal, const float dtime, const float fricCoeff)
{
  ...
  const Vertex3Ds slip =
      surfVel - surfVel.Dot(hitnormal) * hitnormal; // calc the tangential slip velocity
  ...
  if ((normVel <=
       0.025f) || // check for <=0.025 originated from ball<->rubber collisions pushing the ball upwards, but this is still not enough, some could even use <=0.2
#else
  if (
#endif
      (slipspeed < C_PRECISION))
  {
    ...
    const Vertex3Ds slipAcc =
        surfAcc - surfAcc.Dot(hitnormal) * hitnormal; // calc the tangential slip acceleration
...
{
  return m_d.m_vel +
         CrossProduct(m_angularmomentum / Inertia(),
                      surfP); // linear velocity plus tangential velocity due to rotation
}
...
Vertex3Ds Ball::SurfaceAcceleration(const Vertex3Ds& surfP) const
{
  ...
  return g_pplayer->m_gravity / m_d.m_mass // linear acceleration
         + CrossProduct(angularvelocity,
                        CrossProduct(angularvelocity, surfP)); // centripetal acceleration
}
...
void Ball::UpdateVelocities()
{
  ...
  {
    if (g_pplayer->m_ballControl && this == g_pplayer->m_pactiveballBC &&
        g_pplayer->m_pBCTarget != nullptr)
    {
      m_d.m_vel.x *=
          0.5f; // Null out most of the X/Y velocity, want a little bit so the ball can sort of find its way out of obstacles.
      m_d.m_vel.y *= 0.5f;

      m_d.m_vel += Vertex3Ds(
          max(-10.0f, min(10.0f, (g_pplayer->m_pBCTarget->x - m_d.m_pos.x) * (float)(1. / 10.))),
