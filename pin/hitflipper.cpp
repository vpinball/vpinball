...
void HitFlipper::Collide(CollisionEvent *coll)
{
    ...
    */
   const float epsilon = m_elasticity / (1.0f + (m_pflipper->m_d.m_OverridePhysics ? m_pflipper->m_d.m_OverrideElasticityFalloff : m_pflipper->m_d.m_elasticityFalloff)*(float)(1.0/18.53) * fabsf(bnv));
