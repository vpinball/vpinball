...
FlipperMoverObject::FlipperMoverObject(const Vertex2D& center,
                                       const float baser,
                                       const float endr,
                                       const float flipr,
                                       ...
                                       const float zlow,
                                       const float zhigh,
                                       Flipper* const pflipper)
  : m_hitcircleBase(center, baser, zlow, zhigh) // base section
{
  m_pflipper = pflipper;
  ...
  const float mass =
      (m_pflipper->m_d.m_OverridePhysics ||
       (m_pflipper->m_ptable->m_overridePhysicsFlipper && m_pflipper->m_ptable->m_overridePhysics))
          ? m_pflipper->m_d.m_OverrideMass
          : m_pflipper->m_d.m_mass;
  ...
  m_zeroAngNorm.x = sqrtf(
      1.0f -
      ratio *
          ratio); // F2 Norm, used in Green's transform, in FPM time search  // =  sinf(faceNormOffset)
  m_zeroAngNorm.y =
...
HitFlipper::HitFlipper(const Vertex2D& center,
                       const float baser,
                       const float endr,
                       const float flipr,
                       const float angleStart,
...
{
  m_elasticityFalloff = (m_flipperMover.m_pflipper->m_d.m_OverridePhysics ||
                         (m_flipperMover.m_pflipper->m_ptable->m_overridePhysicsFlipper &&
                          m_flipperMover.m_pflipper->m_ptable->m_overridePhysics))
                            ? m_flipperMover.m_pflipper->m_d.m_OverrideElasticityFalloff
                            : m_flipperMover.m_pflipper->m_d.m_elasticityFalloff;
  m_elasticity = (m_flipperMover.m_pflipper->m_d.m_OverridePhysics ||
                  (m_flipperMover.m_pflipper->m_ptable->m_overridePhysicsFlipper &&
                   m_flipperMover.m_pflipper->m_ptable->m_overridePhysics))
                     ? m_flipperMover.m_pflipper->m_d.m_OverrideElasticity
                     : m_flipperMover.m_pflipper->m_d.m_elasticity;
  SetFriction((m_flipperMover.m_pflipper->m_d.m_OverridePhysics ||
               (m_flipperMover.m_pflipper->m_ptable->m_overridePhysicsFlipper &&
                m_flipperMover.m_pflipper->m_ptable->m_overridePhysics))
                  ? m_flipperMover.m_pflipper->m_d.m_OverrideFriction
                  : m_flipperMover.m_pflipper->m_d.m_friction);
  m_scatter = ANGTORAD((m_flipperMover.m_pflipper->m_d.m_OverridePhysics ||
                        (m_flipperMover.m_pflipper->m_ptable->m_overridePhysicsFlipper &&
                         m_flipperMover.m_pflipper->m_ptable->m_overridePhysics))
                           ? m_flipperMover.m_pflipper->m_d.m_OverrideScatterAngle
                           : m_flipperMover.m_pflipper->m_d.m_scatter);
}
...
static FRect3D ExtendBacksideBounds(FRect3D bounds,
...
static FRect3D ExtendBoundsAtPosition(FRect3D bounds,
                                      const Vertex2D& center,
                                      const float length,
                                      const float fixedRadius,
                                      const float angle)
...
static FRect3D ExtendBoundsAtExtreme(const FRect3D& aabb,
                                     const Vertex2D& c,
                                     const float length,
...
void HitFlipper::CalcHitBBox()
{
  ...
  // Allow roundoff
  m_hitBBox.left = m_flipperMover.m_hitcircleBase.center.x -
                   (m_flipperMover.m_flipperradius + m_flipperMover.m_endradius + 0.1f);
  m_hitBBox.right = m_flipperMover.m_hitcircleBase.center.x +
                    (m_flipperMover.m_flipperradius + m_flipperMover.m_endradius + 0.1f);
  m_hitBBox.top = m_flipperMover.m_hitcircleBase.center.y -
                  (m_flipperMover.m_flipperradius + m_flipperMover.m_endradius + 0.1f);
  m_hitBBox.bottom = m_flipperMover.m_hitcircleBase.center.y +
                     (m_flipperMover.m_flipperradius + m_flipperMover.m_endradius + 0.1f);
  ...
  FRect3D aabb = FRect3D(c.x, c.x, c.y, c.y, m_flipperMover.m_hitcircleBase.m_hitBBox.zlow,
                         m_flipperMover.m_hitcircleBase.m_hitBBox.zhigh);
...
{
  return (m_pflipper->m_d.m_OverridePhysics || (m_pflipper->m_ptable->m_overridePhysicsFlipper &&
                                                m_pflipper->m_ptable->m_overridePhysics))
             ? m_pflipper->m_d.m_OverrideReturnStrength
             : m_pflipper->m_d.m_return;
}
...
float FlipperMoverObject::GetStrength() const
{
  return (m_pflipper->m_d.m_OverridePhysics || (m_pflipper->m_ptable->m_overridePhysicsFlipper &&
                                                m_pflipper->m_ptable->m_overridePhysics))
             ? m_pflipper->m_d.m_OverrideStrength
             : m_pflipper->m_d.m_strength;
}

float FlipperMoverObject::GetMass() const
{
  return 3.0f * m_inertia /
         (m_flipperradius *
          m_flipperradius); //!! also change if wiring of moment of inertia happens (see ctor)
}

void FlipperMoverObject::SetMass(const float m)
{
  m_inertia = (float)(1.0 / 3.0) * m *
              (m_flipperradius *
               m_flipperradius); //!! also change if wiring of moment of inertia happens (see ctor)
}
...
void FlipperMoverObject::UpdateDisplacements(const float dtime)
{
  ...
  if (fabsf(m_angleSpeed) <
      0.0005f) // avoids 'jumping balls' when two or more balls held on flipper (and more other balls are in play) //!! make dependent on physics update rate
    return;
  ...
  if (handle_event)
  {
    ...
    if (m_enableRotateEvent > 0)
    {
      ...
      m_pflipper->FireVoidEventParm(DISPID_LimitEvents_BOS,
                                    anglespd); // send Beginning of Stroke/Park event
...
void FlipperMoverObject::UpdateVelocities()
{
  ...
  // hold coil is weaker
  const float EOS_angle = ANGTORAD(
      (m_pflipper->m_d.m_OverridePhysics ||
       (m_pflipper->m_ptable->m_overridePhysicsFlipper && m_pflipper->m_ptable->m_overridePhysics))
          ? m_pflipper->m_d.m_OverrideTorqueDampingAngle
          : m_pflipper->m_d.m_torqueDampingAngle);
  if (fabsf(m_angleCur - m_angleEnd) < EOS_angle)
  {
    const float lerp = sqrf(sqrf(fabsf(m_angleCur - m_angleEnd) /
                                 EOS_angle)); // fade in/out damping, depending on angle to end
    desiredTorque *= lerp + ((m_pflipper->m_d.m_OverridePhysics ||
                              (m_pflipper->m_ptable->m_overridePhysicsFlipper &&
                               m_pflipper->m_ptable->m_overridePhysics))
                                 ? m_pflipper->m_d.m_OverrideTorqueDamping
                                 : m_pflipper->m_d.m_torqueDamping) *
                                (1.0f - lerp);
  }
  ...
  float torqueRampupSpeed =
      (m_pflipper->m_d.m_OverridePhysics ||
       (m_pflipper->m_ptable->m_overridePhysicsFlipper && m_pflipper->m_ptable->m_overridePhysics))
          ? m_pflipper->m_d.m_OverrideCoilRampUp
          : m_pflipper->m_d.m_rampUp;
...
{
  return (m_angleCur - m_angleStart) /
         (m_angleEnd - m_angleStart); // End == Start cannot happen, as handled in ctor
}
...
float FlipperMoverObject::GetHitTime() const
{
  ...
  const float dist = (m_angleSpeed > 0.f) ? angleMax - m_angleCur // >= 0
...
float HitFlipper::HitTest(const BallS& ball, const float dtime, CollisionEvent& coll) const
{
  ...
  if (hittime >= 0.f)
  {
    (const_cast<HitFlipper*>(this))->m_flipperMover.m_lastHitFace =
        !lastface; // change this face to check first // HACK
    return hittime;
  }
...
float HitFlipper::HitTestFlipperEnd(const BallS& ball,
                                    const float dtime,
                                    CollisionEvent& coll) const // replacement
{
  ...
  for (k = 1; k <= C_INTERATIONS; ++k)
  {
    ...
    const float radcos =
        cosf(contactAng); // rotational transform from zero position to position at time t

    //rotate angle delta unit vector, rotates system according to flipper face angle
    const Vertex2D vt(vp.x * radcos - vp.y * radsin +
                          flipperbase.x, //rotate and translate to world position
    ...
    cbcedist =
        sqrtf(ballvtx * ballvtx + ballvty * ballvty); // center ball to center end radius distance

    bfend = cbcedist - ballrEndr; // ball face-to-radius surface distance

    if (fabsf(bfend) <= C_PRECISION)
      break;

    if (k == 1) // end of pass one ... set full interval pass, t = dtime
    { // test for extreme conditions
      if (bfend < -(ball.m_radius + feRadius))
        return -1.0f; // too deeply embedded, ambigious position
      if (bfend <= (float)PHYS_TOUCH)
        break; // inside the clearance limits

      t0 = t1 = dtime;
      d0 = 0;
      d1 = bfend; // set for second pass, force t=dtime
    }
  ...
  if (infNaN(t) || t < 0.f || t > dtime // time is outside this frame ... no collision
      || ((k > C_INTERATIONS) &&
          (fabsf(bfend) > ball.m_radius * 0.25f))) // last ditch effort to accept a solution
  ...
  const float hitz =
      ball.m_pos.z +
      ball.m_vel.z * t; // check for a hole, relative to ball rolling point at hittime
  ...
  const Vertex2D dist(ball.m_pos.x + ballvx * t - ball.m_radius * coll.m_hitnormal.x -
                          m_flipperMover.m_hitcircleBase.center
                              .x, // vector from base to flipperEnd plus the projected End radius
                      ball.m_pos.y + ballvy * t - ball.m_radius * coll.m_hitnormal.y -
                          m_flipperMover.m_hitcircleBase.center.y);

  const float distance =
      sqrtf(dist.x * dist.x + dist.y * dist.y); // distance from base center to contact point

  if ((contactAng >= angleMax && anglespeed > 0.f) ||
      (contactAng <= angleMin && anglespeed < 0.f)) // hit limits ???
    anglespeed = 0.f; // rotation stopped

  const float inv_distance = 1.0f / distance;
  coll.m_hitvel.x =
      -dist.y * inv_distance; //Unit Tangent vector velocity of contact point(rotate normal right)
  ...
  const Vertex2D dv(ballvx - coll.m_hitvel.x * anglespeed * distance,
...
float HitFlipper::HitTestFlipperFace(const BallS& ball,
                                     const float dtime,
                                     CollisionEvent& coll,
                                     const bool face1) const
{
  const float angleCur = m_flipperMover.m_angleCur;
  float anglespeed = m_flipperMover.m_angleSpeed; // rotation rate
  ...
  if (face1)
    ffnx = -ffnx; // negative for face1 (left face)
  ...
  for (k = 1; k <= C_INTERATIONS; ++k)
  {
    ...
    const float radsin = sinf(contactAng); //  Green's transform matrix... rotate angle delta
    const float radcos =
        cosf(contactAng); //  rotational transform from current position to position at time t
    ...
    ballvtx = ball.m_pos.x + ballvx * t -
              vt.x; // new ball position relative to rotated line segment endpoint
    ballvty = ball.m_pos.y + ballvy * t - vt.y;

    bffnd = ballvtx * F.x + ballvty * F.y - ball.m_radius; // normal distance to segment

    if (fabsf(bffnd) <= C_PRECISION)
      break;
    ...
    { // test for already inside flipper plane, either embedded or beyond the face endpoints
      if (bffnd < -(ball.m_radius + feRadius))
  ...
  if (infNaN(t) || t < 0.f || t > dtime // time is outside this frame ... no collision
      || ((k > C_INTERATIONS) &&
          (fabsf(bffnd) > ball.m_radius * 0.25f))) // last ditch effort to accept a near solution
    return -1.0f; // no solution
  ...
  const float len =
      m_flipperMover.m_flipperradius *
      m_flipperMover.m_zeroAngNorm.x; // face segment length ... e.g. same on either face
  ...
  const Vertex2D dist( // calculate moment from flipper base center
      ball.m_pos.x + ballvx * t - ball.m_radius * F.x -
          m_flipperMover.m_hitcircleBase.center
              .x, // center of ball + projected radius to contact point
      ball.m_pos.y + ballvy * t - ball.m_radius * F.y -
          m_flipperMover.m_hitcircleBase.center.y); // all at time t
  ...
  coll.m_hitvel.x =
      -dist.y * inv_dist; // Unit Tangent velocity of contact point(rotate Normal clockwise)
  ...
  const Vertex2D dv(ballvx - coll.m_hitvel.x * (anglespeed * distance),
                    ballvy -
                        coll.m_hitvel.y * (anglespeed * distance)); // delta velocity ball to face
...
void HitFlipper::Collide(const CollisionEvent& coll)
{
  ...
  const Vertex3Ds cF(
      m_flipperMover.m_hitcircleBase.center.x, m_flipperMover.m_hitcircleBase.center.y,
  ...
  if (hdist > 1.0e-4f)
  {
    ...
    pball->m_d.m_pos +=
        hdist *
        coll.m_hitnormal; // push along norm, back to free area; use the norm, but is not correct
  }
#endif
...
#ifdef DEBUG_FLIPPERS
  slintf("  epsilon: %.2f\n  angular response: %.3f\n", epsilon,
         normal.Dot(CrossProduct(angResp / m_flipperMover.m_inertia, rF)));
#endif
  ...
  if (m_flipperMover.m_isInContact)
  {
    if (rotI.z * m_flipperMover.m_contactTorque < 0.f) // pushing against the solenoid?
    {
      ...
      const float recoilTime =
          -rotI.z /
          m_flipperMover.m_contactTorque; // time flipper needs to eliminate this impulse, in 10ms
  ...
  if (tangentSpSq > 1e-6f)
  {
    ...
    kt += tangent.Dot(
        CrossProduct(crossF / m_flipperMover.m_inertia, rF)); // flipper only has angular response
  ...
  if ((bnv < -0.25f) &&
      (g_pplayer->m_time_msec - m_last_hittime) > 250) // limit rate to 250 milliseconds per event
  {
    //!! unused const float distance = coll.m_hitmoment;                     // moment .... and the flipper response
    const float flipperHit = /*(distance == 0.0f)*/ coll.m_hitmoment_bit
                                 ? -1.0f
                                 : -bnv; // move event processing to end of collision handler...
    if (flipperHit < 0.f)
      m_flipperMover.m_pflipper->FireGroupEvent(DISPID_HitEvents_Hit); // simple hit event
    else
      m_flipperMover.m_pflipper->FireVoidEventParm(
          DISPID_FlipperEvents_Collide, flipperHit); // collision velocity (normal to face)
  }
...
void HitFlipper::Contact(CollisionEvent& coll, const float dtime)
{
  ...
  if (coll.m_hitdistance < -C_EMBEDDED)
    pball->m_d.m_vel +=
        normal *
        0.1f; //!! magic to avoid balls being pushed by each other through resting flippers!
#endif
  ...
  if (normVel <= C_CONTACTVEL)
  {
    ...
    pball->m_d.m_vel += ((j * dtime) * invMass - coll.m_hit_org_normalvelocity) *
                        normal; // kill any existing normal velocity
    ...
    if (slipspeed < C_PRECISION)
    {
      ...
      const Vertex3Ds slipAcc =
          arel - arel.Dot(normal) * normal; // calc the tangential slip acceleration
