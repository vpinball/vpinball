...
{
  vector<IFireEvents*>*
      m_vpVolObjs; // vector of triggers and kickers we are now inside (stored as IFireEvents* though, as HitObject.m_obj stores it like that!)
  Vertex3Ds m_pos;
...
public:
  ...
  float HitRadiusSqr() const
  {
    return sqrf((m_hitBBox.right - m_hitBBox.left) * 0.5f);
  } // this returns the extended (by m_vel + magic) squared radius, as needed to be used in the collision detection
  void Collide3DWall(const Vertex3Ds& hitNormal,
                     float elasticity,
  ...
  CollisionEvent
      m_coll; // collision information, may not be a actual hit if something else happens first
  ...
  float
      m_bulb_intensity_scale; // to dampen/increase contribution of the bulb lights (locally/by script)
  ...
  //
  static unsigned int
      ballID; // increased for each ball created to have an unique ID for scripts for each ball
};
