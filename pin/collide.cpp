...
void HitObject::Contact(CollisionEvent& coll, const float dtime)
{
...
void HitObject::FireHitEvent(Ball* const pball)
{
  if (m_obj && m_fe && m_enabled)
  {
    ...
    const float normalDist =
        (m_ObjType == eHitTarget)
            ? 0.0f // hit targets when used with a captured ball have always a too small distance
...
float LineSeg::HitTestBasic(const BallS& ball,
                            const float dtime,
                            CollisionEvent& coll,
                            ...
                            const bool rigid) const
{
  ...
  const float bnv =
      ballvx * normal.x +
      ballvy * normal.y; // ball velocity normal to segment, positive if receding, zero=parallel
  ...
  const float bcpd =
      (ballx - v1.x) * normal.x + (bally - v1.y) * normal.y; // ball center to plane distance
  ...
  const float btv =
      ballvx * normal.y -
      ballvy * normal.x; // ball velocity tangent to segment with respect to direction from V1 to V2
  const float btd =
      (ballx - v1.x) * normal.y - (bally - v1.y) * normal.x // ball tangent distance
      + btv * hittime; // ball tangent distance (projection) (initial position + velocity * hitime)

  if (btd < -C_TOL_ENDPNTS ||
      btd > length + C_TOL_ENDPNTS) // is the contact off the line segment???
  {
    return -1.0f;
  }
  if (!rigid)
  ...
  const float hitz =
      ball.m_pos.z +
      ball.m_vel.z * hittime; // check too high or low relative to ball rolling point at hittime
...
float HitCircle::HitTestBasicRadius(const BallS& ball,
                                    const float dtime,
                                    CollisionEvent& coll,
                                    const bool direction,
                                    const bool lateral,
                                    const bool rigid)
    const // all of these true = bumper/flipperbase/gate/spinner, all false = kicker/trigger
{
  if (!m_enabled || ball.m_frozen)
  {
    return -1.0f;
  ...
  if (m_ObjType == eKicker && bnd <= 0.f && bnd >= -radius && a < C_CONTACTVEL * C_CONTACTVEL &&
      ball.m_vpVolObjs)
  {
  ...
  if (rigid &&
      bnd <
          (float)
              PHYS_TOUCH) // positive: contact possible in future ... Negative: objects in contact now
  {
  ...
  { // here if ... ball inside and no hit set .... or ... ball outside and hit set
    ...
    if (fabsf(bnd - radius) <
        0.05f) // if ball appears in center of trigger, then assumed it was gen'ed there
    {
  ...
  if (((hitz + ball.m_radius * 0.5f) < m_hitBBox.zlow) ||
      ...
      (capsule3D && hitz < m_hitBBox.zhigh))
  {
    return -1.0f;
  }
...
float HitLineZ::HitTest(const BallS& ball, const float dtime, CollisionEvent& coll) const
{
  ...
  if (bnd < (float)PHYS_TOUCH) // already in collision distance?
  {
    ...
    {
      hittime = /*std::max(0.0f,*/ -bnd /
                bnv /*)*/; // estimate based on distance and speed along distance
    }
  }
  else
  {
    ...
    hittime = (time1 * time2 < 0.f) ? max(time1, time2)
                                    : min(time1, time2); // find smallest nonnegative solution
  }
...
void DoHitTest(const Ball* const pball, const HitObject* const pho, CollisionEvent& coll)
{
  ...
  { //!! why is this done here and not in corresponding HitTest()?
    return;
  }
  ...
  if (validhit)
  {
    ...
    if (g_pplayer->m_recordContacts && newColl.m_isContact) // remember all contacts?
    {
      ...
      const std::lock_guard<std::mutex> lg(
          mtx); // multiple threads may end up here and call push_back
#endif
      g_pplayer->m_contacts.push_back(newColl);
    }
    else
    { // record first collision event
      coll = newColl;
    }
  }
}
