...
static const char* regkey_string[eCKeys] = {
    "LFlipKey",   "RFlipKey",    "LTiltKey",   "RTiltKey",     "CTiltKey",      "PlungerKey",
    "FrameCount", "DebugBalls",  "Debugger",   "AddCreditKey", "AddCreditKey2", "StartGameKey",
    "MechTilt",   "RMagnaSave",  "LMagnaSave", "ExitGameKey",  "VolumeUp",      "VolumeDown",
    "LockbarKey", "Enable3DKey", "EscapeKey"};
static constexpr int regkey_defdik[eCKeys] = {
    DIK_LSHIFT,   DIK_RSHIFT, DIK_Z,      DIK_SLASH, DIK_SPACE, DIK_RETURN, DIK_F11,
    DIK_O,        DIK_D,      DIK_5,      DIK_4,     DIK_1,     DIK_T,      DIK_RCONTROL,
    DIK_LCONTROL, DIK_Q,      DIK_EQUALS, DIK_MINUS, DIK_LALT,  DIK_F10,    DIK_ESCAPE};
static constexpr int regkey_idc[eCKeys] = {IDC_LEFTFLIPPER,
                                           IDC_RIGHTFLIPPER,
                                           IDC_LEFTTILT,
                                           IDC_RIGHTTILT,
                                           IDC_CENTERTILT,
...
public:
  ...
  {
    const int EnableLegacyMaximumPreRenderedFrames =
        LoadValueIntWithDefault("Player", "EnableLegacyMaximumPreRenderedFrames", 0);
...
private:
  ...
  void Bloom();
  void SSRefl();
  void StereoFXAA(const bool stereo,
                  const bool SMAA,
                  const bool DLAA,
                  const bool NFAA,
                  const bool FXAA1,
...
public:
...
#ifndef ENABLE_SDL
  void DMDdraw(const float DMDposx,
               const float DMDposy,
...
#endif
  void Spritedraw(const float posx,
                  const float posy,
                  const float width,
                  const float height,
  ...
  std::vector<TimerOnOff>
      m_changed_vht; // stores all en/disable changes to the m_vht timer list, to avoid problems with timers dis/enabling themselves
  ...
  vector<CLSID*>
      m_controlclsidsafe; // ActiveX control types which have already been okayed as being safe
  ...
  int m_lastcursorx,
      m_lastcursory; // used for the dumb task of seeing if the mouse has really moved when we get a WM_MOUSEMOVE message
  ...
  bool
      m_swap_ball_collision_handling; // Swaps the order of ball-ball collision handling around each physics cycle (in regard to the RLC comment block in quadtree.cpp (hopefully ;)))
  ...
  bool m_touchregion_pressed
      [8]; // status for each touch region to avoid multitouch double triggers (true = finger on, false = finger off)
...
private:
  ...
  void SetScreenOffset(
      const float x,
      const float y); // set render offset in screen coordinates, e.g., for the nudge shake
