...
HitPlunger::HitPlunger(const float x,
                       const float y,
                       const float x2,
                       const float zheight,
                       const float frameTop,
                       ...
                       Plunger* const pPlunger)
{
  ...
  const float restPos =
      pPlunger->m_d.m_parkPosition; // The rest position is taken from the "park position" property
...
void PlungerMoverObject::Fire(float startPos)
{
  ...
  m_fireSpeed = -m_plunger->m_d.m_speedFire * dx * m_frameLen / m_mass * normalize;
...
void PlungerMoverObject::UpdateVelocities()
{
  ...
  else if (m_autoFireTimer > 0)
  {
    ...
    if (--m_autoFireTimer == 0)
    {
      if (g_pplayer != 0)
      {
        g_pplayer->m_ptable->FireKeyEvent(DISPID_GameEvents_KeyUp,
  ...
  else
  {
    ...
    m_speed += error * m_frameLen * m_plunger->m_d.m_mechStrength / m_mass * normalize * dt;
...
void HitPlunger::Collide(const CollisionEvent& coll)
{
  ...
  float dot = (pball->m_d.m_vel.x - coll.m_hitvel.x) * coll.m_hitnormal.x +
              (pball->m_d.m_vel.y - coll.m_hitvel.y) * coll.m_hitnormal.y;
  ...
  const float scatter_vel = m_plungerMover.m_scatterVelocity *
                            g_pplayer->m_ptable->m_globalDifficulty; // apply dificulty weighting
  ...
  if (scatter_vel > 0.f && fabsf(pball->m_d.m_vel.y) > scatter_vel) //skip if low velocity
  {
    ...
    scatter *= (1.0f - scatter * scatter) * 2.59808f *
               scatter_vel; // shape quadratic distribution and scale
