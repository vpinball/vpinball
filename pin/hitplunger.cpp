...
void PlungerAnimObject::UpdateVelocities()
{
        ...
        else
        {
                ...
                // this acceleration.  This is in "standard VP 9 physics frame"
                // units, where 1.0 equals the amount of real time in one original
                // VP 9 physics frame.  The other normalization factors were originally
                // chosen for original VP 9 timing, so we need to adjust for the 
                // different timing in the physmod5 version.  PM5 runs physics frames
                // at roughly 10x the rate of standard VP 9, so the time per frame is
                // about 1/10 the VP 9 time.
                const float plungerFriction = .95f;
                const float dt = 0.1f;
                m_speed *= plungerFriction;
                m_speed += error * m_frameLen
                           * m_plunger->m_d.m_mechStrength/m_mass
                           * c_plungerNormalize * dt;
...
void HitPlunger::Collide(CollisionEvent *coll)
{
    ...
    float dot = (pball->vel.x - phitnormal[1].x)* phitnormal->x + (pball->vel.y - phitnormal[1].y) * phitnormal->y;
        ...
        float impulse = dot * -1.45f/(1.0f+1.0f/m_plungeranim.m_mass);
        ...
        // Check for a collision with the tip.  This is the moving
        // part of the plunger, so it has some special handling.
        if (phitnormal->y != 0.0f)
        {
                ...
                const float reverseImpulseFudgeFactor = 0.22f;
