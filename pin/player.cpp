   ...
   , m_ptable(live_table)
{
 ...
 #pragma message ( "Warning: No CPU float ignore denorm implemented" )
   ...
   m_showTouchOverlay = LoadValueWithDefault(regKey[RegName::Player], "TouchOverlay"s, false);
...
#ifdef __STANDALONE__
   m_pB2SWindow = nullptr;
   m_pB2SRenderer = nullptr;
   m_b2sRenderCallback = nullptr;

   m_pDMDWindow = nullptr;
   m_pDMDRenderer = nullptr;
   m_dmdRenderCallback = nullptr;
#endif
}
...
#else
         if (mode.w == m_wnd_width && mode.h == m_wnd_height && (m_refreshrate == 0 || (m_refreshrate > 0 && mode.refresh_rate == m_refreshrate) && mode.format == format))
#endif
         {
...
#ifdef __STANDALONE__
   const bool b2s = LoadValueWithDefault(regKey[RegName::Standalone], "B2S"s, false);
   if (b2s) {
      m_pB2SWindow = SDL_CreateWindow("vpx_b2s",
      ...
      m_pB2SRenderer = SDL_CreateRenderer(m_pB2SWindow, -1, SDL_RENDERER_ACCELERATED);
   }
   const bool dmd = LoadValueWithDefault(regKey[RegName::Standalone], "DMD"s, false);
   if (dmd) {
      m_pDMDWindow = SDL_CreateWindow("vpx_dmd",
      ...
      m_pDMDRenderer = SDL_CreateRenderer(m_pDMDWindow, -1, SDL_RENDERER_ACCELERATED);
   }
   SDL_RaiseWindow(m_sdl_playfieldHwnd);
#endif
...
void Player::Shutdown()
{
   ...
   if (m_pB2SRenderer)
      SDL_DestroyRenderer(m_pB2SRenderer);

   if (m_pB2SWindow)
      SDL_DestroyWindow(m_pB2SWindow);

   m_dmdRenderCallback = nullptr;

   if (m_pDMDRenderer)
      SDL_DestroyRenderer(m_pDMDRenderer);

   if (m_pDMDWindow)
      SDL_DestroyWindow(m_pDMDWindow);
#endif
}
...
#ifdef __STANDALONE__
   const bool webServer = LoadValueWithDefault(regKey[RegName::Standalone], "WebServer"s, false);
...
#else
      m_pin3d.m_pd3dPrimaryDevice->FBShader->SetTechnique(DLAA  ? SHADER_TECHNIQUE_DLAA_edge :
                                                         (NFAA  ? SHADER_TECHNIQUE_NFAA :
                                                         (FXAA3 ? SHADER_TECHNIQUE_FXAA3 :
                                                         (FXAA2 ? SHADER_TECHNIQUE_FXAA2 :
                                                                  SHADER_TECHNIQUE_FXAA1))));
...
void Player::FinishFrame()
{
   ...
   if (m_ptable->m_pcv->m_scriptError)
   {
      ...
      SaveValue(regKey[RegName::Standalone], "LaunchTable"s, "res/exampleTable.vpx");
#endif
...
#ifdef __STANDALONE__
   if (g_pplayer->m_b2sRenderCallback)
      g_pplayer->m_b2sRenderCallback(g_pplayer->m_pB2SRenderer);

   if (g_pplayer->m_dmdRenderCallback)
      g_pplayer->m_dmdRenderCallback(g_pplayer->m_pDMDRenderer);

   if (g_pplayer->m_b2sRenderCallback || g_pplayer->m_dmdRenderCallback)
...
void Player::StopPlayer()
{
   ...
   if ((LoadValueWithDefault(regKey[RegName::Player], "CacheMode"s, 1) > 0) && FileExists(szVPXFile))
   {
      ...
      if (m_dynamicMode && FileExists(path))
      {
         std::ifstream myFile(path);
         std::stringstream buffer;
         buffer << myFile.rdbuf();
...
#ifdef __STANDALONE__
      PLOGI.printf("Saving texture cache to %s", path.c_str());
#endif
