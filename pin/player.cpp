...
// mjr - fencepoint correction (+1 on range)
#define GetUltraNudgeX() (((F32)m_curAccel_x[0]) * (F32)(2.0 / (JOYRANGEMX-JOYRANGEMN+1))) // Get the -1.0f to +1.0f values from joystick input tilt sensor / ushock
#define GetUltraNudgeY() (((F32)m_curAccel_y[0]) * (F32)(2.0 / (JOYRANGEMX-JOYRANGEMN+1)))
...
void NudgeFilter::sample(float &a, U64 now)
{
    ...
    // check for a sign change
        if (fabs(a) > .01 && fabs(prv_) > .01
            && ((a < 0 && prv_ > 0) || (a > 0 && prv_ < 0)))
        {
        ...
        else if (fabs(a) > .05 && now - tzc_ > 500000)
        {
    ...
    // bring the running total toward rest.
        ...
        else if (now - tCorr_ > 50000 || now - tMotion_ > 50000)
        {
        // bring the running total toward neutral
                float corr = exp(0.33f*log(fabs(sum_/.02f))) * (sum_ < 0.0f ? -.02f : .02f);
...
IF_DEBUG_NUDGE(void NudgeFilter::dbg(const char *fmt, ...) {
        ...
        va_start(args, fmt);
        static FILE *fp = 0;
        if (fp == 0) fp = fopen("d:\\per\\temp\\joystick.csv", "w");
...
float PlungerAnimObject::mechPlunger() const
{
    ...
    if (g_pplayer->m_pininput.m_linearPlunger)
    {
        ...
        const float m = (1.0f - m_restPos)/JOYRANGEMX, b = m_restPos;
