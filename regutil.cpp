  ...
  for (unsigned int i = 0; i < 5; ++i)
  {
    ...
    else
    {
      for (
          ...
          child =
              child
                  ->next_sibling()) // exists? -> replace all text-entries (=internally allocated) via value(), with a self-allocated value(textcopy) so that we can overwrite it later-on more easily without leaking mem
      {
...
static HRESULT LoadValue(
    const std::string& szKey, const std::string& szValue, DWORD& type, void* pvalue, DWORD size);
...
HRESULT LoadValue(const std::string& szKey,
...
float LoadValueFloatWithDefault(const std::string& szKey,
                                const std::string& szValue,
                                const float def)
{
  float val;
...
static HRESULT SaveValue(const std::string& szKey,
                         const std::string& szValue,
                         const DWORD type,
                         const void* pvalue,
                         const DWORD size)
