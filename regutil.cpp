...
{
   PLOGI << "Using settings file from: " << path;
   ini.clear();
   iniPath = path;
...
{
   PLOGI << "Using settings override file from: " << path;
   iniOverride.clear();
...
void SaveRegistry()
{
   if (hasIniOverride)
   {
      ...
      PLOGI << "Settings file was saved to: " << iniOverridePath;
   }
   else
   {
      mINI::INIFile file(iniPath);
      file.write(ini, true);
      PLOGI << "Settings file was saved to: " << iniPath;
   }
}
