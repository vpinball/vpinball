...
public:
  ...
  bool Needs_ConvertAlpha_Tonemap() const
  {
    return (m_format == RGB_FP) || ((m_format == RGBA) && m_has_alpha);
  }

  void CopyTo_ConvertAlpha_Tonemap(BYTE* const __restrict bits)
      const // premultiplies alpha (as Win32 AlphaBlend() wants it like that) OR converts rgb_fp format to 32bits
  {
    if (m_format == RGB_FP) // Tonemap for 8bpc-Display
    {
      ...
      for (int j = 0; j < m_height; ++j)
        for (int i = 0; i < m_width; ++i, ++o)
        {
          ...
          const float n =
              (l * (float)(255. * 0.25) + 255.0f) /
              (l + 1.0f); // simple tonemap and scale by 255, overflow is handled by clamp below
          ...
          2600) // For everything newer than Windows XP: use the alpha in the bitmap, thus RGB needs to be premultiplied with alpha, due to how AlphaBlend() works
      {
        ...
        for (int j = 0; j < m_height; ++j)
          for (int i = 0; i < m_width; ++i, ++o)
          {
            ...
            if (alpha ==
                0) // adds a checkerboard where completely transparent (for the image manager display)
            {
