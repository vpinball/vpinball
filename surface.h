...
public:
  ...
  bool m_enabled;
  bool
      m_isBottomSolid; // is the bottom closed (lower side of the 'cube') or not (legacy behavior has bottom open, e.g. balls can drop into walls from below, or leave them if inside walls (if bottom area is large enough of course))
  bool m_slingshotAnimation;
  bool m_topBottomVisible;
  bool
      m_inner; //!! Deprecated, do not use! Always true after loading! (was: Inside or outside wall)
};

/////////////////////////////////////////////////////////////////////////////
// Surface
class Surface : public CComObjectRootEx<CComSingleThreadModel>,
...
public:
  ...
  void SetSlingshotStrength(const float value) { m_d.m_slingshotforce = value * 10.0f; }
...
private:
  ...
  void GenerateMesh(std::vector<Vertex3D_NoTex2>& topBuf,
