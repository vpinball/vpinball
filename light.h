...
class Light : public IDispatchImpl<ILight, &IID_ILight, &LIBID_VPinballLib>,
...
public:
  ...
  {
    return m_d.m_BulbLight && (m_d.m_transmissionScale > 0.f) && !m_backglass;
  }
  virtual float GetDepth(const Vertex3Ds& viewDir) const;
  virtual unsigned long long GetMaterialID() const
  {
    return m_surfaceMaterial ? m_surfaceMaterial->hash() : 64 - 2;
  } //!! 2 = some constant number
  virtual unsigned long long GetImageID() const
  {
    return (m_d.m_BulbLight ? 0 : (unsigned long long)(m_ptable->GetImage(m_d.m_szImage)));
  }
...
private:
  ...
  public:
    ...
    {
      return (m_plight->m_d.m_shape == ShapeCircle) ? 1 : 2;
    } // Don't select light bulb twice if we have drag points
