...
{
  DialogBoxParam(g_pvp->theInstance, MAKEINTRESOURCE(IDD_ROTATE), g_pvp->GetHwnd(), RotateProc,
                 (size_t)this->GetIEditable()->GetISelect()); //(long)this);
}

void IHaveDragPoints::ScaleDialog()
{
  DialogBoxParam(g_pvp->theInstance, MAKEINTRESOURCE(IDD_SCALE), g_pvp->GetHwnd(), ScaleProc,
                 (size_t)this->GetIEditable()->GetISelect());
}

void IHaveDragPoints::TranslateDialog()
{
  DialogBoxParam(g_pvp->theInstance, MAKEINTRESOURCE(IDD_TRANSLATE), g_pvp->GetHwnd(),
                 TranslateProc, (size_t)this->GetIEditable()->GetISelect());
}

void IHaveDragPoints::RotatePoints(const float ang,
...
void IHaveDragPoints::ScalePoints(const float scalex,
                                  const float scaley,
                                  const Vertex2D& pvCenter,
                                  const bool useElementCenter)
{
...
void DragPoint::Init(
    IHaveDragPoints* pihdp, const float x, const float y, const float z, const bool smooth)
{
  m_pihdp = pihdp;
  m_smooth = smooth;
  ...
  m_texturecoord = 0.0f;
  m_menuid =
      (pihdp->GetIEditable()->GetItemType() == eItemRubber) ? IDR_POINTMENU_SMOOTH : IDR_POINTMENU;
}
...
void DragPoint::EditMenu(CMenu& menu)
{
  ...
  menu.CheckMenuItem(ID_POINTMENU_SLINGSHOT,
                     MF_BYCOMMAND | ((m_slingshot && !m_smooth) ? MF_CHECKED : MF_UNCHECKED));
}

void DragPoint::Delete()
{
  if ((int)m_pihdp->m_vdpoint.size() >
      m_pihdp->GetMinimumPoints()) // Can't allow less points than the user can recover from
  {
...
void DragPoint::DoCommand(int icmd, int x, int y)
{
  ...
  switch (icmd)
  {
    case ID_POINTMENU_SMOOTH:
    {
      ...
      m_smooth = !m_smooth;
      const int index2 = (FindIndexOf(m_pihdp->m_vdpoint, (CComObject<DragPoint>*)this) - 1 +
                          (int)m_pihdp->m_vdpoint.size()) %
                         (int)m_pihdp->m_vdpoint.size();
      ...
      switch (LOWORD(wParam))
      {
        case IDC_CHECK_ROTATE_CENTER:
        {
          switch (HIWORD(wParam))
          {
            ...
            {
              string szT;
              if (!(SendDlgItemMessage(hwndDlg, IDC_CHECK_ROTATE_CENTER, BM_GETCHECK, 0, 0) ==
                    BST_CHECKED))
              {
          ...
          switch (LOWORD(wParam))
          {
            case IDOK:
            {
              if (rotateApplyCount == 0)
              {
                ...
                const bool useElementCenter =
                    (SendDlgItemMessage(hwndDlg, IDC_CHECK_ROTATE_CENTER, BM_GETCHECK, 0, 0) ==
                     BST_CHECKED);
            ...
            case IDC_ROTATE_APPLY_BUTTON:
            {
              ...
              const bool useElementCenter = (SendDlgItemMessage(hwndDlg, IDC_CHECK_ROTATE_CENTER,
      ...
      switch (LOWORD(wParam))
      {
        case IDC_CHECK_SCALE_CENTER:
        {
          switch (HIWORD(wParam))
          {
            ...
            {
              string szT;
              if (!(SendDlgItemMessage(hwndDlg, IDC_CHECK_SCALE_CENTER, BM_GETCHECK, 0, 0) ==
                    BST_CHECKED))
              {
                f2sz(g_pvp->m_mouseCursorPosition.x, szT);
                SetDlgItemText(hwndDlg, IDC_CENTERX, szT.c_str());
          ...
          switch (LOWORD(wParam))
          {
            case IDOK:
            {
              if (scaleApplyCount == 0)
              {
                ...
                const bool useElementCenter =
                    (SendDlgItemMessage(hwndDlg, IDC_CHECK_SCALE_CENTER, BM_GETCHECK, 0, 0) ==
                     BST_CHECKED);
            ...
            case IDC_SCALE_APPLY_BUTTON:
            {
              ...
              const bool useElementCenter = (SendDlgItemMessage(hwndDlg, IDC_CHECK_SCALE_CENTER,
                                                                BM_GETCHECK, 0, 0) == BST_CHECKED);
