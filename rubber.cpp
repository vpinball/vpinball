...
{
  char tbuf[128];
  sprintf_s(tbuf, "Height: %.3f | Thickness: %.3f", m_vpinball->ConvertToUnit(m_d.m_height),
            m_vpinball->ConvertToUnit((float)m_d.m_thickness));
...
void Rubber::SetDefaults(bool fromMouseClick)
{
  ...
  m_d.m_staticRendering =
      fromMouseClick ? LoadValueBoolWithDefault(strKeyName, "EnableStaticRendering", true) : true;
  m_d.m_showInEditor =
      fromMouseClick ? LoadValueBoolWithDefault(strKeyName, "EnableShowInEditor", false) : false;
...
void Rubber::DrawRubberMesh(Sur* const psur)
{
  ...
  {
    const Vertex3Ds A = Vertex3Ds(m_vertices[m_ringIndices[i]].x, m_vertices[m_ringIndices[i]].y,
                                  m_vertices[m_ringIndices[i]].z);
    const Vertex3Ds B =
        Vertex3Ds(m_vertices[m_ringIndices[i + 1]].x, m_vertices[m_ringIndices[i + 1]].y,
                  m_vertices[m_ringIndices[i + 1]].z);
    const Vertex3Ds C =
        Vertex3Ds(m_vertices[m_ringIndices[i + 2]].x, m_vertices[m_ringIndices[i + 2]].y,
                  m_vertices[m_ringIndices[i + 2]].z);
...
void Rubber::UIRenderPass1(Sur* const psur)
{
  ...
  if (!m_d.m_showInEditor)
  {
    ...
    const Vertex2D* const rgvLocal =
        GetSplineVertex(cvertex, nullptr, nullptr,
                        4.0f * powf(10.0f, (10.0f - HIT_SHAPE_DETAIL_LEVEL) * (float)(1.0 / 1.5)));
...
void Rubber::UIRenderPass2(Sur* const psur)
{
  ...
  if (!m_d.m_showInEditor)
  {
    ...
    bool* pfCross;
    const Vertex2D* const rgvLocal =
        GetSplineVertex(cvertex, &pfCross, nullptr,
                        4.0f * powf(10.0f, (10.0f - HIT_SHAPE_DETAIL_LEVEL) * (float)(1.0 / 1.5)));
 ...
 */
Vertex2D* Rubber::GetSplineVertex(int& pcvertex,
                                  bool** const ppfCross,
  ...
  else
  {
    ...
    accuracy =
        ...
        powf(
            ...
            (10.0f - accuracy) *
                (float)(1.0 /
                        1.5)); // min = 4 (highest accuracy/detail level), max = 4 * 10^(10/1.5) = ~18.000.000 (lowest accuracy/detail level)
  }
...
void Rubber::AddHitEdge(vector<HitObject*>& pvho,
                        std::set<std::pair<unsigned, unsigned>>& addedEdges,
                        const unsigned i,
                        const unsigned j)
{
...
HRESULT Rubber::InitLoad(IStream* pstm,
                         PinTable* ptable,
                         int* pid,
                         int version,
...
void Rubber::Scale(const float scalex,
                   const float scaley,
...
void Rubber::RenderObject()
{
  ...
  pd3dDevice->DrawIndexedPrimitiveVB(RenderDevice::TRIANGLELIST, MY_D3DFVF_NOTEX2_VERTEX,
                                     m_dynamicVertexBuffer, 0, m_numVertices, m_dynamicIndexBuffer,
                                     0, m_numIndices);
...
void Rubber::GenerateMesh(
    const int _accuracy,
    const bool
        createHitShape) //!! hack, createHitShape==true needs adaption below if changing detail level for hitshape
{
  int accuracy;
  ...
  int splinePoints;
  const Vertex2D* const rgvLocal = GetSplineVertex(
      splinePoints, nullptr, &middlePoints,
      (_accuracy != -1) ? 4.0f * powf(10.0f, (10.0f - HIT_SHAPE_DETAIL_LEVEL) * (float)(1.0 / 1.5))
                        : -1.f);
  ...
  for (int i = 0, index = 0; i < numRings; i++)
  {
    ...
    const Vertex3Ds tangent(middlePoints[i2].x - middlePoints[i].x,
                            middlePoints[i2].y - middlePoints[i].y, 0.0f);

    Vertex3Ds binorm;
    Vertex3Ds normal;
    if (i == 0)
    {
      const Vertex3Ds up(middlePoints[i2].x + middlePoints[i].x,
                         middlePoints[i2].y + middlePoints[i].y, height * 2.f);
      normal = Vertex3Ds(tangent.y * up.z, -tangent.x * up.z,
                         tangent.x * up.y - tangent.y * up.x); // = CrossProduct(tangent, up)
      binorm =
          Vertex3Ds(tangent.y * normal.z, -tangent.x * normal.z,
                    tangent.x * normal.y - tangent.y * normal.x); // = CrossProduct(tangent, normal)
    }
    else
    {
    ...
    for (int j = 0; j < numSegments; j++, index++)
    {
      ...
      Vertex3Ds tmp = GetRotatedAxis((float)j * (360.0f * invNS), tangent, normal) *
                      ((float)m_d.m_thickness * 0.5f);
      m_vertices[index].x = middlePoints[i].x + tmp.x;
      m_vertices[index].y = middlePoints[i].y + tmp.y;
      if (createHitShape &&
          (j == 0 || j == 3)) //!! hack, adapt if changing detail level for hitshape
      {
...
void Rubber::GenerateVertexBuffer()
{
  ...
  SAFE_BUFFER_RELEASE(m_dynamicVertexBuffer);
  VertexBuffer::CreateVertexBuffer(m_numVertices, m_d.m_staticRendering ? 0 : USAGE_DYNAMIC,
                                   MY_D3DFVF_NOTEX2_VERTEX, &m_dynamicVertexBuffer, PRIMARY_DEVICE);
...
void Rubber::SetDefaultPhysics(bool fromMouseClick)
{
  ...
  m_d.m_elasticity =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "Elasticity", 0.8f) : 0.8f;
  m_d.m_elasticityFalloff =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "ElasticityFalloff", 0.3f) : 0.3f;
  ...
  m_d.m_hitHeight =
      fromMouseClick ? LoadValueFloatWithDefault(strKeyName, "HitHeight", 25.0f) : 25.0f;
}
