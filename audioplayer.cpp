...
/*static*/ float convert2decibelvolume(
    const float volume) // 0..100 -> DSBVOLUME_MIN..DSBVOLUME_MAX (-10000..0) (db/log scale)
{
  const float totalvolume = max(min(volume, 100.0f), 0.0f);
  const float decibelvolume = (totalvolume == 0.0f)
                                  ? DSBVOLUME_MIN
                                  : max(logf(totalvolume) * (float)(1000.0 / log(10.0)) - 2000.0f,
                                        (float)DSBVOLUME_MIN); // VP legacy conversion
...
AudioPlayer::AudioPlayer()
{
  ...
  if (!bass_init)
  {
    ...
    { // BASS is already in memory and initialized? (However that would've happened is a mystery to me)
      bass_init = true;
      return;
    }
    ...
    BASS_SetConfig(
        /*BASS_CONFIG_THREAD |*/ BASS_CONFIG_CURVE_PAN,
        fTrue); // logarithmic scale, similar to DSound (although BASS still takes a 0..1 range)
    ...
    BASS_SetConfig(
        /*BASS_CONFIG_THREAD |*/ BASS_CONFIG_VISTA_SPEAKERS,
        fTrue); // to make BASS_ChannelSetAttribute(.., BASS_ATTRIB_PAN, pan); work, needs Vista or later

    for (unsigned int idx = 0; idx < 2; ++idx)
    {
      if (!BASS_Init(
              (idx == 0) ? bass_STD_idx : bass_BG_idx, 44100,
              (SoundMode3D != SNDCFG_SND3D2CH) && (idx == 0)
                  ? 0 /*| BASS_DEVICE_MONO*/ /*| BASS_DEVICE_DSOUND*/
                  : 0,
              g_pvp->GetHwnd(),
              nullptr)) // note that sample rate is usually ignored and set depending on the input/file automatically
      {
...
bool AudioPlayer::MusicInit(const string& szFileName,
                            const string& alt_szFileName,
                            const float volume)
{
  if (bass_BG_idx != -1 && bass_STD_idx != bass_BG_idx)
    BASS_SetDevice(bass_BG_idx);
  ...
  if (m_stream == 0)
  {
    ...
    string bla2;
    BASS_ErrorMapCode(code, bla2);
    const string bla = "BASS music/sound library cannot load \"" + szFileName + "\" (error " +
